...
##  REFACTORED sibling of mah_mrgsolve_model.R -- pluggable-PK naming
##  convention applied to the file's six genuinely externally-dosed
##  compounds: Zoledronate (ZOL), Denosumab (DMB), Calcitonin (CTN),
##  Prednisolone (PRD), Cinacalcet (CIN), Furosemide (FUR). The census's
##  sole listed row, 'Parathyroid (PTH)', is NOT a drug in this model --
##  PTH has no dosing route anywhere in the file and is purely the
##  disease's own endogenous parathyroid-hormone state; the census row
##  has been corrected accordingly. See mah_refactor_notes.md for the
##  full account, including the naming map for all six compounds and
##  the verification result. All numeric parameter values are copied
##  verbatim from the original; only PK/PD naming and structure are
##  reorganized, plus eight new, math-implied GAMMA_*=1.0 Hill-coefficient
##  constants disclosed in the notes. The original (mah_mrgsolve_model.R)
##  is untouched. Refactor added 2026-08-30.
...
EC50_FUR  :   1.2  : furosemide concentration for half TAL block (mg/L)
GAMMA_FUR : 1.0 : Hill coefficient (-) [new; no explicit Hill exponent in original]
EMAX_FUR  :   0.85 : maximal furosemide TAL block (-)
...
EMAX_CIN  :   0.28 : maximal cinacalcet set-point left shift (-)
EC50_CIN  :  25.0  : cinacalcet concentration for half effect (ng/mL)
GAMMA_CIN : 1.0 : Hill coefficient (-) [new; no explicit Hill exponent in original]
...
EMAX_PRD_MAC: 0.90 : maximal glucocorticoid block of extrarenal 1-alpha (-)
EC50_PRD_MAC: 0.055: prednisolone biophase for half block (mg/L)
GAMMA_PRD_MAC: 1.0 : Hill coefficient (-) [new; no explicit Hill exponent in original]
...
EMAX_PRD_GUT   :   0.45 : maximal glucocorticoid cut in Ca absorption (-)
EC50_PRD_GUT: 0.055: prednisolone biophase for half that effect (mg/L)
GAMMA_PRD_GUT: 1.0 : Hill coefficient (-) [new; no explicit Hill exponent in original]
...
EMAX_PRD_OB    :   0.30 : maximal glucocorticoid osteoblast apoptosis (-)
EC50_PRD_OB:  0.045: prednisolone biophase for half that effect (mg/L)
GAMMA_PRD_OB: 1.0 : Hill coefficient (-) [new; no explicit Hill exponent in original]
...
// ================= zoledronate =======================================
CL_ZOL   :   6.0  : renal elimination at reference GFR (1/day)
K12_ZOL   :   9.0  : central to peripheral (1/day)
K21_ZOL   :   2.2  : peripheral to central (1/day)
KB_ZOL    :  22.0  : hydroxyapatite binding rate (1/day)
BMAX_ZOL  : 180.0  : bone-surface binding capacity (umol)
KBURY_ZOL  :   0.055: burial into matrix (1/day)
KUP_ZOL   :   0.45 : osteoclast uptake rate at reference resorption (1/day)
KOUT_ZOL:  0.045: intracellular elimination (1/day)
EMAX_ZOL_APO  :   3.4  : maximal osteoclast apoptosis multiplier (-)
EC50_ZOL_APO  :   0.30 : intracellular drug for half-maximal apoptosis (umol)
GAMMA_ZOL_APO : 1.0 : Hill coefficient (-) [new; no explicit Hill exponent in original]
EMAX_ZOL_RES  :   0.80 : maximal per-cell resorption inhibition (-)
EC50_ZOL_RES  :   0.30 : intracellular drug for half that effect (umol)
GAMMA_ZOL_RES : 1.0 : Hill coefficient (-) [new; no explicit Hill exponent in original]

// ================= denosumab =========================================
KA_DMB    :   0.28 : subcutaneous absorption (1/day)
F_DMB     :   0.62 : subcutaneous bioavailability (-)
V1_DMB    :   8.20 : volume of distribution (L)
CL_DMB   :   0.055: linear elimination (1/day)
KON_DMB   :   0.90 : RANKL association (1/pmol/L/day)
KOFF_DMB  :   2.70 : RANKL dissociation, Kd = 3 pmol/L (1/day)
KINT_DMB  :   0.24 : complex internalisation (1/day)

// ================= calcitonin ========================================
KA_CTN    :  30.0  : subcutaneous absorption (1/day)
CL_CTN   :  16.6  : elimination (1/day)
V1_CTN    :  12.0  : volume of distribution (L)
KE0_CTN   :   3.50 : osteoclast biophase equilibration (1/day)
EMAX_CTN  :   0.85 : maximal inhibition of osteoclast activity (-)
EC50_CTN  :   0.60 : biophase concentration for half effect (IU/L)
GAMMA_CTN : 1.0 : Hill coefficient (-) [new; no explicit Hill exponent in original]
KIN_CTN    :   0.115: calcitonin receptor resynthesis (1/day)
KOUT_CTN   :   1.90 : receptor down-regulation at full occupancy (1/day)

// ================= prednisolone, cinacalcet, furosemide ==============
CL_PRD   :   6.2  : prednisolone elimination (1/day)
KE0_PRD   :   1.20 : genomic biophase equilibration (1/day)
CL_CIN   :   1.9  : cinacalcet elimination (1/day)
CL_FUR   :   8.3  : furosemide elimination (1/day)
...
CENT_ZOL   : zoledronate central (umol)
PERI_ZOL   : zoledronate peripheral (umol)
BONE_ZOL   : zoledronate on bone surface (umol)
BURIED_ZOL   : zoledronate buried in matrix (umol)
OC_ZOL  : intra-osteoclast zoledronate (umol)
GUT_DMB  : denosumab subcutaneous depot (pmol)
CENT_DMB   : denosumab free plasma (pmol/L)
COMPLEX_DMB  : denosumab-RANKL complex (pmol/L)
GUT_CTN  : calcitonin subcutaneous depot (IU)
CENT_CTN   : calcitonin plasma (IU/L)
REC_CTN    : calcitonin receptor availability (fraction)
CENT_PRD   : prednisolone plasma (mg/L)
CENT_CIN   : cinacalcet plasma (ng/mL)
CENT_FUR   : furosemide plasma (mg/L)
BIO_CTN   : calcitonin osteoclast biophase (IU/L)
BIO_PRD   : prednisolone genomic biophase (mg/L)
...
// =====================================================================
C_ZOL = OC_ZOL;
EFFECT_ZOL_APO = EMAX_ZOL_APO * hilln(C_ZOL, EC50_ZOL_APO, GAMMA_ZOL_APO);
EFFECT_ZOL_RES = EMAX_ZOL_RES * hilln(C_ZOL, EC50_ZOL_RES, GAMMA_ZOL_RES);
C_DMB = CENT_DMB;
C_CTN = BIO_CTN;
EFFECT_CTN = EMAX_CTN * REC_CTN * hilln(C_CTN, EC50_CTN, GAMMA_CTN);
C_PRD = BIO_PRD;
EFFECT_PRD_MAC = EMAX_PRD_MAC * hilln(C_PRD, EC50_PRD_MAC, GAMMA_PRD_MAC);
EFFECT_PRD_GUT = EMAX_PRD_GUT * hilln(C_PRD, EC50_PRD_GUT, GAMMA_PRD_GUT);
EFFECT_PRD_OB  = EMAX_PRD_OB  * hilln(C_PRD, EC50_PRD_OB,  GAMMA_PRD_OB);
C_CIN = CENT_CIN;
EFFECT_CIN = EMAX_CIN * hilln(C_CIN, EC50_CIN, GAMMA_CIN);
C_FUR = CENT_FUR;
EFFECT_FUR = EMAX_FUR * hilln(C_FUR, EC50_FUR, GAMMA_FUR);
...
fTAL  = FTAL_MAX * (1.0 - casr) * (1.0 - EFFECT_FUR);
L3    = L2 * (1.0 - fTAL);
Tm_dct = TM_DCT * (0.5 + 0.5 * PS_ren) * (1.0 - CT_RENAL * EFFECT_CTN)
...
// =====================================================================
fabs_ca = (ABS_F0 + ABS_FMAX * hilln(D125, ABS_EC50, ABS_H)) * (1.0 - EFFECT_PRD_GUT);
...
// =====================================================================
RKL_f  = fmax(RKL, 0.0);
J_res  = J_RES0  * OC * (1.0 - EFFECT_ZOL_RES) * (1.0 - EFFECT_CTN) * (1.0 + 0.18 * IMMOB);
...
fur_diu = FUR_DIU * EFFECT_FUR / EMAX_FUR;
...
double u_na  = na_in * fexc + 1.5 * (A_Na - 140.0 * V0) + FUR_NA * EFFECT_FUR / EMAX_FUR;
u_na = clampd(u_na, 0.0, 2200.0);
dxdt_A_Na = na_in - 145.0 * vom - u_na;

dxdt_A_K = ORAL_K * (1.0 - 0.85 * anorexia) - 12.0 * vom
           - (52.0 + 9.0 * Q_u) * (A_K / 3500.0) - 25.0 * EFFECT_FUR;
...
ren_1a = K1A * N_func * D25
         ...
         * (1.0 - ca_1a);
ext_1a = MAC_1A * MAC * D25 * (1.0 - EFFECT_PRD_MAC);
...
dxdt_MAC   = MAC_S * TUM - MAC_KEL * MAC - 0.55 * EFFECT_PRD_MAC * MAC;
...
double bind_d    = KON_DMB * RKL * CENT_DMB;
double unbind_d  = KOFF_DMB * COMPLEX_DMB;
...
dxdt_OC = OC_DIFF * OCP * oc_drive - OC_DEG * OC * (1.0 + EFFECT_ZOL_APO);
...
dxdt_OB  = OBP_OUT * OBP - OB_DEG * OB * (1.0 + ob_pth_apo + EFFECT_PRD_OB);
...
// =====================================================================
double free_surf = fmax(0.0, 1.0 - BONE_ZOL / BMAX_ZOL);
double bind_bone = KB_ZOL * CENT_ZOL * free_surf;
double uptake    = KUP_ZOL * BONE_ZOL * (J_res / J_RES0);

dxdt_CENT_ZOL  = -CL_ZOL * CENT_ZOL * (GFR / GFR0)
              - K12_ZOL * CENT_ZOL + K21_ZOL * PERI_ZOL - bind_bone;
dxdt_PERI_ZOL  = K12_ZOL * CENT_ZOL - K21_ZOL * PERI_ZOL;
dxdt_BONE_ZOL  = bind_bone - KBURY_ZOL * BONE_ZOL - uptake;
dxdt_BURIED_ZOL  = KBURY_ZOL * BONE_ZOL;
dxdt_OC_ZOL = uptake / 12.0 - KOUT_ZOL * OC_ZOL;

// =====================================================================
// 24.  DENOSUMAB — target-mediated disposition, no renal step anywhere
// =====================================================================
dxdt_GUT_DMB = -KA_DMB * GUT_DMB;
dxdt_CENT_DMB  = KA_DMB * GUT_DMB * F_DMB / V1_DMB - CL_DMB * CENT_DMB
              - bind_d + unbind_d;
dxdt_COMPLEX_DMB = bind_d - unbind_d - KINT_DMB * COMPLEX_DMB;

// =====================================================================
// 25.  CALCITONIN — biophase plus receptor down-regulation
// =====================================================================
dxdt_GUT_CTN = -KA_CTN * GUT_CTN;
dxdt_CENT_CTN  = KA_CTN * GUT_CTN / V1_CTN - CL_CTN * CENT_CTN;
dxdt_BIO_CTN  = KE0_CTN * (CENT_CTN - BIO_CTN);
dxdt_REC_CTN   = KIN_CTN * (1.0 - REC_CTN) - KOUT_CTN * REC_CTN * hilln(BIO_CTN, EC50_CTN, GAMMA_CTN);
...
// =====================================================================
dxdt_CENT_PRD = -CL_PRD * CENT_PRD;
dxdt_BIO_PRD = KE0_PRD * (CENT_PRD - BIO_PRD);
dxdt_CENT_CIN = -CL_CIN * CENT_CIN;
dxdt_CENT_FUR = -CL_FUR * CENT_FUR;
...
C_ZOL   : zoledronate intra-osteoclast drug -- the exposed PD-driving concentration (umol)
EFFECT_ZOL_APO : zoledronate-driven osteoclast apoptosis multiplier (-)
EFFECT_ZOL_RES : zoledronate inhibition of resorption (-)
C_DMB   : denosumab free plasma -- the exposed concentration (pmol/L)
C_CTN   : calcitonin osteoclast biophase -- the exposed PD-driving concentration (IU/L)
EFFECT_CTN   : calcitonin effect on osteoclast activity (-)
C_PRD   : prednisolone genomic biophase -- the exposed PD-driving concentration (mg/L)
EFFECT_PRD_MAC : prednisolone suppression of extrarenal 1-alpha-hydroxylase (-)
EFFECT_PRD_GUT : prednisolone suppression of intestinal calcium absorption (-)
EFFECT_PRD_OB  : prednisolone-driven osteoblast apoptosis (-)
C_CIN   : cinacalcet plasma -- the exposed concentration (ng/mL)
EFFECT_CIN   : cinacalcet leftward shift of the CaSR set point (-)
C_FUR   : furosemide plasma -- the exposed concentration (mg/L)
EFFECT_FUR   : furosemide inhibition of TAL calcium reabsorption (-)
...
SCEN <- list(
  ...
  s22 = list("HHM + saline + zoledronate 4 mg q28d (two doses)",
             ...
             events = ev_bolus(c(TX, TX + 28), "CENT_ZOL", ZA_4MG))
)
