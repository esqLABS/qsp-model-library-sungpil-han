...
[`hs_verification_output.txt`](hs_verification_output.txt).
...
| File | Contents |
|---|---|
| [`hs_qsp_model.dot`](hs_qsp_model.dot) | Mechanistic map source — **128 nodes, 13 clusters** |
| [`hs_qsp_model.svg`](hs_qsp_model.svg) / [`.png`](hs_qsp_model.png) | Rendered output (150 dpi) |
| [`hs_mrgsolve_model.R`](hs_mrgsolve_model.R) | **50-ODE** mrgsolve model + 16 scenarios |
| [`hs_shiny_app.R`](hs_shiny_app.R) | **10-tab** Shiny dashboard |
| [`hs_references_en.md`](hs_references_en.md) | **110** PubMed references, with "how the model uses it" stated section by section |
| [`hs_core.py`](hs_core.py) | Independent Python/scipy implementation — the executable source of truth |
| [`hs_calibrate.py`](hs_calibrate.py) | Numerical fit of the cooling conductance UA to published cooling rates |
| [`hs_analysis.py`](hs_analysis.py) | The script that generates every number above |
| [`hs_verification_output.txt`](hs_verification_output.txt) | The raw output of that run |
| [`hs_smoke.py`](hs_smoke.py) | Physiological anchor checks |
