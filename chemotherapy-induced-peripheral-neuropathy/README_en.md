...
[🗺️ Mechanistic map (SVG)](cipn_qsp_model.svg) ·
[⚙️ mrgsolve model](cipn_mrgsolve_model_en.R) ·
[📊 Shiny app](cipn_shiny_app_en.R) ·
[📚 102 references](cipn_references_en.md) ·
[🐍 Python reference implementation](cipn_reference_model.py) ·
[📄 Run output](cipn_reference_output.txt)
...
and the full output is in [`cipn_reference_output.txt`](cipn_reference_output.txt).
...
| File | Contents |
|------|------|
| [`cipn_qsp_model.dot`](cipn_qsp_model.dot) / [`.svg`](cipn_qsp_model.svg) / [`.png`](cipn_qsp_model.png) | Mechanistic map — **16 clusters · 278 nodes · 369 edges** |
| [`cipn_mrgsolve_model_en.R`](cipn_mrgsolve_model_en.R) | **Model of record** — 34 ODEs, PK for 4 drugs, 18 scenarios, population incidence, therapeutic-index optimisation |
| [`cipn_shiny_app_en.R`](cipn_shiny_app_en.R) | 12-tab interactive dashboard |
| [`cipn_references_en.md`](cipn_references_en.md) | 102 references (all with PubMed links, tied to model structure and parameters) |
| [`cipn_reference_model.py`](cipn_reference_model.py) | **Dependency-free Python re-implementation** of the same ODEs with the same parameters — prints every number in this README |
| [`cipn_reference_output.txt`](cipn_reference_output.txt) | The complete run output of that script |
...
All 18 scenarios are in the `scenarios` list of `cipn_mrgsolve_model_en.R` and in
...
```r
shiny::runApp("cipn_shiny_app_en.R")
```
