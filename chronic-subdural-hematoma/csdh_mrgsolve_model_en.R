...
## README_en.md is produced by that file.  The two transcriptions are meant to be
## the same system with the same parameter block; if they disagree, one of them
## is wrong.  Doing this exposed nine defects, listed in README_en.md — including
...
## ---------------------------------------------------------------------------
CSDH_SCENARIOS <- list(
  S01_conservative      = function() csdh_sim("01 conservative observation (no treatment)", 180),
  S02_burrhole_nodrain  = function() csdh_sim("02 burr-hole drainage, no drain", 180,
                              surgery = 0),
  S03_burrhole_drain    = function() csdh_sim("03 burr-hole drainage + drain 48 h", 180,
                              surgery = 0, drain_days = 2),
  S04_burrhole_poorwash = function() csdh_sim("04 burr-hole drainage, inadequate irrigation", 180,
                              surgery = 0, drain_days = 2, wash = 0.55),
  S05_mmae_alone        = function() csdh_sim("05 MMA embolisation alone", 180,
                              embolise_at = 0),
  S06_surg_mmae         = function() csdh_sim("06 burr-hole drainage + drain + MMAE", 180,
                              surgery = 0, drain_days = 2, embolise_at = 0),
  S07_mmae_late         = function() csdh_sim("07 MMAE delayed (30 days after surgery)", 180,
                              surgery = 0, drain_days = 2, embolise_at = 30),
  S08_dex_alone         = function() csdh_sim("08 dexamethasone alone (Dex-CSDH)", 180,
                              doses = ev_dex()),
  S09_dex_surgery       = function() csdh_sim("09 surgery + dexamethasone", 180,
                              surgery = 0, drain_days = 2, doses = ev_dex()),
  S10_atorva            = function() csdh_sim("10 atorvastatin 20 mg x 8 weeks", 180,
                              doses = ev_atv()),
  S11_atorva_dex        = function() csdh_sim("11 atorvastatin + low-dose dex", 180,
                              doses = c(ev_atv(), ev_dex(dose = 2.25, days = 35))),
  S12_txa               = function() csdh_sim("12 tranexamic acid 750 mg x 90 days", 180,
                              doses = ev_txa()),
  S13_surg_txa          = function() csdh_sim("13 surgery + tranexamic acid", 180,
                              surgery = 0, drain_days = 2, doses = ev_txa()),
  S14_doac_resume_early = function() csdh_sim("14 surgery + early DOAC resumption (7 days)", 180,
                              surgery = 0, drain_days = 2,
                              doses = ev_doac(t0 = 7, days = 173)),
  S15_doac_resume_late  = function() csdh_sim("15 surgery + delayed DOAC resumption (30 days)", 180,
                              surgery = 0, drain_days = 2,
                              doses = ev_doac(t0 = 30, days = 150)),
  S16_kitchen_sink      = function() csdh_sim("16 surgery+drain+MMAE+atorvastatin", 180,
                              surgery = 0, drain_days = 2, embolise_at = 0,
                              doses = ev_atv())
)
...
CSDH_natural_history <- function() {
  cat("\n=== Natural history: from a thin membrane to symptomatic presentation (incident cSDH) ===\n")
...
CSDH_flux_ledger <- function() {
  cat("\n=== Flux ledger: the term each treatment touches ===\n")
  cat("Which of J_ex / J_rb / J_abs the treatment acts on, and whether it is a STOCK or a SOURCE\n\n")
...
CSDH_bistability <- function() {
  cat("\n=== Bistability: where the haem switch sits ===\n")
  cat("Change the degree of post-operative irrigation (wash) and the same operation splits into healing and recurrence.\n\n")
  ...
  cat("\n  Without the switch (Hill=1) every row of this table would be the same.\n")
  invisible(NULL)
}

## V_RES protects the patient before the operation and condemns them after.
CSDH_atrophy_two_signs <- function() {
  cat("\n=== The two signs of atrophy (V_RES appears twice, opposite signs) ===\n")
  cat("The same parameter (1) enlarges the volume at presentation and (2) raises the recurrence rate.\n\n")
  ...
  cat("\n  The point is that the two columns (MLS and P(reop)) move in opposite directions.\n")
  invisible(NULL)
}

## Why a 30-day endpoint for embolisation must be null.
CSDH_mmae_timecourse <- function() {
  cat("\n=== MMAE time course: why the 30-day endpoint is null ===\n")
  cat("Embolisation removes the SOURCE rather than the STOCK, so its effect appears as an integral.\n\n")
  ...
  cat("\n  The N_CAP decay time constant is ~9-18 days, so at 30 days the difference is still small.\n")
  invisible(NULL)
}

## 3 hour plasma half-life, 6 day half-life at the site of action.
CSDH_txa_site_delay <- function() {
  cat("\n=== TXA: plasma t1/2 3 hours, site-of-action t1/2 6 days ===\n")
  cat("The drug enters the haematoma not by diffusion but 'carried in the effusate'.\n\n")
  ...
  cat("\n  The turnover time of the cavity, V/J_abs, is the rate-limiting step.\n")
  invisible(NULL)
}

## One drug, two signs: fewer reoperations AND worse patients.
CSDH_dex_two_signs <- function() {
  cat("\n=== The two signs of dexamethasone (the Dex-CSDH paradox) ===\n")
  ...
  cat(sprintf("%-22s %10.3f %10.3f   <- locally beneficial\n", "P(reoperation) 180d",
              .at(a,"P_REOP",180), .at(b,"P_REOP",180)))
  cat(sprintf("%-22s %10.3f %10.3f   <- systemically harmful\n", "P(favourable) 180d",
  ...
  cat("\n  The same drug helps in one equation and harms in another.\n")
  invisible(NULL)
}

## The fibrinolytic loop: report the amplification factor.
CSDH_loop_gain <- function() {
  cat("\n=== Gain of the fibrinolytic loop L2 ===\n")
  ...
  cat(sprintf("\n  1-H (rebleed drive): %.3f -> %.3f  (amplification %.2fx -> %.2fx)\n",
              g1, g2, 1/(1-g1), 1/(1-g2)))
  invisible(NULL)
}

CSDH_trial_ledger <- function() {
  cat("\n=== Model vs published trial numbers (trial ledger) ===\n")
...
CSDH_sensitivity <- function(delta = 0.20) {
  cat(sprintf("\n=== Local sensitivity (+/-%.0f%% on each parameter) ===\n", 100*delta))
...
CSDH_mass_balance <- function() {
  cat("\n=== Mass balance and negative-state check ===\n")
