...
A total of **130** references. Every PMID was checked programmatically against PubMed E-utilities to confirm it actually exists (`csdh_fetch_references_en.py`). To see which literature each of the model's structural claims relies on, see the section breakdown below together with the "What comes from the literature and what is the model's claim" section in `README_en.md`.
