...
| File | Contents |
|---|---|
| [`hk_qsp_model.dot`](hk_qsp_model.dot) · [`.svg`](hk_qsp_model.svg) · [`.png`](hk_qsp_model.png) | Mechanistic map — **134 nodes / 15 clusters / 229 edges** |
| [`hk_mrgsolve_model.R`](hk_mrgsolve_model.R) | mrgsolve ODE model — **30 compartments**, 12 scenario functions |
| [`hk_reference_model.py`](hk_reference_model.py) | Pure-Python reference implementation (zero dependencies) — it **computes** every number below |
| [`hk_model_report.txt`](hk_model_report.txt) | The full output of that script (346 lines) — calibration, validation, discrepancies |
| [`hk_shiny_app_en.R`](hk_shiny_app_en.R) | Shiny dashboard — **9 tabs** |
| [`hk_references_en.md`](hk_references_en.md) | **84 references**, every one cross-checked against PubMed |
...
R -e 'shiny::runApp("hk_shiny_app_en.R")'
```

> **A note on honesty.** R was not installed in this container.
> `hk_mrgsolve_model.R` and `hk_shiny_app_en.R` **have not been verified by running
...
[`hk_references_en.md`](hk_references_en.md) — **84 references**, in 13 sections.
