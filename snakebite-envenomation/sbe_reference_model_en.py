...
Venomous snakebite (venom poisoning) + antivenom — INDEPENDENT PYTHON REFERENCE IMPLEMENTATION
===============================================================================
...
`sbe_mrgsolve_model_en.R` is re-implemented here, term for term, and actually
integrated.  Every number quoted in `README_en.md` is produced by running this
file; the captured log is `sbe_reference_output_en.txt`.
...
SNAKES = {
    "Daboia russelii (Sri Lanka)": dict(
        ...
        f_pre=0.09, f_myo=0.50, f_loc=0.30, koffm=1.00, f_leak=1.00, ko="Russell's viper"),
    "Echis ocellatus": dict(
        dose=22.0, frac=dict(SVMP=0.68, SVSP=0.05, PLA2=0.12, TFTX=0.00),
        f_pre=0.00, f_myo=0.10, f_loc=0.55, koffm=1.00, f_leak=0.60, ko="West African carpet viper"),
    "Naja naja": dict(
        dose=40.0, frac=dict(SVMP=0.10, SVSP=0.02, PLA2=0.28, TFTX=0.55),
        f_pre=0.05, f_myo=0.60, f_loc=1.30, koffm=1.00, f_leak=0.35, ko="Indian cobra"),
    "Bungarus caeruleus": dict(
        dose=10.0, frac=dict(SVMP=0.03, SVSP=0.01, PLA2=0.66, TFTX=0.20),
        f_pre=1.00, f_myo=0.05, f_loc=0.03, koffm=0.15, f_leak=0.05, ko="Indian krait"),
    "Crotalus atrox": dict(
        dose=55.0, frac=dict(SVMP=0.42, SVSP=0.14, PLA2=0.08, TFTX=0.00),
        f_pre=0.03, f_myo=0.30, f_loc=0.70, koffm=1.00, f_leak=0.70, ko="Western diamondback rattlesnake"),
    "Bothrops asper": dict(
        dose=50.0, frac=dict(SVMP=0.50, SVSP=0.06, PLA2=0.30, TFTX=0.00),
        f_pre=0.02, f_myo=0.80, f_loc=1.50, koffm=1.00, f_leak=0.85, ko="Central American lancehead"),
    "Bitis arietans": dict(
        dose=90.0, frac=dict(SVMP=0.62, SVSP=0.04, PLA2=0.16, TFTX=0.00),
        f_pre=0.00, f_myo=0.50, f_loc=1.30, koffm=1.00, f_leak=1.10, ko="Puff adder"),
    "Dry bite": dict(
        dose=0.0, frac=dict(SVMP=0.0, SVSP=0.0, PLA2=0.0, TFTX=0.0),
        f_pre=0.00, f_myo=0.00, f_loc=0.00, koffm=1.00, f_leak=0.00, ko="dry bite"),
}
...
def main():
    ...
    out("Venomous snakebite + antivenom — QSP reference model run")
    out("=" * 78)
    out("")
    ...
    with open(os.path.join(here, "sbe_scenario_results_en.json"), "w") as fh:
    ...
    out("=" * 78)
    out("END OF RUN.  Machine-readable: sbe_scenario_results_en.json,")
    out("             sbe_population_results.json")
    out("=" * 78)

    with open(os.path.join(here, "sbe_reference_output_en.txt"), "w") as fh:
        fh.write("\n".join(lines) + "\n")
