...
#  Postpartum Depression QSP Model · Shiny Dashboard (9 tabs)
#  ---------------------------------------------------------------------------
#  Run:
...
#
#  The app compiles the model defined in ppd_mrgsolve_model_en.R.  To avoid
...
# ---------------------------------------------------------------------------
MODEL_FILE <- "ppd_mrgsolve_model_en.R"
if (!file.exists(MODEL_FILE))
  MODEL_FILE <- file.path("postpartum-depression", "ppd_mrgsolve_model_en.R")
...
# ---------------------------------------------------------------------------
# Event builders (mirror of the helpers in ppd_mrgsolve_model_en.R)
# ---------------------------------------------------------------------------
...
# ===========================================================================
ui <- fluidPage(
  titlePanel("Postpartum Depression QSP Dashboard"),
  ...
  sidebarLayout(
    sidebarPanel(
      width = 3,
      h4("1 · Patient"),
      sliderInput("V", "Vulnerability gain (V)", 0.6, 2.2, 1.70, 0.05),
      ...
      sliderInput("enrol_day", "Enrolment day (days postpartum)",
                  3, 180, 21, 1),
      sliderInput("wt", "Maternal weight (kg)", 45, 120, 70, 1),
      hr(),
      h4("2 · Treatment Arms"),
    ...
    mainPanel(
      ...
      tabsetPanel(
        ...
        # ---------------------------------------------------------------
        tabPanel(
          "⑤ Scenario Comparison · vs Published Trials",
        ...
        tabPanel(
          ...
          hr(),
          tags$p("Model files: ",
                 tags$code("ppd_qsp_model_en.dot"), " · ",
                 tags$code("ppd_mrgsolve_model_en.R"), " · ",
                 tags$code("ppd_reference_check.py"), " · ",
                 tags$code("ppd_references_en.md")),
          tags$p(tags$b("For education and research purposes only. "),
