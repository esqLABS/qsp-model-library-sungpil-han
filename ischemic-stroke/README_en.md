...
**Ischemic stroke** is an emergency neurological disease in which a cerebral vessel is occluded by a thrombus or embolus, blocking blood flow to the affected brain region and causing neuronal injury and death. It accounts for about 80% of all strokes, and is the world's second leading cause of death and leading cause of long-term disability.
...
Thrombus/embolus formation
      ↓
Cerebral vessel occlusion → decreased cerebral blood flow (CBF)
      ↓
Ischemic core (CBF < 10 mL/100g/min)     →  irreversible necrosis
Ischemic penumbra (10–20 mL/100g/min)    →  time-dependent salvageability
      ↓
Energy failure → loss of Na⁺/K⁺-ATPase function
      ↓
Depolarisation → excitotoxic glutamate release → NMDA receptors → Ca²⁺ overload
      ↓
ROS generation · mitochondrial dysfunction · caspase activation
      ↓
Neuroinflammation (microglia, astrocytes, neutrophil infiltration) → MMP-9 → BBB breakdown
      ↓
Vasogenic oedema · haemorrhagic transformation · raised intracranial pressure
```
...
[![Mechanistic Map Preview](is_qsp_model_en.png)](is_qsp_model_en.svg)

> Click to view interactive SVG
...
```bash
# SVG (vector)
dot -Tsvg is_qsp_model_en.dot -o is_qsp_model_en.svg

# PNG (150 dpi)
dot -Tpng -Gdpi=150 is_qsp_model_en.dot -o is_qsp_model_en.png
```
...
Full reference list with 50 PubMed citations: [`is_references_en.md`](is_references_en.md)
