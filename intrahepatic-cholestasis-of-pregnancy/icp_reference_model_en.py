...
the build container: every equation in `icp_mrgsolve_model_en.R` is first written
...
arithmetic; see `icp_calibration.py` and README_en.md.
...
def scenarios():
    S = []
    A = S.append
    A(("01 normal pregnancy (reference)", make_ctl()))
    A(("02 ICP mild, no Rx", make_ctl(gen=MILD)))
    A(("03 ICP severe, no Rx", make_ctl(gen=SEVERE)))
    A(("04 ICP extreme (TBA>=100), no Rx", make_ctl(gen=VSEVERE)))
    A(("05 severe + UDCA 500mg bid @30wk", make_ctl(gen=SEVERE, udca_mg=1000)))
    A(("06 severe + UDCA 750mg bid @30wk", make_ctl(gen=SEVERE, udca_mg=1500)))
    A(("07 severe + UDCA late start @34wk",
       make_ctl(gen=SEVERE, udca_mg=1000, udca_start=34.0)))
    A(("08 extreme + UDCA 500mg bid", make_ctl(gen=VSEVERE, udca_mg=1000)))
    A(("09 severe + rifampicin 300mg bid", make_ctl(gen=SEVERE, rif_mg=600)))
    A(("10 severe + UDCA + rifampicin",
       make_ctl(gen=SEVERE, udca_mg=1000, rif_mg=600)))
    A(("11 extreme + UDCA + rifampicin",
       make_ctl(gen=VSEVERE, udca_mg=1000, rif_mg=600)))
    A(("12 severe + cholestyramine 16g/d", make_ctl(gen=SEVERE, chol_g=16.0)))
    A(("13 severe + cholestyramine + vit K",
       make_ctl(gen=SEVERE, chol_g=16.0, vkdose=1.6)))
    A(("14 severe + SAMe 1000mg IV", make_ctl(gen=SEVERE, sam_mg=1000)))
    A(("15 severe + UDCA + SAMe",
       make_ctl(gen=SEVERE, udca_mg=1000, sam_mg=1000)))
    A(("16 extreme + IBAT inhib (hypoth)", make_ctl(gen=VSEVERE, odev_umol=4.0)))
    A(("17 severe + IBAT inhib (hypoth)", make_ctl(gen=SEVERE, odev_umol=4.0)))
    A(("18 severe + naltrexone 50mg", make_ctl(gen=SEVERE, ntx_mg=50.0)))
    A(("19 severe + antihistamine 12mg/d", make_ctl(gen=SEVERE, ah_mg=12.0)))
    A(("20 twin pregnancy, no Rx", make_ctl(gen=dict(MILD, twin=1.55))))
    A(("21 twin pregnancy + UDCA",
       make_ctl(gen=dict(MILD, twin=1.55), udca_mg=1000)))
    A(("22 ABCB11 V444A homozygous",
       make_ctl(gen=dict(gBSEP=0.55, gMDR3=0.95, gSULT=2.2))))
    A(("23 ABCB4 severe variant (MDR3 30%)",
       make_ctl(gen=dict(gBSEP=0.92, gMDR3=0.30, gSULT=2.2))))
    A(("24 ATP8B1 variant", make_ctl(gen=dict(gFIC1=0.68, gMDR3=0.80, gSULT=2.4))))
    A(("25 NR1H4(FXR) reduced function",
       make_ctl(gen=dict(gFXR=0.55, gBSEP=0.85, gMDR3=0.70, gSULT=2.6))))
    A(("26 low placental transfer (gP 0.6)",
       make_ctl(gen=dict(SEVERE, gP=0.60))))
    A(("27 extreme + 36wk deliv", make_ctl(gen=VSEVERE, tdel_ga=36.0)))
    A(("28 extreme + 36wk deliv + betameth",
       make_ctl(gen=VSEVERE, tdel_ga=36.0, bet_ga=35.0)))
    A(("29 extreme + 37wk deliv", make_ctl(gen=VSEVERE, tdel_ga=37.0)))
    A(("30 extreme + 38wk deliv", make_ctl(gen=VSEVERE, tdel_ga=38.0)))
    A(("31 extreme + 40wk deliv", make_ctl(gen=VSEVERE, tdel_ga=40.0)))
    A(("32 severe + 37wk deliv", make_ctl(gen=SEVERE, tdel_ga=37.0)))
    A(("33 severe + 38wk deliv", make_ctl(gen=SEVERE, tdel_ga=38.0)))
    A(("34 mild + 39wk deliv", make_ctl(gen=MILD, tdel_ga=39.0)))
    A(("35 max combo (UDCA+RIF+IBAT+36wk)",
