...
| Deliverable | File |
|--------|------|
| 🗺️ Mechanistic map (191 nodes · 182 edges · 20 clusters) | [`icp_qsp_model_en.dot`](icp_qsp_model_en.dot) · [`.svg`](icp_qsp_model_en.svg) · [`.png`](icp_qsp_model_en.png) |
| ⚙️ mrgsolve ODE model (78 ODEs, 35 scenarios) | [`icp_mrgsolve_model_en.R`](icp_mrgsolve_model_en.R) |
| 📊 Shiny dashboard (10 tabs) | [`icp_shiny_app_en.R`](icp_shiny_app_en.R) |
| 📚 References (202 items, every PMID looked up) | [`icp_references_en.md`](icp_references_en.md) |
| 🐍 Python RK4 reference implementation | [`icp_reference_model_en.py`](icp_reference_model_en.py) · [output](icp_reference_output_en.txt) |
| 🔬 Fitting, ablation and trial-reproduction analysis | [`icp_calibration.py`](icp_calibration.py) · [output](icp_calibration_output.txt) |
...
Python RK4 (`icp_reference_model_en.py`), and in the course of that nine actual
...
literature range, linked item by item in `icp_references_en.md`), or a scale set so
...
The full table is in the last section of `icp_references_en.md`. The main points:
...
```bash
# render the map
dot -Tsvg icp_qsp_model_en.dot -o icp_qsp_model_en.svg
dot -Tpng -Gdpi=150 icp_qsp_model_en.dot -o icp_qsp_model_en.png

# Python reference implementation (all scenarios, without R; --quick for just 6)
python3 icp_reference_model_en.py
...
# The 35 scenarios are in the comments at the bottom of icp_mrgsolve_model_en.R.

# Shiny (10 tabs)
shiny::runApp("icp_shiny_app_en.R")
```
