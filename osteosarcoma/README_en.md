...
<p align="center">
  <a href="osa_qsp_model_en.svg">
    <img src="osa_qsp_model_en.png" width="900" alt="Osteosarcoma QSP mechanistic map">
  </a>
</p>

| File | Contents |
|------|------|
| [`osa_qsp_model_en.dot`](osa_qsp_model_en.dot) · [`.svg`](osa_qsp_model_en.svg) · [`.png`](osa_qsp_model_en.png) | Mechanistic map — 193 nodes, 15 clusters, 258 edges |
| [`osa_mrgsolve_model_en.R`](osa_mrgsolve_model_en.R) | **The model proper** — 55 ODE compartments, 141 parameters, 20 scenarios, protocol gate layer, virtual population layer |
| [`osa_shiny_app_en.R`](osa_shiny_app_en.R) | Interactive dashboard (8 tabs) |
| [`osa_references_en.md`](osa_references_en.md) | 64 references + calibration comparison table |
| [`osa_reference_model_en.py`](osa_reference_model_en.py) | Python/scipy reimplementation of the same equations (for numerical verification in a container with no R) |
| [`osa_reference_output.txt`](osa_reference_output.txt) | The full text of the actual run output (509 lines) |
| [`osa_scenario_results.json`](osa_scenario_results.json) · [`osa_analysis_results.json`](osa_analysis_results.json) | Machine-readable results |

Every number below is a value that `osa_reference_model_en.py` actually integrated
...
Full text in [`osa_reference_output.txt`](osa_reference_output.txt) §1. (The arm that moves IE pre-operatively is run at a resistance of RES₀ = 0.20 and therefore sits separately in §3.E. The R model includes it as a 20th named scenario.)
...
Rscript osa_mrgsolve_model_en.R

# Interactive dashboard (8 tabs)
shiny::runApp("osa_shiny_app_en.R")
```

```bash
# Numerical reference implementation (needs only numpy/scipy; regenerates every result above)
python3 osa_reference_model_en.py       # -> osa_reference_output.txt + JSON

# Render the mechanistic map
dot -Tsvg osa_qsp_model_en.dot -o osa_qsp_model_en.svg
dot -Tpng -Gdpi=150 osa_qsp_model_en.dot -o osa_qsp_model_en.png
```

Why `osa_reference_model_en.py` exists: the build container of this repository has
