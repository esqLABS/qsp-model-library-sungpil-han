...
#  REFACTORED sibling of cdi_mrgsolve_model.R -- pluggable-PK naming convention
#  applied to Vancomycin (VAN, the drug the corpus census generically listed as
#  "Antibacterial" -- the file's own docstring, its scenario notes, and its
#  primary position in every $PARAM section confirm it as the model's central
#  antibacterial) and Bezlotoxumab (BEZ). Fidaxomicin/OP-1118, metronidazole,
#  rifaximin, ridinilazole, the index (precipitating) antibiotic, and the live
#  biotherapeutic are OUT OF SCOPE for this refactor and are left byte-for-byte
#  untouched (same compartments, same parameter names, same equations). See
#  cdi_refactor_notes.md for the full account. All numeric parameter values are
#  copied verbatim from the original; only PK structure/naming and the
#  Hill-interface expression for VAN and BEZ are reorganized. The original is
#  untouched.
# =============================================================================
#
...
#      toxins ...................  6   (TCDA TCDB COMPLEX_BEZ TCDA_MUC TCDB_MUC CDT) [was TOX_CPLX]
...
#    bezlotoxumab    mg central/peripheral;  LUM_BEZ in mg/L (= ug/mL) of lumen [was BEZ_GUT]
...
KON_BEZ : 4.50  : Bezlotoxumab-TcdB capture rate (per (ug/mL) antibody per day) [was KONB]
KOFF_BEZ: 0.05  : Bezlotoxumab-TcdB dissociation (1/d) [was KOFFB]
KDEG_BEZ: 1.50  : Clearance of the antibody-toxin complex (1/d) [was KDEGCX]
...
// ---------------- bezlotoxumab PK ---------------------------------------
CL_BEZ    : 0.317 : Bezlotoxumab clearance (L/d) [was CLB]
V1_BEZ    : 3.08  : Bezlotoxumab central volume (L) [was VCB]
V2_BEZ    : 4.10  : Bezlotoxumab peripheral volume (L) [was VPB]
...
GAMMA_BEZ: 1.0  : Hill coefficient of the bezlotoxumab PK/PD interface (new)
...
EC50_VAN: 45    : Vancomycin faecal EC50 vs C. difficile (ug/g) [was EC50V]
EMAX_VAN: 1.0   : Hill Emax of the vancomycin PK/PD interface (new; original ratio was implicitly Emax=1)
GAMMA_VAN: 1.0  : Hill coefficient of the vancomycin PK/PD interface (new; original had no explicit Hill exponent)
...
//     content), not plasma)
GUT_VAN    : Vancomycin, gastric/dosing compartment (mg) [was VAN_D]
GUT2_VAN   : Vancomycin, small-bowel transit compartment (mg) [was VAN_T; bespoke 2nd-stage transit, see notes]
CENT_VAN   : Vancomycin, colonic content (mg) [was VAN_COL; this IS the exposed/PD-facing site, not plasma]
// --- 45-48 fidaxomicin + OP-1118 (OUT OF SCOPE, untouched)
...
// --- 49-51 metronidazole (OUT OF SCOPE, untouched)
MTZ_A      : Metronidazole, absorption site (mg)
MTZ_C      : Metronidazole, systemic (mg)
MTZ_COL    : Metronidazole, colonic content (mg)
// --- 52-54 bezlotoxumab (renamed to the pluggable-PK convention)
CENT_BEZ   : Bezlotoxumab, central plasma (mg) [was BEZ_C]
PERI_BEZ   : Bezlotoxumab, peripheral (mg) [was BEZ_P]
LUM_BEZ    : Bezlotoxumab, luminal (mg/L) [was BEZ_GUT; this IS the exposed/PD-facing concentration, not plasma]
// --- 55-56 rifaximin
...
// diagnostic/reporting values, never the actual ODE state trajectories (see
// notes for the full before/after comparison) -- but macros avoid the
// artifact entirely, so they are used here for an exact match.
#define C_VAN         (1000.0 * CENT_VAN / VCOLG)                       // exposed, PD-facing: faecal vancomycin (ug/g) [was macro CVAN]
...
double fVRE = (1.0 - VRE);
double KILL_SBA = KV_SBA*EFFECT_VAN + KF_SBA*FF + KM_SBA*FM + KD_SBA*FD + KR_SBA*FR + KX_SBA*FX;
double KILL_BUT = KV_BUT*EFFECT_VAN + KF_BUT*FF + KM_BUT*FM + KD_BUT*FD + KR_BUT*FR + KX_BUT*FX;
double KILL_BAC = KV_BAC*EFFECT_VAN + KF_BAC*FF + KM_BAC*FM + KD_BAC*FD + KR_BAC*FR + KX_BAC*FX;
double KILL_BIF = KV_BIF*EFFECT_VAN + KF_BIF*FF + KM_BIF*FM + KD_BIF*FD + KR_BIF*FR + KX_BIF*FX;
double KILL_ENC = KV_ENC*EFFECT_VAN*fVRE + KF_ENC*FF + KD_ENC*FD;

// engraftment of an administered consortium, blocked while antibiotics persist
double ABXPRESS = EFFECT_VAN + FF + FM + FD + FR + FX;
...
double KILLCD = EMAXV*EFFECT_VAN + EMAXF*FF + EMAXM*FM + EMAXD*FD + EMAXR*FR + EMAXT*TIGE;
...
// transudated bezlotoxumab.  LUM_BEZ is in ug/mL, converted to IgG-equivalent
// units by WBEZ, so a single 10 mg/kg dose contributes on the same scale as a
// good natural humoral response. [was BEZ_GUT]
double ABEQ   = pos(AB_IGG) + WBEZ*pos(LUM_BEZ);
...
double BIND = KON_BEZ*pos(TCDB)*pos(LUM_BEZ);       // ng/mL/d of TcdB captured [was KONB, BEZ_GUT]
dxdt_TCDB = KTOXB*TOXSRC*FTOX - KDEGB*TCDB - KOUTTOX*TCDB - KTRB*TCDB*PERMF
            - BIND + KOFF_BEZ*pos(COMPLEX_BEZ);
dxdt_COMPLEX_BEZ = BIND - KOFF_BEZ*pos(COMPLEX_BEZ) - KDEG_BEZ*pos(COMPLEX_BEZ);
...
// vancomycin (VAN): renamed compartments/rate constants, identical structure
// and math to the original 3-stage GI-transit chain (no systemic absorption)
dxdt_GUT_VAN  = -KTR_VAN*GUT_VAN;
dxdt_GUT2_VAN =  KTR_VAN*GUT_VAN - KTR_VAN*GUT2_VAN;
dxdt_CENT_VAN =  KTR_VAN*GUT2_VAN - KE_VAN*WASHF*CENT_VAN;
...
dxdt_PERI_BEZ =  (Q_BEZ/V1_BEZ)*CENT_BEZ - (Q_BEZ/V2_BEZ)*PERI_BEZ;
...
dxdt_LUM_BEZ = KTR_BEZ_LUM*Cp_bez_plasma*(0.05 + pos(EPI_PERM)) - KE_BEZ_LUM*LUM_BEZ
...
double CVANo  = C_VAN;                         // kept for R-driver-script column-name compatibility; identical to C_VAN [was 1000.0*VAN_COL/VCOLG]
...
double CBEZo  = Cp_bez_plasma;                 // kept for R-driver-script column-name compatibility; identical to Cp_bez_plasma [was BEZ_C/VCB]
...
double ABTOT = pos(AB_IGG) + WBEZ*pos(LUM_BEZ);   // [was BEZ_GUT]
...
C_VAN     : Vancomycin -- exposed, PD-facing concentration (faecal, ug/g)
EFFECT_VAN: Vancomycin -- fractional target engagement (Hill interface, 0-1)
C_BEZ     : Bezlotoxumab -- exposed, PD-facing concentration (luminal/mucosal, mg/L; NOT plasma)
EFFECT_BEZ: Bezlotoxumab -- fractional contribution to the shared neutralizing-antibody term (Hill interface, 0-1)
Cp_bez_plasma: Bezlotoxumab -- plasma concentration (mg/L; informational only, not read by any PD term)
CVANo  : Faecal vancomycin (ug/g) (duplicate of C_VAN, kept for R-driver-script compatibility)
...
CBEZo  : Plasma bezlotoxumab (mg/L) (duplicate of Cp_bez_plasma, kept for R-driver-script compatibility)
...
ev_vanco <- function(start, days = 10, dose = 125, ii = 0.25) {
  n <- days / ii
  ev(time = start, amt = dose, cmt = "GUT_VAN", ii = ii, addl = n - 1)
}
ev_vanco_taper <- function(start) {
  ...
  c(ev(time = start,      amt = 125, cmt = "GUT_VAN", ii = 0.25,  addl = 55),
    ev(time = start + 14, amt = 125, cmt = "GUT_VAN", ii = 1/3,   addl = 20),
    ev(time = start + 21, amt = 125, cmt = "GUT_VAN", ii = 0.5,   addl = 13),
    ev(time = start + 28, amt = 125, cmt = "GUT_VAN", ii = 1,     addl = 6),
    ev(time = start + 35, amt = 125, cmt = "GUT_VAN", ii = 2.5,   addl = 8))
}
...
ev_bezlo <- function(time, wt = 70) {
  ev(time = time, amt = 10 * wt, cmt = "CENT_BEZ")
}
...
RX_CMT <- c("GUT_VAN", "FDX_D", "MTZ_A", "RID_D", "RFX_D", "LBP", "CENT_BEZ")
rx_end <- function(e) {
  d <- as.data.frame(e)
