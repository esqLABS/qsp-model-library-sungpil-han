...
##      GAMMA_NT  Hill exponent on HPD inhibition    fitted 1.45 [renamed from HNT]
...
##  inhibition curve steeper than simple competition (GAMMA_NT = 1.45, consistent
...
V1_NT    :  12    : central volume (L)
V2_NT    :  10    : peripheral volume (L)
Q_NT     :   3    : intercompartmental clearance (L/day)
CL_NT    :   6.07 : nitisinone clearance (L/day) -> Css 1.0 uM at 2 mg/day
F_NT     :   1    : oral bioavailability x adherence
CYP3A4_NT:   1    : CYP3A4 activity multiplier on CL_NT
...
GAMMA_NT :     1.45  : Hill exponent on HPD inhibition (slow tight binding) -- FITTED [renamed from HNT]
...
$CMT @annotated
GUT_NT  : nitisinone gut depot (umol) [renamed from NTGUT]
CENT_NT : nitisinone central (umol) [renamed from NTCEN]
PERI_NT : nitisinone peripheral (umol) [renamed from NTPER]
...
F_GUT_NT = F_NT;                // adherence / bioavailability acts here [was F_NTGUT = FBIO]
...
double C_NT  = CENT_NT/V1_NT; if(C_NT  < 0.0) C_NT  = 0.0;
...
//    cap a mechanism the original model deliberately leaves uncapped. Kept
//    as a bespoke Hill-shaped-but-unbounded term (rename, not a refit).
// ===========================================================================
double EFFECT_NT = pow(C_NT/KI_NT, GAMMA_NT);
double KMAPP = KMHPD*(1.0 + EFFECT_NT);
...
// ===========================================================================
// -- nitisinone PK
dxdt_GUT_NT  = -KA_NT*GUT_NT;
dxdt_CENT_NT =  KA_NT*GUT_NT - CL_NT*CYP3A4_NT*C_NT - Q_NT*(C_NT - PERI_NT/V2_NT);
dxdt_PERI_NT =  Q_NT*(C_NT - PERI_NT/V2_NT);
...
// uses the renamed compartments/params (was NTCEN/V1).
double CNTo   = CENT_NT/V1_NT; if(CNTo   < 0.0) CNTo   = 0.0;
...
double EFFECT_NTo = pow(CNTo/KI_NT, GAMMA_NT);
double KMAPPo = KMHPD*(1.0 + EFFECT_NTo);
...
calibrate_aku <- function(mod, rounds = 3, verbose = TRUE) {
  fitp <- c(KI_NT = 0.016, GAMMA_NT = 1.45, SRC_INS = 60, KI_OAT = 10,
