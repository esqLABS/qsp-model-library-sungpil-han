...
# Chronic Pyelonephritis Interactive Simulation Dashboard
# Tabs: Patient profile · PK · Bacterial dynamics · Renal function · Scenario comparison · Biomarkers
###############################################################################
...
# --------------------------------------------------------------------------- #
# mrgsolve model (inline) — same as cpn_mrgsolve_model_en.R but streamlined
# --------------------------------------------------------------------------- #
cpn_code <- '
$PARAM
...
ui <- dashboardPage(
  ...
  dashboardHeader(title = "CPN QSP Dashboard — Chronic Pyelonephritis"),

  dashboardSidebar(
    sidebarMenu(
      menuItem("① Patient profile",   tabName = "patient",   icon = icon("user-circle")),
      menuItem("② PK monitoring",     tabName = "pk",        icon = icon("chart-line")),
      menuItem("③ Bacterial dynamics",       tabName = "bacteria",  icon = icon("bacteria")),
      menuItem("④ Renal function (GFR)",    tabName = "renal",     icon = icon("heartbeat")),
      ...
      menuItem("⑦ References",        tabName = "refs",      icon = icon("book-open"))
    ),
    hr(),
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "patient",
        fluidRow(
          box(title = "Patient parameter settings", width = 4, status = "primary",
              solidHeader = TRUE,
              sliderInput("GFR0",    "Baseline GFR (mL/min/1.73m²)",  15, 120, 90, 5),
              selectInput("VUR_risk","VUR severity (structural risk)",
                          c("None (normal)"="low", "Mild I–II"="mild",
                            "Moderate III"="mod", "Severe IV–V"="high")),
              selectInput("Bact_init","Initial bacterial load (log₁₀ CFU/g)",
                          c("Mild 10^4"="4", "Moderate 10^6"="6",
                            "Severe 10^8"="8"), selected="6"),
              checkboxInput("hasDM",   "Diabetes mellitus (DM)",       FALSE),
              checkboxInput("hasImm",  "Immunosuppressed state",      FALSE),
              checkboxInput("hasPreg", "Pregnancy (risk 3×↑)",   FALSE),
              numericInput("Resist_F", "Resistance factor (1=susceptible, 4=moderate resistance)", 1, 1, 8, 0.5)
          ),
          box(title = "Antibiotic regimen settings", width = 4, status = "warning",
              solidHeader = TRUE,
              checkboxInput("use_cipro", "Use ciprofloxacin",    FALSE),
              conditionalPanel("input.use_cipro",
                sliderInput("cipro_dose", "Dose (mg/dose)", 250, 750, 500, 50),
                sliderInput("cipro_days", "Treatment duration (days)", 3, 28, 14, 1)
              ),
              hr(),
              checkboxInput("use_tmp",   "Use TMP-SMX",         FALSE),
              conditionalPanel("input.use_tmp",
                sliderInput("tmp_days", "TMP-SMX duration (days)", 3, 28, 14, 1)
              ),
              hr(),
              checkboxInput("use_nit",   "Use nitrofurantoin prophylaxis", FALSE),
              conditionalPanel("input.use_nit",
                sliderInput("nit_months", "Prophylaxis duration (months)", 1, 12, 6, 1)
              )
          ),
          box(title = "Simulation settings", width = 4, status = "success",
              solidHeader = TRUE,
              sliderInput("sim_months", "Simulation period (months)", 1, 24, 12, 1),
      ...
      tabItem(tabName = "pk",
        fluidRow(
          box(title = "Ciprofloxacin plasma concentration (Cp vs time)", width = 8,
              status = "info", solidHeader = TRUE,
              plotlyOutput("plot_pk_cipro", height = "340px")),
          box(title = "PK/PD index (fAUC/MIC)", width = 4,
              status = "info", solidHeader = TRUE,
              plotlyOutput("plot_fAUC", height = "340px"))
        ),
        fluidRow(
          box(title = "TMP plasma concentration", width = 6,
              status = "primary", solidHeader = TRUE,
              plotlyOutput("plot_pk_tmp", height = "280px")),
          box(title = "Nitrofurantoin urinary concentration", width = 6,
              status = "primary", solidHeader = TRUE,
              plotlyOutput("plot_pk_nit", height = "280px"))
        ),
        fluidRow(
          box(title = "PK parameter summary", width = 12, status = "warning",
      ...
      tabItem(tabName = "bacteria",
        fluidRow(
          box(title = "Renal bacterial load (log₁₀ CFU/g)", width = 8,
              status = "danger", solidHeader = TRUE,
              plotlyOutput("plot_bacteria", height = "360px")),
          box(title = "Biofilm fraction", width = 4,
              status = "danger", solidHeader = TRUE,
              plotlyOutput("plot_biofilm", height = "360px"))
        ),
        fluidRow(
          box(title = "Antibacterial effect (kill rate)", width = 6,
              status = "warning", solidHeader = TRUE,
              plotlyOutput("plot_kill", height = "280px")),
          box(title = "Immune cell recruitment (neutrophil / macrophage)", width = 6,
              status = "warning", solidHeader = TRUE,
      ...
      tabItem(tabName = "renal",
        fluidRow(
          box(title = "GFR trend", width = 7,
              status = "primary", solidHeader = TRUE,
              plotlyOutput("plot_gfr", height = "360px")),
          box(title = "Serum creatinine", width = 5,
              status = "primary", solidHeader = TRUE,
              plotlyOutput("plot_creat", height = "360px"))
        ),
        fluidRow(
          box(title = "Renal scarring progression (scar %)", width = 6,
              status = "warning", solidHeader = TRUE,
              plotlyOutput("plot_scar", height = "280px")),
          box(title = "CKD stage transition", width = 6,
              status = "warning", solidHeader = TRUE,
      ...
      tabItem(tabName = "scenario",
        fluidRow(
          box(title = "Comparison of 5 antibiotic strategies (GFR)", width = 12,
              status = "success", solidHeader = TRUE,
              plotlyOutput("plot_scenario_gfr", height = "380px"))
        ),
        fluidRow(
          box(title = "Bacterial load comparison", width = 6,
              status = "success", solidHeader = TRUE,
              plotlyOutput("plot_scenario_bact", height = "300px")),
          box(title = "Renal scarring comparison", width = 6,
              status = "success", solidHeader = TRUE,
              plotlyOutput("plot_scenario_scar", height = "300px"))
        ),
        fluidRow(
          box(title = "Scenario comparison summary table", width = 12,
      ...
      tabItem(tabName = "biomarker",
        fluidRow(
          box(title = "Inflammation and fibrosis markers (IL-6, TGF-β1, Collagen)", width = 8,
              status = "info", solidHeader = TRUE,
              plotlyOutput("plot_biomarker", height = "380px")),
          box(title = "Clinical significance of biomarkers", width = 4,
              status = "info", solidHeader = TRUE,
              tableOutput("biomarker_legend"))
        ),
        fluidRow(
          box(title = "Urosepsis risk index (Bacteria > 10^7.5)", width = 6,
              status = "danger", solidHeader = TRUE,
              plotlyOutput("plot_sepsis_risk", height = "280px")),
          box(title = "Collagen deposition vs scar formation", width = 6,
              status = "warning", solidHeader = TRUE,
      ...
      tabItem(tabName = "refs",
        box(title = "Key references (chronic pyelonephritis QSP model)", width = 12,
            htmlOutput("ref_html"))
      )
...
server <- function(input, output, session) {
  ...
  scenario_data <- eventReactive(input$run_btn, {
    ...
    sc_list <- list(
      ...
      list(label="NIT prophylaxis alone", use_cipro=0, use_tmp=0, use_nit=1,
  ...
  output$patient_summary_box <- renderUI({
    tags$div(
      tags$b("Current patient settings:"),
      tags$ul(
        tags$li(paste0("Baseline GFR: ", input$GFR0, " mL/min")),
        tags$li(paste0("VUR risk: ", input$VUR_risk)),
        tags$li(paste0("Initial bacteria: 10^", input$Bact_init, " CFU/g")),
        tags$li(paste0("Resistance factor: ", input$Resist_F))
      )
    )
  })
  ...
  output$plot_pk_cipro <- renderPlotly({
    ...
    p <- plot_ly(d, x = ~time, y = ~Cipro_C, type = "scatter", mode = "lines",
      ...
      add_segments(x = 0, xend = max(d$time), y = 0.25, yend = 0.25,
                   ...
                   name = "MIC = 0.25 μg/mL") |>
      layout(title = "Ciprofloxacin plasma concentration profile",
  ...
  output$plot_fAUC <- renderPlotly({
    ...
    plot_ly(d, x = ~time, y = ~fAUC_MIC_OB, type = "scatter", mode = "lines",
      ...
      layout(title = "fAUC/MIC attainment",
             xaxis = list(title = "Time (h)"),
  ...
  output$plot_pk_nit <- renderPlotly({
    ...
    plot_ly(d, x = ~time, y = ~NIT_urine, type = "scatter", mode = "lines",
      ...
      add_segments(x = 0, xend = max(d$time), y = 32, yend = 32,
                   ...
                   name = "MIC = 32 μg/mL") |>
      layout(xaxis = list(title = "Time (h)"),
             yaxis = list(title = "Nitrofurantoin urinary concentration (μg/mL)"))
  })

  output$pk_param_table <- DT::renderDataTable({
    tbl <- data.frame(
      Antibiotic      = c("Ciprofloxacin", "TMP-SMX", "Nitrofurantoin"),
      Bioavailability   = c("70%", "95% / 85%", "75%"),
      Vc_L       = c(140, 85, "-"),
      CL_Lh      = c(25, "3.5 / 1.5", "-"),
  ...
  output$plot_bacteria <- renderPlotly({
    ...
    plot_ly(d, x = ~time / 24, y = ~Bacteria, type = "scatter", mode = "lines",
      ...
      layout(xaxis = list(title = "Time (day)"),
  ...
  output$plot_biofilm <- renderPlotly({
    ...
    plot_ly(d, x = ~time / 24, y = ~Biofilm, type = "scatter", mode = "lines",
      ...
      layout(xaxis = list(title = "Time (day)"),
             yaxis = list(title = "Biofilm fraction (0–1)", range = c(0, 1)))
  })
  ...
  output$plot_kill <- renderPlotly({
    ...
    plot_ly(d, x = ~time / 24, y = ~Kill_drug, type = "scatter", mode = "lines",
      ...
      layout(xaxis = list(title = "Time (day)"),
             yaxis = list(title = "Antibacterial kill rate (log₁₀ CFU/h)"))
  })
  ...
  output$plot_immune <- renderPlotly({
    ...
    plot_ly(d, x = ~time / 24, y = ~Neutrophil, type = "scatter", mode = "lines",
      ...
      layout(xaxis = list(title = "Time (day)"),
             yaxis = list(title = "Normalised cell count (1=baseline)"))
  })
  ...
  output$plot_creat <- renderPlotly({
    ...
    plot_ly(d, x = ~time / 24, y = ~Creatinine, type = "scatter", mode = "lines",
            ...
            line = list(color = "#cc0000", width = 2)) |>
      layout(xaxis = list(title = "Time (day)"),
             yaxis = list(title = "Serum creatinine (mg/dL)"))
  })
  ...
  output$plot_scar <- renderPlotly({
    ...
    plot_ly(d, x = ~time / 24, y = ~ScarPct, type = "scatter", mode = "lines",
      ...
      layout(xaxis = list(title = "Time (day)"),
             yaxis = list(title = "Renal scarring (%)"))
  })
  ...
  output$plot_ckd <- renderPlotly({
    ...
    plot_ly(d, x = ~time / 24, y = ~CKD_Stage, type = "scatter", mode = "lines",
      ...
      layout(xaxis = list(title = "Time (day)"),
             yaxis = list(title = "CKD stage", range = c(0.5, 5.5),
                          tickvals = 1:5,
  ...
  output$scenario_table <- DT::renderDataTable({
    ...
    d |>
      group_by(Scenario) |>
      summarise(
        `Final GFR`       = round(tail(GFR, 1), 1),
        `GFR change`       = round(tail(GFR, 1) - head(GFR, 1), 1),
        `Final CKD stage`  = paste0("G", round(tail(CKD_Stage, 1))),
        `Final scarring (%)`  = round(tail(ScarPct, 1), 1),
        `Bacterial nadir`    = round(min(Bacteria), 2),
        `Final bacterial load` = round(tail(Bacteria, 1), 2)
      ) |>
  ...
  output$plot_biomarker <- renderPlotly({
    ...
    plot_ly(d, x = ~time / 24, y = ~IL6, type = "scatter", mode = "lines",
      ...
      layout(xaxis = list(title = "Time (day)"),
             yaxis = list(title = "Normalised level (1 = baseline)"),
             legend = list(orientation = "h"))
  })

  output$biomarker_legend <- renderTable({
    data.frame(
      Marker      = c("IL-6", "TGF-β1", "Collagen", "Neutrophil", "Macrophage"),
      ClinicalSignificance = c(
        "Acute-phase protein induction, fever signal",
        "Key fibrosis mediator",
        "Degree of interstitial fibrosis",
        "Marker of acute bacterial infection",
        "Chronic inflammation / M2→fibrosis"
      ),
      NormalRange  = c("< 5 pg/mL", "< 10 pg/mL", "baseline=1", "baseline=1", "baseline=1")
    )
  })
  ...
  output$plot_sepsis_risk <- renderPlotly({
    ...
    plot_ly(d, x = ~time / 24, y = ~SepsisRisk, type = "scatter", mode = "lines",
            ...
            line = list(color = "#cc0000", width = 2)) |>
      layout(xaxis = list(title = "Time (day)"),
             yaxis = list(title = "Urosepsis risk (1=High, 0=Low)",
                          range = c(-0.1, 1.2)))
  })
  ...
  output$plot_collagen_scar <- renderPlotly({
    ...
    plot_ly(d, x = ~Collagen, y = ~ScarPct, type = "scatter", mode = "markers",
            marker = list(color = ~time / max(d$time),
                          ...
                          colorbar = list(title = "Time fraction"))) |>
      layout(xaxis = list(title = "Collagen deposition (normalised)"),
             yaxis = list(title = "Renal scarring (%)"))
  })
  ...
  output$ref_html <- renderUI({
    ...
    <h4>Key pathophysiology references</h4>
    <ol>
    ...
    <h4>Renal fibrosis / CKD progression</h4>
    <ol start='13'>
