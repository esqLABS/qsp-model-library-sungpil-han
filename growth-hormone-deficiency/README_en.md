# Paediatric Growth Hormone Deficiency (GHD) — QSP Model
...
| Deliverable | File |
|---|---|
| 🗺️ Mechanistic map (DOT) | [`ghd_qsp_model_en.dot`](ghd_qsp_model_en.dot) |
| 🖼️ Map (SVG, zoomable)   | [`ghd_qsp_model_en.svg`](ghd_qsp_model_en.svg) |
| 🖼️ Map (PNG, 150 dpi)    | [`ghd_qsp_model_en.png`](ghd_qsp_model_en.png) |
| ⚙️ mrgsolve ODE model     | [`ghd_mrgsolve_model.R`](ghd_mrgsolve_model.R) |
| 📊 Shiny dashboard        | [`ghd_shiny_app_en.R`](ghd_shiny_app_en.R) |
| 📚 References (79, PubMed-verified) | [`ghd_references_en.md`](ghd_references_en.md) |
...
Open [`ghd_qsp_model_en.svg`](ghd_qsp_model_en.svg) to zoom.
...
3. **Pituitary Somatotroph** — GHRH-R/Gs/cAMP/PKA/CREB/Pit-1, SSTR2/5, Ca²⁺ influx, granule pool, functional mass, pulse amplitude vs frequency, 22/20-kDa isoforms
4. **GH Circulation · Clearance** — GHBP (shed GHR ectodomain), free GH, peripheral distribution, receptor-mediated vs linear clearance, flip-flop kinetics
...
6. **IGF System** — hepatic IGF-1, IGFBP-3, ALS, the 150-kDa ternary complex, 50-kDa binary complexes, IGFBP-1 (insulin-suppressed), PAPP-A2, free/bioactive IGF-1, IGF-1R/IRS-1, IGF-2R clearance, local growth-plate IGF-1
7. **Growth-Plate Engine** — resting/proliferative/prehypertrophic/hypertrophic zones, IHH–PTHrP loop, FGF18–FGFR3 brake, CNP–NPR2, SOX9, RUNX2/MEF2C, COL10A1/MMP13/VEGF, vascular invasion, senescence, fusion, catch-up capacity
8. **Bone · Mineral** — osteoblast/osteoclast, RANKL/OPG, P1NP/osteocalcin/ALP, remodelling transient, BMD Z, cortical thickness, peak bone mass, SCFE, scoliosis
9. **Pubertal Axis** — KNDy kisspeptin/NKB/dynorphin, MKRN3 brake, GnRH pulse generator, LH/FSH, gonad, testosterone, aromatase, oestradiol, ERα, the spurt, fusion, hypogonadism in CPHD
10. **Body Composition · Intermediary Metabolism** — HSL/ATGL lipolysis, FFA flux, fat and lean mass, hepatic gluconeogenesis, insulin resistance, fasting insulin/glucose, infantile hypoglycaemia, lipids, REE, Na⁺/water retention
11. **Interaction with Other Axes** — type-2 deiodinase and the free-T4 fall, 11β-HSD1 and unmasked adrenal insufficiency, glucocorticoid as a potent growth suppressor, prolactin/ADH
12. **Daily Somatropin PK** — SC depot, ka, CL/F, flip-flop, the single supraphysiological daily peak, adherence decay, devices
13. **Once-Weekly Long-Acting GH** — lonapegsomatropin (TransCon carrier), somatrogon (CTP fusion), somapacitan (albumin binder), release kinetics, non-physiological peak:trough, the within-week IGF-1 swing and why the sampling day matters
14. **Mecasermin (rhIGF-1)** — dosing, hypoglycaemia, lymphoid overgrowth, why it under-performs rhGH in GHD
15. **Adjunct Drugs** — GnRH analogue, aromatase inhibitor, oxandrolone, macimorelin (diagnostic), sex-steroid priming, CNP analogue, glucocorticoid minimisation
16. **Diagnostic Cascade** — height velocity, height SDS, IGF-1/IGFBP-3 SDS, provocation tests, the assay-dependent cut-off, MRI, bone age, gene panel, IGF-1 generation test, retesting
17. **Clinical Endpoints** — annualised height velocity, height SDS, ΔHtSDS year 1, near-adult height, mid-parental target, predicted-adult-height gain, bone-age/chronological-age ratio, HRQoL
18. **Safety** — the IGF-1 SDS > +2 ceiling, benign intracranial hypertension, insulin resistance/T2DM, anti-drug antibodies, injection-site reactions, OSA in Prader-Willi, neoplasia surveillance, SAGhE/KIGS cohorts
19. **Response Predictors** — age at start, dose, provoked GH peak, height deficit vs target, birth weight/SGA, weight SDS, d3-GHR, first-year response, KIGS/Ranke models
20. **Transition · Adult GHD** — stopping criteria, retesting after washout, adult dosing, adult phenotype, cardio-metabolic legacy, structured transition
...
shiny::runApp("ghd_shiny_app_en.R")
```
...
```bash
dot -Tsvg ghd_qsp_model_en.dot -o ghd_qsp_model_en.svg
dot -Tpng -Gdpi=150 -Gsize=60,27 ghd_qsp_model_en.dot -o ghd_qsp_model_en.png
```
...
This is a QSP model for educational and research purposes. It was built based on
published literature but has not been independently validated or certified, and
**must not be used for actual clinical decision-making, prescribing, or
regulatory submission.** The parameters are illustrative approximations and have
not been fitted to individual patient data. In particular, the height SDS and
IGF-1 SDS values output by this model are based on internal reference tables and
do not substitute for a validated growth chart or an assay-specific IGF-1
reference range.
