...
## PK/PD REFACTORED sibling of cppd_mrgsolve_model.R -- pluggable PK,
## named Hill interface, per FORK_WORKFLOW_GUIDE.md Part 2.
##
## Scope: ANA (anakinra), COLCH (colchicine), INDO (indomethacin),
##        PRED (prednisolone) -- all four PK/PD-bearing compounds
##        carrying rows in driver-patches/data/compound_perturbation_census.md.
##        No compound is out of scope in this file.
##
## Author: Claude Code Routine (CCR)
## Date:   2026-06-20 (original) / refactored 2026-08-30
## Ref:    See cppd_references.md for calibration sources; see
##         cppd_refactor_notes.md for the full refactor disclosure.
## ============================================================
...
code <- '
$PROB
Pseudogout (CPPD Crystal Deposition Disease) QSP Model -- PK/PD refactored
...
// --- Colchicine (COLCH) PK: Archetype 3 minus peripheral (depot+central, linear) ---
// F=0.45, Vd=21 L/kg * 70kg=1470L, CL~31L/h, t1/2≈26-31h
F_COLCH  = 0.45,   // oral bioavailability
KA_COLCH = 1.2,    // absorption rate (/h)
CL_COLCH = 31.0,   // clearance (L/h)
V1_COLCH = 1470,   // central volume of distribution (L)

// --- Indomethacin (INDO) PK: Archetype 3 minus peripheral (depot+central, linear) ---
...
// --- Prednisolone (PRED) PK: Archetype 3 minus peripheral (depot+central, linear) ---
// F=0.82, t1/2≈2.5-4h, CL~6.6L/h, Vd~33L
F_PRED   = 0.82,
KA_PRED  = 1.5,
CL_PRED  = 6.6,
V1_PRED  = 33.0,
EC50_PRED = 5.0,     // ng/mL for 50% GR occupancy (was EC50_GR)
EMAX_PRED = 0.95,    // max GR occupancy (was Emax_GR)
GAMMA_PRED = 1,      // Hill exponent, new (original had no explicit exponent)

// --- Anakinra (ANA) PK: Archetype 3 minus peripheral (depot+central, linear) ---
...
// --- Drug PD effect (Hill interface) parameters ---
// Colchicine: two independent physiological effects sharing one EMAX_COLCH,
// each with its own EC50 -- NLRP3 inhibition + neutrophil migration block
EC50_COLCH_NLRP3 = 1.5, // ng/mL plasma for 50% NLRP3 inhibition
EC50_COLCH_NEUT  = 2.0, // ng/mL for 50% neutrophil migration block
EMAX_COLCH       = 0.85,// max inhibition by colchicine (was Emax_COLCH)
GAMMA_COLCH      = 1,   // Hill exponent, new (shared by both effect terms)

// Indomethacin: COX-2 inhibition → PGE2 reduction
EC50_INDO = 0.3,  // µg/mL for 50% COX-2 inhibition (was EC50_INDO_COX)
EMAX_INDO = 0.90, // (was Emax_INDO)
GAMMA_INDO = 1,   // Hill exponent, new

// Prednisolone: NF-κB transrepression → multi-cytokine reduction
// EFFECT_PRED (GR occupancy) is computed from EMAX_PRED/EC50_PRED above;
// EMAX_PRED_NFKB scales GR occupancy down to the NF-kB transrepression
// ceiling actually seen on IL-6/PGE2 (was Emax_PRED). EC50_PRED_NFKB is
// pre-existing dead code in the original -- declared but never referenced
// in $MAIN/$ODE -- preserved unused, see refactor notes.
EC50_PRED_NFKB = 5.0,  // ng/mL -- declared but unused in original, kept unused
EMAX_PRED_NFKB = 0.88, // (was Emax_PRED)

// Anakinra: IL-1R blockade (competitive antagonist model)
// IL-1Ra competitive: fraction = [ANA]/(EC50_ANA + [ANA])
EC50_ANA = 0.5,   // µg/mL, dissociation constant for IL-1R (was KD_ANA)
EMAX_ANA = 0.92,  // (was Emax_ANA)
GAMMA_ANA = 1     // Hill exponent, new

$CMT
// Drug PK compartments (depot + central) -- naming convention: GUT_<STEM>/CENT_<STEM>
GUT_COLCH CENT_COLCH
GUT_INDO  CENT_INDO
GUT_PRED  CENT_PRED
GUT_ANA   CENT_ANA
...
$MAIN
// --- Exposed concentrations (the single point an external covariate could substitute in) ---
double C_COLCH = CENT_COLCH / V1_COLCH * 1000;  // ng/mL
double C_INDO  = CENT_INDO  / V1_INDO;           // µg/mL
double C_PRED  = CENT_PRED  / V1_PRED * 1000;    // ng/mL
double C_ANA   = CENT_ANA   / V1_ANA;            // µg/mL (≈ mg/L)

// --- Prednisolone: GR occupancy (primary Hill effect on C_PRED) ---
double EFFECT_PRED = EMAX_PRED * pow(C_PRED, GAMMA_PRED)
                      / (pow(EC50_PRED, GAMMA_PRED) + pow(C_PRED, GAMMA_PRED));

// --- Colchicine: two independent physiological effects on the same C_COLCH ---
double EFFECT_COLCH_NLRP3 = EMAX_COLCH * pow(C_COLCH, GAMMA_COLCH)
                             / (pow(EC50_COLCH_NLRP3, GAMMA_COLCH) + pow(C_COLCH, GAMMA_COLCH));
double EFFECT_COLCH_NEUT  = EMAX_COLCH * pow(C_COLCH, GAMMA_COLCH)
                             / (pow(EC50_COLCH_NEUT, GAMMA_COLCH)  + pow(C_COLCH, GAMMA_COLCH));

// --- Indomethacin: COX-2 inhibition → PGE2↓ ---
double EFFECT_INDO = EMAX_INDO * pow(C_INDO, GAMMA_INDO)
                      / (pow(EC50_INDO, GAMMA_INDO) + pow(C_INDO, GAMMA_INDO));

// --- Prednisolone: NF-κB transrepression → IL-1β/IL-6/PGE2 all ↓ ---
// (downstream of GR occupancy EFFECT_PRED, not a fresh Hill of concentration)
double EFFECT_PRED_NFKB = EMAX_PRED_NFKB * EFFECT_PRED;

// --- Anakinra: competitive IL-1R blockade ---
double EFFECT_ANA = EMAX_ANA * C_ANA / (EC50_ANA + C_ANA);

// Combined drug inhibition on IL-1β production (multi-drug combination point,
// kept exactly where the disease equations actually use it)
double E_IL1b_total = 1 -
    (1 - EFFECT_COLCH_NLRP3) *
    (1 - EFFECT_PRED_NFKB) *
    (1 - EFFECT_ANA * 0.7);   // anakinra blocks secretion pathway
...
dxdt_CENT_PRED   = KA_PRED * F_PRED * GUT_PRED
...
double nlrp3_inhib = (EFFECT_COLCH_NLRP3 + EFFECT_PRED * 0.3) * NLRP3_act;  // drug inhibition
...
double neut_ingress = IL8_recruit * (1 - EFFECT_COLCH_NEUT) / resolve_factor;
...
double IL6_prod = kIL6_syn * (1 + 0.05 * fmax(IL1b, 0)) * (1 - EFFECT_PRED_NFKB * 0.7);
dxdt_IL6 = IL6_prod - kIL6_deg * IL6;

// === PGE2 (COX-2 mediated) ===
double pge2_prod = kPGE2_syn * (1 + 0.02 * fmax(IL1b, 0)) * (1 - EFFECT_INDO) * (1 - EFFECT_PRED_NFKB * 0.5);
...
$CAPTURE
C_COLCH C_INDO C_PRED C_ANA
EFFECT_COLCH_NLRP3 EFFECT_COLCH_NEUT EFFECT_INDO EFFECT_PRED EFFECT_PRED_NFKB EFFECT_ANA
...
capture ColchCp_out    = C_COLCH;
capture IndoCp_out     = C_INDO;
capture PredCp_out     = C_PRED;
capture AnaCp_out      = C_ANA;
capture GR_out         = EFFECT_PRED;
'

## ---- 2. Compile Model ----
mod <- mcode("CPPD_QSP_refactored", code)

## ---- 3. Initial Conditions ----
init_vals <- list(
  GUT_COLCH     = 0, CENT_COLCH    = 0,
  GUT_INDO      = 0, CENT_INDO     = 0,
  GUT_PRED      = 0, CENT_PRED     = 0,
  GUT_ANA       = 0, CENT_ANA      = 0,
...
evs_2 <- ev(time = c(0, 1, 12, 24, 36, 48, 60, 72, 84, 96, 108, 120, 132, 144, 156),
             cmt = "GUT_COLCH", amt = 0.5) +
         make_acute_event()

# Scenario 3: Indomethacin 50 mg TID × 7 days
evs_3 <- ev(time = seq(0, by = 8, length.out = 21), cmt = "GUT_INDO", amt = 50) +
         make_acute_event()

# Scenario 4: Prednisolone 30 mg/day → taper (30→20→10→5 over 14 days)
pred_taper <- bind_rows(
  data.frame(time = seq(0, 71, by = 24),  cmt = "GUT_PRED", amt = 30),
  data.frame(time = seq(72, 143, by = 24), cmt = "GUT_PRED", amt = 20),
  data.frame(time = seq(144, 215, by = 24),cmt = "GUT_PRED", amt = 10),
  data.frame(time = seq(216, 287, by = 24),cmt = "GUT_PRED", amt = 5)
)
evs_4 <- as_data_set(pred_taper) + make_acute_event()

# Scenario 5: Anakinra 100 mg SC/day × 14 days
evs_5 <- ev(time = seq(0, by = 24, length.out = 14), cmt = "GUT_ANA", amt = 100) +
         make_acute_event()

# Scenario 6: Colchicine + Indomethacin (combination)
evs_6 <- ev(time = seq(0, by = 12, length.out = 15), cmt = "GUT_COLCH", amt = 0.5) +
         ev(time = seq(0, by = 8,  length.out = 21), cmt = "GUT_INDO",  amt = 50) +
         make_acute_event()
...
evs_7 <- ev(time = seq(0, by = 24, length.out = 180), cmt = "GUT_COLCH", amt = 0.5)
...
#
## ---- 11. PK/PD Refactor Notes ----
# See cppd_refactor_notes.md for the full disclosure. Summary:
# - All four compounds (ANA, COLCH, INDO, PRED) are Archetype 3 minus
#   peripheral (depot+central, linear PK), renamed to GUT_<STEM>/CENT_<STEM>,
#   with parameters renamed to KA_/F_/CL_/V1_<STEM>.
# - Each exposes C_<STEM> (the single point an external covariate could
#   substitute in) and one or more named EFFECT_<STEM> terms.
# - Colchicine has two independent physiological effects on one C_COLCH
#   (EFFECT_COLCH_NLRP3, EFFECT_COLCH_NEUT), both pure renames (gamma=1).
# - Prednisolone's GR occupancy (EFFECT_PRED) and its downstream NF-kB
#   transrepression magnitude (EFFECT_PRED_NFKB = EMAX_PRED_NFKB *
#   EFFECT_PRED) are kept as two named quantities, matching how the
#   original used GR_occ directly in one place and Emax_PRED*GR_occ in
#   another.
# - Build-compat fix (syntax-only, disclosed): the original's $CAPTURE
#   duplicates all 12 disease-side $CMT compartment names, which fails to
#   compile under mrgsolve 2.0.1 ("compartment should not be in
#   $CAPTURE"). Removed from $CAPTURE here (every compartment remains
#   available via /model_manifest's outputPaths / mrgsolve's own state
#   output regardless). Logged as translations/UPSTREAM_ISSUES.md #68.
# - Verified via qspserver mrgsolve_api against all 7 of the original's
#   own dosing scenarios: exact match (max abs diff 0.0) on every shared
#   output.
