...
## =============================================================================
##
## ── PK/PD REFACTOR NOTE (fork-only; see aip_refactor_notes.md) ───────────────
## This sibling file makes givosiran (GIV) and hemin (HEM) "plumbable" per
## FORK_WORKFLOW_GUIDE.md Part 2: each compound's PK now lives in its own
## clearly-named block and exposes exactly one $CAPTURE-visible concentration
## (`C_GIV`, `C_HEM`) and one named effect (`EFFECT_GIV`, `EFFECT_HEM`) that
## the ALAS1 synthesis term reads. No other compound exists in this file.
## Every parameter VALUE below is copied verbatim from the original; only
## names changed (old name given as an inline `[was ...]` comment) plus the
## small number of new, algebra-forced constants noted in the refactor notes
## (`GAMMA_GIV`, `EMAX_HEM`, `GAMMA_HEM`). The original file is untouched.
## =============================================================================
...
Refactored (fork): givosiran (GIV) and hemin (HEM) PK isolated into named
GUT_/CENT_/PERI_/LIV_ blocks, each exposing one C_<STEM>/EFFECT_<STEM> pair.

$PARAM @annotated
// ── Givosiran Population PK (Balwani 2020 popPK analysis) ──────────────────
KA_GIV    = 0.50   // SC absorption rate constant (d⁻¹); Balwani 2020
F_GIV     = 0.90   // SC bioavailability (fraction); ENVISION phase 3 [was F1_GIV]
CL_GIV    = 0.038  // Total plasma clearance (L/kg/d); popPK central estimate
V1_GIV    = 0.22   // Central volume of distribution (L/kg) [unused in original ODE; see refactor notes]
Q_GIV     = 0.12   // Inter-compartmental clearance (L/kg/d)
V2_GIV    = 1.60   // Peripheral volume (L/kg) [unused in original ODE; see refactor notes]
BW        = 65.0   // Body weight (kg)

// ── Givosiran Liver Distribution & RISC Kinetics ────────────────────────────
KUP_GIV     = 4.0    // First-order hepatic uptake from plasma (d⁻¹) [was KUP_LIV]
GALN_KM_GIV = 12.0   // GalNAc-ASGPR Michaelis-Menten Km (ng/mL) [was GALN_KM]
KCLEAR_GIV  = 0.0039 // Liver givosiran elimination (d⁻¹; t½ ~6 months) [was KCLEAR_LIV]
MW_GIV      = 14540  // Givosiran molecular weight (Da)
EC50_GIV    = 450.0  // EC50 for ALAS1 mRNA knockdown (ng/g liver) [was EC50_siRNA]
EMAX_GIV    = 0.92   // Maximum knockdown fraction (~92%); ENVISION trough [was EMAX_siRNA]
GAMMA_GIV   = 1.50   // Hill coefficient [was HILL_siRNA]
...
$CMT @annotated
GUT_GIV   : Givosiran SC depot (mg) [was GIV_SC]
CENT_GIV  : Givosiran plasma central (µg/L) [was GIV_C]
PERI_GIV  : Givosiran plasma peripheral (µg/L) [was GIV_P]
LIV_GIV   : Givosiran liver concentration (ng/g) [was GIV_LIV]
...
CENT_HEM  : Hemin plasma (µg/L) [was HEM_C]
LIV_HEM   : Hemin liver (nmol/g) [was HEM_LIV]
...
$ODE
// ── Givosiran PK (GUT_GIV -> CENT_GIV <-> PERI_GIV) ─────────────────────────
double abs_GIV = KA_GIV * GUT_GIV;
dxdt_GUT_GIV  = -abs_GIV;
dxdt_CENT_GIV = F_GIV * abs_GIV / BW * 1000     // mg→µg/L adjustment
               - CL_GIV * CENT_GIV
               - Q_GIV * CENT_GIV
               + Q_GIV * PERI_GIV
               - KUP_GIV * CENT_GIV;            // hepatic uptake (first-order approx)
dxdt_PERI_GIV = Q_GIV * CENT_GIV - Q_GIV * PERI_GIV;

// GalNAc-ASGPR hepatic uptake (Michaelis-Menten saturation) into liver tissue
double CENT_GIV_ng = CENT_GIV * MW_GIV / 1e6;   // µg/L → ng/mL approximation
double UPT_LIV_GIV = KUP_GIV * CENT_GIV_ng / (GALN_KM_GIV + CENT_GIV_ng) * CENT_GIV;
dxdt_LIV_GIV = UPT_LIV_GIV - KCLEAR_GIV * LIV_GIV;

// ── Givosiran exposed concentration + Hill effect (RISC/siRNA knockdown) ────
// LIV_GIV is already a tissue concentration (ng/g), not an amount divided by
// a volume — same "compartment IS the concentration" pattern documented for
// tocilizumab in sepsis/sep_refactor_notes.md — so no division is applied.
double C_GIV      = LIV_GIV;
double EFFECT_GIV = EMAX_GIV * pow(C_GIV, GAMMA_GIV) /
                    (pow(EC50_GIV, GAMMA_GIV) + pow(C_GIV, GAMMA_GIV));
...
// ── Hemin PK (single central compartment, dosed/read directly as a
//    concentration — same quirk as CENT_GIV/tocilizumab above) + liver ───────
dxdt_CENT_HEM = -(CL_HEM + KUP_HEM * V_HEM) * CENT_HEM;
dxdt_LIV_HEM  = KUP_HEM * CENT_HEM * V_HEM / (BW * 0.025) // liver g = 2.5% BW
              - KHO1_HEM * LIV_HEM;
...
// — algebraic identity, not a fit (see aip_refactor_notes.md).
double C_HEM      = LIV_HEM;
double X_HEM      = KALAS1_HEM * C_HEM;
double EFFECT_HEM = EMAX_HEM * pow(X_HEM, GAMMA_HEM) /
                    (pow(EC50_HEM, GAMMA_HEM) + pow(X_HEM, GAMMA_HEM));
double HEM_EFF    = 1.0 - EFFECT_HEM;           // original suppression factor, preserved

// ── ALAS1 mRNA dynamics ───────────────────────────────────────────────────────
// Synthesis: baseline × hormonal trigger × heme feedback × hemin effect × (1 - siRNA KD)
// Degradation: first-order
double ALAS1_SYN = KDEG_mRNA * HORMTRIG * HEME_FB * HEM_EFF * (1.0 - EFFECT_GIV) * (1.0 - GLU_EFF);
...
dxdt_HEME_LIV = HEME_PROD - KD_HEME * HEME_LIV
               + KHO1_HEM * LIV_HEM * 0.3;            // hemin catabolism product
...
capture GIV_LIVER_ng   = LIV_GIV;
capture GIV_PLASMA_ug  = CENT_GIV;
capture HEMIN_PLASMA   = CENT_HEM;
capture HEMIN_LIVER    = LIV_HEM;
capture siEFF_frac     = EFFECT_GIV;
...
capture C_GIV_out      = C_GIV;
capture EFFECT_GIV_out = EFFECT_GIV;
capture C_HEM_out      = C_HEM;
capture EFFECT_HEM_out = EFFECT_HEM;
'

## =============================================================================
## Build mrgsolve model object
## =============================================================================
mod <- mcode("aip_qsp_refactored", code)
...
make_givosiran_events <- function(bw = 65, dose_mgkg = 2.5, n_months = 12) {
  ...
  ev(amt = dose_mg, cmt = "GUT_GIV", time = days)
}
...
make_hemin_events <- function(bw = 65, start_day = 30, dose_mgkg = 3, days = 4) {
  ...
  ev(amt = dose_ug, cmt = "CENT_HEM", rate = -2,  # rate=-2 → zero-order 1h infusion
