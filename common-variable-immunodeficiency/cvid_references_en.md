# Common Variable Immunodeficiency (CVID) — References

**Common Variable Immunodeficiency · QSP Model Reference List**

**Every PMID in this list was looked up through the NCBI E-utilities to confirm its author, year,
journal and title**. No PMID written from memory has been used (during drafting, 4 of the 5 written
from memory pointed to entirely different papers, and all of them were replaced with the results of
a lookup).

---

## The load-bearing anchors this model depends on quantitatively

The six papers below are the ones to which the model's parameters are **pinned directly**. The
remaining papers justify the structure (which node connects to which node); these six settle the
numbers.

| Anchor | Reference | Observed | Model |
|---|---|---|---|
| Pneumonia exposure-response slope | Orange 2010 [PMID 20675197](https://pubmed.ncbi.nlm.nih.gov/20675197/) | −27% per 100 mg/dL of IgG trough | −29.0% |
| Pneumonia incidence anchor point | Orange 2010 | 0.113/year at a trough of 500 mg/dL | 0.113/year |
| SC:IV dose adjustment factor | EMA / Berger·Jolles [PMID 25384609](https://pubmed.ncbi.nlm.nih.gov/25384609/) | 1.37 (AUC matching) | 1.341 |
| Mortality RR from non-infectious complications | Resnick 2012 [PMID 22180439](https://pubmed.ncbi.nlm.nih.gov/22180439/) | about 11-fold | 10.5-fold |
| smB− switched memory B cells | Wehr 2008 EUROclass [PMID 17898316](https://pubmed.ncbi.nlm.nih.gov/17898316/) | <2% of B cells | 1.67% |
| Bronchiectasis progression versus trough | Lucas 2010 [PMID 20471071](https://pubmed.ncbi.nlm.nih.gov/20471071/) | progression suppressed at a trough ≥600–800 | 0.049 → 0.002 Reiff points/year |

**An item whose under-prediction is reported as it stands:** the 12-week index-lesion response to
leniolisib is −39% observed (Rao 2023 Blood [PMID 36399712](https://pubmed.ncbi.nlm.nih.gov/36399712/))
against −21% in the model. That is the result of removing an arbitrary lymph-node shrinkage term and
leaving only PI3Kδ inhibition, so it is left unfitted, as it stands.

---

## 1. Definition, Diagnostic Criteria, Epidemiology & Registries
...
## 2. B-cell Development, the Class-switch Block & the EUROclass Classification
...
## 3. T-cell Help, Regulatory Failure & CTLA-4
...
## 4. Genetic Substrate — TACI, BAFF-R, CTLA4, LRBA, NFKB1/2, PIK3CD
...
## 5. The BAFF-APRIL Axis and the B-cell Sink
...
## 6. Immunoglobulin Replacement Pharmacokinetics
...
## 7. Exposure-Response — Trough IgG and Infection
...
## 8. Vaccine Response, Specific Antibody & Opsonic Function
...
## 9. Mucosal Immunity, the IgA Gap & Microbial Translocation
...
## 10. Lung Disease — Bronchiectasis & Structural Damage
...
## 11. GLILD, Granuloma & Interstitial Lung Disease
...
## 12. Autoimmune Cytopenias — ITP, AIHA, Evans
...
## 13. Enteropathy, Protein Loss & Liver Disease
...
## 15. Targeted and Genotype-directed Therapy
...
## 16. Adverse Effects of Immunoglobulin Replacement
...
## 17. Additional Mechanism, Review & Supporting Literature
...
| Model element | Supporting section |
|---|---|
| Class-switch block (`FCSR`), EUROclass phenotype | 2 |
| Tfh expansion · Treg/CTLA-4 failure (the root of ARM 2) | 3, 4 |
| Loss of the BAFF sink → rise in BAFF after rituximab | 5 |
| Two-compartment IgG PK, FcRn saturation, SC bioavailability | 6 |
| Pneumonia exposure-response Hill function, individual biological IgG level | 7 |
| Mucosal IgA gap → the dose-independent floor for sinusitis | 8, 9 |
| The bronchiectasis ratchet, Reiff/Bhalla scores, azithromycin | 10 |
| The structural null for GLILD, rituximab + azathioprine | 11 |
| Autoantibody · platelets · the splenectomy trade-off | 12 |
| Protein-losing enteropathy → IgG clearance feedback | 13 |
| Cumulative malignancy risk | 14 |
| Abatacept (CTLA4/LRBA), leniolisib (APDS), HSCT | 15 |
| The price of treatment — infusion reactions, thrombosis, haemolysis | 16 |

## Search Strategy

Title and author field queries were sent to the PubMed E-utilities (`esearch` → `esummary`) to
settle the PMID, and the metadata returned (author · year · journal · title) was used verbatim in
the citation. Where the paper returned was off-topic it was excluded from the citations.
The citation strings in this list are therefore generated from lookup results, not from memory.

## Disclaimer

This QSP model is **for education and research**. Its parameters were calibrated against aggregate
values from the published literature but have not been validated on individual patient data, and it
must not be used directly for clinical decision-making, prescribing or regulatory submission.
