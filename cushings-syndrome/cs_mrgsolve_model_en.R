...
# Cushing's syndrome quantitative systems pharmacology model
#
...
scen1 <- mod %>%
  ...
  as_tibble() %>%
  mutate(scenario = "CS natural history (Cushing disease, untreated)")
...
scen2 <- mod %>%
  ...
  as_tibble() %>%
  mutate(scenario = "Pasireotide 0.6mg BID SC")
...
scen3 <- mod %>%
  ...
  mrgsim(end = 720, delta = 2.0) %>%
  as_tibble() %>%
  mutate(scenario = "Ketoconazole 400mg BID")
...
scen4 <- mod %>%
  ...
  mrgsim(end = 720, delta = 2.0) %>%
  as_tibble() %>%
  mutate(scenario = "Osilodrostat 5mg BID")
...
scen5 <- mod %>%
  ...
  mrgsim(end = 720, delta = 2.0) %>%
  as_tibble() %>%
  mutate(scenario = "Mifepristone 600mg QD (GR antagonist)")
...
scen6 <- mod %>%
  ...
  as_tibble() %>%
  mutate(scenario = "Post-surgical remission (after pituitary adenoma removal)")
...
cols_scen <- c(
  "CS natural history (Cushing disease, untreated)" = "#E53935",
  "Pasireotide 0.6mg BID SC"                       = "#1E88E5",
  "Ketoconazole 400mg BID"                         = "#43A047",
  "Osilodrostat 5mg BID"                           = "#8E24AA",
  "Mifepristone 600mg QD (GR antagonist)"          = "#F4511E"
)
...
p1 <- all_scens %>%
  ...
  annotate("text", x = 0.5, y = 7, label = "Normal range (6-23 μg/dL)", hjust = 0, size = 3) +
  scale_color_manual(values = cols_scen) +
  labs(x = "Time (days)", y = "Plasma free cortisol (μg/dL)",
       title = "A. Plasma cortisol kinetics") +
  theme_cs
...
p3 <- all_scens %>%
  ...
  scale_color_manual(values = cols_scen) +
  labs(x = "Time (days)", y = "Blood glucose (mmol/L)",
       title = "C. Blood glucose change (metabolic effect)") +
  theme_cs
...
p4 <- all_scens %>%
  ...
  annotate("text", x = 0.5, y = 55, label = "Upper limit of normal (50 μg/24h)", hjust = 0, size = 3) +
  scale_color_manual(values = cols_scen) +
  labs(x = "Day", y = "24 h urinary free cortisol (μg/24h)",
       title = "D. UFC (urinary free cortisol)") +
  theme_cs
...
p5 <- all_scens %>%
  ...
  geom_line(linewidth = 1.0) +
  scale_color_manual(values = cols_scen) +
  labs(x = "Time (days)", y = "Skeletal muscle mass (kg)",
       title = "E. Skeletal muscle wasting") +
  theme_cs
...
p6 <- all_scens %>%
  ...
  scale_color_manual(values = cols_scen) +
  labs(x = "Time (days)", y = "Systolic blood pressure (mmHg)",
       title = "F. Blood pressure change") +
  theme_cs

# Plot 7: Post-surgery recovery
p7 <- scen6 %>%
  ggplot(aes(time / (24 * 30))) +
  geom_line(aes(y = cortisol_free, color = "Cortisol (μg/dL)")) +
  geom_line(aes(y = ACTH_pg_mL / 5, color = "ACTH/5 (pg/mL)")) +
  geom_hline(yintercept = c(6, 23) / 30, linetype = "dashed") +
  scale_color_manual(values = c("Cortisol (μg/dL)" = "#1565C0", "ACTH/5 (pg/mL)" = "#C62828")) +
  labs(x = "Time since surgery (months)", y = "Hormone level",
       title = "G. Post-operative HPA axis recovery (6 months)") +
  theme_cs
...
p8 <- all_scens %>%
  ...
  geom_line(linewidth = 1.0) +
  scale_color_manual(values = cols_scen) +
  labs(x = "Time (days)", y = "GR nuclear activity (%)",
       title = "H. GR nuclear activation kinetics") +
  theme_cs

# Plot 9: Drug PK profiles (all drugs first 5 days)
pk_data <- bind_rows(
  scen2 %>% filter(time <= 120) %>% mutate(Drug = "Pasireotide (ng/mL)", Conc = pas_conc),
  scen3 %>% filter(time <= 120) %>% mutate(Drug = "Ketoconazole (μg/mL)", Conc = keto_conc),
  scen4 %>% filter(time <= 120) %>% mutate(Drug = "Osilodrostat (μg/mL)", Conc = osilo_conc),
  scen5 %>% filter(time <= 120) %>% mutate(Drug = "Mifepristone (μg/mL)", Conc = mife_conc)
)
...
p9 <- pk_data %>%
  ...
  labs(x = "Time (days)", y = "Drug plasma concentration", title = "I. Drug PK profiles (5 days)") +
  theme_cs
...
p10 <- all_scens %>%
  ...
  annotate("text", x = 0.5, y = -0.95, label = "Osteopenia threshold", hjust = 0, size = 3) +
  annotate("text", x = 0.5, y = -2.45, label = "Osteoporosis diagnosis", hjust = 0, size = 3, color = "red") +
  scale_color_manual(values = cols_scen) +
  labs(x = "Time (days)", y = "BMD T-score",
       title = "J. Bone mineral density change") +
  theme_cs
...
fig_main <- (p1 | p2) /
            ...
            (p8 | p10) +
  plot_annotation(
    title = "Cushing's syndrome QSP model — treatment scenario comparison",
...
biomarker_ref <- tribble(
  ...
  "LNSC",                     "< 4",             "> 10",                "nmol/L",
  "Morning serum cortisol",   "6-23",            "> 23 (or cycling)",   "μg/dL",
  "Plasma ACTH",              "10-46",           "40-200 (pit CD)",     "pg/mL",
  "1mg DST serum cortisol",   "< 1.8",           "> 1.8",               "μg/dL",
  "Fasting glucose",          "< 5.6",           "5.6-11.1+",           "mmol/L",
  "BMD T-score",              "> -1.0",           "< -1.0 (often -2+)",  "",
  "Systolic BP",              "< 140",           "140-180+",            "mmHg"
)
