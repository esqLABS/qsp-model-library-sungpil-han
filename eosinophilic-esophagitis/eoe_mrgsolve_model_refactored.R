...
# =============================================================================
# PK/PD REFACTOR (fork sibling; original eoe_mrgsolve_model.R untouched)
# =============================================================================
#  Scope: MEPOLIZUMAB and CENDAKIMAB only -- their PK blocks and their
#  downstream IL-5 / IL-13 effect equations. Budesonide, dupilumab, and every
#  disease equation not driven by mepolizumab/cendakimab concentration are
#  byte-for-byte identical to eoe_mrgsolve_model.R.
#
#  Archetype: for BOTH compounds, the original already isolates PK into its
#  own depot + single "central" compartment, with NO peripheral compartment,
#  and (like tocilizumab in sepsis/sep_refactor_notes.md and the antiviral
#  compound in dengue/denv_refactor_notes.md) that central compartment is
#  integrated directly AS a concentration (mg/L), not as an amount that PD
#  divides by volume: dxdt_MEPO_C's/dxdt_CENDA_C's own input term divides by
#  Vd on the way in, and their elimination term is (CL/Vd)*state, a first-
#  order rate on the concentration state itself. This is the guide's
#  Archetype 3 minus the peripheral compartment, combined with Archetype 1's
#  "compartment IS concentration" behavior -- kept exactly as the original
#  has it, not forced into a plain amount/V1 division.
#    - Renamed to convention: MEPO_SC/MEPO_C/ka_mepo/F_mepo/CL_mepo/Vd_mepo ->
#      GUT_MEPO/CENT_MEPO/KA_MEPO/F_MEPO/CL_MEPO/V1_MEPO;
#      CENDA_GUT/CENDA_C/ka_cenda/F_cenda/CL_cenda/Vd_cenda ->
#      GUT_CENDA/CENT_CENDA/KA_CENDA/F_CENDA/CL_CENDA/V1_CENDA. Same
#      compartment POSITIONS in $CMT (6th/7th and 8th/9th respectively), so
#      the numeric `cmt=6`/`cmt=8` dosing already used by this file's own
#      build_events() is unaffected -- verified below.
#    - C_MEPO = CENT_MEPO and C_CENDA = CENT_CENDA, both undivided (not
#      C/V1), for exact numerical equivalence with the originals' Inh_mepo_IL5
#      / Inh_cenda_IL13.
#    - EFFECT_MEPO / EFFECT_CENDA are RENAMES of the originals' Inh_mepo_IL5 /
#      Inh_cenda_IL13, not refits: both original terms
#      (Emax_mepo_IL5*MEPO_C/(MEPO_C+IC50_mepo_IL5) and
#       Emax_cenda_IL13*CENDA_C/(CENDA_C+IC50_cenda_IL13)) are already exact
#      Hill ratios with an implicit gamma=1, so EMAX_/EC50_ carry the
#      originals' exact values and GAMMA_MEPO=GAMMA_CENDA=1 are added
#      explicitly. EFFECT_MEPO/EFFECT_CENDA replace Inh_mepo_IL5/
#      Inh_cenda_IL13 at every downstream use site (IL5_effective, dxdt_IL5,
#      IL13_signal, EPBAR_ss, dxdt_IL13, dxdt_FIBRO) -- those equations
#      themselves are untouched, only the aliased inputs feeding them.
#    - C_MEPO/EFFECT_MEPO/C_CENDA/EFFECT_CENDA are state-dependent quantities
#      computed in $MAIN (not $PARAM -- a fixed parameter cannot depend on
#      compartment state); added to $TABLE/$CAPTURE (additive only) so they
#      are discoverable as outputs via the qspserver mrgsolve_api
#      /model_manifest and /run_simulation endpoints, per
#      FORK_WORKFLOW_GUIDE.md's qspserver compatibility requirements and the
#      precedent set in thyroid-eye-disease/ted_refactor_notes.md and
#      dengue/denv_refactor_notes.md.
#
#  Verified against the original (via the qspserver mrgsolve_api container)
#  on the original file's own "mepolizumab" and "cendakimab" build_events()
#  scenarios (300 mg SC q28d into cmt=6; 160 mg PO QD into cmt=8), end=364
#  days, delta=7: exact match, max abs/rel deviation 0.0 across all 35
#  shared $CAPTURE/compartment outputs at every timepoint. See
#  eoe_refactor_notes.md for the full verification record and two logged
#  pre-existing upstream defects (not fixed here -- see
...
// ---- Mepolizumab PK/PD (renamed to fork PK/PD convention) ----
KA_MEPO    : 0.34  : Mepolizumab SC absorption (1/day) [was ka_mepo]
F_MEPO     : 0.81  : Mepolizumab SC bioavailability (fraction) [was F_mepo]
CL_MEPO    : 0.28  : Mepolizumab clearance (L/day) [was CL_mepo]
V1_MEPO    : 3.6   : Mepolizumab volume of distribution (L) [was Vd_mepo]
EMAX_MEPO  : 0.95  : Max mepolizumab neutralization of IL-5 [was Emax_mepo_IL5]
EC50_MEPO  : 1.0   : Mepolizumab EC50 for IL-5 blockade (mg/L) [was IC50_mepo_IL5]
GAMMA_MEPO : 1     : Mepolizumab Hill coefficient [new; original had no explicit Hill exponent, i.e. gamma=1]

// ---- Cendakimab PK/PD (renamed to fork PK/PD convention) ----
KA_CENDA    : 14.4  : Cendakimab oral absorption rate (1/day) [was ka_cenda]
F_CENDA     : 0.35  : Cendakimab bioavailability (fraction) [was F_cenda]
CL_CENDA    : 360   : Cendakimab apparent clearance CL/F (L/day) [was CL_cenda]
V1_CENDA    : 100   : Cendakimab apparent volume Vd/F (L) [was Vd_cenda]
EMAX_CENDA  : 0.90  : Max cendakimab blockade of IL-13 [was Emax_cenda_IL13]
EC50_CENDA  : 0.05  : Cendakimab EC50 for IL-13 blockade (mg/L) [was IC50_cenda_IL13]
GAMMA_CENDA : 1     : Cendakimab Hill coefficient [new; original had no explicit Hill exponent, i.e. gamma=1]
...
GUT_MEPO  : Mepolizumab SC depot (mg) [was MEPO_SC]
CENT_MEPO : Mepolizumab central (mg/L) [was MEPO_C]
GUT_CENDA : Cendakimab gut depot (mg) [was CENDA_GUT]
CENT_CENDA: Cendakimab central (mg/L) [was CENDA_C]
...
GUT_MEPO  = 0     : Initial mepolizumab SC (mg)
CENT_MEPO = 0     : Initial mepolizumab central (mg/L)
GUT_CENDA= 0     : Initial cendakimab gut (mg)
CENT_CENDA = 0     : Initial cendakimab central (mg/L)
...
dxdt_GUT_MEPO  = -KA_MEPO * GUT_MEPO;
dxdt_CENT_MEPO = KA_MEPO * F_MEPO * GUT_MEPO / V1_MEPO
                 - (CL_MEPO / V1_MEPO) * CENT_MEPO;

// --- Cendakimab PK (oral 1-compartment, apparent params) ---
dxdt_GUT_CENDA  = -KA_CENDA * GUT_CENDA;
dxdt_CENT_CENDA = KA_CENDA * F_CENDA * GUT_CENDA / V1_CENDA
                  - (CL_CENDA / V1_CENDA) * CENT_CENDA;
...
double MEPO_TROUGH = CENT_MEPO;

// Cendakimab Cmax (proxy)
double CENDA_CONC = CENT_CENDA;
...
double C_MEPO_OUT = C_MEPO;
double EFFECT_MEPO_OUT = EFFECT_MEPO;
double C_CENDA_OUT = C_CENDA;
double EFFECT_CENDA_OUT = EFFECT_CENDA;
...
C_MEPO_OUT EFFECT_MEPO_OUT C_CENDA_OUT EFFECT_CENDA_OUT
