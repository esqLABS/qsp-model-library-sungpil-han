...
| Deliverable | File |
|--------|------|
| 🗺️ Mechanistic map (148 nodes · 18 clusters · 224 edges) | [`hsph_qsp_model_en.dot`](hsph_qsp_model_en.dot) · [SVG](hsph_qsp_model_en.svg) · [PNG](hsph_qsp_model_en.png) |
| 📐 Geometry kernel (closed-form, self-verifying) | [`hsph_geometry.py`](hsph_geometry.py) |
| ⚙️ mrgsolve model (72 ODEs) | [`hsph_mrgsolve_model_en.R`](hsph_mrgsolve_model_en.R) |
| 🐍 The reference implementation that was actually run (the model of record) | [`hsph_python_reference.py`](hsph_python_reference.py) |
| 🎯 Calibration ledger | [`hsph_calibrate.py`](hsph_calibrate.py) · [`hsph_calibration.json`](hsph_calibration.json) · [`calib.log`](calib.log) |
| 🔬 Full run output of all 18 analysis sections | [`hsph_analysis.py`](hsph_analysis.py) → [`hsph_reference_output.txt`](hsph_reference_output.txt) |
| 📊 Shiny dashboard (14 tabs) | [`hsph_shiny_app_en.R`](hsph_shiny_app_en.R) |
| 📚 References (115 papers, live PubMed lookup) | [`hsph_references_en.md`](hsph_references_en.md) · [`mkrefs_en.py`](mkrefs_en.py) |
...
- **There was no R toolchain in the environment.** `hsph_mrgsolve_model_en.R` and
  `hsph_shiny_app_en.R` **were not run.** The model of record is
...
python3 mkrefs_en.py --refresh         # re-query the references from PubMed
dot -Tsvg hsph_qsp_model_en.dot -o hsph_qsp_model_en.svg
dot -Tpng -Gdpi=150 hsph_qsp_model_en.dot -o hsph_qsp_model_en.png
```
...
shiny::runApp("hsph_shiny_app_en.R")
```
