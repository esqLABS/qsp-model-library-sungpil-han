#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Build hsph_references_en.md from PubMed.
...
def main():
    ...
    out = []
    out.append("# Hereditary Spherocytosis — references\n")
    out.append("> **Every entry was queried live through the NCBI E-utilities("
               "`esearch` + `esummary`, `sort=relevance`).** Not one of the "
               "titles, journals, years, authors or PMIDs was written from "
               "memory. The **intent** given for each entry states what the "
               "model took from that paper, and is printed alongside it so that "
               "any mismatch between the search result and the intent is plain.\n")
    ...
    for sec, items in secs.items():
        ...
        for intent, q in items:
            ...
            body.append(
                ...
                "  <sub>Role in the model: %s</sub>\n"
                % (title, alist, jr, yr, pid, pid, intent))
    out.append("**%d papers** organised into %d sections.\n"
               % (n_tot, len(secs)))
    out.extend(body)
    open("hsph_references_en.md", "w").write("".join(out))
    print("wrote hsph_references_en.md with %d resolved references" % n_tot)
