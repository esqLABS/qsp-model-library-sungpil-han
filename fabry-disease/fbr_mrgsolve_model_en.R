...
# Disease  : Fabry Disease (α-Galactosidase A Deficiency)
...
s1 <- mod %>%
  ...
  mrgsim(data = s1_data,
         ...
         carry_out = "evid") %>%
  as.data.frame() %>%
  mutate(Scenario = "S1: Natural history (untreated)", Year = time/365)
...
s2 <- mod %>%
  ...
  as.data.frame() %>%
  mutate(Scenario = "S2: Agalsidase beta 1mg/kg Q2W", Year = time/365)
...
s3 <- mod %>%
  ...
  as.data.frame() %>%
  mutate(Scenario = "S3: Agalsidase alfa 0.2mg/kg Q2W", Year = time/365)
...
s4 <- mod %>%
  ...
  as.data.frame() %>%
  mutate(Scenario = "S4: Migalastat 150mg QOD (chaperone)", Year = time/365)
...
s5 <- mod %>%
  ...
  as.data.frame() %>%
  mutate(Scenario = "S5: Pegunigalsidase alfa 1mg/kg Q4W", Year = time/365)
...
s6 <- mod %>%
  ...
  as.data.frame() %>%
  mutate(Scenario = "S6: ERT + lucerastat 1000mg TID combination", Year = time/365)
...
scenario_colors <- c(
  "S1: Natural history (untreated)"             = "#616161",
  "S2: Agalsidase beta 1mg/kg Q2W"              = "#1565C0",
  "S3: Agalsidase alfa 0.2mg/kg Q2W"            = "#0288D1",
  "S4: Migalastat 150mg QOD (chaperone)"        = "#2E7D32",
  "S5: Pegunigalsidase alfa 1mg/kg Q4W"         = "#7B1FA2",
  "S6: ERT + lucerastat 1000mg TID combination" = "#E65100"
)
...
p1 <- all_scenarios %>%
  ...
  labs(title = "Plasma Lyso-Gb3 (core Fabry disease biomarker)",
       x = "Years", y = "Lyso-Gb3 (μg/L)") +
  geom_hline(yintercept = 2.0, linetype = "dashed", color = "gray50") +
  annotate("text", x = 4.5, y = 2.5, label = "Upper limit of normal (<2 μg/L)",
...
p2 <- all_scenarios %>%
  ...
  geom_line(size = 1.2) +
  scale_color_manual(values = scenario_colors) +
  labs(title = "eGFR trajectory (renal function preservation)",
       x = "Years", y = "eGFR (mL/min/1.73m²)") +
  geom_hline(yintercept = 60, linetype = "dashed", color = "red") +
  annotate("text", x = 4.5, y = 62, label = "CKD G3 boundary (60)",
           size = 3.5, color = "red") +
  theme_fabry
print(p2)
...
p3 <- all_scenarios %>%
  ...
  geom_line(size = 1.2) +
  scale_color_manual(values = scenario_colors) +
  labs(title = "LVMi — left ventricular mass index (reversal of cardiac hypertrophy)",
       x = "Years", y = "LVMi (g/m²)") +
  geom_hline(yintercept = 115, linetype = "dashed", color = "blue") +
  annotate("text", x = 4.5, y = 118, label = "Male upper limit of normal (115 g/m²)",
           size = 3.5, color = "blue") +
  theme_fabry
print(p3)
...
p4 <- all_scenarios %>%
  ...
  scale_color_manual(values = scenario_colors) +
  ylim(0, 10) +
  labs(title = "Neuropathic pain (BPI-SF, 0–10)",
       x = "Years", y = "Pain score (0–10)") +
  theme_fabry
print(p4)
...
p5 <- all_scenarios %>%
  ...
  geom_line(size = 1.1) +
  scale_color_manual(values = scenario_colors) +
  labs(title = "α-Galactosidase A enzyme activity (first 6 months)",
       x = "Days", y = "α-Gal A (nmol/h/mg)") +
  theme_fabry
print(p5)
...
p_combined <- (p1 + p2) / (p3 + p4) +
  plot_annotation(
    title = "Fabry disease QSP model: treatment scenario comparison (5-year simulation)",
    subtitle = "ERT vs migalastat chaperone vs SRT combination — composite renal · cardiac · neurological endpoints",
