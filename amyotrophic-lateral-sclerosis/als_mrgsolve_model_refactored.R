## =============================================================================
## ALS (Amyotrophic Lateral Sclerosis) — QSP mrgsolve Model — REFACTORED
## =============================================================================
## Refactor sibling of als_mrgsolve_model.R (never edited — see fork workflow
## guide). Four compounds isolated to the pluggable-PK naming convention:
...
##                       C_TOF exposes the CSF concentration, NOT plasma —
##                       that is what the original's own EC50_TOF effect term
##                       reads (Ccsf_TOF). See als_refactor_notes.md.
##   Phenylbutyrate/AMX0035 (PB) — archetype 3 minus peripheral (depot+central);
##                       the original's own compartment was literally named
##                       "C_PB" (an mg AMOUNT) which collided with the naming
##                       convention's own token for the exposed CONCENTRATION —
##                       renamed to CENT_PB to free up C_PB for its intended
##                       meaning. See als_refactor_notes.md.
## All PK/PD parameter VALUES are copied verbatim from the original; only PK
## structure/naming and the Hill-interface expression are reorganized (Riluzole,
## Edaravone, Tofersen all already used a plain Emax*C/(EC50+C) ratio in the
## original — pure renames, gamma=1, not refits). The original's two pre-
## existing mrgsolve 2.0.1 build defects (unrelated to any compound's own PK)
## are fixed syntax-only here — see als_refactor_notes.md for the full account
...
## 2. Compile the model
## -----------------------------------------------------------------------------
als_mod <- mcode("als_qsp_refactored", als_refactored_code, quiet = TRUE)
...
## -----------------------------------------------------------------------------
## 3. Helper: build dosing events (compartment names renamed to the pluggable-
##    PK convention; the schedule logic itself is unchanged from the original)
## -----------------------------------------------------------------------------
...
p_alsfrs <- ggplot(results,
  ...
  scale_linetype_manual(values = c("solid", "longdash", "dashed",
                        ...
                        labels = sc_labels) +
  labs(
    title    = "ALSFRS-R Progression — ALS QSP Model (refactored)",
...
## --- Plot 5: Riluzole PK (BID, first 7 days) ---
p_pk_ril <- results %>%
  filter(scenario == "riluzole", time_days <= 7) %>%
  ggplot(aes(x = time_days * 24, y = C_RIL)) +
  geom_line(color = "#1976D2", size = 1) +
  geom_hline(yintercept = 0.5, linetype = "dashed", color = "red") +
  annotate("text", x = 5, y = 0.55, label = "EC50 = 0.5 μg/mL",
           hjust = 0, size = 3, color = "red") +
  labs(
    title = "Riluzole PK — First 7 Days (C_RIL)",
    x     = "Time (h)",
    y     = "Riluzole Cp (μg/mL)"
  ) +
  theme_bw(base_size = 12)
...
if (requireNamespace("gridExtra", quietly = TRUE)) {
  ...
  ggsave("als_qsp_results_refactored.pdf", g, width = 16, height = 18)
  cat("Plots saved to als_qsp_results_refactored.pdf\n")
} else {
  print(p_alsfrs)
}
...
## -----------------------------------------------------------------------------
pk_table <- tribble(
  ~Drug,        ~Route,   ~F_pct, ~t_half_h, ~Vd_L,  ~CL_Lh, ~Dose,          ~EC50_Emax,
  "Riluzole",   "PO BID", 60,     12,         357,    28,     "50 mg q12h",   "EC50_RIL=0.5 μg/mL, EMAX_RIL=60%",
  "Edaravone",  "IV/PO",  99,     4.5,        120,    18,     "60 mg/day IV", "EC50_EDA=1.2 μg/mL, EMAX_EDA=70%",
  "Tofersen",   "SC q4w", NA,     168,        20,     0.5,    "100 mg q4w",   "EC50_TOF=0.1 μg/mL CSF, EMAX_TOF=80%",
  "PB (AMX)",   "PO BID", 85,     3.0,        50,     12,     "3g q12h",      "EC50_PB=50 μmol/L, EMAX_PB_MITO=30%"
)
...
cat("\n✓ ALS QSP mrgsolve model (refactored) run complete.\n")
