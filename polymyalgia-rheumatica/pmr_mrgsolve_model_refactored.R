...
## =============================================================================
##
## NOTE (fork refactor): This is `pmr_mrgsolve_model_refactored.R`, a sibling
## of the original `pmr_mrgsolve_model.R`. ONLY tocilizumab's PK block and its
## downstream disease-effect interface have been rewritten, to the pluggable
## CL/Q/V + named-Hill-interface convention described in
## `FORK_WORKFLOW_GUIDE.md` Part 2. Every other compound (prednisolone), every
## disease equation not driven by TCZ, and all scenario/simulation/plotting
## code below are byte-for-byte copies of the original. See
## `pmr_refactor_notes.md` for the archetype determination, the parameter
## conversion applied, and verification results.
## =============================================================================
...
// ── Tocilizumab PK (SC) ── [REFACTORED to canonical CL/Q/V naming; see
// pmr_refactor_notes.md. Values are unchanged from the original.] ──────────
...
KINT_TCZ : 0.003  : Internalization rate TCZ:mIL-6R complex (1/h) [renamed from KINT]
KON_TCZ  : 0.45   : TCZ association rate constant (L/nmol/h) [renamed from KON; unused, see notes]
KOFF_TCZ : 0.00045: TCZ dissociation rate constant (1/h) [renamed from KOFF; unused, see notes]
// Note: Kd = KOFF_TCZ/KON_TCZ ~ 1 nM (high-affinity IL-6R binding)
...
GAMMA_TCZ : 1     : Hill coefficient, TCZ IL-6R occupancy (fixed; original had none)
...
GAMMA_TCZ_PMRAS : 1   : Hill coefficient, TCZ PMR-AS effect (fixed; original had none)
...
// Tocilizumab [REFACTORED: DEPOT_TCZ renamed GUT_TCZ per naming convention;
// same position (4th compartment), so numeric dosing (cmt=4) is unaffected]
GUT_TCZ    : Tocilizumab SC depot (mg)
...
// ── Tocilizumab PK ────────────────────────────────────────────
// [REFACTORED to canonical CL/Q/V parameterization — Archetype 3 (depot +
// central + peripheral, linear elimination), per FORK_WORKFLOW_GUIDE.md.
// The original expressed the same dynamics via derived micro-constants
// (k12_T = Q_TCZ/V1_TCZ, k21_T = Q_TCZ/V2_TCZ, kel_T = CL_TCZ/V1_TCZ); the
// direct CL/Q/V form below is algebraically identical (see
// pmr_refactor_notes.md for the derivation and verification). kel_TMDD is
// kept verbatim: it is NOT receptor-binding TMDD (the original never
// instantiates REC_FREE/COMPLEX compartments — KON/KOFF are declared but
// unused dead parameters), but a disease-state-coupled extra clearance term
// (a function of SIL6R). Removing it would change model behaviour, so the
// coupling is preserved exactly as written.]
double C_TCZ    = CENT_TCZ / V1_TCZ / 148.0 * 1e6; // nM — the exposed TCZ concentration (PD-facing)
double kel_TMDD = KINT_TCZ * SIL6R / (SIL6R_BASE + SIL6R); // simplified, disease-state-coupled
...
// [REFACTORED: TCZ_occ_sR/inh_IL6_TCZ renamed to the Hill-interface
// convention (OCC_TCZ / EFFECT_TCZ), reading C_TCZ instead of Cp_TCZnM.
// Same formula, same parameter values — a rename, not a refit.]
double OCC_TCZ      = (C_TCZ > 0) ? pow(C_TCZ, GAMMA_TCZ) / (pow(EC50_TCZ, GAMMA_TCZ) + pow(C_TCZ, GAMMA_TCZ)) : 0.0;
double EFFECT_TCZ   = EMAX_TCZ * OCC_TCZ; // max 85% IL-6 signaling blockade
...
double sIL6R_stim = 1.0 + 0.4 * OCC_TCZ;
...
// [REFACTORED: eff_TCZ_PMRAS renamed EFFECT_TCZ_PMRAS, reading C_TCZ and an
// explicit GAMMA_TCZ_PMRAS=1 instead of Cp_TCZnM — a rename, not a refit.]
double EFFECT_TCZ_PMRAS = (EMAX_TCZ_PMRAS * pow(C_TCZ, GAMMA_TCZ_PMRAS)) / (pow(EC50_TCZ_PMRAS, GAMMA_TCZ_PMRAS) + pow(C_TCZ, GAMMA_TCZ_PMRAS));
double combined_eff = 1 - (1 - eff_GC_PMRAS) * (1 - EFFECT_TCZ_PMRAS);
