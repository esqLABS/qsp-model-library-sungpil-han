...
CL_AUG         = 0.052   // Elimination rate from central compartment (per day)
K12_AUG        = 0.032   // Central → peripheral transfer (per day)
K21_AUG        = 0.022   // Peripheral → central transfer (per day)
V1_AUG         = 3.76    // Central Vd (L/kg) [disclosed-dead: unused, see notes]
...
F_AUG          = 1.00    // IV bioavailability [disclosed-dead: unused, see notes]
KA_AUG         = 9999    // IV bolus (rate → infinity) [disclosed-dead: unused, see notes]
...
CL_NEI         = 2.40    // NE inhibitor elimination (t1/2 ~7h; CL/F)
EMAX_NEI       = 0.75    // Max NE suppression
EC50_NEI       = 0.18    // EC50 (µg/mL normalized)
GAMMA_NEI      = 1       // Hill coefficient [math-implied =1; original had no explicit exponent]
...
CENT_AUG     = 0.0    // Augmentation drug central (= C_AUG, itself the concentration)
PERI_AUG     = 0.0    // Augmentation drug peripheral
GUT_NEI      = 0.0    // NE inhibitor absorption compartment
CENT_NEI     = 0.0    // NE inhibitor central (= C_NEI, itself the concentration)
...
dxdt_PERI_AUG =  K12_AUG*CENT_AUG - K21_AUG*PERI_AUG ;

// ── 2. NE INHIBITOR PK (1-compartment with absorption) ───────
dxdt_GUT_NEI = -KA_NEI * GUT_NEI ;
dxdt_CENT_NEI = KA_NEI * GUT_NEI - CL_NEI * CENT_NEI ;
// CENT_NEI is itself the concentration, no volume in the original.
double C_NEI = CENT_NEI ;   // exposed concentration = compartment itself
double EFFECT_NEI = EMAX_NEI * pow(C_NEI, GAMMA_NEI)
                     / (pow(EC50_NEI, GAMMA_NEI) + pow(C_NEI, GAMMA_NEI)) ;
...
// Augmentation adds external M-AAT (already in CENT_AUG mg/dL)
double AAT_total_input = ZAAT_endogenous + Gene_MAAT + CENT_AUG ;
...
double NE_inhib_total = AAT_inhib_eff + (1.0 - AAT_inhib_eff) * EFFECT_NEI ;
...
aug_dose <- function(start = 0, end = sim_end, interval = 7, amt = 55) {
  ...
  ev(time = times, amt = amt, cmt = "CENT_AUG", rate = -2)
}
...
nei_dose <- function(start = 0, end = sim_end, interval = 0.5, amt = 0.30) {
  times <- seq(start, end, by = interval)
  ev(time = times, amt = amt, cmt = "GUT_NEI")
}
