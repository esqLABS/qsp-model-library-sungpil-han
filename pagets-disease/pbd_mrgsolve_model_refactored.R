...
# pagets-disease/pbd_refactor_notes.md for full detail):
#   Only Calcitonin (CTN), Denosumab (DMB), and Zoledronic Acid (ZA) PK
#   blocks and their interfaces into the disease equations were touched.
#   Alendronate (ALN) — which shares ZA's distribution compartments via its
#   own ka_ALN/F_ALN — is byte-for-byte identical in structure/behavior;
#   only the compartment name it doses/flows into was mechanically updated
#   (ZA_cen -> CENT_ZA) since that compartment itself was renamed.
#   - ZA_cen/ZA_per/ZA_bon/Vc_ZA/Vp_ZA/k_bon_ZA renamed to convention:
#     CENT_ZA/PERI_ZA/BON_ZA/V1_ZA/V2_ZA/KBON_ZA. Archetype 2 (2-cpt linear)
#     + a bespoke irreversible bone-trap sink (BON_ZA), preserved exactly.
#     ZA_abs (Alendronate's own oral depot) left untouched by name.
#   - CTN_abs/CTN_cen/Vc_CTN/ka_CTN renamed to GUT_CTN/CENT_CTN/V1_CTN/
#     KA_CTN. Archetype 3 without a peripheral compartment (depot+central).
#   - DMB_abs/DMB_cen/DMB_per/Vc_DMB/Vp_DMB/ka_DMB renamed to GUT_DMB/
#     CENT_DMB/PERI_DMB/V1_DMB/V2_DMB/KA_DMB. Archetype 3 + a bespoke
#     target-binding sink (RANKL neutralization) preserved exactly; NOT
#     full TMDD (no complex/RTOT compartment pair in the original).
#   - Emax_ZA_OC/IC50_ZA_OC and Emax_CTN_OC/IC50_CTN_OC renamed to
#     EMAX_ZA/EC50_ZA and EMAX_CTN/EC50_CTN; both already exactly
#     Hill-shaped (gamma=1) in the original, so this is a rename, not a
#     fit. GAMMA_ZA=1/GAMMA_CTN=1 added per the guide's Hill-interface
#     convention. EFFECT_DMB is bespoke (see notes): the original never
#     computes a bounded Emax-Hill effect for DMB, only an unbounded
#     mass-action RANKL-binding rate term, so EFFECT_DMB = KBIND_DMB*C_DMB
#     (a pseudo-first-order rate constant, not a 0-1 fraction) is exposed
#     faithfully instead of forcing a Hill shape the original doesn't have.
#   - F_ZA/F_CTN/F_DMB and KDISS_DMB (was kdiss_DMB) are declared but
#     unused anywhere in the original's $ODE/$MAIN; preserved as unused,
#     not fixed, not removed.
#   - $CAPTURE fix (build-compat, non-numeric): the original lists 15
#     compartment names in $CAPTURE, which mrgsolve 2.0.1 rejects outright
#     ("compartment should not be in $CAPTURE") — confirmed on the
#     untouched original via the qspserver mrgsolve_api. Removed (mrgsolve
#     always includes every compartment in its output regardless of
#     $CAPTURE), and C_ZA/EFFECT_ZA/C_CTN/EFFECT_CTN/C_DMB/EFFECT_DMB added
#     so the new interface variables are discoverable in output. Logged as
#     a new entry in translations/UPSTREAM_ISSUES.md.
#
# Verified against the original (via the qspserver mrgsolve_api service,
# original's own build-compat-patched-only-for-$CAPTURE copy) on the
# original file's own ZA IV, CTN SC, and DMB SC dosing scenarios: see
# pbd_refactor_notes.md for the full verification result.
# =============================================================================
...
$PARAM
// ---- Zoledronic Acid (ZA) PK Parameters [REFACTORED: renamed to convention] ----
CL_ZA    = 0.8      // Clearance (L/h), Cremers 2005 NEJM
V1_ZA    = 18.0     // Central volume (L) [was Vc_ZA]
...
GAMMA_ZA = 1         // no explicit Hill coefficient in the original; fixed at 1 per guide

// ---- Calcitonin (CTN) PK Parameters [REFACTORED: renamed to convention] ----
...
GAMMA_CTN = 1        // no explicit Hill coefficient in the original; fixed at 1 per guide

// ---- Denosumab (DMB) PK Parameters (RANKL MAb) [REFACTORED: renamed to convention] ----
CL_DMB   = 0.00875  // Clearance (L/h) = 0.21 L/day, Keizer 2010
V1_DMB   = 2.86     // Central volume (L) [was Vc_DMB]
...
// (`C_ZA = CENT_ZA;`) is a compile error ("assignment of read-only
// reference"), confirmed via the qspserver mrgsolve_api. This is the same
...
// [REFACTORED] IC50_ZA_OC/Emax_ZA_OC and IC50_CTN_OC/Emax_CTN_OC relocated
// and renamed to EC50_ZA/EMAX_ZA and EC50_CTN/EMAX_CTN in the ZA/CTN PK
// $PARAM blocks above (with GAMMA_ZA/GAMMA_CTN added), per the Hill
// interface naming convention -- not duplicated here.
...
$CMT
// Drug PK compartments [REFACTORED: renamed to convention]
ZA_abs       // ZA absorption depot (for oral/SC formulations; not used IV
             // by ZA itself) -- shared entry point for the out-of-scope
             // Alendronate scenario (ka_ALN/F_ALN, neither a ZA-owned
             // parameter); left untouched by name, see refactor notes
CENT_ZA      // ZA central compartment (ng/mL) [was ZA_cen]
PERI_ZA      // ZA peripheral compartment (ng/mL) [was ZA_per]
BON_ZA       // ZA bone-bound (irreversible trap; ng/g bone) [was ZA_bon]
GUT_CTN      // Calcitonin SC depot (IU equiv, pg/mL scale) [was CTN_abs]
CENT_CTN     // Calcitonin plasma (pg/mL) [was CTN_cen]
GUT_DMB      // Denosumab SC depot (μg) [was DMB_abs]
CENT_DMB     // Denosumab central (μg/mL) [was DMB_cen]
PERI_DMB     // Denosumab peripheral (μg/mL) [was DMB_per]
// Signaling
...
// --- Zoledronic Acid (ZA) [REFACTORED: renamed to convention; Archetype 2
...
double ke_ZA  = CL_ZA / V1_ZA;

dxdt_ZA_abs = -ka_ALN * ZA_abs;                        // oral depot (alendronate; untouched)
dxdt_CENT_ZA = ka_ALN * ZA_abs
              - ke_ZA  * CENT_ZA
              - k12_ZA * CENT_ZA
              + k21_ZA * (V2_ZA / V1_ZA) * PERI_ZA
              - KBON_ZA * CENT_ZA;                      // bone trapping (irreversible)
dxdt_PERI_ZA = k12_ZA * CENT_ZA - k21_ZA * (V2_ZA / V1_ZA) * PERI_ZA;
dxdt_BON_ZA = KBON_ZA * CENT_ZA * V1_ZA;              // cumulative bone binding (ng/g)

double C_ZA = CENT_ZA;   // exposed concentration -- CENT_ZA is already concentration-
                  // space (ng/mL) in the original, so this is a pass-through

// --- Calcitonin (CTN) SC [REFACTORED: renamed to convention; Archetype 3
//     without a peripheral compartment (depot + central only)] ---
// Dose in IU, converted: 1 IU salmon CTN ~ 8 pg/mL at Cmax
// Model in pg/mL equivalent plasma concentration
double ke_CTN = CL_CTN / V1_CTN;
dxdt_GUT_CTN = -KA_CTN * GUT_CTN;
dxdt_CENT_CTN = KA_CTN * GUT_CTN - ke_CTN * CENT_CTN;

double C_CTN = CENT_CTN;  // exposed concentration -- pass-through, already pg/mL

// --- Denosumab (DMB) SC [REFACTORED: renamed to convention; Archetype 3
//     (depot + central + peripheral) + a bespoke target-binding sink term
//     (RANKL neutralization), preserved exactly -- see refactor notes for
//     why this is not modeled as full Archetype-4 TMDD] ---
double k12_DMB = Q_DMB / V1_DMB;
double k21_DMB = Q_DMB / V2_DMB;
double ke_DMB  = CL_DMB / V1_DMB;
...
// This is the same "kbind_DMB * DMB_cen" product the original computed
// twice under two different expressions -- exact rename/factoring, not a
// refit; see refactor notes.
...
dxdt_CENT_DMB = KA_DMB * (GUT_DMB / V1_DMB)
               - ke_DMB  * CENT_DMB
               - k12_DMB * CENT_DMB
               + k21_DMB * (V2_DMB / V1_DMB) * PERI_DMB
               - EFFECT_DMB * RANKL_free;     // RANKL binding reduces DMB
dxdt_PERI_DMB = k12_DMB * CENT_DMB - k21_DMB * (V2_DMB / V1_DMB) * PERI_DMB;
...
// ZA inhibits OC apoptosis (Emax model) [REFACTORED: Hill interface rename,
// not a fit -- already exactly this shape (gamma=1) in the original]
double EFFECT_ZA = EMAX_ZA * pow(C_ZA, GAMMA_ZA) / (pow(EC50_ZA, GAMMA_ZA) + pow(C_ZA, GAMMA_ZA));

// CTN inhibits OC directly [REFACTORED: same rename pattern]
double EFFECT_CTN = EMAX_CTN * pow(C_CTN, GAMMA_CTN) / (pow(EC50_CTN, GAMMA_CTN) + pow(C_CTN, GAMMA_CTN));

// Combined OC inhibition (cannot exceed 1) -- dead code in the original:
// computed here but immediately overwritten below by the eff_kdeg_OC
// formulation (never actually read). Preserved verbatim (renamed only);
// removing it would not be a pure rename. See refactor notes.
double OC_inhib_total = 1.0 - EFFECT_ZA - EFFECT_CTN
                        + EFFECT_ZA * EFFECT_CTN; // Bliss independence
...
// Redo: use inhibition as: effective_kdeg = kdeg_OC * (1 + EFFECT_ZA + EFFECT_CTN)
// Using cleaner formulation:
// dxdt_OC already set above — overwrite with cleaner version:
{
  double eff_kdeg_OC = kdeg_OC * (1.0 + 3.0*EFFECT_ZA + 1.5*EFFECT_CTN);
...
double ZA_cen_ngmL  = CENT_ZA;   // was ZA_cen
double CTN_pgmL     = CENT_CTN;  // was CTN_cen
double DMB_ugmL     = CENT_DMB;  // was DMB_cen

$CAPTURE
// [REFACTORED build-compat fix, non-numeric] The original listed 15 $CMT
// compartment names here (ZA_cen ZA_bon CTN_cen DMB_cen RANKL_free OPG_free
// OCpre OC OBpre OB BMD bsALP NTX CTX_s Pain), which mrgsolve 2.0.1 rejects
// ("compartment should not be in $CAPTURE") -- confirmed on the untouched
// original via the qspserver mrgsolve_api. Removed (every compartment still
// appears in mrgsolve output automatically regardless of $CAPTURE); the new
// exposed interface variables are added below. See refactor notes and
// translations/UPSTREAM_ISSUES.md.
RANKL_OPG_R OC_fold OB_fold pct_bsALP pct_NTX
C_ZA EFFECT_ZA C_CTN EFFECT_CTN C_DMB EFFECT_DMB
'
...
ev_notx <- ev(time = 0, amt = 0, cmt = "CENT_ZA")  # placeholder null event
ev_notx$amt <- 0
...
run_za_iv <- function(model) {
  ...
  ev_za <- ev(time = 0, amt = dose_amt, cmt = "CENT_ZA", evid = 1)
  mrgsim(model,
         events = ev_za,
         init   = pbd_init,
         end    = SIM_HRS,
