...
## dm1_refactor_notes.md for full detail):
##   Only Mexiletine was in scope, per its existing row in
##   driver-patches/data/compound_perturbation_census.md (classified
##   "Redirect concentration (clean single site)"). ASO's own PK/PD block
##   is completely untouched -- no renaming, no restructuring.
##
##   Mexiletine -- Archetype 3 (depot + central + peripheral, linear).
##   MEX_GUT/MEX_CENT/MEX_PERI renamed GUT_MEX/CENT_MEX/PERI_MEX;
##   Ka_mex/F_mex/Vc_mex/Vp_mex/CL_mex/Q_mex/MW_mex/fu_mex renamed
##   KA_MEX/F_MEX/V1_MEX/V2_MEX/CL_MEX/Q_MEX/MW_MEX/FU_MEX (values
##   unchanged). The exposed concentration PD reads is the free plasma
##   concentration (uM) -- Cp_mex_free in the original -- renamed C_MEX;
##   the total (protein-bound+free) concentration is kept as a diagnostic
##   capture, renamed C_MEX_TOTAL (was Cp_mex). Nav1.4 blockade (`nav_block`,
##   already an exact Emax*C^gamma/(EC50^gamma+C^gamma) ratio in the
##   original, with EMAX implicit=1) renamed EFFECT_MEX;
##   IC50_mex_nav/hill_mex_nav renamed EC50_MEX/GAMMA_MEX; EMAX_MEX=1 added
##   explicitly. Rename only, not a refit -- the original was already
##   exactly this shape. `F_mex`/`F_MEX` is declared but was never applied
##   anywhere in the original's own `$ODE` (the absorption term omits it);
##   preserved exactly as-is (not newly applied) to match the original's
##   actual behaviour -- see dm1_refactor_notes.md and
...
##   refactor; logged as translations/UPSTREAM_ISSUES.md #65, not fixed
...
##     1. `$CMT` and `$INIT` jointly redeclared all 19 compartments --
##        `$CMT` removed (its per-compartment documentation folded into a
##        comment above `$MAIN`), `$INIT` alone now declares+initializes
##        every compartment, same order, same 1-based compartment numbers.
##     2. `$PARAM`'s `HOMA_IR_0` collided with mrgsolve's own
##        auto-reserved `<CMT>_0` symbol for the `HOMA_IR` compartment --
##        renamed `HOMA_IR_BASELINE`, one read site in `$MAIN` updated to
##        match.
##   Neither fix changes any number; see verification result below and
##   dm1_refactor_notes.md for full disclosure.
##
##   Verified via the qspserver mrgsolve_api service (`POST /model_manifest`,
##   `POST /run_simulation`) against a no-dosing natural-history check and
##   the original's own mexiletine 300 mg TID (q8h) dosing regimen with
##   MEX_ON=1: every shared `$CAPTURE`d output matched the original EXACTLY
##   (max abs diff 0.0), 30-day verification window (shortened from the
##   original's own 365-day scenario for verification-request convenience
##   only -- no solver step-count issue was encountered).
## ============================================================
...
F_MEX     = 0.85    // Bioavailability (declared but not applied anywhere in
                     // the original ODE; preserved exactly as the original
                     // behaved -- see dm1_refactor_notes.md)
V1_MEX    = 65      // L, central volume (V/F ≈ 76 L)
V2_MEX    = 195     // L, peripheral volume
...
// ---- ASO PK (DYNE-101/Ionis data) -- untouched, out of scope
...
// Nav1.4 blockade by mexiletine (Hill interface; EMAX_MEX=1 implicit in the
// original -- no separate multiplier in front of the pow()/pow() ratio --
// made explicit here per the named-interface convention; rename, not a refit)
EC50_MEX  = 3.0    // μM (therapeutic target range 3–10 μM)
GAMMA_MEX  = 1.8
EMAX_MEX   = 1.0   // Max fractional Nav1.4 blockade (implicit=1 in the original)
...
// Metabolic
HOMA_IR_BASELINE     = 3.5    // DM1 baseline HOMA-IR (normal < 2.5)
k_HOMA_eq     = 0.01
...
// Build-compat fix (syntax-only, non-numeric; see UPSTREAM_ISSUES.md #65
// and dm1_refactor_notes.md): the original had BOTH a `$CMT` block AND an
// `$INIT` block declaring the same 19 compartment names. mrgsolve 2.0.1
// refuses to build any model with duplicated compartment declarations:
//   Error in validObject(.Object) :
//     invalid class "mrgmod" object: Duplicated model names: GUT_MEX
//     CENT_MEX PERI_MEX ASO_PLASMA ASO_MUSCLE ASO_NUCL CUG_FOCI MBNL1_FREE
//     CUGBP1_ACT CLCN1_FETAL SERCA_FETAL INSR_FETAL MYOTONIA GRIP_STR
//     MUSCLE_MASS PR_INT QTc_INT HOMA_IR FVC_PCT
// `$INIT` alone both declares and initializes every compartment (standard
// mrgsolve idiom), so the `$CMT` block is removed here; compartment order
// (and 1-based compartment number) is unchanged, since it now follows the
// order of the (unchanged) `$INIT` list below.
//
// Mexiletine PK (3 compartments): GUT_MEX (mg), CENT_MEX (mg), PERI_MEX (mg)
// ASO PK (3 compartments): ASO_PLASMA (μg), ASO_MUSCLE (μg/g, normalized),
//   ASO_NUCL (active nuclear concentration)
// Disease biology (6 states, normalized 0-1): CUG_FOCI (relative foci
//   burden), MBNL1_FREE (free MBNL1 fraction), CUGBP1_ACT (CUGBP1 relative
//   activity), CLCN1_FETAL / SERCA_FETAL / INSR_FETAL (fetal-isoform
//   fractions)
// Clinical endpoints (6 states): MYOTONIA (VAS 0-10), GRIP_STR (kg),
//   MUSCLE_MASS (kg), PR_INT (ms), QTc_INT (ms), HOMA_IR, FVC_PCT (%)
...
// ---- Nav1.4 blockade by mexiletine (Hill interface; rename, not a refit --
// already this exact shape in the original with EMAX_MEX=1 implicit)
double EFFECT_MEX = MEX_ON * EMAX_MEX * pow(C_MEX, GAMMA_MEX) /
                   (pow(EC50_MEX, GAMMA_MEX) + pow(C_MEX, GAMMA_MEX));
...
double QTc_mex_effect = 15.0 * EFFECT_MEX;  // shortens by up to 15 ms
...
double HOMA_IR_target = HOMA_IR_BASELINE * (1.0 + 1.5 * (INSR_FETAL - INSR_fetal_norm));
...
$ODE
// ============================
// MEXILETINE PK [archetype 3: depot + central + peripheral, linear]
// ============================
...
build_events <- function(mex_on, aso_on, gt_on, end_days = 365) {
  ...
  if (mex_on == 1) {
    ...
    ev_mex <- ev(cmt = "GUT_MEX", amt = mex_dose, time = times)
...
plot_mex_pk <- sim_all %>%
  ...
  ggplot(aes(Day, C_MEX_TOTAL, color = Label)) +
