...
K_INS_EGP = 2.7e-3 // EGP suppression per unit insulin effect-site level (per mU/L) [was kp1]
kp2      = 0.002   // EGP suppression by portal insulin (declared, unused -- see refactor notes)
...
K_EFF_INS = 0.0331 // Insulin effect-compartment rate constant; also reused below
                    // as the Rd sensitivity coefficient -- see refactor notes [was p2U]
...
// ── Insulin PK (compound: INS) ────────────────────────────────
KA1_INS  = 0.003   // Hexamer → dimer absorption rate (SC; min⁻¹) [was ka1]
KA2_INS  = 0.01    // Dimer/monomer → plasma absorption rate (min⁻¹) [was ka2]
CL_INS   = 1.0     // Insulin clearance (L/h) [was CL_ins]
V1_INS   = 3.5     // Central volume (L) [was Vc_ins]
V2_INS   = 2.5     // Peripheral volume (L); declared, not referenced anywhere in
                    // $ODE -- see refactor notes [was Vp_ins]
...
ka_type  = 0.05    // Current bolus absorption rate (declared, not referenced
                    // anywhere in $ODE -- see refactor notes)
...
// (insulin-mediated glucagon suppression coefficient moved to the Insulin PK
// block above as K_INS_GCG -- was kGcg_ins)

// ── Drug: Teplizumab (anti-CD3, compound: TEP) ────────────────
CL_TEP   = 0.3     // Teplizumab clearance (L/day) [was CL_tep]
V1_TEP   = 5.0     // Central "volume" (L) [was Vc_tep]
// MOA: rate of CTL reduction per unit teplizumab concentration
KCTL_TEP = 0.05    // (L/μg/day) [was kCTL_tep]
// MOA: rate of Treg expansion per unit teplizumab concentration
KTREG_TEP= 0.03    // (L/μg/day) [was kTreg_tep]
...
// Insulin PK (compound: INS)
GUT1_INS     // 7. SC depot 1 (hexamer, nmol) [was SC1]
GUT2_INS     // 8. SC depot 2 (dimer/monomer, nmol) [was SC2]
CENT_INS     // 9. Central insulin (mU/L) [was Ic]
PERI_INS     // 10. Peripheral insulin (mU/L) [was Ip]

// Insulin action
EFF_INS      // 11. Remote insulin effect compartment (action on Rd/EGP) [was X_ins]
...
// Teplizumab PK (compound: TEP)
CENT_TEP     // 14. Plasma teplizumab (μg/mL) [was Ctep]
...
double Abs_INS = KA2_INS * GUT2_INS;   // U/h scaled to mU/L via V1_INS [was Ins_from_SC]
dxdt_CENT_INS  =  (Abs_INS * 1000.0 / V1_INS) + (secR / V1_INS)
             - (CL_INS / V1_INS) * C_INS
             - K12_INS * C_INS + K21_INS * PERI_INS
             - APC_infusion * 1000.0 / V1_INS * (-1.0); // APC adds to CENT_INS
// Note: APC infusion sign correction — adds insulin to plasma
dxdt_CENT_INS  =  (Abs_INS * 1000.0 / V1_INS) + (secR / V1_INS)
             + (APC_infusion * 1000.0 / V1_INS)
             - (CL_INS / V1_INS) * C_INS
             - K12_INS * C_INS + K21_INS * PERI_INS;

dxdt_PERI_INS  =  K12_INS * C_INS - K21_INS * PERI_INS;

// ── Remote insulin action (effect compartment) ──────────────
dxdt_EFF_INS = K_EFF_INS * (C_INS - EFF_INS);

// ── Glucagon ─────────────────────────────────────────────────
double hypo_stim = (Gp < 70) ? kGLU_GCG * (70 - Gp) : 0.0;
double EFFECT_INS_GCG = K_INS_GCG * (C_INS / 15.0);  // insulin suppresses glucagon [was: ins_sup = kGcg_ins*(Ic/15.0)]
dxdt_Gcg = kGCG_EGP * (Gcg0 - Gcg) + hypo_stim - EFFECT_INS_GCG * Gcg;
...
// ── Teplizumab PK (IV dosing) (compound: TEP) ────────────────
dxdt_CENT_TEP = -(CL_TEP / V1_TEP) * C_TEP;
...
double C_INS_report          = CENT_INS;
double EFFECT_INS_EGP_report = K_INS_EGP * EFF_INS;
double EFFECT_INS_UP_report  = K_EFF_INS * EFF_INS;
double EFFECT_INS_GCG_report = K_INS_GCG * (CENT_INS / 15.0);
double C_TEP_report          = CENT_TEP;
double EFFECT_TEP_CTL_report  = KCTL_TEP  * CENT_TEP;
double EFFECT_TEP_TREG_report = KTREG_TEP * CENT_TEP;

$CAPTURE
C_INS_report EFFECT_INS_EGP_report EFFECT_INS_UP_report EFFECT_INS_GCG_report
C_TEP_report EFFECT_TEP_CTL_report EFFECT_TEP_TREG_report
...
bolus_event <- function(time_min, dose_U, cmt = "GUT1_INS") {
  ev(cmt = cmt, time = time_min, amt = dose_U * 6, evid = 1)
}

# Teplizumab IV dose (μg/mL · L = μg; 14 day course: 51 μg/kg/day)
tepliz_dose <- function(day, dose_ug = 51 * 70, cmt = "CENT_TEP") {
...
run_MDI <- function(sim_days = 180) {
  ...
  build_day_events <- function(day_offset) {
    ...
    b_ev <- lapply(mt, function(t) bolus_event(t, base_bolus, "GUT2_INS"))
    # Basal glargine 20U at 22:00 (slow GUT1_INS)
    bg <- bolus_event(day_offset * 1440 + 1320, 20, "GUT1_INS")
  ...
  mod %>%
    ...
    init(Bm = 0.05, Gp = 160, CENT_INS = 8) %>%
...
run_CSII <- function(sim_days = 180) {
  ...
  mod %>%
    ...
    init(Bm = 0.05, Gp = 150, CENT_INS = 10) %>%
    mrgsim(events = all_ev, end = sim_days * 1440, delta = 10) %>%
    as.data.frame() %>%
...
run_HCL <- function(sim_days = 180) {
  ...
  ev_meals <- lapply(0:(sim_days - 1), function(d) {
    lapply(meal_times + d * 1440, function(t) {
      list(meal_event(t, 75), bolus_event(t, 5, "GUT2_INS"))  # smaller manual bolus
    })
  })
  ...
  mod %>%
    ...
    init(Bm = 0.05, Gp = 140, CENT_INS = 10) %>%
    mrgsim(events = all_ev, end = sim_days * 1440, delta = 10) %>%
    as.data.frame() %>%
...
run_teplizumab <- function(years = 5) {
  ...
  mod %>%
    ...
    init(Bm = 0.5, CTL = 0.7, Treg = 0.4, Gp = 100, CENT_TEP = 0) %>%
...
run_tepliz_MDI <- function(sim_days = 730) {  # 2 years
  ...
  day_ev <- lapply(0:(sim_days - 1), function(d) {
    ...
    m_ev <- lapply(mt, function(t) meal_event(t, 75))
    b_ev <- lapply(mt, function(t) bolus_event(t, 8, "GUT2_INS"))
    bg   <- bolus_event(d * 1440 + 1320, 18, "GUT1_INS")
    do.call(c, c(m_ev, b_ev, list(bg)))
  })
  ...
  mod %>%
    ...
    init(Bm = 0.15, CTL = 0.8, Gp = 180, CENT_INS = 5, CENT_TEP = 0) %>%
    mrgsim(events = all_ev, end = sim_days * 1440, delta = 10) %>%
    as.data.frame() %>%
