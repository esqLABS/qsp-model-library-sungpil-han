...
## ---------------------------------------------------------------------
##  Refactored sibling of lpa_mrgsolve_model.R. The original is left
##  completely untouched; this file only reorganizes/renames SIX
##  compounds' PK/PD blocks into the guide's pluggable naming convention
##  (GUT_/CENT_/C_/EFFECT_<STEM>), per their existing rows in
##  driver-patches/data/compound_perturbation_census.md:
##    EVO  evolocumab   (anti-PCSK9 mAb)         MUV  muvalaplin (oral)
##    NIA  niacin ER                              OBI  obicetrapib (CETP inhibitor)
##    STA  statin (rosuvastatin-equivalent)       ZIL  ziltivekimab (anti-IL-6 mAb)
##  Pelacarsen (PEL_*, an ASO) and the shared olpasiran/lepodisiran/
##  zerlasiran siRNA block (SIR_*) are OUT OF SCOPE (no census row for
##  either in this file) and are completely untouched.
##
##  No compound here is TMDD or an siRNA/ASO despite the task brief's
##  a-priori expectation for OBI/ZIL -- checked against the actual code,
##  not assumed: in THIS file OBI is obicetrapib (a small-molecule CETP
##  inhibitor) and ZIL is ziltivekimab (a plain-PK anti-IL-6 ligand mAb);
##  the real siRNA/ASO compounds (olpasiran et al., pelacarsen) use the
##  SIR_*/PEL_* prefixes instead and are not among the six requested.
##
##  Every compound's own PK is plain linear depot(+central) disposition
##  (Archetype 1 or the depot+central-no-peripheral variant of Archetype
##  3) -- no peripheral compartment, no receptor-binding ODEs anywhere in
##  any of the six. Full per-compound archetype classification, the
##  complete renaming map, the Hill-interface derivations (all renames or
##  exact algebraic derivations -- no nls() fitting was needed anywhere),
##  and the qspserver mrgsolve_api verification results (max abs diff 0.0
##  across two independent scenarios) are in lpa_refactor_notes.md.
## =====================================================================
...
// muvalaplin (oral assembly inhibitor, cmt 7-8) [refactor: KAMUV->KA_MUV,
// KEMUV->KE_MUV, VMUV->V1_MUV, KIMUV->EC50_MUV; EMAX_MUV/GAMMA_MUV new]
KA_MUV  : 3.0  : Muvalaplin absorption (/day)
KE_MUV  : 1.5  : Muvalaplin elimination (/day)
V1_MUV  : 60.0 : Muvalaplin volume of distribution (L)
EC50_MUV: 0.10 : Muvalaplin EC50 for step-1 assembly blockade (mg/L) [was KIMUV]
EMAX_MUV: 1.0  : Muvalaplin max fractional block of assembly (-) [new; implicit 1 in original 1/(1+C/Ki) form]
GAMMA_MUV: 1   : Muvalaplin Hill coefficient (-) [new; original had none, i.e. implicit 1]
...
V1_EVO      : 3.30 : Evolocumab volume of distribution (L)
K_EVO_PCSK9 : 0.65 : Evolocumab-mediated PCSK9 removal rate constant (per mg/L per day) [was KONE]

// statin (cmt 12-13) [refactor: KASTA->KA_STA, KESTA->KE_STA, VSTA->V1_STA,
// EC50STA->EC50_STA, ESRE->EMAX_STA_LDLR, EPSK->EMAX_STA_PCSK9, ESTA->EMAX_STA_LPA;
// GAMMA_STA new. One shared saturating fraction (EFFECT_STA_FRAC) feeds three
// independent named downstream effects, each its own EMAX -- same pattern as the
// abdominal-aortic-aneurysm statin refactor (EFFECT_STAT_MMP9/NFKB/ROS).]
KA_STA        : 6.0  : Statin absorption (/day)
KE_STA        : 1.4  : Statin elimination (/day)
V1_STA        : 134  : Statin volume of distribution (L)
EC50_STA      : 0.012: Statin conc for half-max effect (mg/L)
GAMMA_STA     : 1    : Statin Hill coefficient (-) [new; original had none, i.e. implicit 1]
EMAX_STA_LDLR : 1.24 : Statin Emax on LDLR synthesis (SREBP-2) (-) [was ESRE]
EMAX_STA_PCSK9: 0.40 : Statin Emax on PCSK9 synthesis (-) [was EPSK]
EMAX_STA_LPA  : 0.30 : Statin Emax on LPA transcription (-) [was ESTA]

// niacin ER (cmt 14) [refactor: KENIA->KE_NIA, VNIA->V1_NIA, EC50NIA->EC50_NIA,
// ENIA->EMAX_NIA_LPA, ENIALDL->EMAX_NIA_LDL; GAMMA_NIA new. One shared saturating
// fraction (EFFECT_NIA_FRAC) feeds two independent named downstream effects.]
KE_NIA        : 1.5  : Niacin elimination (/day)
V1_NIA        : 60.0 : Niacin volume of distribution (L)
EC50_NIA      : 3.0  : Niacin conc for half-max effect (mg/L)
GAMMA_NIA     : 1    : Niacin Hill coefficient (-) [new; original had none, i.e. implicit 1]
EMAX_NIA_LPA  : 0.22 : Niacin Emax on LPA transcription (fractional decrease) (-) [was ENIA]
EMAX_NIA_LDL  : 0.15 : Niacin Emax on LDL particle production (-) [was ENIALDL]

// ziltivekimab (anti-IL-6 ligand mAb; cmt 15-16) [refactor: KAZIL->KA_ZIL,
// KEZIL->KE_ZIL, VZIL->V1_ZIL, IC50Z->EC50_ZIL; GAMMA_ZIL new]
KA_ZIL   : 0.30 : Ziltivekimab SC absorption (/day)
KE_ZIL   : 0.05 : Ziltivekimab elimination (/day)
V1_ZIL   : 4.0  : Ziltivekimab volume of distribution (L)
EC50_ZIL : 0.11 : Ziltivekimab conc for half of IL-6 signalling blocked (mg/L) [was IC50Z]
GAMMA_ZIL: 1    : Ziltivekimab Hill coefficient (-) [new; original had none, i.e. implicit 1]

// obicetrapib (CETP inhibitor, cmt 17) [refactor: KEOBI->KE_OBI, VOBI->V1_OBI,
// EC50OBI->EC50_OBI, ECETPR->EMAX_OBI_LDLR, EOBI->EMAX_OBI_LPA, EHDL->EMAX_OBI_HDL;
// GAMMA_OBI new. One shared saturating fraction (EFFECT_OBI_FRAC) feeds three
// independent named downstream effects.]
KE_OBI        : 0.35 : Obicetrapib elimination (/day)
V1_OBI        : 100  : Obicetrapib volume of distribution (L)
EC50_OBI      : 0.05 : Obicetrapib conc for half-max effect (mg/L)
GAMMA_OBI     : 1    : Obicetrapib Hill coefficient (-) [new; original had none, i.e. implicit 1]
EMAX_OBI_LDLR : 0.74 : Obicetrapib Emax on LDLR synthesis (-) [was ECETPR]
EMAX_OBI_LPA  : 0.23 : Obicetrapib Emax on LPA transcription (fractional decrease) (-) [was EOBI]
EMAX_OBI_HDL  : 1.50 : Obicetrapib Emax on HDL-C (-) [was EHDL]
...
GUT_MUV  : 0    : Muvalaplin gut depot (mg) [cmt 7; was MUV_GU]
CENT_MUV : 0    : Muvalaplin plasma (mg) [cmt 8; was MUV_CE]
GUT_EVO  : 0    : Evolocumab SC depot (mg) [cmt 9; was EVO_SC]
CENT_EVO : 0    : Evolocumab plasma (mg) [cmt 10; was EVO_CE]
PCSK9    : 1    : Free plasma PCSK9 (relative; shared target -- statin drives its synthesis, evolocumab drives its clearance) [cmt 11]
GUT_STA  : 0    : Statin gut (mg) [cmt 12; was STA_GU]
CENT_STA : 0    : Statin plasma (mg) [cmt 13; was STA_CE]
CENT_NIA : 0    : Niacin plasma (mg) [cmt 14; was NIA_CE]
GUT_ZIL  : 0    : Ziltivekimab SC depot (mg) [cmt 15; was ZIL_SC]
CENT_ZIL : 0    : Ziltivekimab plasma (mg) [cmt 16; was ZIL_CE]
CENT_OBI : 0    : Obicetrapib plasma (mg) [cmt 17; was OBI_CE]
...
dxdt_GUT_MUV  = -KA_MUV*GUT_MUV;
dxdt_CENT_MUV =  KA_MUV*GUT_MUV - KE_MUV*CENT_MUV;

dxdt_GUT_EVO  = -KA_EVO*GUT_EVO;
dxdt_CENT_EVO =  KA_EVO*GUT_EVO - KE_EVO*CENT_EVO;

dxdt_GUT_STA  = -KA_STA*GUT_STA;
dxdt_CENT_STA =  KA_STA*GUT_STA - KE_STA*CENT_STA;

dxdt_CENT_NIA = -KE_NIA*CENT_NIA;

dxdt_GUT_ZIL  = -KA_ZIL*GUT_ZIL;
dxdt_CENT_ZIL =  KA_ZIL*GUT_ZIL - KE_ZIL*CENT_ZIL;

dxdt_CENT_OBI = -KE_OBI*CENT_OBI;

// --- drug concentrations / effect fractions --------------------------
double CPELL = PEL_LIV/VLIVP;                       // hepatic ASO (mg/L) [out of scope]
double C_MUV = CENT_MUV/V1_MUV;                     // muvalaplin -- exposed pluggable PK covariate
double C_EVO = CENT_EVO/V1_EVO;                     // evolocumab -- exposed pluggable PK covariate
double C_STA = CENT_STA/V1_STA;                     // statin -- exposed pluggable PK covariate
double C_NIA = CENT_NIA/V1_NIA;                     // niacin -- exposed pluggable PK covariate
double C_ZIL = CENT_ZIL/V1_ZIL;                     // ziltivekimab -- exposed pluggable PK covariate
...
double EFFECT_OBI_FRAC = pow(C_OBI,GAMMA_OBI)/(pow(EC50_OBI,GAMMA_OBI) + pow(C_OBI,GAMMA_OBI));

double EFFECT_STA_PCSK9 = EMAX_STA_PCSK9*EFFECT_STA_FRAC;
double EFFECT_STA_LDLR  = EMAX_STA_LDLR*EFFECT_STA_FRAC;
double EFFECT_STA_LPA   = EMAX_STA_LPA*EFFECT_STA_FRAC;
double EFFECT_NIA_LPA   = EMAX_NIA_LPA*EFFECT_NIA_FRAC;
double EFFECT_NIA_LDL   = EMAX_NIA_LDL*EFFECT_NIA_FRAC;
double EFFECT_OBI_LDLR  = EMAX_OBI_LDLR*EFFECT_OBI_FRAC;
...
// Muvalaplin: original 1/(1+C/Ki) competitive-blockade fraction is an exact Hill
// ratio in disguise (Ki/(Ki+C) == 1 - C/(Ki+C)); EFFECT_MUV pulls out the Hill form.
double EFFECT_MUV = EMAX_MUV*pow(C_MUV,GAMMA_MUV)/(pow(EC50_MUV,GAMMA_MUV) + pow(C_MUV,GAMMA_MUV));

// =====================================================================
//  PCSK9  (cmt 11)   — statin raises synthesis, mAb removes free protein
// =====================================================================
dxdt_PCSK9 = KSP*(1.0 + EFFECT_STA_PCSK9) - KDP*PCSK9 - EFFECT_EVO*PCSK9;
...
// =====================================================================
double EFFECT_ZIL = pow(C_ZIL,GAMMA_ZIL)/(pow(EC50_ZIL,GAMMA_ZIL) + pow(C_ZIL,GAMMA_ZIL));
double IL6EFF = IL6*(1.0 - EFFECT_ZIL);
...
dxdt_LDL_P  = gPRODLDL*(1.0 - EFFECT_NIA_LDL) + KVL*VLDL_P
              - KCATD*LDL_P - FRECY*ASSEM - APH*LDL_P;
dxdt_VLDL_P = gPRODV*(1.0 - EFFECT_NIA_LDL) - (KVL + KCLV)*VLDL_P;

// =====================================================================
//  LDL RECEPTOR  (cmt 24)
// =====================================================================
dxdt_LDLR = KSR*(1.0 + EFFECT_STA_LDLR)*(1.0 + EFFECT_OBI_LDLR)*FEZE
...
// =====================================================================
dxdt_HDL_C = gKSYNH*(1.0 + EFFECT_OBI_HDL) - KHD*HDL_C;
dxdt_EGFR  = -KEGFRD;
...
CPELL C_MUV C_EVO C_STA C_NIA C_ZIL C_OBI IL6EFF FIL6 TRANS KNOCK FMUV KCATL KCATD ASSEM
EFFECT_EVO EFFECT_MUV EFFECT_STA_LDLR EFFECT_STA_PCSK9 EFFECT_STA_LPA
EFFECT_NIA_LPA EFFECT_NIA_LDL EFFECT_OBI_LDLR EFFECT_OBI_LPA EFFECT_OBI_HDL EFFECT_ZIL
'

mod <- mcode_cache("lpa_qsp_refactored", lpa_code, atol = 1e-9, rtol = 1e-7, maxsteps = 1e6)

## =====================================================================
##  COMPARTMENT / DOSING INDEX
## ---------------------------------------------------------------------
##   1 PEL_SC    pelacarsen SC        80 mg  q28d                 [out of scope, untouched]
##   4 SIR_SC    siRNA SC             75 mg  q84d  (olpasiran)     [out of scope, untouched]
##                                   400 mg  q168d (lepodisiran)
##   7 GUT_MUV   muvalaplin PO       240 mg  q1d    [refactored; was MUV_GU]
##   9 GUT_EVO   evolocumab SC       420 mg  q28d    [refactored; was EVO_SC]
##  12 GUT_STA   statin PO            20 mg  q1d (rosuvastatin-equivalent) [refactored; was STA_GU]
##  14 CENT_NIA  niacin ER          2000 mg  q1d    [refactored; was NIA_CE]
##  15 GUT_ZIL   ziltivekimab SC      30 mg  q28d    [refactored; was ZIL_SC]
##  17 CENT_OBI  obicetrapib PO       10 mg  q1d    [refactored; was OBI_CE]
##  (compartment numbering/order is unchanged -- events below still dose by number)
## =====================================================================
