...
// [REFACTORED per FORK_WORKFLOW_GUIDE.md Part 2, Archetype 3: renamed to the
// GUT_/CENT_/PERI_/CL_/V1_/V2_/Q_/KA_/F_ convention. k12_LT4/k21_LT4 micro-
// constants rewritten as Q_LT4/V2_LT4 (Q_LT4 = k12_LT4*V1_LT4 = 0.3 L/h;
// V2_LT4 = Q_LT4/k21_LT4 = 75 L) -- algebraically identical dynamics, no
// invented or refit values. VSS_LT4 is NOT one of the 2-compartment PK slots;
// it is a separate apparent whole-body volume the original used only to scale
// the exogenous T4 input (see EFFECT_LT4 below), kept unchanged.]
...
V2_LT4      : 75.0   : LT4 peripheral volume (L) [derived: Q_LT4 / k21_LT4]
Q_LT4       : 0.3    : LT4 inter-compartmental clearance (L/h) [derived: k12_LT4 * V1_LT4]
CL_LT4      : 0.50   : LT4 systemic clearance (L/h, t1/2 ~ 168h in VSS_LT4)
VSS_LT4     : 700.0  : LT4 apparent whole-body distribution volume for exogenous input scaling (L, ~10 L/kg, 70-kg patient); not part of the 2-cmt PK system
...
GUT_LT4     : LT4 gut absorption compartment (ug) [renamed from A_LT4_gut]
CENT_LT4    : LT4 central compartment (ug) [renamed from A_LT4_c]
PERI_LT4    : LT4 peripheral compartment (ug) [renamed from A_LT4_p]
...
// ---- LT4 contribution to TT4 pool (ug → nmol/L) ----
// [REFACTORED] renamed convention: GUT_LT4/CENT_LT4/PERI_LT4, VSS_LT4.
// [GUT_LT4 ug] absorbed at KA_LT4*F_LT4 feeds the TT4 pool as a synthesis-
// equivalent source term, scaled by the whole-body VSS_LT4 and MW_T4 (see
// EFFECT_LT4 in $ODE) -- this is substrate replacement, not a Hill-shaped
// receptor effect, so it bypasses C_LT4 (the central-compartment
// concentration) entirely; see refactor notes.
...
// ---- LT4 PK: depot + central + peripheral (Archetype 3) ----
// [REFACTORED] renamed to convention; k12_LT4/k21_LT4 rewritten as
// CL_LT4/Q_LT4/V1_LT4/V2_LT4 (Q_LT4 = k12_LT4*V1_LT4, V2_LT4 = Q_LT4/k21_LT4)
// -- algebraically identical dynamics, per FORK_WORKFLOW_GUIDE Archetype 3.
...
double C_LT4_out      = CENT_LT4 / V1_LT4;         // ug/L in central compartment [renamed from LT4_conc; suffixed _out because mrgsolve hoists $ODE/$TABLE locals into one shared scope, so this name cannot collide with the $ODE-computed C_LT4]
double EFFECT_LT4_out = KA_LT4 * GUT_LT4 * F_LT4 / VSS_LT4 / MW_T4 * 1.0e6; // nmol/L/h [REFACTORED, recomputed for reporting, identical formula to the $ODE C_LT4/EFFECT_LT4]
...
$CAPTURE fT4_pmol fT3_pmol fT4_ngdL fT3_pgmL TT4_ugdL TT3_ngdL
         A_TSH A_TT4 A_TT3 A_rT3 C_LT4_out EFFECT_LT4_out
...
make_ev <- function(LT4_dose_ug = 0, LT3_dose_ug = 0,
  ...
  evs <- list()
  if (LT4_dose_ug > 0) {
    evs[[1]] <- ev(amt = LT4_dose_ug, cmt = "GUT_LT4",
