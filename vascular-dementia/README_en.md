...
[![Mechanistic map preview](vad_qsp_model_en.png)](vad_qsp_model_en.svg)
...
| File | Description |
|------|------|
| `vad_qsp_model_en.dot` | Graphviz mechanistic map source |
| `vad_qsp_model_en.svg` | SVG vector image |
| `vad_qsp_model_en.png` | PNG raster image (150 dpi) |
| `vad_mrgsolve_model_en.R` | mrgsolve ODE model (18 compartments, 6 scenarios) |
| `vad_shiny_app_en.R` | Shiny interactive dashboard (7 tabs) |
| `vad_references_en.md` | References (70) |
| `README.md` | This file |
...
```bash
dot -Tsvg vad_qsp_model_en.dot -o vad_qsp_model_en.svg
dot -Tpng -Gdpi=150 vad_qsp_model_en.dot -o vad_qsp_model_en.png
```

### Run the mrgsolve Model (requires R)
```r
install.packages(c("mrgsolve", "dplyr", "ggplot2", "tidyr"))
source("vad_mrgsolve_model_en.R")
```

### Run the Shiny App
```r
install.packages(c("shiny", "shinydashboard", "ggplot2", "dplyr", "DT", "tidyr"))
shiny::runApp("vad_shiny_app_en.R")
```
