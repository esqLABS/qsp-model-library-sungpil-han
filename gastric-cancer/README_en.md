...
### 1. Mechanistic Map (gc_qsp_model_en.dot / .svg / .png)
...
### 2. mrgsolve ODE Model (gc_mrgsolve_model_en.R)
...
### 3. Shiny Dashboard (gc_shiny_app_en.R)

| Tab | Content |
|---|---|
| Patient Profile | ECOG, stage, HER2/MSI/CLDN18.2/CPS input; TCGA subtype classifier |
| Drug PK | Per-drug PK simulation, Cmax/Ctrough/AUC table, therapeutic window |
| Tumor Dynamics | Simeoni TGI, waterfall plot, spider plot, RECIST assessment |
| Clinical Endpoints | KM OS/PFS (survminer), ORR/DCR/CR bar charts |
| Treatment Scenario Comparison | Side-by-side TV dynamics, AUC comparison, summary table |
| Biomarker Analysis | CEA/CA19-9/ctDNA/VAF dynamics, HER2 IHC distribution, CPS, TCGA pie chart, correlation matrix |

### 4. References (gc_references_en.md)
...
```bash
cd gastric-cancer/
dot -Tsvg gc_qsp_model_en.dot -o gc_qsp_model_en.svg
dot -Tpng -Gdpi=150 gc_qsp_model_en.dot -o gc_qsp_model_en.png
```
...
# Run the app
shiny::runApp("gc_shiny_app_en.R")
```
...
```
gastric-cancer/
├── gc_qsp_model_en.dot         # Graphviz mechanistic map (212 nodes, 10 clusters)
├── gc_qsp_model_en.svg         # SVG render
├── gc_qsp_model_en.png         # PNG render (150 dpi)
├── gc_mrgsolve_model_en.R      # mrgsolve ODE model (28 compartments, 6 scenarios)
├── gc_shiny_app_en.R           # Shiny interactive dashboard (6 tabs)
├── gc_references_en.md         # 60 PubMed references (10 sections)
└── README.md                # This file
```
