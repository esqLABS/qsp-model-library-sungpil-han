...
## =====================================================================
##  REFACTORED sibling of cos_mrgsolve_model.R -- pluggable-PK naming
##  convention applied to five compounds: recombinant FSH (FSH),
##  corifollitropin alfa (CORI), ganirelix (ANT, the GnRH antagonist --
##  calibrated explicitly to ganirelix per the header above), triptorelin
##  (AGO, the GnRH agonist -- named explicitly in the original's own
##  parameter comments), and hCG (HCG, the trigger). Cabergoline (CAB),
##  letrozole (LET) and exogenous vaginal/IM progesterone (P4D) are real,
##  separately-dosed compounds this file also models but are OUT OF SCOPE
##  for this refactor and are left byte-for-byte untouched (same
##  compartments, same parameter names, same equations) -- see
##  cos_refactor_notes.md for the full account of what was found and why
##  each was or was not in scope. All numeric parameter values for the five
##  in-scope compounds are copied verbatim from the original; only PK
...
V1_FSH  : 26.0   : rFSH V/F (L) [was VF]
KA_CORI : 0.60   : corifollitropin absorption (1/d) [was KACO]
CL_CORI : 0.241  : corifollitropin elimination (1/d, t1/2 69 h) [was KELCO]
POT_CORI: 10.0   : FSH-equivalent potency of the corifollitropin state (shares V1_FSH, no own volume) [was POTCO]
KA_ANT  : 100.0  : ganirelix absorption (1/d, tmax 1.1 h) [was KAANT]
CL_ANT  : 1.28   : ganirelix elimination (1/d, t1/2 13 h) [was KELANT]
V1_ANT  : 20.0   : ganirelix V/F (L) [was VANT]
IC50_ANT: 0.44   : GnRH-receptor IC50 of the antagonist (ng/mL) [was IC50ANT]
KA_HCG  : 1.40   : hCG absorption (1/d) [was KAHCG]
CL_HCG  : 0.50   : hCG elimination (1/d, t1/2 33 h) [was KELHCG]
V1_HCG  : 20.0   : hCG V/F (L) [was VHCG]
KA_AGO  : 40.0   : triptorelin absorption (1/d) [was KAAGO]
CL_AGO  : 5.55   : triptorelin elimination (1/d, t1/2 3 h) [was KELAGO]
V1_AGO  : 30.0   : triptorelin V/F (L) [was VAGO]
EC50_AGO: 0.05   : GnRH-receptor EC50 of the agonist (ng/mL) [was EC50AGO]
...
// --- drug PK (renamed to GUT_<STEM>/CENT_<STEM>; CAB/LET compartment
//     names kept as-is -- out of scope for this refactor) --------------
GUT_FSH  : rFSH subcutaneous depot (IU) [was FSHDEP]
CENT_FSH : rFSH central (IU) [was FSHC]
GUT_CORI : corifollitropin depot (IU-equivalent) [was CORID]
CENT_CORI: corifollitropin central (IU-equivalent) [was CORIC]
GUT_ANT  : ganirelix depot (ug) [was ANTD]
CENT_ANT : ganirelix central (ug) [was ANTC]
GUT_HCG  : hCG depot (IU) [was HCGD]
CENT_HCG : hCG central (IU) [was HCG]
GUT_AGO  : triptorelin depot (ug) [was AGOD]
CENT_AGO : triptorelin central (ug) [was AGOC]
CAB    : cabergoline effect compartment (units) [out of scope, unchanged]
LET    : letrozole effect compartment (units) [out of scope, unchanged]
// --- endocrine ------------------------------------------------------
...
// corpus census flagged this file for. Normalized here into ONE $GLOBAL
...
#define C_HCG    (fmax(CENT_HCG, 0.0) / V1_HCG)                   // hCG (IU/L)
...
#define EFFECT_AGO (XOCC_AGO / (1.0 + XOCC_AGO + XOCC_ANT))       // fractional GnRH-receptor occupancy by the agonist
#define EFFECT_ANT (XOCC_ANT / (1.0 + XOCC_AGO + XOCC_ANT))       // fractional GnRH-receptor occupancy by the antagonist
...
MULT = AFC / 10.0;
for (int iTH = 0; iTH < 10; ++iTH) TH[iTH] = T50 * exp(SIGT * ZQ[iTH]);
...
for (int iG = 0; iG < 10; ++iG) {
  Dv[iG] = fmax(Dv[iG], 1.0e-6);
  Gv[iG] = fmax(Gv[iG], 0.0);
  Vv[iG] = fmin(fmax(Vv[iG], 0.0), 1.0);
}

// =====================================================================
//  1. drug concentrations
//     C_FSH/C_CORI/C_ANT/C_HCG/C_AGO and EFFECT_AGO/EFFECT_ANT are $GLOBAL
//     macros (see above) -- CAB/LET local doubles below are unchanged,
//     out of scope for this refactor.
// =====================================================================
double CCAB   = fmax(CAB,  0.0);
double CLET   = fmax(LET,  0.0);
double FSHTOT = fmax(FSHE, 0.0) + C_FSH + C_CORI;  // IU/L total FSH-equivalent drive

// --- LHCGR: ONE ligand pool read by THREE kernels -------------------
double LHEQ = fmax(LH, 0.0) + C_HCG + fmax(PHCG, 0.0);
...
for (int iF = 0; iF < 10; ++iF) {
  ...
  double TEFF = TH[iF] * (1.0 + KAMHT*(fmax(AMH,0.0)/AMHREF - 1.0))
                      * (1.0 - KDTH*HL(Dv[iF], DTH, 4.0));
  if (TEFF < 0.5) TEFF = 0.5;
  double A    = HL(FSHTOT, TEFF, HT);
  double LHR  = HL(Dv[iF], DLH, 6.0);               // LHCGR acquisition
  ...
  double GCAP = pow(Dv[iF]/DPRE, 2.0); if (GCAP > 1.0) GCAP = 1.0;
  double GFAC = WGR + (1.0 - WGR)*HL(Dv[iF], DGR, NGR);
  double GATE = HL(Dv[iF], DASP, NASP);
  double RUPI = RUP * LHR;                         // only LHCGR-competent

  dD[iF] = KGR*DRV*GFAC*(1.0 - Dv[iF]/DMAX)
          - KRUP*RUPI*(Dv[iF] - 3.0)
          - KCOL*COLL*GATE*(Dv[iF] - 3.0);
  dG[iF] = KG*DRV*(GCAP - Gv[iF]) - KGD*(1.0 - DRV)*Gv[iF]
          - KRUP*RUPI*Gv[iF]
          - FGRAN*KCOL*COLL*GATE*Gv[iF];
  dV[iF] = -KATR*pow(1.0 - DRV, 2.0)*Vv[iF];

  MG    += MULT*Gv[iF]*Vv[iF];
  MGA   += MULT*Gv[iF]*Vv[iF]*HL(Dv[iF], DARO, NARO);
  NSM   += MULT*Vv[iF]*(1.0 - HL(Dv[iF], DAMH, 4.0));
  SMALL += MULT*(1.0 - HL(Dv[iF], DAMH, 4.0));
  NASPF += MULT*Vv[iF]*GATE;
  NMIIF += MULT*Vv[iF]*GATE*HL(Dv[iF], DMII, NMII);
  CLASP += MULT*Gv[iF]*Vv[iF]*GATE;
  CLRUP += MULT*Gv[iF]*Vv[iF]*LHR;
  if (Dv[iF] > DMAXF) DMAXF = Dv[iF];
}
...
//  4. drug PK
// =====================================================================
dxdt_GUT_FSH  = -KA_FSH*GUT_FSH;
dxdt_CENT_FSH =  KA_FSH*GUT_FSH - CL_FSH*CENT_FSH;
dxdt_GUT_CORI  = -KA_CORI*GUT_CORI;
dxdt_CENT_CORI =  KA_CORI*GUT_CORI - CL_CORI*CENT_CORI;
dxdt_GUT_ANT  = -KA_ANT*GUT_ANT;
dxdt_CENT_ANT =  KA_ANT*GUT_ANT - CL_ANT*CENT_ANT;
dxdt_GUT_HCG  = -KA_HCG*GUT_HCG;
dxdt_CENT_HCG =  KA_HCG*GUT_HCG - CL_HCG*CENT_HCG;
dxdt_GUT_AGO  = -KA_AGO*GUT_AGO;
dxdt_CENT_AGO =  KA_AGO*GUT_AGO - CL_AGO*CENT_AGO;
dxdt_CAB    = -KELCAB*CAB;
dxdt_LET    = -KELLET*LET;
...
dxdt_RS = KR*HL(E2, E2S, NE2S)*HL(DMAXF, DSURG, NDSURG)*P4BLK
            *(1.0 - FANTL*EFFECT_ANT)*(1.0 - RS)
          - KROFF*RS;
double SURGE  = AMPS*HL(fmax(RS,0.0), 0.5, NRS)*P4BLK;
double GNRHLH = (TONE + SURGE)*(1.0 - FANTL*EFFECT_ANT) + AMPA*EFFECT_AGO;
double GNRHF  = (TONE + 0.25*SURGE)*(1.0 - FANTF*EFFECT_ANT) + 0.15*AMPA*EFFECT_AGO;
...
dxdt_GRR = KGRREC*(1.0 - GRRC) - KGRDOWN*EFFECT_AGO*GRRC;
GNRHLH *= P4FB*GRRC;
...
double FSHTOTAL = fmax(FSHE,0.0) + C_FSH + C_CORI;
double LHEQOUT  = fmax(LH,0.0) + C_HCG + fmax(PHCG,0.0);
double HCT      = RBCV/(RBCV + fmax(VP, 0.2))*100.0;

// follicle counts (a scan report, not a state)
// split for the same multi-declarator hoisting defect as section 3 of $ODE
double NF11 = 0.0;
double NF14 = 0.0;
double NF17 = 0.0;
double MGOUT = 0.0;
...
for (int iC = 0; iC < 10; ++iC) {
  double mm = AFC/10.0;
  if (DD[iC] >= 11.0) NF11 += mm*VVv[iC];
  if (DD[iC] >= 14.0) NF14 += mm*VVv[iC];
  if (DD[iC] >= 17.0) NF17 += mm*VVv[iC];
  MGOUT += mm*GG[iC]*fmin(fmax(VVv[iC],0.0),1.0);
  OVOL  += mm*3.14159265/6.0*pow(DD[iC]/10.0, 3.0);   // mL, sphere
}
...
$CAPTURE FSHTOTAL LHEQOUT HCT NF11 NF14 NF17
         MGOUT OVOL OHSSG TWOPN BLAST EUPL CLBR
         C_FSH C_CORI C_ANT C_HCG C_AGO EFFECT_AGO EFFECT_ANT
'
...
cos_events <- function(m, p, ttrig = NA, tend = 30) {
  ...
  ## daily gonadotropin --------------------------------------------------
  if (p$cori > 0) {
    e[[length(e) + 1]] <- rec(p$fsh_start, FB$CORI * p$cori * 100, "GUT_CORI")
    if (p$fsh_dose > 0 && stim_end >= p$fsh_start + 7)      # ENGAGE day 8 on
      e[[length(e) + 1]] <- rec(seq(p$fsh_start + 7, stim_end),
                                FB$FSH * p$fsh_dose, "GUT_FSH")
  } else if (p$fsh_dose > 0 && stim_end >= p$fsh_start) {
    dd <- seq(p$fsh_start, stim_end)
    e[[length(e) + 1]] <- rec(dd, FB$FSH * p$fsh_dose, "GUT_FSH")
    if (p$hmg_lh > 0)
      e[[length(e) + 1]] <- rec(dd, FB$HCG * p$hmg_lh * FB$HMG, "GUT_HCG")
  }
  ...
  if (p$ant_start > 0) {
    ...
    if (last >= p$ant_start)
      e[[length(e) + 1]] <- rec(seq(p$ant_start, last),
                                FB$ANT * p$ant_dose * 1000, "GUT_ANT")
  }
  ...
  if (!is.na(ttrig) && p$trigger != "none") {
    ...
    if (hd > 0) e[[length(e) + 1]] <- rec(ttrig, FB$HCG * hd, "GUT_HCG")
    if (ad > 0) e[[length(e) + 1]] <- rec(ttrig, FB$AGO * ad * 1000, "GUT_AGO")
