...
`cos_mrgsolve_model_en.R` (Controlled Ovarian Stimulation QSP model).
...
the numbers printed by this script are the numbers quoted in README_en.md.
Writing the system twice is also the cheapest available bug detector: five
structural defects were found this way (see README, "Defects revealed by verification").
...
#     compartments instead ($CMT in cos_mrgsolve_model_en.R), so the two files
