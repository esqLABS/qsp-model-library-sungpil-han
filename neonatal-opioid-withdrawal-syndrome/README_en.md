...
Full output: [`nows_verification_output.txt`](nows_verification_output.txt)
...
| File | Contents |
|---|---|
| [`nows_qsp_model.dot`](nows_qsp_model.dot) | Mechanistic map source (188 nodes / 17 clusters) |
| [`nows_qsp_model.svg`](nows_qsp_model.svg) · [`.png`](nows_qsp_model.png) | Rendering (PNG 150 dpi) |
| [`nows_mrgsolve_model.R`](nows_mrgsolve_model.R) | 43-ODE mrgsolve model · 25 scenarios · calibration sources |
| [`nows_shiny_app.R`](nows_shiny_app.R) | 12-tab interactive dashboard |
| [`nows_verify_python.py`](nows_verify_python.py) | Independent Python re-implementation · exhaustive parameter cross-check · 61 anchors |
| [`nows_verification_output.txt`](nows_verification_output.txt) | Full verification output |
| [`nows_population_sim.py`](nows_population_sim.py) | Virtual cohort · decomposition of the ESC effect |
| [`nows_population_output.txt`](nows_population_output.txt) | Cohort run results |
| [`nows_references.md`](nows_references.md) | 143 papers · every PMID verified with PubMed E-utilities |
