##############################################################################
# Hashimoto Thyroiditis - Quantitative Systems Pharmacology Model
# mrgsolve ODE-based PK/PD Model (REFACTORED, pluggable PK)
#
# Refactor of ht_mrgsolve_model.R per FORK_WORKFLOW_GUIDE.md Part 2.
# Original untouched (see ht_refactor_notes.md for full rationale).
#
# Scope: of the two rows this file carries in
# driver-patches/data/compound_perturbation_census.md, BOTH are genuine
# externally-dosed drugs (confirmed by their own dosing-rate parameters,
# LT4_dose/LiT3_dose, distinct from the endogenous T4p/T3p hormone pools and
# T4thy/T3thy thyroid-synthesis compartments, which have no dosing route):
#   - Levothyroxine (LT4) -- Archetype 3 (depot + central + peripheral,
#     linear), continuous zero-order infusion input (not an event)
#   - Liothyronine (LIT3, was LiT3) -- Archetype 1 (no depot, single
#     compartment, linear), continuous zero-order infusion input
# Selenium (Se) is not a census row for this file and is untouched except
# for one build-compatibility rename (F_Se -> FBIO_Se, see below).
#
# Both compounds' "effect on disease" is substrate-replacement (an
# exogenous, synthesis-equivalent hormone source added directly into the
# endogenous T4p/T3p mass balance), NOT a Hill/occupancy relationship --
# same finding as chronic-hypothyroidism's LT4
# (chronic-hypothyroidism/hypo_refactor_notes.md). No EMAX/EC50/GAMMA were
# invented for either compound; EFFECT_LT4/EFFECT_LIT3 are exact algebraic
# renames of the original's LT4c_nmol*kel_LT4 / LiT3c_nmol*kel_LiT3 terms.
#
# Build-compatibility fixes (disclosed, syntax-only, non-numeric -- see
# notes): the ORIGINAL does not compile under mrgsolve 2.0.1 for three
# reasons, all fixed here and logged as UPSTREAM_ISSUES.md #113:
#   1. $PARAM F_Se collided with mrgsolve's own reserved F_<compartment>
#      bioavailability symbol because a compartment is literally named Se.
#      Renamed the parameter to FBIO_Se (Se itself, out of scope, is
#      untouched).
#   2. $CAPTURE re-listed thirteen $CMT/$INIT compartment names directly
#      (AntiTPOAb, AntiTgAb, DmgThy, Th1, Treg, Bcell, T4p, T3p, T4t, T3t,
#      LT4c, LiT3c, Se), which mrgsolve 2.0.1 rejects. Compartments are
#      exposed as output columns automatically with no $CAPTURE entry
#      needed, so those names are dropped (replaced with the refactor's own
#      new C_<STEM>/EFFECT_<STEM> capture entries).
#   3. $PARAM declared five names (Th1_0, Treg_0, Bcell_0, DmgThy_0, Se_0)
#      that collide with mrgsolve's auto-generated <compartment>_0 initial-
#      value symbols (Th1, Treg, Bcell, DmgThy, Se are all $CMT names).
#      Renamed to Th1P0/TregP0/BcellP0/DmgThyP0/SeP0 (all five are
#      declared-but-unused or feed only already-dead $MAIN locals, see
#      notes -- no numeric behavior change).
#
# Model states (18 ODEs, unchanged count):
#   HPT axis     : TRH, TSH
...
#   Drug PK      : GUT_LT4/CENT_LT4/PERI_LT4 (was LT4g/LT4c/LT4p),
#                  CENT_LIT3 (was LiT3c)
#   Selenium     : Se (untouched)
#
# Treatment scenarios:
#   1. Untreated Hashimoto's (natural progression)
#   2. Standard levothyroxine (LT4 1.6 ug/kg/day)
#   3. Selenium supplementation (200 ug/day)
...
#   - Gartner R et al. J Clin Endocrinol Metab 2002 (Selenium RCT)
...
Th1P0       = 0.15     // Baseline Th1 (elevated in Hashimoto)
TregP0      = 0.65
BcellP0     = 0.20
...
DmgThyP0    = 0.25     // Initial damage (moderate, established Hashimoto)
k_dmg_scale = 0.001    // Scale factor

// ---- Levothyroxine PK (Archetype 3: depot + central + peripheral, linear) ----
// Refactored to CL/Q/V form; Q_LT4/V2_LT4 derived by exact algebraic identity
// from the original ODE own k12_LT4/k21_LT4 -- see ht_refactor_notes.md
...
KA_LT4      = 0.48     // Absorption rate (1/day) [was ka_LT4]
CL_LT4      = 1.3      // Clearance (L/day) [was declared-but-unused in original $ODE; now operative -- exact match kel_LT4*Vc_LT4=0.13*10.0=1.3]
V1_LT4      = 10.0     // Central volume (L) [was Vc_LT4]
Q_LT4       = 2.5      // Inter-compartmental CL (L/day) [new, derived: k12_LT4*Vc_LT4 = 0.25*10.0]
V2_LT4      = 22.727272727272727 // Peripheral volume (L) [new, derived: Q_LT4/k21_LT4 = 2.5/0.11, full double precision so Q_LT4/V2_LT4 round-trips bit-exact to k21_LT4; replaces the original declared-but-unused, internally-inconsistent Vp_LT4=22.0 -- see notes]
f_T4conv    = 0.35     // Fraction LT4 → T3 via deiodinase [declared, unused in original -- preserved as-is]

// ---- Liothyronine PK (Archetype 1: no depot, single compartment, linear) ----
KA_LIT3     = 5.0      // T3 rapid absorption (1/day) [was ka_LiT3; declared-unused, no depot in this archetype -- preserved as-is]
CL_LIT3     = 25.0     // T3 clearance (L/day) [was CL_LiT3; declared-but-unused in original $ODE; now operative -- exact match kel_LiT3*Vc_LiT3=0.625*40.0=25.0]
V1_LIT3     = 40.0     // T3 volume of distribution (L) [was Vc_LiT3]
F_LIT3      = 0.95     // Bioavailability [new -- promoted from original inline magic number LiT3_dose*0.95, same value]
CL_LIT3_from_LT4 = 0.125 // Extra clearance (L/day) [new, isolated constant -- original dxdt_LiT3c read k12_LT4*0.5 as a live cross-compound term (0.25*0.5=0.125); extracted here so LIT3 block is self-contained -- see notes]

// ---- Selenium PK/PD ----
FBIO_Se     = 0.85     // Selenium bioavailability [renamed from F_Se -- build fix, see notes]
k_Se_abs    = 2.0      // Absorption rate (1/day)
k_Se_elim   = 0.15     // Selenium elimination (1/day)
Vd_Se       = 50.0     // Se distribution volume (L)
SeP0        = 80.0     // Baseline plasma Se (μg/L, normal ~90-120, low in HT)
...
LIT3_dose   = 0.0      // Liothyronine dose (μg/day, typical 5-10 μg) [was LiT3_dose]
...
// LT4 PK (Archetype 3: depot + central + peripheral)
GUT_LT4   = 0.0
CENT_LT4  = 0.0
PERI_LT4  = 0.0

// Liothyronine (Archetype 1: no depot, single compartment)
CENT_LIT3 = 0.0
...
double DmgSS  = DmgThyP0 * exp(ETA(2));
...
// LT4 contribution to plasma T4 (was LT4c_nmol, computed from LT4c/Vc_LT4)
// NOTE: kept in $MAIN, same placement as original -- see ht_refactor_notes.md
// ("$MAIN vs $ODE placement is numerically load-bearing here").
double C_LT4 = CENT_LT4 / V1_LT4;                                  // μg/L, central concentration
double EFFECT_LT4 = C_LT4 * (CL_LT4 / V1_LT4) / (MW_T4 * 1e-3);    // nmol/L; was LT4c_nmol*kel_LT4

// LiT3 contribution to plasma T3 (was LiT3c_nmol, computed from LiT3c/Vc_LiT3)
double C_LIT3 = CENT_LIT3 / V1_LIT3;                                // μg/L
double EFFECT_LIT3 = C_LIT3 * (CL_LIT3 / V1_LIT3) / (65.5 * 1e-3); // nmol/L; was LiT3c_nmol*kel_LiT3; T3 MW handling (65.5) copied verbatim -- see notes on 65.5-vs-651-Da discrepancy
...
dxdt_T4p = k_T4sec * T4thy                         // from thyroid
           + EFFECT_LT4                              // from LT4 PK (was LT4c_nmol*kel_LT4)
...
dxdt_T3p = k_T3sec * T3thy                          // thyroidal T3
           ...
           + EFFECT_LIT3                             // from LiT3 PK (was LiT3c_nmol*kel_LiT3)
           - k_T3_deg * T3p
           - k_T3_tissue * T3p
           + k_T3_tissue_ret * T3t;
...
// LEVOTHYROXINE PK (Archetype 3: depot + central + peripheral, linear)
// ================================================================
// Dose enters gut compartment (μg/day, continuous infusion model -- guide
// "continuous infusion input, not an event" edge case, preserved from original)
double LT4_input = LT4_dose * F_LT4;  // effective daily LT4 absorbed (μg); bioavailability applied at entry, matching original
dxdt_GUT_LT4  = LT4_input - KA_LT4 * GUT_LT4;  // gut absorption (day units)
dxdt_CENT_LT4 = KA_LT4 * GUT_LT4
...
// LIOTHYRONINE PK (Archetype 1: no depot, single compartment, linear)
// ================================================================
double LIT3_input = LIT3_dose * F_LIT3;  // was: LiT3_dose * 0.95 (magic number promoted to F_LIT3)
dxdt_CENT_LIT3 = LIT3_input - (CL_LIT3/V1_LIT3 + CL_LIT3_from_LT4) * CENT_LIT3;

// ================================================================
// SELENIUM KINETICS
// ================================================================
double Se_input = Se_dose * FBIO_Se;
...
double BMD_risk = -0.5 * (CENT_LT4 / 100.0);  // T-score units [was LT4c]
...
$CAPTURE
fT4_pmolL fT3_pmolL TSH_clinical ThyVol_rel HypoScore HR_effect LDL_rel Fatigue_score BMD_risk
fT4_obs TSH_obs Ab_obs
C_LT4 EFFECT_LT4 C_LIT3 EFFECT_LIT3
'
