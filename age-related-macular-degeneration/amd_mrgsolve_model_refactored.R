...
## amd_refactor_notes.md for full detail):
##   Only the single modeled compound (census stem "VIT", the generic
##   anti-VEGF agent switched via DRUG_TYPE among ranibizumab/aflibercept/
##   bevacizumab/faricimab/brolucizumab) is renamed to the fork's naming
##   convention. There is no second compound in this file to leave alone.
##   - DRUG_VIT/DRUG_RET/DRUG_SYS renamed to CENT_VIT/PERI_VIT/SYS_VIT.
##     Bespoke structure (none of the guide's 4 archetypes fit exactly):
##     a one-way, non-mammillary 3-compartment cascade (vitreous dosing site
##     -> retinal effect site -> systemic monitoring sink, no back-flux
##     anywhere), not the depot/central/peripheral or bidirectional-exchange
##     shapes the archetypes assume. See refactor notes for why this was
##     kept bespoke rather than forced into archetype 2 or 3.
##   - New: C_VIT (nM), the retinal-site concentration read by every disease
##     equation that used to read DRUG_RET directly — the single point a
##     future external-covariate patch would substitute. Recomputed every
##     $ODE step from PERI_VIT. NOT also declared in $PARAM: this mrgsolve
##     build treats a name already in $PARAM as a read-only reference inside
##     $ODE/$MAIN, so "double C_VIT = ...;" there would silently drop
##     "double" and become an illegal reassignment (confirmed empirically —
##     see refactor notes). A per-step recomputed value and a genuinely
##     settable $PARAM default are mutually exclusive here; exposed via
##     $CAPTURE instead (same choice the tocilizumab calibration runs made
##     for C_TCZ/EFFECT_TCZ).
##   - New: EFFECT_VIT (fraction 0-1), a captured diagnostic = fractional
##     local VEGF neutralization (VEGF_BOUND / (VEGF_FREE+VEGF_BOUND)). This
##     is NOT a Hill-fit of concentration -> effect: the original has no
##     single-line algebraic effect term to rename or fit in the first
##     place. Drug action here is fully mechanistic target-binding kinetics
##     (mass-action VEGF/Ang-2 binding) threaded through several disease
##     state variables (VEGFR2 activation, CNV growth/regression, fluid
##     influx) — genuinely richer than one Hill curve, so it was kept
##     completely intact and unflattened, per the guide's explicit
##     instruction not to simplify real TMDD-style pharmacology. EFFECT_VIT
##     is an added summary readout for future driver-patch work, not
##     something the disease ODEs themselves consume.
##   - VEGF_FREE/VEGF_BOUND/ANG2_FREE/ANG2_BOUND were deliberately NOT
##     renamed to REC_FREE_VIT/COMPLEX_VIT. Unlike a dedicated TMDD receptor
##     pool, VEGF is the disease's own multi-purpose pathway state (drives
##     VEGFR2 activation, CNV growth, and its own feedback loop even absent
##     any drug) — renaming it into the compound's block would misrepresent
##     it as drug-owned. See refactor notes.
##   - All PK/binding parameter VALUES are copied verbatim from the
##     original; several were already unused/dead in the original
##     (Dose_vit, Fabs_ret, kon_VEGF, koff_VEGF, n_loading, inter_maint) and
##     remain unused/dead here too, renamed but otherwise untouched.
##
## Verified against the original (via the qspserver mrgsolve_api service)
## on the original file's own Ranibizumab, Aflibercept, and Faricimab
## dosing scenarios: every $CAPTURE'd output matched the original exactly
## across the full 730-day daily time grid. See amd_refactor_notes.md.
## ============================================================
...
$PARAM
// === Drug PK parameters (compound: VIT — generic anti-VEGF agent, ===
// === selected via DRUG_TYPE among ranibizumab/aflibercept/bevacizumab/  ===
// === faricimab/brolucizumab) ===
// Ranibizumab (default drug)
DOSE_VIT = 0.5          // mg IVT dose [unused/dead in the original DSL — dosing is driven entirely by the R-side make_events()/ev() calls below, not this parameter; preserved as-is]
FABS_VIT  = 0.3         // fraction drug reaching retina from vitreous [unused/dead in the original DSL, preserved as-is]
KEL_VIT   = 0.0965      // /day CENT_VIT (vitreous) elimination (t1/2 ~7.2d Fab) [was kel_vit]
KEL_SYS_VIT = 0.693     // /day SYS_VIT (systemic) elimination (t1/2~1d Fab) [was kel_sys]
KTR_CENT_PERI_VIT = 0.02  // /day CENT_VIT->PERI_VIT (vitreous->retina) transfer [was ktr_vit2ret]
KTR_CENT_SYS_VIT  = 0.005 // /day CENT_VIT->SYS_VIT (vitreous->systemic) drainage via AH outflow [was ktr_vit2sys]
KTR_PERI_SYS_VIT  = 0.01  // /day PERI_VIT->SYS_VIT (retina->systemic) clearance [was ktr_ret2sys]
V1_VIT     = 0.0046     // L vitreous (CENT_VIT) volume (~4.6 mL) [was V_vit]

// === VEGF kinetics (VIT:VEGF binding — the exposed C_VIT drives this) ===
k_VEGF_syn  = 0.5       // nM/day VEGF production by RPE/hypoxia [disease-level parameter, not compound VIT-specific — left untouched]
k_VEGF_deg  = 0.2       // /day VEGF degradation (t1/2 ~3.5d) [disease-level parameter, left untouched]
KON_VEGF_VIT = 5.0      // nM^-1 day^-1 generic drug-VEGF binding [was kon_VEGF; unused/dead in the original DSL — superseded by the DRUG_TYPE-dependent KON_USED_VIT below, preserved as-is]
KOFF_VEGF_VIT = 0.002   // /day generic drug-VEGF dissociation [was koff_VEGF; unused/dead, preserved as-is]
KD_VIT_RNB = 0.04       // nM VEGF:ranibizumab Kd [was Kd_VEGF_RNB]
KD_VIT_AFL = 0.0005     // nM VEGF:aflibercept Kd [was Kd_VEGF_AFL]
KD_VIT_BEV = 0.2        // nM VEGF:bevacizumab Kd [was Kd_VEGF_BEV]
KD_VIT_FAR = 0.0003     // nM VEGF:faricimab (VEGF arm) Kd [was Kd_VEGF_FAR]
KD_VIT_BRO = 0.06       // nM VEGF:brolucizumab Kd [was Kd_VEGF_BRO]
VEGF_baseline = 2.5     // nM baseline vitreous VEGF (wet AMD ~10x normal) [disease-level parameter, not compound VIT-specific — left untouched]

// === VEGFR2 activation (disease pathway, not renamed) ===
...
// === Ang-2 / Tie2 (faricimab Ang-2 arm; VIT:Ang-2 binding) ===
ANG2_baseline = 1.0     // nM vitreous Ang-2 (elevated in AMD) [disease-level parameter, left untouched]
k_ANG2_syn  = 0.15      // nM/day Ang-2 production
k_ANG2_deg  = 0.1       // /day Ang-2 degradation
KON_ANG2_VIT    = 2.0   // nM^-1 day^-1 faricimab Ang-2 arm binding [was kon_ANG2]
KOFF_ANG2_VIT   = 0.00087 // /day (Kd~0.9pM for faricimab Ang-2) [was koff_ANG2]

// === Complement activation (disease pathway, not renamed) ===
...
DRUG_TYPE   = 1         // 1=RNB, 2=AFL, 3=BEV, 4=FAR, 5=BRO [drug-variant selector, not a physical PK/PD parameter — left unrenamed]

// === Dosing schedule ===
N_LOADING_VIT   = 3        // number of loading doses (monthly) [was n_loading; unused/dead in the original DSL, preserved as-is]
INTER_MAINT_VIT = 56.0     // days between maintenance injections (q8w=56) [was inter_maint; unused/dead, preserved as-is]

$CMT
// Compartments
CENT_VIT    // Drug in vitreous (dosing site) [nM] [was DRUG_VIT]
PERI_VIT    // Drug in retina/RPE (effect site) [nM] [was DRUG_RET]
SYS_VIT     // Drug in systemic circulation [mg] [was DRUG_SYS]
...
// $PARAM default are mutually exclusive under this mrgsolve build; the
// recomputed value (needed for a correct, verifiable simulation) took
// precedence, matching the precedent set by the tocilizumab calibration
// runs (C_TCZ/EFFECT_TCZ are $ODE-local + $CAPTURE only, not $PARAM, there
// too).
double C_VIT = PERI_VIT;

double KD_EFF_VIT = (DRUG_TYPE==1) ? KD_VIT_RNB :
               (DRUG_TYPE==2) ? KD_VIT_AFL  :
               (DRUG_TYPE==3) ? KD_VIT_BEV  :
               (DRUG_TYPE==4) ? KD_VIT_FAR  :
               KD_VIT_BRO;

double KON_EFF_VIT = 0.693 / (KD_EFF_VIT * 1.0);   // derived kon for each drug [unused/dead — superseded by KON_USED_VIT below, preserved as-is]
// kon = koff/Kd; koff ~ 0.001-0.01/day
double KOFF_EFF_VIT = (DRUG_TYPE==1) ? 0.0039 :
                 (DRUG_TYPE==2) ? 0.0005 :
                 (DRUG_TYPE==3) ? 0.002 :
                 (DRUG_TYPE==4) ? 0.0003 :
                 0.006;
double KON_USED_VIT = KOFF_EFF_VIT / KD_EFF_VIT;

// VEGF binding
double R_BIND_VEGF_VIT = KON_USED_VIT * C_VIT * VEGF_FREE - KOFF_EFF_VIT * VEGF_BOUND;

// Ang-2 binding (only faricimab DRUG_TYPE==4)
double R_BIND_ANG2_VIT = (DRUG_TYPE==4) ?
    (KON_ANG2_VIT * C_VIT * ANG2_FREE - KOFF_ANG2_VIT * ANG2_BOUND) : 0.0;

dxdt_CENT_VIT = -KEL_VIT * CENT_VIT
                - KTR_CENT_PERI_VIT * CENT_VIT
                - KTR_CENT_SYS_VIT * CENT_VIT;

dxdt_PERI_VIT = KTR_CENT_PERI_VIT * CENT_VIT
                - KTR_PERI_SYS_VIT * C_VIT
                - KON_USED_VIT * C_VIT * VEGF_FREE
                + KOFF_EFF_VIT * VEGF_BOUND
                - R_BIND_ANG2_VIT;

dxdt_SYS_VIT = KTR_CENT_SYS_VIT * CENT_VIT * V1_VIT
                + KTR_PERI_SYS_VIT * C_VIT
                - KEL_SYS_VIT * SYS_VIT;

// ============================================================
// VEGF dynamics (disease pathway state — not a compound-owned receptor
// pool: it drives VEGFR2/CNV/fluid on its own even absent any drug, so it
// is NOT renamed into the VIT block; only the drug-input term uses C_VIT)
// ============================================================
...
dxdt_VEGF_BOUND = R_BIND_VEGF_VIT
                  - (k_VEGF_deg + KEL_VIT) * VEGF_BOUND;  // complex cleared

// ============================================================
// EFFECT_VIT — captured diagnostic only (see header note): fractional
// local VEGF neutralization achieved by the drug. Not consumed by any
// dxdt_ — the real disease consequence of C_VIT is the full mechanistic
// chain above and below (VEGF_FREE depletion -> VEGFR2_ACT -> CNV/fluid),
// which is kept completely intact rather than flattened into this ratio.
// ============================================================
double EFFECT_VIT = VEGF_BOUND / (VEGF_FREE + VEGF_BOUND + 1e-9);
...
dxdt_ANG2_BOUND = R_BIND_ANG2_VIT
...
capture DRUG_VIT_out   = CENT_VIT;
capture DRUG_RET_out   = PERI_VIT;
