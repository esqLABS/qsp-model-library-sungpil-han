...
K12_AMIO : 0.020  : Central→depot transfer (1/h)  [kept as literal micro-constant -- see refactor notes]
K21_AMIO : 0.0002 : Depot→central transfer (1/h)  [slow re-equilibration; kept literal]
...
// EC50_APIX_FXA = 75 ng/mL (0.075 µg/mL) per ARISTOTLE anti-FXa substudy
...
// ===========================================================================
EC50_AMIO_ERP  : 0.50  : Amiodarone EC50 for ERP prolongation (µg/mL)
EMAX_AMIO_ERP  : 0.35  : Amiodarone max fractional ERP increase
GAMMA_AMIO_ERP : 1     : Amiodarone Hill coefficient for ERP (implicit in original)
EC50_AMIO_HR   : 1.00  : Amiodarone EC50 for HR reduction (µg/mL)
EMAX_AMIO_HR   : 0.45  : Amiodarone max fractional HR reduction
GAMMA_AMIO_HR  : 1     : Amiodarone Hill coefficient for HR (implicit in original)
EC50_AMIO_IKR  : 0.30  : Amiodarone EC50 for IKr block (µg/mL)
EMAX_AMIO_IKR  : 0.85  : Amiodarone max IKr inhibition
GAMMA_AMIO_IKR : 1     : Amiodarone Hill coefficient for IKr (implicit in original)
EC50_AMIO_CA   : 0.80  : Amiodarone EC50 for ICaL block (µg/mL)
EMAX_AMIO_CA   : 0.60  : Amiodarone max ICaL inhibition
GAMMA_AMIO_CA  : 1     : Amiodarone Hill coefficient for ICaL (implicit in original)
...
// ===========================================================================
EC50_METRO_HR  : 50    : Metoprolol EC50 for HR reduction (ng/mL)
EMAX_METRO_HR  : 0.40  : Metoprolol max fractional HR reduction
GAMMA_METRO_HR : 1     : Metoprolol Hill coefficient for HR (implicit in original)
...
// ===========================================================================
EC50_APIX_FXA  : 0.075 : Apixaban EC50 for FXa inhibition (µg/mL)
EMAX_APIX_FXA  : 0.95  : Apixaban max fractional FXa inhibition
GAMMA_APIX_FXA : 1     : Apixaban Hill coefficient for FXa (implicit in original)
...
// Inflammatory
IL6_BASE       : 1.40  : Baseline IL-6 (elevated in AF)  [renamed from IL6_0 -- upstream defect, see refactor notes]
...
GUT_AMIO   : Amiodarone gut absorption (mg)
CENT_AMIO  : Amiodarone central plasma (mg)
PERI_AMIO  : Amiodarone fat/tissue depot (mg)
GUT_APIX   : Apixaban gut absorption (mg)
CENT_APIX  : Apixaban central plasma (mg)
GUT_METRO  : Metoprolol gut absorption (mg)
CENT_METRO : Metoprolol central plasma (mg)
...
// Amiodarone (µg/mL) -- the single exposed C_AMIO (assign, not re-declare)
C_AMIO      = CENT_AMIO / V1_AMIO;

// Apixaban (µg/mL for PD; multiply × 1000 for ng/mL display)
C_APIX      = CENT_APIX / V1_APIX;

// Metoprolol (ng/mL for PD)
C_METRO     = CENT_METRO / V1_METRO * 1000.0;

// ============================================================
// SECTION 2 — PHARMACODYNAMIC EFFECTS (Hill equation, n=1)
// ============================================================

// --- Amiodarone effects (renames -- original was already a plain Hill
//     ratio Emax*C/(IC50+C), i.e. n=1; GAMMA_AMIO_* pulled out explicit) ---
// ERP prolongation
EFFECT_AMIO_ERP = EMAX_AMIO_ERP * pow(C_AMIO, GAMMA_AMIO_ERP)
                  / (pow(EC50_AMIO_ERP, GAMMA_AMIO_ERP) + pow(C_AMIO, GAMMA_AMIO_ERP));

// Heart rate slowing (class IV-like; secondary to class III)
EFFECT_AMIO_HR  = EMAX_AMIO_HR  * pow(C_AMIO, GAMMA_AMIO_HR)
                  / (pow(EC50_AMIO_HR, GAMMA_AMIO_HR) + pow(C_AMIO, GAMMA_AMIO_HR));

// IKr block (Kv11.1/hERG; "reverse use-dependence")
EFFECT_AMIO_IKR = EMAX_AMIO_IKR * pow(C_AMIO, GAMMA_AMIO_IKR)
                  / (pow(EC50_AMIO_IKR, GAMMA_AMIO_IKR) + pow(C_AMIO, GAMMA_AMIO_IKR));

// ICaL (L-type Ca2+ channel) block → reduces Ca2+ overload
EFFECT_AMIO_CA  = EMAX_AMIO_CA  * pow(C_AMIO, GAMMA_AMIO_CA)
                  / (pow(EC50_AMIO_CA, GAMMA_AMIO_CA) + pow(C_AMIO, GAMMA_AMIO_CA));

// --- Metoprolol effects ---
// HR reduction (beta-1 blockade)
EFFECT_METRO_HR = EMAX_METRO_HR * pow(C_METRO, GAMMA_METRO_HR)
                  / (pow(EC50_METRO_HR, GAMMA_METRO_HR) + pow(C_METRO, GAMMA_METRO_HR));

// --- Apixaban effects ---
// FXa inhibition (direct anti-Xa)
EFFECT_APIX_FXA = EMAX_APIX_FXA * pow(C_APIX, GAMMA_APIX_FXA)
                  / (pow(EC50_APIX_FXA, GAMMA_APIX_FXA) + pow(C_APIX, GAMMA_APIX_FXA));

// --- Combined HR reduction (additive with ceiling) ---
// Bloch interaction model (Greco): combined = E1+E2 - E1*E2 (independence)
HR_red_combined = EFFECT_AMIO_HR + EFFECT_METRO_HR - EFFECT_AMIO_HR * EFFECT_METRO_HR;
...
dxdt_GUT_AMIO  = -KA_AMIO * GUT_AMIO;
dxdt_CENT_AMIO =  F_AMIO * KA_AMIO * GUT_AMIO
                 - ke_AMIO  * CENT_AMIO
                 - K12_AMIO * CENT_AMIO
                 + K21_AMIO * PERI_AMIO;
dxdt_PERI_AMIO =  K12_AMIO * CENT_AMIO - K21_AMIO * PERI_AMIO;
...
dxdt_GUT_APIX  = -KA_APIX * GUT_APIX;
dxdt_CENT_APIX =  F_APIX * KA_APIX * GUT_APIX - ke_APIX * CENT_APIX;
...
dxdt_GUT_METRO  = -KA_METRO * GUT_METRO;
dxdt_CENT_METRO =  F_METRO * KA_METRO * GUT_METRO - ke_METRO * CENT_METRO;
...
// Amiodarone block
double IKr_drug_block = EFFECT_AMIO_IKR * IKr;
...
double Ca_AMIO_blk  = EFFECT_AMIO_CA * 0.04 * Ca_i;               // ICaL block → less entry
...
double dERP_drug    =  EFFECT_AMIO_ERP * ERP0 / 720.0;             // slow accumulation
...
double IL6_decay = 0.03 / 24.0 * (IL6 - IL6_BASE);
...
double FXa_inhib  = EFFECT_APIX_FXA * FXa_act * 0.20 / 24.0;  // apixaban direct inhibition
...
double anticoag_effect = EFFECT_APIX_FXA * 0.65;  // max 65 % stroke RRR (ARISTOTLE upper bound)
...
// Drug concentrations for display -- single source of truth: the $GLOBAL
// C_<STEM> assigned in $ODE (this used to be a second, independently-
// recomputed `double Cp_AMIO_out = CENT_AMIO/V1_AMIO`; Cp_METRO_ng in
// particular used to be declared `double` a second time here with the
// exact same name as its own $ODE computation, which mrgsolve 2.0.1
// rejects as a redefinition -- see refactor notes upstream-defect entry)
double Cp_AMIO_out   = C_AMIO;                 // µg/mL
double Cp_APIX_ng    = C_APIX  * 1000.0;       // ng/mL (ARISTOTLE units)
double Cp_METRO_ng = C_METRO;                  // ng/mL (single declaration -- see refactor notes)
double Cp_AMIO_depot = PERI_AMIO / V2_AMIO;    // µg/mL (fat compartment)

// PD effect outputs
double AntiXa_pct    = EFFECT_APIX_FXA * 100.0;       // % FXa inhibition
...
cat("Compiling AF QSP model (24-compartment ODE system, refactored pluggable-PK)...\n")
af_mod <- mcode("AF_QSP_v2_refactored", af_model_code_refactored)
...
build_events <- function(scenario, t_max_days = 730) {
  ...
  if (scenario %in% c("S2_rate_metro", "S5_metro_apix")) {
    ...
    ev_list[["metro"]] <- data.frame(
      time = t_metro,
      cmt  = 6,       # GUT_METRO compartment index (was GI_METRO; same 1-based index)
      amt  = 50,      # 50 mg
      evid = 1,
      ii   = 0,
      addl = 0
  ...
  if (scenario %in% c("S3_rhythm_amio", "S6_amio_apix")) {
    ...
    ev_load <- data.frame(
      time = t_load,
      cmt  = 1,       # GUT_AMIO compartment index (was GI_AMIO; same 1-based index)
  ...
  if (scenario %in% c("S4_apix_only", "S5_metro_apix", "S6_amio_apix")) {
    ...
    ev_list[["apix"]] <- data.frame(
      time = t_apix,
      cmt  = 4,       # GUT_APIX compartment index (was GI_APIX; same 1-based index)
      amt  = 5,       # 5 mg per dose
      evid = 1,
      ii   = 0,
      addl = 0
