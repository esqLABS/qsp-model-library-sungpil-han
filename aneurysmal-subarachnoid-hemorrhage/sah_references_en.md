...
This list gives the evidence for the structure that `sah_qsp_model_en.dot` ·
`sah_mrgsolve_model_en.R` · `sah_shiny_app_en.R` claim. The annotation on each entry records
...
the "Where the model diverges from the literature or fails" section of [README_en.md](README_en.md) in the model's own directory.
