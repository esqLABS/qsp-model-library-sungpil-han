...
// ---------------- drug PK ----------------------------------------------------
KA_NIM  : 36.0   : nimodipine absorption (1/d)
CL_NIM  : 979.0  : nimodipine clearance (L/d) [= KELNIM 9.79/d x VNIM 100 L]
V1_NIM  : 100.0  : nimodipine apparent volume (L)
F_NIM   : 0.13   : nimodipine oral bioavailability, CYP3A4 first pass (-)
...
// ---------------- drug PD ----------------------------------------------------
EC50_NIM    : 30.0   : nimodipine EC50 (ng/mL)
GAMMA_NIM   : 1.0    : nimodipine Hill coefficient (-) [original had no explicit Hill term]
EMAX_NIM_SP : 0.16   : nimodipine Emax, large-artery relaxation (-)
EMAX_NIM_RM : 0.45   : nimodipine Emax, microvascular relaxation (-)
EMAX_NIM_SD : 0.55   : nimodipine Emax, SD suppression (-)
EMAX_NIM_NP : 0.50   : nimodipine Emax, infarct-conversion block (-)
...
$CMT @annotated
// ---- drug PK / exposure (8) ----
GUT_NIM  : nimodipine gut depot (mg)
CENT_NIM : nimodipine plasma concentration (ng/mL)
...
// fork PK/PD interface: C_NIM is the single exposed nimodipine concentration
// (the CENT_NIM compartment already stores it directly, as in the original);
// EFFECT_NIM is the fractional occupancy (Hill, gamma=1) every downstream
// nimodipine site-specific Emax multiplies.
...
// 2.  DRUG PK
// ===========================================================================
dxdt_GUT_NIM  = -KA_NIM * GUT_NIM;
dxdt_CENT_NIM = KA_NIM * F_NIM * GUT_NIM / V1_NIM * 1000.0 - (CL_NIM / V1_NIM) * CLF * CENT_NIM
              + (NIMIV > 0.0 ? NIMIV * 24.0 * 1000.0 / V1_NIM * ON : 0.0);
...
// ===========================================================================
double vasod_sp = EMAX_NIM_SP * EFFECT_NIM + EMXNIC_SP * ENIC
...
double vasod_rm = EMAX_NIM_RM * EFFECT_NIM + EMXCLZ_RM * ECLZ + EMXCIL_RM * ECIL
...
double sdrate = SDMAX * SDS * SDS * (1.0 - EMAX_NIM_SD * EFFECT_NIM - EMXKET_SD * EKET);
...
double map_t = MAP0 + MAPH * ONH
             - (NIMPO > 0.0 ? DMAP_NIM_PO * EFFECT_NIM : 0.0)
             - (NIMIV > 0.0 ? DMAP_NIM_IV * EFFECT_NIM : 0.0)
...
double npro = 1.0 - EMAX_NIM_NP * EFFECT_NIM;
...
// scope, so reusing the $ODE names here fails to compile with a
// "redefinition" error (confirmed empirically). Named NIM_conc/NIM_effect
// instead, mirroring the same workaround in ted_refactor_notes.md.
double oC_NIM      = CENT_NIM;
double oEFFECT_NIM = pow(oC_NIM, GAMMA_NIM) / (pow(EC50_NIM, GAMMA_NIM) + pow(oC_NIM, GAMMA_NIM));

capture NIM_conc   = oC_NIM;                    // exposed nimodipine concentration (ng/mL)
capture NIM_effect = oEFFECT_NIM;                // fractional nimodipine occupancy (Hill, gamma=1)
...
sah_events <- function(pop, sc) {
  ...
  n  <- floor((t1 - t0) * 6) + 1
  ev(amt = sc$NIMPO, cmt = "GUT_NIM", time = t0, ii = 1 / 6, addl = n - 1)
}
