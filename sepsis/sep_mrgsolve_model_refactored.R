...
// Fork PK/PD refactor: renamed stem Toci -> TCZ; Archetype 1 (no depot,
// single compartment, linear elimination) per FORK_WORKFLOW_GUIDE.md Part 2.
CL_TCZ    = 0.28    // clearance (L/h)
V1_TCZ    = 4.2     // volume (L) -- only sets elimination rate CL_TCZ/V1_TCZ;
                    // original doses/reads this compartment directly as a
                    // concentration (no separate amt/V1 conversion), preserved
                    // unchanged from the original TOCI_C compartment behavior
EMAX_TCZ  = 0.90    // maximum IL-6 signaling suppression (was Emax_Toci)
EC50_TCZ  = 1.5     // mcg/mL, half-max concentration (was EC50_Toci)
GAMMA_TCZ = 1        // Hill coefficient (original effect term had no explicit
                    // Hill exponent, i.e. a plain Emax ratio -> gamma = 1)
...
// ── Tocilizumab effect on IL-6 signaling (Hill interface) ─────────────────
// C_TCZ is the single exposed concentration PD reads (fork PK/PD convention).
// The original model doses directly into the tocilizumab compartment and
// reads it directly as a concentration (no amt -> conc division by V1_TCZ);
// preserved unchanged here for exact numerical equivalence with the original.
double C_TCZ = CENT_TCZ;
...
// IL-6
double Stim_IL6 = kprod_IL6 * TNF / (EC50_IL6 + TNF) * Inh_combined * (1.0 - EFFECT_TCZ);
...
// ─── TOCILIZUMAB PK (Archetype 1: no depot, single compartment, linear) ────
...
NE_C HC_C CENT_TCZ C_TCZ EFFECT_TCZ
...
make_events <- function(
  ...
  if (use_toci)
    ev <- ev + ev(amt = toci_dose, cmt = "CENT_TCZ", time = 6)
