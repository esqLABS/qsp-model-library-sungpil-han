...
##  Necrotising enterocolitis of the newborn — quantitative systems pharmacology model (mrgsolve)
##
...
##  term in `nec_reference_model_en.py` (pure-Python RK4) and actually integrated;
##  the captured log is `nec_reference_output_en.txt` and every number quoted in
##  `README_en.md` comes from that run.  If you change an equation here, change it
...
## =============================================================================
scenarios <- list(
  S1  = list(label = "28 wk · maternal milk · no antibiotics",
             args  = list(GA = 28, BW = 1.05, milk = "MOM")),
  S2  = list(label = "28 wk · formula · no antibiotics",
             args  = list(GA = 28, BW = 1.05, milk = "FORMULA")),
  S3  = list(label = "28 wk · formula · empirical antibiotics 7 d",
             args  = list(GA = 28, BW = 1.05, milk = "FORMULA", abx_days = 7)),
  S4  = list(label = "28 wk · formula · probiotics",
             args  = list(GA = 28, BW = 1.05, milk = "FORMULA", prob_dose = 1.10)),
  S5  = list(label = "25 wk · formula · antibiotics 7 d + indomethacin",
             args  = list(GA = 25, BW = 0.68, milk = "FORMULA", abx_days = 7,
                          ind_days = 3, dysbio = 1.8)),
  S6  = list(label = "25 wk · maternal milk + probiotics · antibiotics 2 d",
             args  = list(GA = 25, BW = 0.68, milk = "MOM", prob_dose = 1.10,
                          abx_days = 2, dysbio = 1.8)),
  S7  = list(label = "32 wk · formula",
             args  = list(GA = 32, BW = 1.75, milk = "FORMULA", feedrate = 30)),
  S8  = list(label = "28 wk · donor milk (pasteurised)",
             args  = list(GA = 28, BW = 1.05, milk = "DONOR")),
  S9  = list(label = "28 wk · nil by mouth + TPN",
             args  = list(GA = 28, BW = 1.05, milk = "MOM", NPO_ON = 1)),
  S10 = list(label = "28 wk · formula · antibiotics 10 d + metronidazole",
             args  = list(GA = 28, BW = 1.05, milk = "FORMULA", abx_days = 10,
                          mtz_days = 10)),
  S11 = list(label = "27 wk · formula · severe (no rescue intervention)",
...
##  Reproduces section 1 of nec_reference_output_en.txt.  Holds B, HMO, IgA and the
