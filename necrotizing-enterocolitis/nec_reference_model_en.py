...
Neonatal necrotising enterocolitis — INDEPENDENT PYTHON REFERENCE MODEL
===============================================================================

WHY THIS FILE EXISTS
--------------------
This build environment has no R runtime, so `nec_mrgsolve_model_en.R` cannot be
...
numpy).  Every number quoted in `README_en.md` is produced by running this file;
the captured log is `nec_reference_output_en.txt` and the machine-readable results
are `nec_scenario_results_en.json` / `nec_population_results.json`.
...
SCENARIOS = [
    ("S1_28wk_MOM", "28 wk · mother's own milk · no antibiotics"),
    ("S2_28wk_formula", "28 wk · formula · no antibiotics"),
    ("S3_28wk_formula_abx7", "28 wk · formula · 7 d empirical antibiotics"),
    ("S4_28wk_formula_probiotic", "28 wk · formula · probiotics"),
    ("S5_25wk_formula_abx7_indo", "25 wk · formula · 7 d antibiotics + indomethacin"),
    ("S6_25wk_MOM_probiotic", "25 wk · mother's own milk + probiotics · 2 d antibiotics"),
    ("S7_32wk_formula", "32 wk · formula"),
    ("S8_28wk_donor_milk", "28 wk · donor milk (pasteurised)"),
    ("S9_28wk_NPO_TPN", "28 wk · nil by mouth + TPN"),
    ("S10_28wk_formula_abx10_mtz", "28 wk · formula · 10 d antibiotics + metronidazole"),
    ("S11_28wk_formula_severe", "27 wk · formula · 7 d antibiotics + indomethacin + severe hypotensive episode"),
]
...
def main():
    ...
    W("Neonatal necrotising enterocolitis QSP reference model — actual integration output\n")
    W(hr() + "\n")
    ...
    W("   The critical pathobiont load is not one but two\n")
    W(hr("-") + "\n")
    W("   g(E) = kE(1-E)·Ftroph/(1+(INJ/Ki)^2) - kA·E·INJ ,  INJ = TLR4s(E;B)\n")
    W("   B_lo = the load at which NEC becomes 'possible'  (bistability appears)\n")
    W("   B_hi = the load at which NEC becomes 'inevitable'  (healthy root annihilates)\n")
    ...
    W("\n   READ: between B_lo and B_hi the outcome is decided not by the load but by\n")
    W("   the 'initial condition' — why two babies on one ward, one feed, diverge.\n\n")
    ...
    W("   The separatrix E* and the log dependence of the time to fulminant collapse\n")
    W(hr("-") + "\n")
    ...
    if Estar and Elow is not None and Elow < Estar:
        ...
        if len(rows) >= 2 and rows[-1]["sim_h"] > 0:
            ...
            W("   transit time — the signature of a logarithm.  This is\n")
        ...
        W("\n   The arithmetic reason for 'Bell II to III is always about a day', and the\n")
        W("   reason why a marker that rings at Bell II buys so little time.\n\n")
    ...
    W("   Treatment · exposure scenarios (integration of the full system)\n")
    W(hr("-") + "\n")
    ...
    W("   Feeding advance has both signs; in this parameterisation the substrate wins\n")
    W(hr("-") + "\n")
    ...
    W("   READ: the same equation is gentle at 30 weeks and steep at 25 — so the\n")
    W("   null result of the large advancement-rate trial (SIFT) and the clinical\n")
    W("   wariness about the most preterm infants are one equation at two\n")
    W("   gestational ages, not a contradiction.  Yet on NEC alone in this model\n")
    W("   the slowest advance is always lowest; its price falls not on NEC but on\n")
    W("   days to full feeds, TPN exposure and growth — the last two columns above.\n\n")
    ...
    W("   Empirical antibiotics change sign at a computable duration\n")
    W(hr("-") + "\n")
    ...
    W("   Threshold-moving drugs and state-moving drugs multiply, they do not add\n")
    W(hr("-") + "\n")
    ...
    W("   Calibration check against aggregate endpoints from the literature\n")
    W(hr("-") + "\n")
    ...
    W("   Lead time of the markers relative to the Bell II criteria\n")
    W(hr("-") + "\n")
    ...
    W("\n   READ: clinical markers move only after the switch has flipped.  What\n")
    W("   leads is the state variables (E, Jtr) — why prevention beats early diagnosis.\n\n")
    ...
    W("   Extra-intestinal outcomes (liver · brain · kidney · growth)\n")
    W(hr("-") + "\n")
    W("   %-30s %-9s %-9s %-9s %-9s\n"
    ...
    with open(os.path.join(here, "nec_scenario_results_en.json"), "w") as f:
