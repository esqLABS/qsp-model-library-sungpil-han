...
> no R runtime, so `nec_mrgsolve_model_en.R` cannot be executed. Committing an ODE
...
> `nec_reference_model_en.py` (pure Python, RK4) and **actually integrated**. Every
> table below is transcribed as it stands from that run log,
> `nec_reference_output_en.txt`.
...
| File | Contents |
|---|---|
| [`nec_qsp_model_en.dot`](nec_qsp_model_en.dot) | Mechanistic map source (167 nodes · 15 clusters) |
| [`nec_qsp_model_en.svg`](nec_qsp_model_en.svg) | Vector render |
| [`nec_qsp_model_en.png`](nec_qsp_model_en.png) | 150 dpi raster |
| [`nec_mrgsolve_model_en.R`](nec_mrgsolve_model_en.R) | mrgsolve 39-ODE model · scenarios · bifurcation analysis · population simulation |
| [`nec_reference_model_en.py`](nec_reference_model_en.py) | Pure-Python RK4 reimplementation of the same equations (for verification; actually executed) |
| [`nec_reference_output_en.txt`](nec_reference_output_en.txt) | The run log of the file above — **the source of every number in this README** |
| [`nec_scenario_results_en.json`](nec_scenario_results_en.json) | Bifurcation points · separatrix · scenarios · feeding · antibiotic results (machine-readable) |
| [`nec_population_results.json`](nec_population_results.json) | Factorial design · calibration results |
| [`nec_shiny_app_en.R`](nec_shiny_app_en.R) | 9-tab interactive dashboard |
| [`nec_references_en.md`](nec_references_en.md) | 106 references — each section marked with **which term of the model it supports** |

Reproduction:

```bash
# render the map
dot -Tsvg nec_qsp_model_en.dot -o nec_qsp_model_en.svg
dot -Tpng -Gdpi=150 nec_qsp_model_en.dot -o nec_qsp_model_en.png

# regenerate every number (pure standard library, about 8 minutes)
python3 nec_reference_model_en.py > nec_reference_output_en.txt
```

```r
# in an environment that has an R runtime
source("nec_mrgsolve_model_en.R")
sim <- run_all_scenarios();  summarise_scenarios(sim)
critical_load_table();       point_of_no_return()
factorial_arms();            abx_duration_sweep()
shiny::runApp("nec_shiny_app_en.R")
```

> `nec_mrgsolve_model_en.R` and `nec_reference_model_en.py` are **two implementations
