...
# PK/PD REFACTOR of dili_mrgsolve_model.R, per FORK_WORKFLOW_GUIDE.md Part 2.
# =============================================================================
#
# Original file is untouched; this is a new sibling. Two compounds refactored
# to this fork's pluggable-PK naming convention:
#
#   * APAP  -- the culprit hepatotoxin. NOT one of the census's two rows for
#     this file (the census had missed it entirely) -- added as a new row,
#     see dili_refactor_notes.md. Archetype 3 (depot+central+peripheral,
#     linear) plus a bespoke 4th tissue compartment (LIV_APAP, was ALIV)
#     because hepatic bioactivation genuinely reads a different concentration
#     (intrahepatic unbound, CU_APAP) than the plasma PK the
#     depot/central/peripheral triad describes (C_APAP). Two disease-effect
#     hookups are named explicitly (EFFECT_APAP, EFFECT_APAP_BSEP) but
#     neither is force-fit into an Emax/EC50 Hill shape -- see notes for why.
#   * NAC (N-acetylcysteine) -- census row "Redirect concentration inside
#     #define macro". Archetype 1 (no depot, single compartment, linear).
#     CENT_NAC (was NACC); its concentration was pulled out of the original's
#     #define macro into a named $ODE-local double, C_NAC, and its effect
#     promoted to an explicit EMAX_NAC/EC50_NAC/GAMMA_NAC Hill (GAMMA_NAC=1,
#     new, rename not a fit -- the original's own "enac" term was already
#     exactly this shape).
#
# The census's other row for this file, "Innate immunity (EL)", is NOT a
# compound at all -- see dili_refactor_notes.md. It is left alone; DAMP, KC,
# TNF, IL10 (the file's innate-immunity compartments) are all endogenous,
# with no dosing route anywhere in the file, and no parameter or compartment
# named "EL" exists -- "EL" is a fragment of several unrelated elimination-
# rate constants in that section (KDAMP_EL, KTNF_EL, KIL10_EL), not a drug.
#
# Every disease-side equation (thiol resynthesis, mitochondrial bioenergetics,
# the bistable JNK-Sab loop, BSEP-mediated cholestasis, DAMP-driven innate
# immunity, HLA-restricted adaptive immunity, regeneration, all six
# biomarkers) is byte-for-byte identical to the original, modulo the renames
# above and one disclosed, syntax-only upstream build-compatibility fix (see
# below and dili_refactor_notes.md / translations/UPSTREAM_ISSUES.md).
#
# UPSTREAM BUILD DEFECT (fixed here, syntax-only, disclosed):
#   The original's $PARAM @annotated block uses a 3-line continuation idiom
#   for three parameter descriptions (KM_CYP, STER_TAU, KBIL_ALT) -- a
#   leading-":" continuation line under the first line of a param entry.
#   mrgsolve 2.0.1's @annotated parser rejects this ("improper annotation
#   format"), so the ORIGINAL FILE DOES NOT COMPILE under mrgsolve 2.0.1 at
#   all (confirmed via the qspserver mrgsolve_api container's own
#   /model_manifest endpoint). Fixed here by collapsing each 3-line
#   description into one line -- purely textual, no numeric or behavioral
#   change. Logged as translations/UPSTREAM_ISSUES.md #113.
#
# VERIFICATION (qspserver mrgsolve_api, http://localhost:8007): the bare DSL
# text below was extracted and run, alongside the *original* file's own DSL
# with the same 3-line-continuation fix applied (nothing else changed, per
# the guide's "apply the same syntax-only fix to a patched-original baseline"
# policy), through /run_simulation for two of the original file's own
# scenarios:
#   - S5 "APAP 350 mg/kg + NAC at 8 h" (single oral APAP dose + the file's
#     own Prescott 3-phase IV NAC regimen), end=336h/delta=0.25 (1349 pts):
#     every shared output matches to floating-point-solver noise (max abs
#     diff ~1e-9 to 1e-13 for every state/derived variable except FBIOACT,
#     see below).
#   - S10 "Drug B (BSEP inhibitor) 8 mg/kg q12h x 28 d" (DRUG_B parameter
#     archetype: KI_BSEP=0.5 activates the BSEP-inhibition pathway, unlike
#     S5's inert default KI_BSEP=1e9), end=1400h/delta=0.25 (5657 pts): every
#     shared output matches to floating-point-solver noise (max abs diff
#     <=1e-4, most <=1e-10); peak ALT, peak TBIL, and HYLAW are identical.
#   FBIOACT (a ratio of near-zero numerator/denominator once the drug is
#   fully cleared, e.g. post-260h in S5) swings between the two runs by up
#   to ~4.6 in absolute terms at isolated late timepoints purely because
#   CU/CU_APAP themselves are ~1e-13-1e-14 (ODE-solver floating-point noise)
#   at those times -- a near-zero/near-zero artifact, not a real mismatch
#   (same category as documented in aip_refactor_notes.md for GIV/HEM).
#   Confirmed via /model_manifest that C_APAP, CU_APAP, EFFECT_APAP,
#   EFFECT_APAP_BSEP, C_NAC, EFFECT_NAC, and every renamed PK parameter are
#   discoverable (outputPaths / parameters respectively).
#
# See dili_refactor_notes.md for full detail.
# =============================================================================
...
V1_APAP :   63.0 : Central volume of distribution (L) [APAP ~0.9 L/kg]
V2_APAP :   25.0 : Peripheral volume (L)
Q_APAP  :   25.0 : Intercompartmental clearance (L/h)
CL_APAP :    1.4 : Renal clearance of parent drug (L/h)
KP_APAP :    1.0 : Liver-to-plasma partition coefficient (OATP uptake raises it)
FU_APAP :    0.8 : Unbound fraction in liver
F_APAP  :   0.88 : Oral bioavailability x fraction absorbed
KA_APAP :    1.2 : First-order absorption rate (1/h)
...
KM_CYP   : 6000.0 : Km CYP bioactivation (uM) — near-linear, so it does NOT saturate while phase II does; the shunt to reactive metabolite therefore rises with dose ON ITS OWN
...
KI_APAP_BSEP  :  1e9   : Drug Ki for BSEP inhibition (uM); 1e9 = no liability
...
STER_TAU :    6.0 : Onset time constant of the steroid effect (h). A smooth onset is both more physiological than a step and far kinder to the ODE solver.
...
KBIL_ALT :  0.003 : Non-hepatic (renal/alternative) bilirubin elimination (1/h) Without this term TBIL diverges as hepatocyte mass -> 0.
...
// ------------------------------------------------------------------- NAC --
V1_NAC   :   33.0 : NAC volume of distribution (L)
CL_NAC   :   11.0 : NAC clearance (L/h)
...
GAMMA_NAC:    1.0 : Hill coefficient (no exponent in original -> 1; new, explicit)

$CMT @annotated
GUT_APAP  : Drug in gut lumen (umol)
CENT_APAP : Drug in central compartment (umol)
PERI_APAP : Drug in peripheral compartment (umol)
LIV_APAP  : Drug in liver (umol)
...
CENT_NAC  : N-acetylcysteine in central compartment (umol)
...
// =========================================================================
// 1. PHARMACOKINETICS -- compound APAP (the culprit hepatotoxin; see
//    dili_refactor_notes.md -- this row was missing from the census
//    entirely). Archetype 3 (depot+central+peripheral, linear) plus a
//    bespoke 4th tissue compartment (LIV_APAP) because hepatic bioactivation
//    genuinely happens on a different concentration (intrahepatic unbound)
//    than the plasma PK the depot/central/peripheral triad describes.
// =========================================================================
double C_APAP     = CENT_APAP / V1_APAP;   // *** exposed concentration 1/2: plasma ***
double CPERI_APAP = PERI_APAP / V2_APAP;   // peripheral concentration (internal only)
double CLIV_APAP  = LIV_APAP / VLIV;       // total hepatic concentration (internal only)
double CU_APAP    = FU_APAP * CLIV_APAP;   // *** exposed concentration 2/2: unbound hepatic ***
double C_NAC      = CENT_NAC / V1_NAC;     // *** NACs exposed concentration (pulled out of the originals #define macro) ***

double v_sult = VMAX_SULT * CU_APAP / (KM_SULT + CU_APAP) * PAPS;
double v_ugt  = VMAX_UGT  * CU_APAP / (KM_UGT  + CU_APAP) * UDPGA;
double v_cyp  = VMAX_CYP  * CU_APAP / (KM_CYP  + CU_APAP) * FCYP;

dxdt_GUT_APAP  = -KA_APAP * GUT_APAP;
dxdt_CENT_APAP =  KA_APAP*GUT_APAP*F_APAP - QH*C_APAP + QH*CLIV_APAP/KP_APAP - CL_APAP*C_APAP - Q_APAP*C_APAP + Q_APAP*CPERI_APAP;
dxdt_PERI_APAP =  Q_APAP*C_APAP - Q_APAP*CPERI_APAP;
dxdt_LIV_APAP  =  QH*C_APAP - QH*CLIV_APAP/KP_APAP - (v_sult + v_ugt + v_cyp);
...
// =========================================================================
double EFFECT_APAP = v_cyp / VLIV;           // *** APAPs effect on disease: uM/h reactive-metabolite production flux (rename of prod_RM; deliberately NOT Hill-shaped -- see notes) ***
double v_conj  = KGST * gsh * RM;           // uM/h
double v_bind  = KBIND * RM;                // uM/h
dxdt_RM = EFFECT_APAP - v_conj - v_bind - KNQO * RM;

// =========================================================================
// 3. GLUTATHIONE AND CYSTEINE — the disposal rate, and its ceiling
// =========================================================================
double EFFECT_NAC = EMAX_NAC * pow(C_NAC, GAMMA_NAC) / (pow(EC50_NAC, GAMMA_NAC) + pow(C_NAC, GAMMA_NAC)); // *** NACs effect on disease (rename of enac; exact Hill shape already, GAMMA_NAC=1 explicit not a fit) ***
double v_gsh_syn = VMAX_GSH * FGSH * CYS / (KCYS + CYS) * NRF2;

dxdt_GSH = v_gsh_syn - KDEGG*gsh - v_conj/1000.0
           - KOX * ((ROS > 1.0) ? (ROS - 1.0) : 0.0) * gsh;
dxdt_CYS = KCYSIN * (CYSBASE*(1.0 + EFFECT_NAC) - CYS) - ALPHA_CYS * v_gsh_syn;
...
double ros_el = KROS_EL * ROS * redox_cap + KROS_NAC * C_NAC * ROS;
...
double EFFECT_APAP_BSEP = 1.0 / (1.0 + CU_APAP/KI_APAP_BSEP); // *** APAPs 2nd disease effect: competitive BSEP-transport inhibition (rename only; linear competitive-inhibition shape, not a Hill -- see notes) ***
double v_bsep   = VMAX_BSEP * BAH/(KM_BSEP + BAH) * EFFECT_APAP_BSEP * atp * hep;
...
// 12. ANTIDOTE PK
// =========================================================================
dxdt_CENT_NAC = -CL_NAC/V1_NAC * CENT_NAC;

$TABLE
double CP      = CENT_APAP/V1_APAP;                       // plasma drug (uM)
double CP_UGML = CP * 151.16 / 1000.0;        // ug/mL if MW = APAP
double CLIVER  = LIV_APAP/VLIV;
double CU      = FU_APAP * CLIVER;
...
C_APAP      : APAP plasma concentration (uM) -- exposed PK covariate 1/2
CU_APAP     : APAP unbound hepatic concentration (uM) -- exposed PK covariate 2/2 (distinct tissue site)
EFFECT_APAP : APAPs effect on disease -- reactive-metabolite production flux (uM/h); not Hill-shaped by design
EFFECT_APAP_BSEP : APAPs competitive BSEP-transport-inhibition factor (fraction of transport activity remaining)
C_NAC       : NAC plasma concentration (uM) -- exposed PK covariate
EFFECT_NAC  : NACs effect on disease -- fractional boost to cysteine supply
'

mod <- mcode("dili_refactored", dili_code)
...
# NOTE: compartment order is unchanged from the original (renamed in place),
# so the 1-based positions below are identical to the original's
# CMT_AGUT/CMT_NACC.
CMT_GUT_APAP <- 1L    # position of GUT_APAP in $CMT (was CMT_AGUT)
CMT_CENT_NAC <- 33L   # position of CENT_NAC in $CMT (was CMT_NACC)
...
oral <- function(mgkg, times = 0, wt = 70, mw = MW_APAP) {
  data.frame(ID = 1, time = times, amt = mgkg_to_umol(mgkg, wt, mw),
             rate = 0, cmt = CMT_GUT_APAP, evid = 1)
}
...
# Parameter names updated to the APAP_<role> convention (VC/VP/QD/CLR/KP/
# FULIV/KI_BSEP -> V1_APAP/V2_APAP/Q_APAP/CL_APAP/KP_APAP/FU_APAP/
# KI_APAP_BSEP); values unchanged from the original.
# =============================================================================
SLOW_CL <- list(VMAX_UGT = 2500, VMAX_SULT = 200, CL_APAP = 0.5)

# Drug B: low-clearance, OATP-concentrated BSEP inhibitor, weak bioactivator
#         (troglitazone / bosentan archetype)
DRUG_B <- c(SLOW_CL, list(KP_APAP = 25, FU_APAP = 0.05, KI_APAP_BSEP = 0.5,
                          VMAX_CYP = 150))
...
DRUG_C <- c(SLOW_CL, list(KP_APAP = 6, FU_APAP = 0.5, VMAX_CYP = 400,
                          KADD_REP = 0.001))
