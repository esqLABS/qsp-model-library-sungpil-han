...
## REFACTORED sibling of aki_mrgsolve_model.R -- pluggable-PK naming
## convention applied to the file's three externally-dosed compounds:
## Furosemide (FUR), Norepinephrine (NE), N-Acetylcysteine (NAC).
## All numeric parameter values are copied verbatim from the original;
## only PK/PD naming and structure are reorganized, plus:
##   - three new, math-implied Hill constants (EMAX_FUR=1.0, GAMMA_FUR=1.0,
##     GAMMA_NE=1.0, GAMMA_NAC=1.0) disclosed in aki_refactor_notes.md
##   - one new PK parameter, KA_FUR=1.5, pulled out of a gsub() text-patch
##     the original's own R wrapper applied to the raw DSL before compiling
##     it (see notes -- also logged as an upstream defect)
##   - two build-compatibility fixes (deduplicated $CAPTURE; explicit
##     $MAIN <cmt>_0 wiring for the original's own R-side post-mcode()
##     `init()` overrides) so this file compiles and reproduces the
##     original's own scenarios through the qspserver mrgsolve_api with
##     no R wrapper required -- both disclosed in aki_refactor_notes.md.
## The original (aki_mrgsolve_model.R) is untouched.
## Refactor added 2026-08-31.
## ============================================================
...
// ─── Furosemide PK (renamed to the pluggable-PK convention) ───
KA_FUR     = 1.5    // Absorption rate (h-1) [new, explicit; see refactor notes -- the
                     //   original never declares this in $PARAM despite using it in $ODE;
                     //   its own R wrapper papers over the gap with a gsub() text patch
                     //   applied to the raw DSL string before compiling. Value 1.5 is
                     //   exactly what that patch hardcodes.]
F_FUR      = 0.6    // Oral bioavailability (was F_fur)
CL_FUR     = 8      // Clearance (L/h) (was CL_fur)
V1_FUR     = 8      // Central volume (L) (was Vd_fur1)
V2_FUR     = 14     // Peripheral volume (L) (was Vd_fur2)
Q_FUR      = 4      // Inter-compartment clearance (L/h) (was Q_fur)
OAT_eff    = 1.0    // OAT1/3 secretion efficiency (reduced in AKI) -- unchanged, not a
                     //   compound PK/Hill role, kept as the original named it
EC50_FUR   = 0.5    // Furosemide EC50 for NKCC2 block (was NKCC2_IC50)
EMAX_FUR   = 1.0    // Maximal NKCC2 block fraction [new; the original ratio term has no
                     //   separate Emax -- its own max value is 1, made explicit here]
GAMMA_FUR  = 1.0    // Hill coefficient [new; no explicit Hill exponent in original]

// ─── Norepinephrine PK (renamed to the pluggable-PK convention) ───
CL_NE      = 250    // Clearance (L/h, very fast t1/2 ~2.5 min) (was CL_ne)
V1_NE      = 10     // Volume (L) (was Vd_ne)
EMAX_NE    = 0.4    // Maximum MAP increase fraction (was Emax_map)
EC50_NE    = 0.01   // NE EC50 for MAP (mcg/mL) (was EC50_ne)
GAMMA_NE   = 1.0    // Hill coefficient [new; no explicit Hill exponent in original]

// ─── N-Acetylcysteine PK (renamed to the pluggable-PK convention) ───
CL_NAC       = 15   // NAC clearance (L/h) (was CL_nac)
V1_NAC       = 30   // NAC volume (L) (was Vd_nac)
EMAX_NAC_GSH = 0.6  // Max GSH replenishment by NAC (was Emax_gsh)
EC50_NAC     = 10   // NAC EC50 for GSH/NO effects (mg/L) (was EC50_nac) -- shared by both
                     //   effect terms in the original, kept shared here (not split into
                     //   two EC50s the original never had)
EMAX_NAC_NO  = 0.3  // Max NO enhancement by NAC (was Emax_no)
GAMMA_NAC    = 1.0  // Hill coefficient [new; no explicit Hill exponent in original]
...
$CMT
// Drug PK compartments (renamed; order unchanged from the original so
// 1-based dosing positions used by the original\'s own scenarios are
// identical: GUT_FUR=1, CENT_FUR=2, PERI_FUR=3, CENT_NE=4, CENT_NAC=5)
GUT_FUR     // Furosemide gut (oral)      -- was FUR_GUT
CENT_FUR    // Furosemide central         -- was FUR_CENT
PERI_FUR    // Furosemide peripheral      -- was FUR_PERI
CENT_NE     // Norepinephrine central     -- was NE_CENT
CENT_NAC    // NAC central                -- was NAC_CENT
...
$MAIN
// [Build-compatibility fix, disclosed in aki_refactor_notes.md: the
//  original never wires its own $PARAM baselines into $CMT initial
//  conditions inside the DSL -- its R wrapper does this afterwards via
//  a post-mcode() `mod %>% init(...)` call that lives OUTSIDE the quoted
//  model_content string entirely. A client driving only the bare DSL
//  (as the qspserver API does) would otherwise start every compartment,
//  including GFR and TCV, at the mrgsolve default of 0. These lines
//  reproduce the original R wrapper\'s own init() values exactly, sourced
//  from the file\'s own $PARAM baselines where one exists.]
GFR_0        = GFR0;
ATP_0        = ATP0;
ROS_0        = ROS0;
GSH_0        = GSH0;
...
// ─── Furosemide: exposed concentration + effect on NKCC2 / diuresis ───
double C_FUR = CENT_FUR / V1_FUR;          // exposed PK covariate (mg/L) -- NEW
// FUR_TUB (the OAT-mediated, disease-state-modulated tubular delivery term) is
// preserved EXACTLY as the original computed it: from the raw central-compartment
// content, not from C_FUR. The original compares this against NKCC2_IC50 despite
// documenting it in mg/L, i.e. it never divides by Vd_fur1 -- an apparent
// concentration/amount inconsistency in the original, preserved here rather than
// silently corrected (see aki_refactor_notes.md "Findings, not fixed").
double FUR_TUB = CENT_FUR * OAT_eff * TCV;
double EFFECT_FUR = EMAX_FUR * pow(FUR_TUB, GAMMA_FUR)
                     / (pow(EC50_FUR, GAMMA_FUR) + pow(FUR_TUB, GAMMA_FUR));
double diuresis = 1.0 + 3.0 * EFFECT_FUR;   // diuresis fold over baseline

// ─── Norepinephrine: exposed concentration + effect on MAP/RBF ───
double C_NE = CENT_NE / V1_NE;             // exposed PK covariate (mcg/mL) -- NEW
// EFFECT_NE\'s Hill argument is likewise the raw compartment content (CENT_NE),
// exactly as the original used it, not C_NE -- same preserved quirk as FUR/NAC.
double EFFECT_NE = EMAX_NE * pow(CENT_NE, GAMMA_NE)
                    / (pow(EC50_NE, GAMMA_NE) + pow(CENT_NE, GAMMA_NE));
double RBF  = RBF0 * (1 + EFFECT_NE);      // MAP↑ → RBF autoregulation
double E_RBF_gfr = RBF / RBF0;             // relative RBF effect on GFR

// ─── NAC: exposed concentration + effects on GSH resynthesis / NO scavenging ───
double C_NAC = CENT_NAC / V1_NAC;          // exposed PK covariate (mg/L) -- NEW
double EFFECT_NAC_GSH = EMAX_NAC_GSH * pow(CENT_NAC, GAMMA_NAC)
                         / (pow(EC50_NAC, GAMMA_NAC) + pow(CENT_NAC, GAMMA_NAC));
double EFFECT_NAC_NO  = EMAX_NAC_NO  * pow(CENT_NAC, GAMMA_NAC)
                         / (pow(EC50_NAC, GAMMA_NAC) + pow(CENT_NAC, GAMMA_NAC));
...
dxdt_CENT_FUR = F_FUR * KA_FUR * GUT_FUR
                - (CL_FUR / V1_FUR) * CENT_FUR
                - (Q_FUR / V1_FUR) * CENT_FUR
                + (Q_FUR / V2_FUR) * PERI_FUR;
dxdt_PERI_FUR = (Q_FUR / V1_FUR) * CENT_FUR
                - (Q_FUR / V2_FUR) * PERI_FUR;

// ─── Norepinephrine PK ───
dxdt_CENT_NE  = - (CL_NE / V1_NE) * CENT_NE;  // continuous infusion via RATE
...
dxdt_ROS = k_ros_prod * (1 + 2 * inj_stim + (1 - TCV) + (1 - ATP))
           - k_ros_deg * GSH * ROS
           - 0.2 * EFFECT_NAC_NO * ROS;  // NAC scavenging

// ─── Glutathione ───
dxdt_GSH = k_gsh_prod * (1 + EFFECT_NAC_GSH)
           - k_gsh_dep * ROS * GSH;
...
$CAPTURE
AKI_stage_out eGFR UO_mLkgh FE_Na diuresis
C_FUR EFFECT_FUR
C_NE EFFECT_NE
C_NAC EFFECT_NAC_GSH EFFECT_NAC_NO
'

## ============================================================
## Compile Model
## ============================================================
## NOTE: unlike the original, no post-hoc gsub() text patch is needed --
## KA_FUR is now a genuine, explicit $PARAM entry (see notes).

mod <- mcode("AKI_QSP_refactored", code)

cat("=== AKI QSP Model (refactored) compiled successfully ===\\n")
cat("Compartments:", length(Init(mod)), "\\n")

## ============================================================
## INITIAL CONDITIONS
## ============================================================
## Same values as the original's own mod %>% init(...) call; also wired
## directly into $MAIN above (see refactor notes) so a driver that does
## not call init() -- e.g. the qspserver API -- still reproduces them.
...
ev2 <- ev(amt=40, ii=12, addl=13, cmt="GUT_FUR", time=2)  # start at 2h
scen2 <- mod %>%
  param(AKI_type=1, isch_sev=0.6) %>%
...
ev3 <- ev(amt=0.1, rate=0.1, cmt="CENT_NE", time=0, evid=1)  # NE continuous
scen3 <- mod %>%
  param(AKI_type=1, isch_sev=0.6) %>%
...
ev4 <- ev(amt=10000, rate=1000, cmt="CENT_NAC", time=-2)  # pre-injury NAC
scen4 <- mod %>%
  param(AKI_type=1, isch_sev=0.6) %>%
...
ev6a <- ev(amt=0.1, rate=0.1, cmt="CENT_NE", time=0)
ev6b <- ev(amt=40, ii=12, addl=13, cmt="GUT_FUR", time=6)
ev6  <- ev6a + ev6b
scen6 <- mod %>%
...
cat("\\n=== AKI QSP Model (refactored): Run Complete ===\\n")
