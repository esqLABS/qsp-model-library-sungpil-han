## =============================================================================
##  tts_mrgsolve_model_refactored.R
##  PK/PD REFACTOR of tts_mrgsolve_model.R (fork sibling - original untouched).
##  See tts_refactor_notes.md for the per-compound archetype/rename record and
##  qspserver verification. All 11 exogenous compounds (APX, CAR, DOB, ESM,
##  FUR, LEV, MET, MIL, OR (OR-1896), PHE, RAM) renamed to this fork's
##  C_<STEM>/EFFECT_<STEM>/CENT_<STEM>/GUT_<STEM>/PERI_<STEM> convention and
##  pulled out of the TTS_ALGEBRA #define macro into named, $CAPTURE'd
##  quantities. Disease-side mechanics are byte-for-byte unchanged.
...
##      Levosimendan/OR-1896 enters EFFECT_LEV (was CASENS), which multiplies myofilament
...
// Dobutamine
V1_DOB   : 25.0  : Dobutamine central volume of distribution (L)
...
GAMMA_DOB: 1     : Dobutamine Hill coefficient (implicit =1 in original) (-)
FB2_DOB  : 0.25  : Dobutamine beta2 potency relative to beta1 (-)
RATE_DOB : 0.0   : Dobutamine infusion rate, continuous zero-order input (mg/h)
...
// Milrinone
V1_MIL   : 31.0  : Milrinone volume of distribution (L)
CL_MIL   : 9.1   : Milrinone clearance (L/h)
EC50_MIL : 0.20  : Milrinone EC50 (was IC50) on PDE3 (mg/L)
EMAX_MIL : 0.90  : Milrinone maximal PDE3 inhibition (was IMAX) (-)
GAMMA_MIL: 1     : Milrinone Hill coefficient (implicit =1 in original) (-)
RATE_MIL : 0.0   : Milrinone infusion rate, continuous zero-order input (mg/h)
// Levosimendan and OR-1896 (active metabolite)
V1_LEV   : 14.0  : Levosimendan central volume (L)
V2_LEV   : 20.0  : Levosimendan peripheral volume (L)
...
V1_OR    : 48.0  : OR-1896 volume of distribution (L)
CL_OR    : 0.43  : OR-1896 clearance (L/h ; t1/2 ~77 h)
EC50_LEV : 0.030 : Levosimendan EC50 for Ca sensitisation (composite parent+metabolite conc.) (mg/L)
EMAX_LEV : 0.55  : Maximal myofilament Ca sensitisation (-)
GAMMA_LEV: 1     : Levosimendan Hill coefficient (implicit =1 in original) (-)
POT_OR   : 1.20  : OR-1896 potency relative to parent (-)
KATP_LEV : 0.30  : Levosimendan K_ATP vasodilator effect on SVR (-)
RATE_LEV : 0.0   : Levosimendan infusion rate, continuous zero-order input (mg/h)
// Esmolol
V1_ESM   : 240   : Esmolol volume of distribution (L)
CL_ESM   : 1200  : Esmolol clearance (L/h ; t1/2 ~8 min)
KI1_ESM  : 0.15  : Esmolol Ki at beta1 (mg/L)
KI2_ESM  : 5.00  : Esmolol Ki at beta2 (mg/L)
RATE_ESM : 0.0   : Esmolol infusion rate, continuous zero-order input (mg/h)
// Metoprolol (oral)
KA_MET   : 1.50  : Metoprolol absorption rate (1/h)
F_MET    : 0.40  : Metoprolol bioavailability (-)
V1_MET   : 280   : Metoprolol volume of distribution (L)
...
V1_CAR   : 115   : Carvedilol volume of distribution (L)
...
// Phenylephrine
V1_PHE   : 25.0  : Phenylephrine volume of distribution (L)
CL_PHE   : 200   : Phenylephrine clearance (L/h ; t1/2 ~5 min)
POT_PHE  : 45.0  : Phenylephrine alpha1 potency (nmol/L equivalent per mg/L)
RATE_PHE : 0.0   : Phenylephrine infusion rate, continuous zero-order input (mg/h)
// Ramipril / ramiprilat
KA_RAM   : 1.20  : Ramipril absorption rate (1/h)
F_RAM    : 0.55  : Ramipril bioavailability to ramiprilat (-)
V1_RAM   : 100   : Ramiprilat volume of distribution (L)
CL_RAM   : 6.00  : Ramiprilat clearance (L/h)
EC50_RAM : 0.0020: Ramiprilat EC50 (was IC50) on ACE (mg/L)
EMAX_RAM : 0.85  : Maximal ACE inhibition (was IMAX) (-)
GAMMA_RAM: 1     : Ramiprilat Hill coefficient (implicit =1 in original) (-)
// Furosemide
KA_FUR   : 1.50  : Furosemide absorption rate (1/h)
F_FUR    : 0.50  : Furosemide bioavailability (-)
V1_FUR   : 12.0  : Furosemide volume of distribution (L)
CL_FUR   : 9.00  : Furosemide clearance (L/h)
EC50_FUR : 0.50  : Furosemide EC50 for natriuresis (mg/L)
EMAX_FUR : 1     : Furosemide maximal natriuretic effect (implicit =1 in original) (-)
GAMMA_FUR: 1     : Furosemide Hill coefficient (implicit =1 in original) (-)
...
V1_APX   : 21.0  : Apixaban volume of distribution (L)
CL_APX   : 1.30  : Apixaban clearance (L/h ; t1/2 ~11 h)
EC50_APX : 0.10  : Apixaban EC50 for anticoagulation (mg/L)
EMAX_APX : 1     : Apixaban maximal anticoagulant effect (implicit =1 in original) (-)
GAMMA_APX: 1     : Apixaban Hill coefficient (implicit =1 in original) (-)
...
// -------------------------------------------------------------------- drug PK
CENT_DOB : 0.0 : Dobutamine central amount (mg)
CENT_MIL : 0.0 : Milrinone central amount (mg)
CENT_LEV : 0.0 : Levosimendan central amount (mg)
PERI_LEV : 0.0 : Levosimendan peripheral amount (mg)
CENT_OR  : 0.0 : OR-1896 (levosimendan active metabolite) central amount (mg)
CENT_ESM : 0.0 : Esmolol central amount (mg)
GUT_MET  : 0.0 : Metoprolol depot (mg)
CENT_MET : 0.0 : Metoprolol central amount (mg)
GUT_CAR  : 0.0 : Carvedilol depot (mg)
CENT_CAR : 0.0 : Carvedilol central amount (mg)
CENT_PHE : 0.0 : Phenylephrine central amount (mg)
GUT_RAM  : 0.0 : Ramipril depot (mg)
CENT_RAM : 0.0 : Ramiprilat central amount (mg)
GUT_FUR  : 0.0 : Furosemide depot (mg)
CENT_FUR : 0.0 : Furosemide central amount (mg)
GUT_APX  : 0.0 : Apixaban depot (mg)
CENT_APX : 0.0 : Apixaban central amount (mg)
// ------------------------------------------------------------ receptor / cAMP
...
double GS_BS, GI_AP, GI_MD, GI_BS, EFFECT_MIL, KDEG;
double AC_AP, AC_MD, AC_BS, CQ_AP, CQ_MD, CQ_BS;
double X_LEV, EFFECT_LEV, ATPF_AP, ATPF_MD, ATPF_BS, DR0;
...
double EFFECT_RAM, EFFECT_FUR, EFFECT_APX, ACN_AP, ACN_MD, ACN_BS;
...
#define TTS_ALGEBRA                                                           \
  ...
  /* --- receptor availability after downregulation and antagonists --- */    \
         AVAIL   = 1.0 - RDN;                                                 \
         C_ESM   = CENT_ESM / V1_ESM;                                        \
         C_MET   = CENT_MET / V1_MET;                                        \
         C_CAR   = CENT_CAR / V1_CAR;                                        \
         C_DOB   = CENT_DOB / V1_DOB;                                        \
         C_MIL   = CENT_MIL / V1_MIL;                                        \
         C_LEV   = CENT_LEV / V1_LEV;                                        \
         C_OR    = CENT_OR  / V1_OR;                                         \
         C_PHE   = CENT_PHE / V1_PHE;                                        \
         C_RAM   = CENT_RAM / V1_RAM;                                        \
         C_FUR   = CENT_FUR / V1_FUR;                                        \
         C_APX   = CENT_APX / V1_APX;                                        \
  /* --- each beta-blocker's own competitive-occupancy term at beta1/beta2;  \
     combined ONLY at INH1/INH2 below (guide: combine them only at the      \
     point the disease equations actually use them) --- */                   \
         EFFECT_ESM_B1 = C_ESM/KI1_ESM;                                       \
         EFFECT_ESM_B2 = C_ESM/KI2_ESM;                                       \
         EFFECT_MET_B1 = C_MET/KI1_MET;                                       \
         EFFECT_MET_B2 = C_MET/KI2_MET;                                       \
         EFFECT_CAR_B1 = C_CAR/KI1_CAR;                                       \
         EFFECT_CAR_B2 = C_CAR/KI2_CAR;                                       \
         EFFECT_CAR_A1 = C_CAR/KIA_CAR;                                       \
  ...
  /* --- dobutamine acts as an added beta agonist, not as a cAMP source --- */ \
         EFFECT_DOB = EMAX_DOB*pow(C_DOB,GAMMA_DOB)                          \
                      /(pow(EC50_DOB,GAMMA_DOB)+pow(C_DOB,GAMMA_DOB));        \
  /* --- local agonist concentrations per segment --- */                      \
         ...
         O2_AP   = AVAIL*INH2*(AG2_AP/(AG2_AP + KD_B2) + FB2_DOB*EFFECT_DOB); \
         O2_MD   = AVAIL*INH2*(AG2_MD/(AG2_MD + KD_B2) + FB2_DOB*EFFECT_DOB); \
         O2_BS   = AVAIL*INH2*(AG2_BS/(AG2_BS + KD_B2) + FB2_DOB*EFFECT_DOB); \
         AG1_AP  = AE1*EPI + NEL_AP;                                          \
         AG1_MD  = AE1*EPI + NEL_MD;                                          \
         AG1_BS  = AE1*EPI + NEL_BS;                                          \
         O1_AP   = AVAIL*INH1*(AG1_AP/(AG1_AP + KD_B1) + EFFECT_DOB);         \
         O1_MD   = AVAIL*INH1*(AG1_MD/(AG1_MD + KD_B1) + EFFECT_DOB);         \
         O1_BS   = AVAIL*INH1*(AG1_BS/(AG1_BS + KD_B1) + EFFECT_DOB);         \
  /* --- receptor densities --- */                                            \
  ...
  /* --- adenylyl cyclase and PDE (milrinone enters the PDE3 term) --- */     \
         EFFECT_MIL = EMAX_MIL*pow(C_MIL,GAMMA_MIL)                          \
                      /(pow(EC50_MIL,GAMMA_MIL)+pow(C_MIL,GAMMA_MIL));       \
         KDEG    = KDEG_CA*(1.0 - FPDE3*EFFECT_MIL);                          \
  ...
  /* --- bespoke: EFFECT_LEV is shared between LEV and its own active       \
     metabolite OR (OR-1896) over their potency-weighted composite exposure  \
     X_LEV - same molecule's own metabolite acting via the same mechanism    \
     (myofilament Ca sensitisation), not two independent drugs collapsed     \
     together. No independent EFFECT_OR (precedent: acne-vulgaris ISO/OXO). \
     --- */                                                                   \
         X_LEV   = C_LEV + POT_OR*C_OR;                                       \
         EFFECT_LEV = EMAX_LEV*pow(X_LEV,GAMMA_LEV)                          \
                      /(pow(EC50_LEV,GAMMA_LEV)+pow(X_LEV,GAMMA_LEV));       \
  ...
  /* --- drug effects on the neurohormonal and volume arms --- */             \
         EFFECT_RAM = EMAX_RAM*pow(C_RAM,GAMMA_RAM)                          \
                      /(pow(EC50_RAM,GAMMA_RAM)+pow(C_RAM,GAMMA_RAM));       \
         EFFECT_FUR = EMAX_FUR*pow(C_FUR,GAMMA_FUR)                          \
                      /(pow(EC50_FUR,GAMMA_FUR)+pow(C_FUR,GAMMA_FUR));       \
         EFFECT_APX = EMAX_APX*pow(C_APX,GAMMA_APX)                          \
                      /(pow(EC50_APX,GAMMA_APX)+pow(C_APX,GAMMA_APX));       \
  /* --- counterfactuals, using the exact QSS cAMP --- */                     \
         ...
         CO2_AP  = POSPART((DR_AP/DRR_AP)*ATPF_AP*(1.0-STU_AP)*(1.0+EFFECT_LEV)); \
         CO2_MD  = POSPART((DR_MD/DRR_MD)*ATPF_MD*(1.0-STU_MD)*(1.0+EFFECT_LEV)); \
         CO2_BS  = POSPART((DR_BS/DRR_BS)*ATPF_BS*(1.0-STU_BS)*(1.0+EFFECT_LEV)); \
...
// ------------------------------------------------------------- drug PK
// Continuous zero-order infusion inputs (RATE_<STEM>), not ev()/dosing
// events - preserved exactly as the original models them (guide's
// "continuous infusion, not an event" edge case): DOB, MIL, LEV, ESM, PHE.
dxdt_CENT_DOB = RATE_DOB - (CL_DOB/V1_DOB)*CENT_DOB;
dxdt_CENT_MIL = RATE_MIL - (CL_MIL/V1_MIL)*CENT_MIL;
dxdt_CENT_LEV = RATE_LEV - (CL_LEV/V1_LEV)*CENT_LEV
            - (Q_LEV/V1_LEV)*CENT_LEV + (Q_LEV/V2_LEV)*PERI_LEV;
dxdt_PERI_LEV = (Q_LEV/V1_LEV)*CENT_LEV - (Q_LEV/V2_LEV)*PERI_LEV;
// OR-1896 (bespoke): fed by LEV's own clearance fraction FM_LEV, not its own
// dose/depot - same pattern as acne-vulgaris OXO (fed from ISO's KMET_ISO).
dxdt_CENT_OR  = FM_LEV*(CL_LEV/V1_LEV)*CENT_LEV - (CL_OR/V1_OR)*CENT_OR;
dxdt_CENT_ESM = RATE_ESM - (CL_ESM/V1_ESM)*CENT_ESM;
dxdt_GUT_MET  = -KA_MET*GUT_MET;
dxdt_CENT_MET = F_MET*KA_MET*GUT_MET - (CL_MET/V1_MET)*CENT_MET;
dxdt_GUT_CAR  = -KA_CAR*GUT_CAR;
dxdt_CENT_CAR = F_CAR*KA_CAR*GUT_CAR - (CL_CAR/V1_CAR)*CENT_CAR;
dxdt_CENT_PHE = RATE_PHE - (CL_PHE/V1_PHE)*CENT_PHE;
dxdt_GUT_RAM  = -KA_RAM*GUT_RAM;
dxdt_CENT_RAM = F_RAM*KA_RAM*GUT_RAM - (CL_RAM/V1_RAM)*CENT_RAM;
dxdt_GUT_FUR  = -KA_FUR*GUT_FUR;
dxdt_CENT_FUR = F_FUR*KA_FUR*GUT_FUR - (CL_FUR/V1_FUR)*CENT_FUR;
dxdt_GUT_APX  = -KA_APX*GUT_APX;
dxdt_CENT_APX = F_APX*KA_APX*GUT_APX - (CL_APX/V1_APX)*CENT_APX;
...
dxdt_ANGII = KREN_I*RENDRV*(1.0 - EFFECT_RAM) - KANG_O*ANGII;
...
double SVRT = SVR0*(1.0 + EA1_SVR*(1.0 - ENO*NOF)*(OA1 - OA1_0)/fmax(OA1_0,1e-6)
                    ...
                    - KATP_LEV*EFFECT_LEV - E_IABP*IABP);
...
dxdt_VOL = KVIN*(1.0 + EALD*(ALDO - 1.0))
           - KVOUT*VOL*(1.0 + EBNP*BNPF/(BNPF + KBNPV)) - KFUR_V*EFFECT_FUR;
...
// -------------------------------------------------------- electrophysiology
dxdt_KPL = KK_IN + KSUP - KK_OUT*KPL - KK_FUR*EFFECT_FUR - KK_CAT*POSPART(O2_BS - O20_BS);
...
// ---------------------------------------------------------------- thrombus
dxdt_THR  = KTF*STASIS*HYPERC*(1.0 - 0.75*EFFECT_APX) - KTL*(1.0 + KAC_LYS*EFFECT_APX)*THR;
dxdt_HEMB = H0_EMB*THR*(1.0 - 0.70*EFFECT_APX);

// --------------------------------------------------------------- recurrence
double PHBAR = FAP*PHOS_AP + FMD*PHOS_MD + FBS*PHOS_BS;
dxdt_HREC = H0_REC*(1.0 + W_PHREC*PHBAR)*(1.0 - E_ACEREC*EFFECT_RAM)
...
C_DOB  : Dobutamine plasma concentration (mg/L)
EFFECT_DOB : Dobutamine added beta-agonist intrinsic-activity fraction (-)
C_MIL  : Milrinone plasma concentration (mg/L)
EFFECT_MIL : Milrinone fractional PDE3 inhibition (-)
C_LEV  : Levosimendan plasma concentration (mg/L)
C_OR   : OR-1896 (levosimendan active metabolite) plasma concentration (mg/L)
X_LEV  : Levosimendan + OR-1896 potency-weighted composite concentration (mg/L)
EFFECT_LEV : Myofilament Ca sensitisation, shared LEV+OR-1896 Hill effect (-)
C_ESM  : Esmolol plasma concentration (mg/L)
EFFECT_ESM_B1 : Esmolol competitive-occupancy term at beta1 (-)
EFFECT_ESM_B2 : Esmolol competitive-occupancy term at beta2 (-)
C_MET  : Metoprolol plasma concentration (mg/L)
EFFECT_MET_B1 : Metoprolol competitive-occupancy term at beta1 (-)
EFFECT_MET_B2 : Metoprolol competitive-occupancy term at beta2 (-)
C_CAR  : Carvedilol plasma concentration (mg/L)
EFFECT_CAR_B1 : Carvedilol competitive-occupancy term at beta1 (-)
EFFECT_CAR_B2 : Carvedilol competitive-occupancy term at beta2 (-)
EFFECT_CAR_A1 : Carvedilol competitive-occupancy term at alpha1 (-)
C_PHE  : Phenylephrine plasma concentration (mg/L)
EFFECT_PHE : Phenylephrine potency-weighted alpha1 agonist drive (nmol/L equiv.)
C_RAM  : Ramiprilat plasma concentration (mg/L)
EFFECT_RAM : Fractional ACE inhibition = ramiprilat Hill effect (-)
C_FUR  : Furosemide plasma concentration (mg/L)
EFFECT_FUR : Fractional natriuretic effect = furosemide Hill effect (-)
C_APX  : Apixaban plasma concentration (mg/L)
EFFECT_APX : Fractional anticoagulation = apixaban Hill effect (-)
...
##  BUILD
## -----------------------------------------------------------------------------
mod <- mcode_cache("tts_qsp_refactored", tts_code_refactored, atol = 1e-8, rtol = 1e-8,
                   maxsteps = 2000000)
...
oral_events <- list(
  ramipril   = ev(amt = 5,  cmt = "GUT_RAM", ii = 24, addl = 364, time = 24),
  metoprolol = ev(amt = 50, cmt = "GUT_MET", ii = 12, addl = 729, time = 24),
  apixaban   = ev(amt = 5,  cmt = "GUT_APX", ii = 12, addl = 179, time = 24),
  furosemide = ev(amt = 40, cmt = "GUT_FUR", ii = 12, addl = 13,  time = 12),
  carvedilol = ev(amt = 12.5, cmt = "GUT_CAR", ii = 12, addl = 729, time = 24)
)
...
recurrence_1y <- function() {
  ...
  do.call(rbind, lapply(names(arms), function(a) {
    ...
    data.frame(arm = a,
               ...
               ACEINH = round(max(o$EFFECT_RAM), 3),
               row.names = NULL)
  }))
}
