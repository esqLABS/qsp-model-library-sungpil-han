...
(the derivation is in CALIBRATION §D of `gdm_mrgsolve_model_en.R`):
...
| File | Contents |
|---|---|
| [`gdm_qsp_model.dot`](gdm_qsp_model.dot) | Mechanistic map source (Graphviz) — 208 nodes · 14 clusters · 316 edges |
| [`gdm_qsp_model.svg`](gdm_qsp_model.svg) | Vector map (zoomable, searchable) |
| [`gdm_qsp_model.png`](gdm_qsp_model.png) | Raster map (150 dpi) |
| [`gdm_mrgsolve_model_en.R`](gdm_mrgsolve_model_en.R) | **31-ODE** mrgsolve model + 10 scenarios + calibration rationale |
| [`gdm_shiny_app_en.R`](gdm_shiny_app_en.R) | **10-tab** Shiny dashboard |
| [`gdm_references_en.md`](gdm_references_en.md) | **87** references (every PMID machine-verified) |
...
source("gdm_mrgsolve_model_en.R")
sims <- run_all()
endpoint_table(sims)
...
```r
shiny::runApp("gdm_shiny_app_en.R")
```
