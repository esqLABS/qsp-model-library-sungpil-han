# Major Thermal Burn Injury — QSP model references
# References for the Major Thermal Burn Injury QSP model

> **On provenance.** The 352 entries below were produced by actually querying NCBI E-utilities (`esearch` + `esummary`)
> with search terms matching the subject of each section, and transferring **only the records returned**.
> Titles, authors, journals, years, and PMIDs are all exactly as NCBI returned them, and not one entry was
> filled in from memory or by guesswork. The searches were run in 2026-08.
>
> **Which reference each quantitative claim of the model hangs on** is tabulated at the very end in
> [Appendix A](#appendix-a--which-section-each-component-of-the-model-hangs-on).

---

## The numbers this model took from the literature (calibration targets)

| # | Calibration target | Literature value | Model value | Section |
|---|-----------|--------|--------|---------|
| 1 | 24-hour fluid volume of UO-titrated Parkland | 5.2–6.7 mL/kg/%TBSA | 5.97 | C |
| 2 | in:out ratio (volume given ÷ Parkland prescription) | 1.2–1.6 | 1.49 | C |
| 3 | Intra-abdominal hypertension threshold | about 250 mL/kg/24 h | 269 mL/kg → IAP 14.2 mmHg | D |
| 4 | Plasma volume nadir | 60–80 % of baseline | 62 % | B |
| 5 | 24-hour plasma colloid osmotic pressure | 10–16 mmHg (baseline about 26) | 13.0 | B |
| 6 | Maximum weight gain | +15~30 % | +20 % | C·D |
| 7 | Maximum REE | 120–180 % of predicted | 157 % (45 %TBSA) | F |
| 8 | Persistence of the raised REE | months to years after wound closure | 135 % on day 60 | F |
| 9 | Change in lean body mass (14 days, no drug) | about −9 % | −7.1 % | F·G |
| 10 | Change in lean body mass (14 days, propranolol) | about +9 % | see the text below | G |
| 11 | Fluid sparing with high-dose ascorbate | −45 % | −36 % | E |
| 12 | Fluid sparing with albumin | −20~−40 % | −5 % (started at 8 hours) / −38 % (started at 0 hours) | C |
| 13 | Invasive wound infection threshold | above 10⁵ CFU/g | model BTHRESH = 5 log₁₀ | K |
| 14 | Agreement with the revised Baux score | LD50 about 110 | mean error 3.2 %p across 8 patients | A |
| 15 | Augmented renal clearance in burn patients | CrCl above 130 mL/min | ×1.55 in the flow phase | L |

**Where the model disagrees with the literature, it has been left in place rather than erased.** The most exposed prediction is number 12.
The model says that albumin started at 8 hours barely reduces the fluid volume, and it offers a mechanistic
reason for that — under Parkland's front-loaded first-8-hour delivery, most of the dilution of colloid osmotic
pressure is over by the 8-hour mark. Start the same colloid fraction at 0 hours and it is −38 %.
That is a testable claim, and if the start time does not matter that much in a trial then this structure is
wrong. That number 11 (ascorbate) reproduces Tanaka's value well is not a victory but a problem —
no subsequent trial has reproduced that result, so the model has in effect been tuned to a finding that may not be
true.

---

## A. Epidemiology · severity · prognosis
...
## B. The microvascular mechanism of burn oedema
...
## C. Resuscitation formulae and fluid creep
...
## E. High-dose vitamin C (ascorbate)
...
## K. Infection and immunoparalysis
...
## N. Inflammation, cytokines, liver
...
## O. Bone · growth · long-term outcome
...
## P. Special populations · electrolytes
...
## Q. Nutrition and micronutrients
...
## R. QSP and modelling methodology
...
## Appendix A — Which section each component of the model hangs on

| Model component | Location in the file | Section |
|---|---|---|
| The Landis–Pappenheimer colloid osmotic pressure equation | `landis_pappenheimer()` / `LP()` in R | B |
| The Starling equation (regional Kf, sigma, Pc, Pi) | `Jvb` and `Jvu` in `derive()` | B |
| Negative pressure in burned tissue (imbibition) | `IMB0`, `TAUIMB` | B |
| Perfusion limitation (a delivery ceiling and no-reflow) | `Jvb_cap`, `STASIS0`, `QPB0` | B |
| Saturation of lymphatic return | `lymph()`, `LBASE`, `LMAX`, `KL` | B |
| The closed-loop resuscitation controller | the `RSTATE` ODE, `GCTRL`, `DRMAX`, `TAUUO` | C |
| Fluid creep · the in:out ratio | the `creep_uncapped` scenario | C |
| Opioid creep | `KOPI` | C |
| Colloid restores f_ret | `FCOL`, `ALBSTART` | C |
| Intra-abdominal pressure · abdominal compartment syndrome | `IAPK`, `IAPB`, `IAPQ` | D |
| The action of ascorbate on Kf | `EMAXVIT`, `KROSKF` | E |
| Hypermetabolism (driven by A_open) | the `REE` ODE, `EMAXHM`, `A50`, `HILLA` | F |
| Evaporative heat loss | `KEVAP` | F |
| Propranolol (blockade of the transmitter) | `prBlock`, `EC50PR`, `KSPR` | G |
| Oxandrolone (the synthesis term) | `oxEff`, `EMAXOX` | H |
| Insulin · glucose · hypoglycaemic exposure | the `GLC`/`INS`/`HYPOH` ODEs, `GFEED` | I |
| Early excision · grafting · the donor site constraint | `apply_events()`, `DONPOOLF`, `MESH` | J |
| The quantitative bacteriology threshold · immunoparalysis | the `BWD`/`HLADR`/`BSYS` ODEs | K |
| Augmented renal clearance · vancomycin | `arc`, `CLVC`, `VVC` | L |
| The fluid multiplier and risk multiplier of inhalation injury | `INH`, `HZINH` | M |
| IL-6 · negative acute-phase proteins | the `IL6` ODE, `albSyn` | N |
| Bone · growth | the `BMC` ODE, `KBONE` | O |
| Paediatric · elderly · pregnancy · electrical burns | `build(..., WT/AGE/TBSA/INH)` | P |
| Early enteral nutrition · glutamine | `sc["nutrition"]`, `GFEED`, `TFEED` | Q |
| QSP methodology · mrgsolve | the whole model | R |

---

## Appendix B — How to reproduce this list
...
The search terms for each section were queried as above, then the bibliographic details were fetched with `esummary.fcgi`
and transferred verbatim. Duplicate PMIDs were kept only in the section where they first appeared.

---

## ⚠️ Disclaimer

This reference list exists **to make it possible to trace where the assumptions and parameters of the model came
from**; it is not a clinical practice guideline. The points at which the conclusions of an individual paper and the model
implementation do not agree are set out in the table above and in the "Where the model is wrong" section of `README_en.md`.
