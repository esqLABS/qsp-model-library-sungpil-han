...
# =====================================================================
# REFACTOR (this file only; original vwd_mrgsolve_model.R is untouched):
#   Per FORK_WORKFLOW_GUIDE.md Part 2, all 4 compounds this file models were
#   rewritten to the guide's pluggable-PK naming convention, per their
#   existing rows in driver-patches/data/compound_perturbation_census.md
#   (all classified "Redirect concentration (clean single site)"):
#     - DDAVP: Archetype 3 minus peripheral (depot+central, linear),
#       DDAVP_DEPOT/DDAVP_CP -> GUT_DDAVP/CENT_DDAVP, KE_DDAVP+V_DDAVP ->
#       CL_DDAVP+V1_DDAVP, HILL_DDAVP -> GAMMA_DDAVP, new EMAX_DDAVP=1
#       (implicit ceiling of the original ratio made explicit), effect
#       exposed as EFFECT_DDAVP.
#     - RVWF (recombinant VWF, vonicog alfa-like): Archetype 1 (no depot,
#       single compartment, linear), RVWF_CP -> CENT_RVWF, KE_RVWF+V_RVWF ->
#       CL_RVWF+V1_RVWF. No EFFECT_RVWF -- see notes (bespoke: linear
#       mass-balance replacement, not a saturating Hill effect).
#     - PDVWF (plasma-derived VWF/FVIII concentrate): Archetype 1, same
#       treatment as RVWF, PDVWF_CP -> CENT_PDVWF, KE_PDVWF+V_PDVWF ->
#       CL_PDVWF+V1_PDVWF. No EFFECT_PDVWF, same rationale as RVWF.
#     - TXA: Archetype 3 minus peripheral (depot+central, linear),
#       TXA_DEPOT/TXA_CP -> GUT_TXA/CENT_TXA, KE_TXA+V_TXA -> CL_TXA+V1_TXA,
#       EMAX_TXA_BLEED -> EMAX_TXA, new GAMMA_TXA=1 (original had no
#       explicit Hill exponent), effect exposed as EFFECT_TXA.
#   Two pre-existing mrgsolve-2.0.1 build defects (unrelated to any
#   compound's own PK) were fixed with a syntax-only, non-numeric patch so
#   this file actually compiles -- logged as upstream issue #72 in
#   translations/UPSTREAM_ISSUES.md: (1) $CAPTURE duplicated 12 compartment
#   names, removed (compartments are already reported automatically); (2)
#   $MAIN set 14 bare compartment names directly (e.g. "WPB_STORE = WPB0;"),
#   converted to the "<CMT>_0" initial-value idiom this mrgsolve build
#   requires (e.g. "WPB_STORE_0 = WPB0;").
#   A third, DDAVP-specific numerical-robustness fix (inside this refactor's
#   own scope, so fixed as part of it, not logged separately): EFFECT_DDAVP's
#   fractional pow(C_DDAVP, 1.3) NaN's if C_DDAVP numerically undershoots
#   zero (which the original's equivalent DDAVP_CP does too, at ~1e-10
#   scale, in this file's own repeat-dosing scenario) -- guarded with
#   fmax(C_DDAVP, 0.0) before the pow(). See vwd_refactor_notes.md for full
#   detail, renaming tables, and qspserver verification results.
# =====================================================================
...
CL_DDAVP   : 6.9    : DDAVP clearance (L/h) [= KE_DDAVP 0.23 x V_DDAVP 30.0; CL/V1 = 0.23 /h, t1/2 ~3h]
V1_DDAVP   : 30.0   : DDAVP apparent central volume (L) [renamed from V_DDAVP]

// ============================================
// Recombinant VWF (vonicog alfa-like) PK — Mannucci 2013 / Gill 2015
// ============================================
CL_RVWF    : 1.155  : rVWF:RCo clearance (dL/h) [= KE_RVWF 0.033 x V_RVWF 35.0; CL/V1 = 0.033 /h, t1/2 ~21h]
V1_RVWF    : 35.0   : rVWF apparent Vd (dL; ~plasma volume, ~2 IU/dL rise per IU/kg dosed) [renamed from V_RVWF]

// ============================================
// Plasma-derived VWF/FVIII concentrate PK — Dobrkovska 1998 (Humate-P-like)
// ============================================
CL_PDVWF     : 1.575  : PD-concentrate VWF:RCo clearance (dL/h) [= KE_PDVWF 0.045 x V_PDVWF 35.0; CL/V1 = 0.045 /h, t1/2 ~15h]
V1_PDVWF     : 35.0   : PD-concentrate VWF apparent Vd (dL; ~plasma volume) [renamed from V_PDVWF]
...
CL_TXA     : 7.25   : TXA clearance (L/h) [= KE_TXA 0.29 x V_TXA 25.0; CL/V1 = 0.29 /h, t1/2 ~2.4h]
V1_TXA     : 25.0   : TXA apparent Vd (L) [renamed from V_TXA]
...
EMAX_TXA       : 0.30   : Max fractional bleeding-score reduction from TXA [renamed from EMAX_TXA_BLEED]
EC50_TXA       : 8.0    : TXA conc. for half-max antifibrinolytic effect (mcg/mL)
GAMMA_TXA      : 1.0    : Hill coefficient (new; original had no explicit Hill exponent)
...
$CMT @annotated
GUT_DDAVP   : DDAVP SC/IN depot (mcg)
CENT_DDAVP  : DDAVP central amount (mcg equiv; C_DDAVP = CENT_DDAVP/V1_DDAVP is the exposed conc., ng/mL)
CENT_RVWF   : Recombinant VWF circulating amount (IU; C_RVWF = CENT_RVWF/V1_RVWF is the exposed conc., IU/dL)
CENT_PDVWF  : Plasma-derived VWF/FVIII concentrate circulating amount (IU; C_PDVWF = CENT_PDVWF/V1_PDVWF is the exposed conc., IU/dL)
GUT_TXA     : Tranexamic acid oral depot (mg)
CENT_TXA    : Tranexamic acid central amount (mg equiv; C_TXA = CENT_TXA/V1_TXA is the exposed conc., mcg/mL)
...
if (NEWIND <= 1) {
  WPB_STORE_0   = WPB0;
  VWFPP_0       = VWFAG_BASE * 0.10;
  VWF_AG_0      = VWFAG_BASE;
  VWF_RCO_0     = VWFRCO_BASE;
  HMWM_0        = HMWM_BASE;
  ADAMTS13_ACT_0= ADAMTS13_BASE;
  FVIII_C_0     = FVIII_BASE;
  PLT_COUNT_0   = PLT_BASE;
  BLEED_SCORE_0 = BLEED_BASE;
  MENS_LOSS_0   = MENS_BASE;
  GI_LOSS_0     = GI_BASE;
  HB_0          = HB_BASE;
  NA_SERUM_0    = NA_BASE;
  THROMB_RISK_0 = 0;
...
$ODE
// ---- PK ----
double C_DDAVP = CENT_DDAVP / V1_DDAVP;
dxdt_GUT_DDAVP  = -KA_DDAVP * GUT_DDAVP;
dxdt_CENT_DDAVP =  KA_DDAVP * GUT_DDAVP - CL_DDAVP/V1_DDAVP * CENT_DDAVP;
// NB: F_GUT_DDAVP (mrgsolve F_<cmt> bioavailability, set in $MAIN) is applied
// once at the dosing event, exactly as the original applied F_DDAVP_DEPOT --
// no separate F_DDAVP factor is added in this flow term (that would double it).

double C_RVWF  = CENT_RVWF / V1_RVWF;      // IU/dL
double C_PDVWF = CENT_PDVWF / V1_PDVWF;    // IU/dL
dxdt_CENT_RVWF   = -CL_RVWF  * C_RVWF;     // dosed directly (IU) via bolus into CENT_RVWF
dxdt_CENT_PDVWF  = -CL_PDVWF * C_PDVWF;    // dosed directly (IU) via bolus into CENT_PDVWF

double C_TXA = CENT_TXA / V1_TXA;
dxdt_GUT_TXA  = -KA_TXA * GUT_TXA;
dxdt_CENT_TXA =  KA_TXA * GUT_TXA - CL_TXA/V1_TXA * CENT_TXA;
// NB: F_GUT_TXA is applied at the dosing event (same mechanism as DDAVP above).

// ---- DDAVP -> WPB release (V2-like endothelial receptor) ----
double C_DDAVP_nn = fmax(C_DDAVP, 0.0); // guard: fractional pow() on a state that can numerically undershoot 0 at ~1e-10 scale would otherwise NaN (see refactor notes)
double EFFECT_DDAVP = EMAX_DDAVP * pow(C_DDAVP_nn, GAMMA_DDAVP) / (pow(EC50_DDAVP, GAMMA_DDAVP) + pow(C_DDAVP_nn, GAMMA_DDAVP));
double WPB_RELEASE_RATE = K_WPBDEPLETE * WPB_STORE * EFFECT_DDAVP;
...
double EXO_VWF_TOTAL = C_RVWF + C_PDVWF;    // exogenous VWF:RCo-equivalent contribution (own PK decay)
...
// ---- FVIII:C: endogenous stabilization target raised by BOTH endogenous VWF:Ag and exogenous rVWF (C_RVWF);
...
double FVIII_STAB_DRIVE = fmin(1.0, (VWF_AG + FVIII_STAB_GAIN * C_RVWF) / (VWFAG_BASE + 1e-6));
...
double FVIII_EXO_DIRECT = C_PDVWF / VWF_FVIII_RATIO;   // co-formulated FVIII in PD concentrate: immediate, own PK decay
...
double EFFECT_TXA = EMAX_TXA * pow(C_TXA, GAMMA_TXA) / (pow(EC50_TXA, GAMMA_TXA) + pow(C_TXA, GAMMA_TXA));
double BLEED_TARGET = BLEED_TARGET_RAW * (1 - EFFECT_TXA);
...
double MENS_TARGET = MENS_TARGET_RAW * (1 - HORMONAL_REDUX * HORMONAL_ON) * (1 - EFFECT_TXA);
...
double NA_DROP = EMAX_NA_DROP * (1 - 0.6*FLUID_RESTRICT) * C_DDAVP / (EC50_NA + C_DDAVP);
...
$CAPTURE VWF_AG_TOTAL VWF_RCO_TOTAL HMWM_EFFECTIVE FVIII_C_TOTAL C_DDAVP EFFECT_DDAVP C_RVWF C_PDVWF C_TXA EFFECT_TXA
...
scenarios <- list(
  ...
  "2_Type1_DDAVP_IV_single"         = make_ev(0.3 * WT, 0, 0, "GUT_DDAVP"),
  "3_Type1_DDAVP_Intranasal_Repeat" = make_ev(300, 12, 5, "GUT_DDAVP"),
  "4_Type2B_DDAVP_Contraindicated"  = make_ev(0.3 * WT, 0, 0, "GUT_DDAVP"),
  "5_Type3_Severe_rVWF_Vonvendi"    = make_ev(50 * WT, 24, 6, "CENT_RVWF"),
  "6_Type3_Severe_PDConcentrate"    = make_ev(50 * WT, 24, 6, "CENT_PDVWF"),
  "7_Menorrhagia_TXA_Hormonal"      = make_ev(1300, 8, 89, "GUT_TXA"),
  "8_Acquired_VWS_DDAVP_transient"  = make_ev(0.3 * WT, 0, 0, "GUT_DDAVP"),
  "9_Pregnancy_Type1_Peripartum"    = NULL,
  "10_Surgical_Major_PKguided_PDConc" = make_ev(60 * WT, 12, 20, "CENT_PDVWF")
)
...
#  - rVWF (CENT_RVWF; C_RVWF = CENT_RVWF/V1_RVWF) CL_RVWF/V1_RVWF corresponds
#    to VWF:RCo t1/2 ~21h (Mannucci 2013 Blood phase-1 PK); because rVWF only
#    raises the FVIII_STAB_TARGET (and FVIII_C relaxes toward it at
#    KOUT_FVIII, t1/2 ~9h) rather than adding directly to plasma FVIII, the
#    model reproduces the genuinely delayed/gradual post-infusion FVIII:C
#    climb reported for a VWF-only product in the Gill 2015 Blood pivotal
#    trial (vonicog alfa has no co-formulated FVIII, unlike plasma-derived
#    concentrate).
#  - Plasma-derived concentrate (CENT_PDVWF; C_PDVWF = CENT_PDVWF/V1_PDVWF)
#    CL_PDVWF/V1_PDVWF and VWF_FVIII_RATIO=2.4 approximate Humate-P-class
#    VWF:RCo t1/2 ~12-20h / FVIII:C t1/2 ~8-12h (Dobrkovska 1998 Haemophilia).
