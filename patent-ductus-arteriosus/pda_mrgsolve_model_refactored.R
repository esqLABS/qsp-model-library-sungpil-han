...
KE0_IND  :   0.12    : effect-site equilibration, indomethacin (1/h) - slow, representing slowly-reversible tight COX binding
...
GAMMA_APAP:  1.0     : Hill coefficient, acetaminophen ductal peroxidase effect [new]
...
IC50_APAP_K: 260.0   : unbound IC50, acetaminophen at renal COX (uM) - renal peroxide tone is high, so the peroxidase mechanism is weak
...
$CMT @annotated
CENT_IBU    : ibuprofen central (mg/kg)
PERI_IBU    : ibuprofen peripheral (mg/kg)
EFF_IBU    : ibuprofen ductal effect site (mg/L)
CENT_IND    : indomethacin central (mg/kg)
PERI_IND    : indomethacin peripheral (mg/kg)
EFF_IND    : indomethacin ductal effect site (mg/L)
GUT_APAP   : acetaminophen enteral depot (mg/kg)
CENT_APAP   : acetaminophen central (mg/kg)
PERI_APAP   : acetaminophen peripheral (mg/kg)
...
#define PDA_ALG(TT)                                                        \\
  double PNAd = (TT) / 24.0;                                               \\
  /* ---- drug concentrations ---- */                                      \\
  /* ---- exposed concentrations: the single redirect point per compound ---- */\\
  double C_IBU  = fmax(0.0, CENT_IBU)  / V1_IBU;                               \\
  double C_IND  = fmax(0.0, CENT_IND)  / V1_IND;                               \\
  double C_APAP = fmax(0.0, CENT_APAP) / V1_APAP;                              \\
  double U_IBU  = C_IBU  * FU_IBU  * 1000.0 / MW_IBU;                      \\
  double U_IND  = C_IND  * FU_IND  * 1000.0 / MW_IND;                      \\
  double U_APAP = C_APAP * FU_APAP * 1000.0 / MW_APAP;                     \\
  double U_IBU_EFF = fmax(0.0, EFF_IBU) * FU_IBU * 1000.0 / MW_IBU;           \\
  double U_IND_EFF = fmax(0.0, EFF_IND) * FU_IND * 1000.0 / MW_IND;           \\
  ...
  /* ---- per-compound ductal (central) COX effect: the Hill/occupancy    \\
     ...
     part of the real pharmacology, not an artifact of the old macro; this\\
     is a rename that splits the original single `occD` sum into its two \\
     named addends, not a re-fit (see refactor notes).  EFFECT_APAP is    \\
     already exactly Hill-shaped in the original (Emax=1, gamma=1,        \\
     EC50=ic50D) and is pulled out unchanged. ---- */                     \\
  double EFFECT_IBU  = U_IBU_EFF / KI_IBU;                                 \\
  double EFFECT_IND  = U_IND_EFF / KI_IND;                                 \\
  ...
  double vcind = VC_IND * C_IND / (C_IND + VC50_IND);                        \\
  double vcibu = VC_IBU * C_IBU / (C_IBU + VC50_IBU);                        \\
  ...
  double NAPQI = FNAPQI * CL_APAP0 * C_APAP;
...
double c1i = CENT_IBU / V1_IBU;
double c2i = PERI_IBU / V2_IBU;
dxdt_CENT_IBU = -clibu * c1i - Q_IBU * (c1i - c2i);
dxdt_PERI_IBU =  Q_IBU * (c1i - c2i);
dxdt_EFF_IBU =  KE0_IBU * (C_IBU - EFF_IBU);

double c1n = CENT_IND / V1_IND;
double c2n = PERI_IND / V2_IND;
dxdt_CENT_IND = -clind * c1n - Q_IND * (c1n - c2n);
dxdt_PERI_IND =  Q_IND * (c1n - c2n);
dxdt_EFF_IND =  KE0_IND * (C_IND - EFF_IND);

double c1a = CENT_APAP / V1_APAP;
double c2a = PERI_APAP / V2_APAP;
dxdt_GUT_APAP = -KA_APAP * GUT_APAP;
dxdt_CENT_APAP =  F_APAP * KA_APAP * GUT_APAP - clapap * c1a - Q_APAP * (c1a - c2a);
dxdt_PERI_APAP =  Q_APAP * (c1a - c2a);
...
double bftgt = (TBILI / TBILI0) * (1.0 + BILIDISP_IBU * C_IBU);
...
double indprot = 1.0 - IVH_IND_PROT * (EFF_IND / (EFF_IND + 0.25));
...
dxdt_HSIP = H_SIP0 * exp(0.30 * fmax(0.0, 28.0 - GA))
          * (1.0 + B_SIP_IND * (EFF_IND / (EFF_IND + 0.3)))
          * (1.0 + B_SIP_HC * HCORT);
...
C_IBU    : total ibuprofen concentration (mg/L)
C_IND    : total indomethacin concentration (mg/L)
C_APAP   : total acetaminophen concentration (mg/L)
...
rx_ibuprofen <- function(start_h = 48, high = FALSE) {
  ...
  ev(time = start_h + c(0, 24, 48), amt = d, cmt = "CENT_IBU")
}
...
rx_indomethacin <- function(start_h = 48, dose = 0.2, maint = 0.1, n = 3,
  ...
  ev(time = start_h + interval * (0:(n - 1)),
     amt  = c(dose, rep(maint, n - 1)), cmt = "CENT_IND")
}
...
rx_acetaminophen <- function(start_h = 48, dose = 15, interval = 6,
  ...
  ev(time = start_h + interval * (0:(n - 1)), amt = dose,
     cmt = if (iv) "CENT_APAP" else "GUT_APAP")
}
...
maturation_table <- function() {
  ...
  auc <- function(out, d0, d1) {
    ...
    sum(diff(d$DAY[m]) * (head(d$C_IBU[m], -1) + tail(d$C_IBU[m], -1)) / 2)
  }
  data.frame(
