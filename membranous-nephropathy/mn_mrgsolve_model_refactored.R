...
#   Drug PK:
#     1.  CENT_RTX       — Rituximab central compartment (µg/mL)
#     2.  PERI_RTX       — Rituximab peripheral compartment (µg/mL)
#     3.  COMPLEX_RTX    — RTX-CD20 bound complex (TMDD, µg/mL equiv.)
#     4.  CENT_TAC       — Tacrolimus whole blood (ng/mL)
#     5.  CENT_CPX       — Cyclophosphamide central (µg/mL)
#     6.  MET_CPX        — Active 4-OH-cyclophosphamide metabolite (µg/mL)
#
#   [PK/PD refactor] Rituximab (RTX), Tacrolimus (TAC), and Cyclophosphamide (CPX)
#   were renamed to this fork's pluggable-PK naming convention (CENT_/PERI_/
#   COMPLEX_/GUT_/C_/EFFECT_<STEM>) and each now exposes exactly one named
#   concentration (C_RTX, C_TAC, C_CPX) and effect term (EFFECT_RTX, EFFECT_TAC,
#   EFFECT_CPX) for external driving. See mn_refactor_notes.md for the full
#   archetype determination, naming table, and verification result. TAC's
#   absorption depot is GUT_TAC (not separately numbered above, matches the
#   original's own unnumbered TAC_gut). Every other compound (avacopan, ACEi) is
#   untouched.
#   Immune/Disease:
...
V2_RTX   = 2.8     // L      peripheral volume
Q_RTX    = 0.7     // L/day  intercompartmental clearance
// TMDD parameters for RTX-CD20 binding
KD_RTX   = 0.010   // µg/mL  dissociation constant (~10 nM) -- unused in $ODE in the
                    // original (koff_RTX is set directly, not derived from Kd*kon at
                    // runtime); kept as documentation-only, exactly as in the original
...
// -------------------------------------------------------
// [refactor] renamed: Vd_TAC->V1_TAC, ka_TAC->KA_TAC. Values unchanged.
CL_TAC   = 25.0    // L/h  apparent oral clearance (CL/F)
V1_TAC   = 900.0   // L    apparent volume of distribution
F_TAC    = 0.25    // bioavailability (oral) -- declared but NOT applied anywhere in the
                    // original $ODE (dxdt_TAC_blood has no F_TAC term); kept unused,
                    // exactly as in the original, to preserve numerical behavior
...
// -------------------------------------------------------
// [refactor] renamed: CL_CPx->CL_CPX, Vd_CPx->V1_CPX, km_CPx->KM_CPX,
// kel_met->KEL_CPX. Values unchanged.
CL_CPX   = 4.5     // L/h  CPx (parent prodrug) clearance
V1_CPX   = 35.0    // L    CPx (parent prodrug) volume
KM_CPX   = 0.15    // 1/h  metabolic activation rate (CPx → 4OH-CPx)
KEL_CPX  = 0.8     // 1/h  elimination of active metabolite
...
// -------------------------------------------------------
// [refactor] renamed to EMAX_TAC/EC50_TAC/GAMMA_TAC convention; GAMMA_TAC is new
// (original had no explicit Hill coefficient, i.e. GAMMA_TAC = 1 reproduces it exactly)
EC50_TAC  = 5.0    // ng/mL  half-maximal effect on Ab production
EMAX_TAC  = 0.6    // maximal fractional inhibition of Ab production (60%)
GAMMA_TAC = 1.0    // Hill coefficient (none in original; 1 = exact reproduction)

// -------------------------------------------------------
// Cyclophosphamide effect on plasma cells
// Reference: Ponticelli C et al., NEJM 1989; Scolari JASN 2021
// -------------------------------------------------------
// [refactor] renamed to EMAX_CPX/EC50_CPX/GAMMA_CPX convention; GAMMA_CPX is new
// (original had no explicit Hill coefficient, i.e. GAMMA_CPX = 1 reproduces it exactly)
EC50_CPX  = 0.5    // µg/mL  active metabolite EC50 on plasma cells
EMAX_CPX  = 0.85   // maximal kill fraction of plasma cells (85%)
GAMMA_CPX = 1.0    // Hill coefficient (none in original; 1 = exact reproduction)
...
// [refactor] renamed to convention: RTX_cent->CENT_RTX, RTX_peri->PERI_RTX,
// RTX_CD20_bound->COMPLEX_RTX, TAC_gut->GUT_TAC, TAC_blood->CENT_TAC,
// CPx_cent->CENT_CPX, CPx_metab->MET_CPX
CENT_RTX
PERI_RTX
COMPLEX_RTX
GUT_TAC
...
// (RTX_conc = RTX_cent; etc.) computed it.
C_RTX = CENT_RTX;                         // µg/mL
C_TAC = CENT_TAC;                         // ng/mL
C_CPX = MET_CPX;                          // µg/mL  -- active metabolite (this is what
                                           // the Emax term reads, not the parent CENT_CPX)

// -------------------------------------------------------
// Drug effect terms (EFFECT_<STEM>)
// -------------------------------------------------------
// Tacrolimus: inhibits Ab production (via calcineurin → NFAT → plasma cell function)
// [refactor] renamed Emax_TAC->EFFECT_TAC; original ratio was already a plain
// Emax/EC50 shape, so this is a rename with GAMMA_TAC=1, not a refit.
EFFECT_TAC = EMAX_TAC * pow(C_TAC, GAMMA_TAC) / (pow(EC50_TAC, GAMMA_TAC) + pow(C_TAC, GAMMA_TAC) + 1e-9);

// Cyclophosphamide active metabolite: alkylates and kills plasma cells
// [refactor] renamed Emax_CPx->EFFECT_CPX; same rename-not-refit rationale.
EFFECT_CPX = EMAX_CPX * pow(C_CPX, GAMMA_CPX) / (pow(EC50_CPX, GAMMA_CPX) + pow(C_CPX, GAMMA_CPX) + 1e-9);

// Rituximab: [refactor] new named term, EFFECT_RTX. The original never wrote this as
// a standalone variable -- it inlined "keff_RTX * RTX_conc * CD20_B" directly into
// dxdt_CD20_B. Unlike TAC/CPX, this is NOT an Emax/EC50 saturating ratio: it is a
// bilinear (mass-action) per-capita kill-rate constant, unbounded in C_RTX. Forcing
// an EMAX_RTX/EC50_RTX/GAMMA_RTX Hill fit onto it would misrepresent the original,
// which never saturates as C_RTX grows -- see refactor notes for why the native
// functional form is kept instead of a forced Hill fit.
EFFECT_RTX = KEFF_RTX * C_RTX;            // 1/day, per-capita CD20+ B-cell kill rate

// Avacopan: reduces complement MAC (C5aR block) -- unchanged, out of scope
...
//    There is also no RTOT_RTX: CD20_B is a dynamic compartment with its own
//    turnover, not a fixed total receptor pool, so no fractional-occupancy term
//    (COMPLEX_RTX/RTOT_RTX) is constructed. This binding sink only depletes
//    CENT_RTX/PERI_RTX drug mass -- it does NOT feed back into CD20_B (that
//    depletion instead comes from the separate EFFECT_RTX term below, exactly
//    as the original wired it).
// -------------------------------------------------------
double ON_RTX  = KON_RTX * C_RTX * CD20_B;   // association flux
double OFF_RTX = KOFF_RTX * COMPLEX_RTX;      // dissociation flux
double INT_RTX = KINT_RTX * COMPLEX_RTX;      // internalization
...
// 2. PERI_RTX: peripheral compartment
dxdt_PERI_RTX = (Q_RTX/V1_RTX)*CENT_RTX - (Q_RTX/V2_RTX)*PERI_RTX;

// 3. COMPLEX_RTX: bound drug-receptor complex (TMDD)
dxdt_COMPLEX_RTX = ON_RTX - OFF_RTX - INT_RTX;

// -------------------------------------------------------
// 4. GUT_TAC: oral absorption compartment (transit)
// -------------------------------------------------------
dxdt_GUT_TAC   = -(KA_TAC/24.0)*GUT_TAC;   // ka converted to 1/day

// 5. CENT_TAC: whole blood compartment (ng/mL)
//    CL/F in L/h → convert to 1/day (* 24)
//    Dose enters GUT_TAC as amount (mg) in event table
dxdt_CENT_TAC = (KA_TAC/24.0)*GUT_TAC/(V1_TAC/1000.0)
                 - (CL_TAC*24.0/V1_TAC)*CENT_TAC;
...
// -------------------------------------------------------
dxdt_CENT_CPX  = -(CL_CPX*24.0/V1_CPX)*CENT_CPX
                 - KM_CPX*24.0*CENT_CPX;

// 7. MET_CPX: active 4-OH-cyclophosphamide
dxdt_MET_CPX = KM_CPX*24.0*CENT_CPX - KEL_CPX*24.0*MET_CPX;
...
// -------------------------------------------------------
double B_KILL_RTX = EFFECT_RTX * CD20_B;   // = keff_RTX * C_RTX * CD20_B, unchanged

dxdt_CD20_B    = kprol_B * Bss                       // constitutive replenishment
                 - kdeg_B * CD20_B                    // natural turnover
                 - B_KILL_RTX;                        // RTX depletion
...
double PC_kill = EFFECT_CPX * kdeg_PC * Plasma_cells;   // CPx cytotoxic effect
...
// -------------------------------------------------------
double Ab_prod = kdeg_Ab * Ab_ss * (Plasma_cells / PC_ss) * (1.0 - EFFECT_TAC);
...
// they are visible in API output (they are $ODE-local/$GLOBAL doubles, not
// compartments, so mrgsolve does not capture them automatically the way it does
// CENT_RTX/PERI_RTX/etc.). Note: they could not also be declared in $PARAM (see
// refactor notes -- mrgsolve 2.0.1 treats $PARAM members as read-only in $ODE,
// confirmed by test, same constraint already documented in the AMD refactor).
C_RTX C_TAC C_CPX EFFECT_RTX EFFECT_TAC EFFECT_CPX
...
# We model as direct bolus addition to CENT_RTX for simplicity with mrgsolve ev()
...
make_events <- function(scenario, bwt = 70) {
  ...
  if(scenario == "no_treatment") {
    ...
    ev_list <- ev(time = 0, cmt  = "CENT_RTX", amt = 0)
  }
  ...
  if(scenario == "TAC_mono") {
    ...
    # mrgsolve: daily dosing, absorbed via GUT_TAC
    # Amount in mg → ng: 1 mg = 1e6 ng; Vd = 900 L = 900,000 mL
    # CENT_TAC in ng/mL: 1 mg dose / 900 L = 1000/900 ng/mL ≈ 1.11 ng/mL per mg
    daily_mg  <- 0.05 * bwt
    dose_mg   <- daily_mg / 2   # BID
    # amt deposited into GUT_TAC (mg → converted internally)
  ...
  if(scenario == "RTX_TAC_combo") {
    ...
    ev_tac <- ev(time = tac_times, cmt  = "GUT_TAC", amt = dose_mg)
    dose_rtx <- 1000 / 3.1 * 1000 / 1000
    ev_rtx <- ev(time = 180, cmt  = "CENT_RTX", amt = dose_rtx)
...
pk1 <- ggplot(df_pk %>% filter(scenario %in% c("RTX_mono_MENTOR", "RTX_TAC_combo")),
              aes(x = time_months, y = CENT_RTX, color = scenario_label)) +
  geom_line(linewidth = 0.8) +
  scale_color_manual(values = pal) +
...
pk2 <- ggplot(df_pk %>% filter(scenario %in% c("TAC_mono", "RTX_TAC_combo")),
              aes(x = time_months, y = CENT_TAC, color = scenario_label)) +
  geom_line(linewidth = 0.8) +
...
pk3 <- ggplot(df_pk %>% filter(scenario == "Ponticelli"),
              aes(x = time_months, y = MET_CPX, color = scenario_label)) +
  geom_line(linewidth = 0.8) +
  scale_color_manual(values = pal) +
