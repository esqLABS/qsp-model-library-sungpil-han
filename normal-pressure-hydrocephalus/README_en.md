...
| File | Contents |
|---|---|
| [`inph_qsp_model.dot`](inph_qsp_model.dot) · [`.svg`](inph_qsp_model.svg) · [`.png`](inph_qsp_model.png) | Mechanistic map — **20 clusters, 255 nodes** |
| [`inph_reference_model.py`](inph_reference_model.py) | **The reference implementation.** Pure standard-library Python, 45 ODEs + 1 closed-form fast state. Every number below is computed here |
| [`inph_model_report.txt`](inph_model_report.txt) | The **committed output** of that script. The source of every figure in this README |
| [`inph_mrgsolve_model_en.R`](inph_mrgsolve_model_en.R) | mrgsolve implementation (46 compartments), 21 scenarios, titration-map and delay-sweep functions |
| [`inph_shiny_app_en.R`](inph_shiny_app_en.R) | 13-tab interactive dashboard |
| [`inph_references_en.md`](inph_references_en.md) | **156 PubMed links, all with author, year and journal verified against the E-utilities API.** §19 conversely states which parts are *not* supported by the literature |
...
> For the full elasticity table see §11 of [`inph_model_report.txt`](inph_model_report.txt).
...
source("inph_mrgsolve_model_en.R")
all <- run_all()
...
# Shiny dashboard
shiny::runApp("inph_shiny_app_en.R")
```
...
the header of `inph_mrgsolve_model_en.R` (the integration scheme for `dVac` and the
