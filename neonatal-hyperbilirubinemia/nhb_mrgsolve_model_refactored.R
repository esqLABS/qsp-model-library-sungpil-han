...
EC50_SNMP:  8.0  : stannsoporfin Hill EC50 against HO-1 (renamed KISNMP) (ug/mL)
EMAX_SNMP:  1.0  : stannsoporfin Hill Emax (fractional HO-1 block) [new, none explicit in original] (-)
GAMMA_SNMP: 1.0  : stannsoporfin Hill coefficient [new, none explicit in original] (-)
...
EMAX_PB :   2.50 : phenobarbital Hill Emax, maximal induction fold-1 (renamed EPB) (-)
EC50_PB :  20.0  : phenobarbital Hill EC50 (renamed EC50PB) (mg/L)
GAMMA_PB:   1.0  : phenobarbital Hill coefficient [new, none explicit in original] (-)
...
// ---- drug PK --------------------------------------------------------------
// Naming convention (FORK_WORKFLOW_GUIDE.md Part 2): GUT_<STEM>/CENT_<STEM>
// compartments, KA_/V1_/KE_ rate & volume params, C_<STEM> exposed
// concentration, EMAX_/EC50_/GAMMA_<STEM> Hill params, EFFECT_<STEM> the
// named disease-facing effect. Stannsoporfin (SNMP), phenobarbital (PB) and
// UDCA all refactored; IVIG (IGGC/IGGP, a real two-compartment PK already
// following a CENT/PERI shape) is NOT in scope for this pass and is left
// exactly as in the original.
KA_SNMP :   0.173: stannsoporfin (SNMP) IM absorption, t1/2 4 h (renamed KASNMP) (1/h)
V1_SNMP :   3.00 : stannsoporfin (SNMP) volume of distribution (renamed VSNMP) (dL/kg)
KE_SNMP :   0.023: stannsoporfin (SNMP) elimination, t1/2 30 h (renamed KESNMP) (1/h)
KA_PB   :   0.400: phenobarbital (PB) absorption (renamed KAPB) (1/h)
V1_PB   :   9.00 : phenobarbital (PB) volume of distribution (renamed VPB) (dL/kg)
KE_PB   :   0.00693: phenobarbital (PB) elimination, t1/2 100 h (renamed KEPB) (1/h)
...
KA_UDCA :   0.500: ursodeoxycholic acid (UDCA) absorption (renamed KAUDCA) (1/h)
V1_UDCA :   3.00 : ursodeoxycholic acid (UDCA) volume of distribution (renamed VUDCA) (dL/kg)
KE_UDCA :   0.140: ursodeoxycholic acid (UDCA) elimination (renamed KEUDCA) (1/h)
EMAX_UDCA:  0.80 : UDCA Hill Emax, maximal effect (renamed EMAXU) (-)
EC50_UDCA:  4.00 : UDCA Hill EC50 (renamed EC50UDCA) (umol/L)
GAMMA_UDCA: 1.0  : UDCA Hill coefficient [new, none explicit in original] (-)
...
GUT_SNMP  : stannsoporfin (SNMP) IM depot, was ASNMP (mg)
CENT_SNMP : stannsoporfin (SNMP) plasma, was CSNMP (ug/mL)
GUT_PB    : phenobarbital (PB) gut depot, was APB (mg)
CENT_PB   : phenobarbital (PB) plasma, was CPB (mg/L)
IGGC  : IVIG central (g)
IGGP  : IVIG peripheral (g)
GUT_UDCA  : ursodeoxycholic acid (UDCA) gut depot, was AUDCA (mg)
CENT_UDCA : ursodeoxycholic acid (UDCA) plasma, was CUDCA (umol/L)
...
// -- named PK/PD interface (pluggable-PK refactor). Declared once at file
// scope (same pattern as CPNOW etc. above) and only ASSIGNED (never
// re-declared with "double") inside $ODE/$TABLE, because mrgsolve 2.0.1
// hoists every block-local "double NAME" into one shared anonymous
// namespace -- declaring the same name as a local in two blocks collides
// even though each block is textually separate (see UPSTREAM_ISSUES.md
...
// -- pluggable-PK refactor: each compound exposes exactly one named
// concentration (C_<STEM>) and one named Hill effect (EFFECT_<STEM>).
...
EFFECT_SNMP = EMAX_SNMP*pow(C_SNMP, GAMMA_SNMP)
               / (pow(EC50_SNMP, GAMMA_SNMP) + pow(C_SNMP, GAMMA_SNMP));
EFFECT_PB   = EMAX_PB*pow(C_PB, GAMMA_PB)
               / (pow(EC50_PB, GAMMA_PB) + pow(C_PB, GAMMA_PB));
EFFECT_UDCA = EMAX_UDCA*pow(C_UDCA, GAMMA_UDCA)
               / (pow(EC50_UDCA, GAMMA_UDCA) + pow(C_UDCA, GAMMA_UDCA));
...
// ---- drug PK ---------------------------------------------------------
dxdt_GUT_SNMP  = -KA_SNMP*GUT_SNMP;
dxdt_CENT_SNMP = KA_SNMP*GUT_SNMP/(V1_SNMP*Wc)*10.0 - KE_SNMP*CENT_SNMP;
dxdt_GUT_PB    = -KA_PB*GUT_PB;
dxdt_CENT_PB   = KA_PB*GUT_PB/(V1_PB*Wc)*10.0 - KE_PB*CENT_PB;
...
dxdt_CENT_UDCA = KA_UDCA*GUT_UDCA/(V1_UDCA*Wc)*10.0 - KE_UDCA*CENT_UDCA;
...
// C_SNMP/C_PB/C_UDCA/EFFECT_SNMP/EFFECT_PB/EFFECT_UDCA are already the
// file-scope globals set above -- no separate hand-off needed.
...
dose_snmp  <- function(wt, time = 24, mg_per_kg = 4.5)
  ev(time = time, amt = mg_per_kg*wt, cmt = "GUT_SNMP")

dose_pheno <- function(wt, start = 0, days = 14, mg_per_kg = 5.0)
  ev(time = start, amt = mg_per_kg*wt, cmt = "GUT_PB", ii = 24, addl = days-1)

dose_udca  <- function(wt, start = 0, days = 14, mg_per_kg = 10.0)
  ev(time = start, amt = mg_per_kg*wt, cmt = "GUT_UDCA", ii = 12, addl = 2*days-1)
