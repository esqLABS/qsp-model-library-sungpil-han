...
##  Common variable immunodeficiency · primary antibody deficiency
## =====================================================================
##
##  THE ORGANISING IDEA
##  -------------------
...
S$s01 <- run_scn("01 Untreated natural history",
                 events = NULL, years = YRS, arm = "A")

for (d in c(1, 4, 7, 15)) {
  S[[sprintf("s0%d", 1 + which(c(1,4,7,15) == d))]] <-
    run_scn(sprintf("%02d Diagnostic delay of %2d y, then IVIG 500 q4w (delay %d y)",
...
S$s15 <- run_scn("15 Protein-losing enteropathy + standard IVIG 500 q4w (trough collapse)",
                 pars = ple_geno,
                 events = ig_regimen(500, 28, "IV", end_day = YRS*365),
                 years = YRS, arm = "D")
...
cat(sprintf("  solved PLE escalation dose = %d mg/kg q4w (target trough %.0f)\n",
            PLE_DOSE, TR_REF))
S$s16 <- run_scn(sprintf("16 Protein-losing enteropathy + IVIG %d q4w (solved escalation)",
                         PLE_DOSE),
                 pars = ple_geno,
                 events = ig_regimen(PLE_DOSE, 28, "IV", end_day = YRS*365),
                 years = YRS, arm = "D")
S$s17 <- run_scn("17 Protein-losing enteropathy + sirolimus + IVIG 500 q4w",
                 pars = ple_geno,
                 events = mk_ev(
...
S$s18 <- run_scn("18 GLILD phenotype + optimal IgG replacement only (the structural null)",
                 pars = gl_geno, events = igbase, years = YRS, arm = "E")
S$s19 <- run_scn("19 GLILD + prednisone (0.5 mg/kg -> taper)",
                 pars = gl_geno,
                 events = mk_ev(
                   igbase,
                 ...
                 years = YRS, arm = "E")
S$s20 <- run_scn("20 GLILD + rituximab + azathioprine (Chase protocol)",
                 ...
                 events = mk_ev(
                   ...
                   ev(amt = 700, cmt = "RTX_C", time = 5*365 + 182, ii = 182,
                      addl = 28)),
                 years = YRS, arm = "E")
S$s21 <- run_scn("21 GLILD + JAK inhibitor (ruxolitinib)",
...
S$s22 <- run_scn("22 Refractory ITP + high-dose IVIG + prednisone",
                 pars = itp_geno,
                 events = mk_ev(
                   igbase,
                 ...
                 years = YRS, arm = "F")
S$s23 <- run_scn("23 Refractory ITP + rituximab",
                 pars = itp_geno,
                 events = mk_ev(
                   igbase,
                   ...
                   ev(amt = 700, cmt = "RTX_C", time = 6*365 + 365, ii = 365,
                      addl = 12)),
                 years = YRS, arm = "F")
S$s24 <- run_scn("24 Refractory ITP + splenectomy",
                 pars = c(itp_geno, list(SPLENEC = 1, TSPLENEC = 6*365)),
                 events = igbase, years = YRS, arm = "F")
S$s25 <- run_scn("25 Refractory ITP + eltrombopag (non-immunosuppressive)",
                 pars = itp_geno,
                 events = mk_ev(
                   igbase,
...
S$s26 <- run_scn("26 CTLA4 haploinsufficiency (CHAI) — IgG replacement only",
                 pars = list(CTLA4G = 0.5, DYSGENO = 0.45),
                 events = igbase, years = YRS, arm = "G")
S$s27 <- run_scn("27 CTLA4 haploinsufficiency + abatacept (mechanism-matched)",
                 pars = list(CTLA4G = 0.5, DYSGENO = 0.45),
                 events = mk_ev(
                   igbase,
                 ...
                 years = YRS, arm = "G")
S$s28 <- run_scn("28 APDS (PIK3CD GOF) — IgG replacement only",
                 pars = list(PI3KGOF = 0.8, DYSGENO = 0.50),
                 events = igbase, years = YRS, arm = "G")
S$s29 <- run_scn("29 APDS + leniolisib 70 mg BID",
                 pars = list(PI3KGOF = 0.8, DYSGENO = 0.50),
                 events = mk_ev(
                   igbase,
...
S$s30 <- run_scn("30 Early diagnosis + SCIG trough~950 + azithromycin prophylaxis",
