...
## Myasthenia Gravis Quantitative Systems Pharmacology Interactive Dashboard
##
## Tabs:
##   1. Patient Profile & Disease Overview
##   2. Pharmacokinetics (PK — Pyridostigmine, Prednisolone, Eculizumab, Efgartigimod)
##   3. Key PD Indicators (AChR density, AChE inhibition, Complement, NMJ Safety Factor)
##   4. Clinical Endpoints (QMG, MG-ADL, MGFA)
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    width = 280,
    sidebarMenu(
      menuItem("1. Patient Profile", tabName = "profile",
               icon = icon("user-circle")),
      menuItem("2. Pharmacokinetics (PK)", tabName = "pk",
               icon = icon("pills")),
      menuItem("3. Key PD Indicators", tabName = "pd",
               icon = icon("microscope")),
      menuItem("4. Clinical Endpoints", tabName = "clinical",
               icon = icon("chart-line")),
      menuItem("5. Scenario Comparison", tabName = "scenario",
               icon = icon("layer-group")),
      menuItem("6. Biomarker Panel", tabName = "biomarker",
               icon = icon("flask"))
    ),
    hr(),
    ## ---- Drug Dosing Controls ----
    h5("Treatment Setup", style = "text-align:center; color:#E0E0E0;"),
    ...
    sliderInput("sim_wk", "Simulation period (weeks)",
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(
          ...
          box(
            ...
            fluidRow(
              column(6,
                h4("Pathophysiology"),
                tags$ul(
                  tags$li("AChR (acetylcholine receptor) autoantibodies destroy NMJ acetylcholine receptors"),
                  tags$li("Complement activation → MAC (membrane attack complex) formation → NMJ structural damage"),
                  tags$li("AChR antibody cross-linking → increased receptor internalisation"),
                  tags$li("Reduced AChR density → reduced NMJ safety factor"),
                  tags$li("On repetitive stimulation, the end-plate potential (EPP) falls below threshold → muscle weakness")
                ),
                h4("Epidemiology"),
                tags$ul(
                  tags$li("Prevalence: about 20 per 100,000 population"),
                  tags$li("AChR-Ab (+): about 85%; MuSK-Ab (+): about 5-8%"),
                  tags$li("Seronegative: about 10%"),
                  tags$li("Women: peak onset before age 50; men: peak onset after age 50"),
                  tags$li("Thymoma association: about 10-15% of AChR-MG")
                )
              ),
              column(6,
                h4("Treatment Goals"),
                tags$ul(
                  tags$li(strong("QMG ≤ 6"), ": Minimal symptoms or remission"),
                  tags$li(strong("MG-ADL ≤ 2"), ": Minimal symptom state (MMS)"),
                  tags$li(strong("CSR"), ": Complete stable remission (QMG=0 without medication, ≥12 months)"),
                  tags$li("Thymectomy: effective in non-thymomatous AChR-MG (MGTX trial)"),
                  tags$li("Crisis prevention: FVC and NIF monitoring essential")
                ),
                h4("MGFA Classification"),
        ...
        fluidRow(
          box(
            title = "Natural History Simulation",
      ...
      tabItem(
        ...
        fluidRow(
          box(
            title = "Pyridostigmine PK — 24h Profile",
            status = "success", solidHeader = TRUE, width = 6,
            plotlyOutput("plot_pyr_pk", height = "280px")
          ),
          box(
            title = "Prednisolone PK",
      ...
      tabItem(
        ...
        fluidRow(
          box(
            title = "AChE Inhibition by Pyridostigmine",
            status = "success", solidHeader = TRUE, width = 6,
            plotlyOutput("plot_ache_inh", height = "280px")
          ),
          box(
            title = "NMJ AChR Density over Time",
            status = "danger", solidHeader = TRUE, width = 6,
        ...
        fluidRow(
          box(
            title = "Complement Activity: C5 Inhibition",
            status = "warning", solidHeader = TRUE, width = 6,
        ...
        fluidRow(
          box(
            title = "Immune Cell Dynamics: B Cells / Plasma Cells",
            status = "primary", solidHeader = TRUE, width = 12,
      ...
      tabItem(
        ...
        fluidRow(
          box(
            title = "AChR Antibody Titer over Time",
            status = "info", solidHeader = TRUE, width = 6,
            plotlyOutput("plot_ab_titer", height = "280px")
          ),
          box(
            title = "FcRn Blockade: IgG Reduction %",
            status = "success", solidHeader = TRUE, width = 6,
        ...
        fluidRow(
          box(
            title = "Clinical Indicator Summary by Key Time Point",
            status = "primary", solidHeader = TRUE, width = 12,
      ...
      tabItem(
        ...
        fluidRow(
          box(
            title = "Treatment Scenario Comparison Settings",
          ...
          box(
            title = "Treatment Scenario Comparison Results",
        ...
        fluidRow(
          box(
            title = "52-Week Clinical Outcome Comparison Table",
            status = "primary", solidHeader = TRUE, width = 12,
      ...
      tabItem(
        ...
        fluidRow(
          box(
            title = "Diagnostic Biomarker Panel",
            status = "primary", solidHeader = TRUE, width = 5,
            tableOutput("biomarker_table")
          ),
          box(
            title = "Antibody Titer Change Over Time",
        ...
        fluidRow(
          box(
            title = "SFEMG Jitter (single-fibre EMG, degree of NMJ transmission abnormality)",
            status = "warning", solidHeader = TRUE, width = 6,
        ...
        fluidRow(
          box(
            title = "Biomarker Clinical Interpretation Guide",
...
server <- function(input, output, session) {
  ...
  output$mgfa_table <- renderTable({
    data.frame(
      ...
      Description = c(
        "Ocular muscles only, ptosis or diplopia",
        "Predominantly limb/trunk involvement (mild)",
        "Predominantly oropharyngeal/respiratory muscle involvement (mild)",
        "Predominantly limb/trunk involvement (moderate)",
        "Predominantly oropharyngeal/respiratory involvement (moderate)",
        "Predominantly limb/trunk involvement (severe)",
        "Predominantly oropharyngeal/respiratory involvement (severe)",
        "Requires intubation (crisis)"
      )
    )
  ...
  output$biomarker_guide_table <- renderDT({
    df <- data.frame(
      Biomarker = c("Anti-AChR Ab", "Anti-MuSK Ab", "Anti-LRP4 Ab",
                   "Anti-Titin Ab", "SFEMG Jitter", "RNS decrement",
                   "Total IgG", "sC5b-9", "Blood Tfh %", "CH50"),
      Normal range = c("<0.4 nmol/L (RIA)", "Negative", "Negative",
                   "Negative", "<55 µs", "<10% at 3Hz",
                   "7-16 g/L", "<244 ng/mL", "~1-2% CD4+", "60-150 U/mL"),
      Clinical significance = c("MG diagnosis and disease activity", "MuSK-MG (chest CT, IgG4)",
                   "Seronegative MG diagnosis", "Thymoma-associated MG",
                   "High sensitivity for NMJ transmission abnormality", "Objective indicator of neuromuscular transmission block",
                   "FcRn therapy monitoring", "Degree of MAC formation (eculizumab treatment response)",
                   "Immune activity", "Complement consumption (decreased in active MG)")
    )
  ...
  output$vb_qmg <- renderValueBox({
    ...
    valueBox(value = val, subtitle = "QMG Score (final, 0-39)",
  ...
  output$vb_mgadl <- renderValueBox({
    ...
    valueBox(value = val, subtitle = "MG-ADL (final, 0-24)",
