# Infective Endocarditis — References
### References for the IE QSP Model

> **A note on how these are linked.**
> Each entry, instead of writing the PMID number directly, uses **a link that searches
> PubMed by the paper's title**. A hand-transcribed PMID that is off by even a single
> digit will silently point to *a different paper*, and that kind of error is not
> visible to the eye. If a title-search link is wrong, the result is empty or is
> obviously a different paper, so **the error is visible**. To check the evidence behind any parameter in the model, please follow the link and check the source directly.
>
...
This list corresponds directly to the parameters and structure of `ie_mrgsolve_model.R` and to each
cluster of `ie_qsp_model.dot`. Of the 10 validation targets the model is required to reproduce
(T1-T10), which are concentrated in sections 5, 8, 10, and 13 below, **T5, T6, and T7 are negative-
result trials** — a model that cannot reproduce a negative result is not a mechanistic model but
merely curve fitting.

---

## 1. Epidemiology, Definitions, Diagnostic Criteria
...
## 2. Pathogenesis: NBTE, Adhesion, Vegetation Assembly
...
## 3. Bacterial Density, Growth State and Phenotypic Tolerance in the Vegetation
...
## 4. Antibiotic Penetration into Vegetations: the Diffusion Problem
...
## 5. Vancomycin: AUC Targets, MIC, Nephrotoxicity
...
## 6. Daptomycin and the Lipopeptides
...
## 7. Beta-lactams: MSSA, the Cefazolin Debate, Continuous Infusion
...
## 8. Aminoglycosides and Rifampicin: Two Informative Negatives
...
## 9. Population Genetics of Resistance and Tolerance
...
## 11. Heart Failure, Valve Destruction, Perivalvular Extension
...
## 12. Persistent Bacteraemia, Metastatic Foci, the Intracellular Reservoir
...
## 13. From IV to Oral: POET and Duration of Therapy
...
## 16. Experimental Endocarditis Models — where most of the mechanism is measured
...
## Mapping This Model's Core Claims to the Evidence Literature

| Model Claim | Location in Code | Primary Evidence |
|---|---|---|
| Bacterial density within the vegetation is 10^9-10^11 CFU/g and in stationary phase | `DENSCAP`, `MUMAX`, `AVL_*` | Durack & Beeson 1972 (3 papers) |
| The drug enters only by diffusion and decays exponentially with depth | `LAM_*`, `exp(-d/lambda)` | Cremieux 1989 autoradiography; Stewart 1996 |
| Beta-lactam killing is proportional to growth rate, so it is lost in the core | `FSTAT_BL = 0.02` | Craig 1998; Lewis 2007; Nielsen 2007 |
| Daptomycin also acts on non-dividing cells | `FSTAT_DAP = 0.55` | Mascio 2007 |
| Aminoglycoside uptake is PMF-dependent, so it fails in the anoxic core | `OXY = AVL^1.5` | Taber 1987; Bryan & Kwan 1983 → **T5** |
| rpoB mutants are already present at the time of diagnosis | `MUTRATE * mu * B` | Luria & Delbruck 1943; Wichelhaus 2002 |
| Vancomycin AKI depends steeply on AUC and creates positive feedback | `CAKI50_V`, `E_AKI` | Rybak 2020; Aljefri 2019 → **T10** |
| Embolic risk tracks the surface *bacterial* population, not the substrate | `FRAG = f(NSG)` | Dickerman 2007; Thuny 2005 → **T3, T4, T7** |
| Only rifampicin penetrates the intracellular reservoir | `FCELL_*` | Barcia-Macay 2006; Lemaire 2007 |
| Free-drug concentration and lambda determine the outcome, not the route (IV/oral) | `IE_poet()` | POET 2019, 2022; OVIVA 2019 → **T8** |

---

## Disclaimer

This model and the accompanying reference list were prepared for **educational and research purposes**. They must not be used for actual clinical decision-making, prescribing, antibiotic selection, surgical
timing decisions, or regulatory submission.
Treatment of infective endocarditis must always follow current guidelines (2023 ESC, AHA) and the judgement of the infectious-disease,
cardiology, cardiac-surgery, and clinical-pharmacy **endocarditis team**, whose
collective judgement must be followed.
