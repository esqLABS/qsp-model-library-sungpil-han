...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      ...
      menuItem("Drug PK (Pharmacokinetics)",       tabName = "pk",         icon = icon("flask")),
      menuItem("Sleep-wake regulation",         tabName = "sleep_wake", icon = icon("moon")),
      menuItem("Clinical endpoints",        tabName = "endpoints",  icon = icon("chart-line")),
      menuItem("Treatment scenario comparison",     tabName = "comparison", icon = icon("balance-scale")),
      menuItem("Biomarkers",             tabName = "biomarkers", icon = icon("vial")),
      menuItem("Autoimmune mechanism",          tabName = "autoimmune", icon = icon("dna")),
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "patient",
        h4(class = "tab-title", icon("user"), " Patient Profile"),
        fluidRow(
          box(title = "Basic Settings", width = 4, status = "primary",
            radioButtons("nt_type", "Narcolepsy Type:",
              choices = c("NT1 (With Cataplexy)" = "NT1",
                          "NT2 (Without Cataplexy)" = "NT2"),
              selected = "NT1"),
            selectInput("hla_status", "HLA-DQB1*06:02 status:",
              choices = c("Positive" = "pos",
                          "Negative" = "neg",
                          "Unknown" = "unk")),
            sliderInput("orexin_survival", "Orexin neuron survival (%)",
              min = 0, max = 100, value = 15, step = 5),
            sliderInput("disease_duration", "Disease Duration (years)",
              min = 0, max = 40, value = 5, step = 1)
          ),
          box(title = "Symptom Settings", width = 4, status = "warning",
            sliderInput("baseline_ess", "Baseline ESS (0-24)",
              min = 0, max = 24, value = 18, step = 1),
            sliderInput("cataplexy_freq", "Cataplexy Frequency (episodes/week)",
              min = 0, max = 50, value = 10, step = 1),
            sliderInput("mslt_latency", "Mean Sleep Latency (MSLT, min)",
              min = 0, max = 20, value = 3, step = 0.5),
            numericInput("soremp_count", "Number of SOREMPs (MSLT SOREMPs)",
              min = 0, max = 5, value = 3)
          ),
          box(title = "Comorbidities & Other", width = 4, status = "info",
            checkboxGroupInput("comorbidities", "Comorbidities:",
              choices = c("Obesity" = "obesity",
                          "Depression" = "depression",
                          "ADHD" = "adhd",
                          "Sleep Apnea" = "osa",
                          "Anxiety" = "anxiety"),
              selected = c("obesity")),
            numericInput("patient_age", "Age", min = 5, max = 90, value = 28),
            selectInput("patient_sex", "Sex",
              choices = c("Male" = "M", "Female" = "F")),
            numericInput("patient_weight", "Weight (kg)", min = 20, max = 200, value = 72)
          )
        ),
        fluidRow(
          box(width = 12,
            h4("Patient Summary Card"),
      ...
      # -----------------------------------------------------------------------
      tabItem(tabName = "pk",
        h4(class = "tab-title", icon("flask"), " Drug PK — Pharmacokinetics"),
        fluidRow(
          box(title = "Drug & Dose Settings", width = 3, status = "primary",
            selectInput("pk_drug", "Drug:",
              choices = names(drug_params), selected = "Modafinil"),
            numericInput("pk_dose", "Dose (mg)", value = 200, min = 10, max = 9000, step = 10),
            selectInput("pk_freq", "Frequency:",
              choices = c("Once daily (QD)" = 1, "Twice daily (BID)" = 2, "Three times daily (TID)" = 3),
              selected = 1),
            numericInput("pk_timing", "Administration Time (h after midnight)",
            ...
            actionButton("pk_reset", "Restore defaults", icon = icon("undo"), class = "btn-sm btn-default")
          ),
          box(title = "Concentration-Time Profile", width = 9, status = "info",
            plotlyOutput("pk_plot", height = "350px"),
            hr(),
            h5("PK Summary Parameters"),
      ...
      # -----------------------------------------------------------------------
      # TAB 3: Sleep-Wake Regulation
      # -----------------------------------------------------------------------
      tabItem(tabName = "sleep_wake",
        h4(class = "tab-title", icon("moon"), " Sleep-Wake Regulation"),
        fluidRow(
          box(title = "System Parameters", width = 3, status = "primary",
            sliderInput("sw_orexin", "Orexin Activity (%)", min = 0, max = 100, value = 15),
            sliderInput("sw_adenosine", "Adenosine Pressure (AU)",
              min = 0, max = 10, value = 4, step = 0.5),
            sliderInput("sw_circadian_phase", "Circadian Phase (h)",
              min = 0, max = 24, value = 8, step = 0.5),
            selectInput("sw_treatment", "Treatment:",
              choices = c("None", "Modafinil", "Pitolisant",
                          "Sodium Oxybate", "Solriamfetol")),
            sliderInput("sw_drug_effect", "Drug Effect Size (%)", min = 0, max = 100, value = 50)
          ),
          box(title = "Orexin System & Wake/Sleep Activity (24h)", width = 9, status = "info",
            plotlyOutput("sw_system_plot", height = "400px")
          )
        ),
        fluidRow(
          box(title = "Flip-Flop Switch & Sleep Architecture", width = 6, status = "warning",
            plotlyOutput("flipflop_plot", height = "300px")
          ),
          box(title = "Circadian Rhythm & Orexin Influence (24h)", width = 6, status = "success",
      ...
      tabItem(tabName = "endpoints",
        h4(class = "tab-title", icon("chart-line"), " Clinical Endpoints"),
        fluidRow(
          box(title = "Treatment Settings", width = 3, status = "primary",
            selectInput("ep_drug", "Drug:",
              choices = names(drug_params), selected = "Modafinil"),
            sliderInput("ep_dose_pct", "Dose Efficacy (%)",
              min = 20, max = 100, value = 70, step = 5),
            numericInput("ep_weeks", "Treatment Duration (Weeks)", value = 12, min = 2, max = 52),
            hr(),
            h5("Baseline"),
          ...
          box(title = "ESS Score Over Time", width = 9, status = "info",
            plotlyOutput("ess_time_plot", height = "280px")
          )
        ),
        fluidRow(
          box(title = "Cataplexy Frequency Over Time", width = 6, status = "warning",
            plotlyOutput("cat_time_plot", height = "280px")
          ),
          box(title = "Sleep Architecture", width = 6, status = "success",
            plotlyOutput("sleep_arch_plot", height = "280px")
          )
        ),
        fluidRow(
          box(title = "PSG Parameters & Sleep Efficiency", width = 6, status = "primary",
            plotlyOutput("psg_plot", height = "260px")
          ),
          box(title = "MSLT & CSF Orexin Change", width = 6, status = "info",
      ...
      tabItem(tabName = "comparison",
        h4(class = "tab-title", icon("balance-scale"), " Treatment Scenario Comparison"),
        fluidRow(
          box(title = "Comparison Settings", width = 3, status = "primary",
            checkboxGroupInput("comp_drugs", "Select drugs to compare:",
            ...
            hr(),
            p(icon("info-circle"), " Uses clinical-trial reference values"),
            p(class = "trial-badge", "HARMONY I · TONES 3 · REST-ON")
          ),
          box(title = "ESS Reduction — Waterfall Plot", width = 9, status = "info",
            plotlyOutput("waterfall_ess", height = "320px")
          )
        ),
        fluidRow(
          box(title = "Cataplexy Reduction (%)", width = 6, status = "warning",
            plotlyOutput("cat_reduction_plot", height = "300px")
          ),
          box(title = "Forest Plot — Excessive Daytime Sleepiness Treatment Effect (ESS)", width = 6, status = "success",
      ...
      tabItem(tabName = "biomarkers",
        h4(class = "tab-title", icon("vial"), " Biomarkers"),
        fluidRow(
          box(title = "CSF Hypocretin-1 Distribution", width = 6, status = "primary",
            plotlyOutput("csf_plot", height = "320px")
          ),
          box(title = "MSLT Diagnostic Criteria Visualisation", width = 6, status = "warning",
            plotlyOutput("mslt_diag_plot", height = "320px")
          )
        ),
        fluidRow(
          box(title = "Dose-Response Curve (Emax Model)", width = 6, status = "info",
            selectInput("bm_drug_dr", "Drug:", choices = names(drug_params)),
            plotlyOutput("dose_response_plot", height = "280px")
          ),
          box(title = "HLA Test Sensitivity/Specificity & Treatment Biomarker Trajectory", width = 6, status = "success",
      ...
      # -----------------------------------------------------------------------
      # TAB 7: Autoimmune Mechanism
      # -----------------------------------------------------------------------
      tabItem(tabName = "autoimmune",
        h4(class = "tab-title", icon("dna"), " Autoimmune Mechanism"),
        fluidRow(
          box(title = "Orexin Neuron Loss Timeline", width = 6, status = "danger",
            plotlyOutput("neuron_loss_plot", height = "320px")
          ),
          box(title = "HLA-DQB1*06:02 Association Visualisation", width = 6, status = "warning",
            plotlyOutput("hla_assoc_plot", height = "320px")
          )
        ),
        fluidRow(
          box(title = "Autoantibody Levels", width = 6, status = "primary",
            plotlyOutput("autoab_plot", height = "300px")
          ),
          box(title = "Environmental Triggers", width = 6, status = "info",
      ...
      tabItem(tabName = "references",
        h4(class = "tab-title", icon("book"), " References"),
        fluidRow(
          box(title = "Key Clinical Trials", width = 12, status = "primary",
            tableOutput("trial_table")
          )
        ),
        fluidRow(
          box(title = "Model Assumptions & Limitations",
              width = 6, status = "warning",
            tags$ul(
              tags$li("PK parameters based on literature mean values (one-compartment model)"),
              tags$li("VLPO/LC/TMN activity is a literature-based normalised unit"),
              tags$li("Dose-response simplified with an Emax model"),
              tags$li("Inter-individual variability (IIV) not included (planned NLME extension)"),
              tags$li("Autoimmune timeline based on cross-sectional cohort data"),
              tags$li("CSF Hcrt-1 distribution assumed normal")
            )
          ),
...
server <- function(input, output, session) {
  ...
  output$patient_card <- renderUI({
    nt   <- input$nt_type
    hla  <- switch(input$hla_status, pos = "Positive (+)", neg = "Negative (-)", unk = "Unknown")
    ...
    if (nchar(comor) == 0) comor <- "None"

    sev <- if (ess >= 18) "Severe" else if (ess >= 14) "Moderate" else "Mild"

    div(class = "patient-card",
      fluidRow(
        column(3,
          div(class = "card-label", "Type"),
        ...
        column(3,
          div(class = "card-label", "Orexin Survival"),
          div(class = "card-value", paste0(orex, "%")),
          br(),
          div(class = "card-label", "Disease Duration"),
          div(class = "card-value", paste0(dur, " yr"))
        ),
        column(3,
          div(class = "card-label", "ESS Score"),
          div(class = "card-value", paste0(ess, " / 24  (", sev, ")")),
          br(),
          div(class = "card-label", "Cataplexy/week"),
          div(class = "card-value", paste0(cat, " episodes"))
        ),
        column(3,
          div(class = "card-label", "Age / Sex / Weight"),
          div(class = "card-value",
              paste0(input$patient_age, " yr / ", input$patient_sex, " / ", input$patient_weight, " kg")),
          br(),
          div(class = "card-label", "Comorbidities"),
  ...
  output$waterfall_ess <- renderPlotly({
    ...
    plot_ly(df, x = ~reorder(Drug, ESS_change), y = ~ESS_change,
      ...
      layout(
        title = "ESS Change (Waterfall Plot) — Clinical Trial Data",
  ...
  output$forest_plot <- renderPlotly({
    ...
    plot_ly() %>%
      ...
      add_segments(x = 0, xend = 0, y = 0.5, yend = nrow(df_ess) + 0.5,
                   line = list(dash = "dash", color = "gray")) %>%
      layout(
        title = "Forest Plot — ESS Reduction Effect (95% CI)",
