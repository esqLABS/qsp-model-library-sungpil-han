...
##
##  The app loads the model from pji_mrgsolve_model.R unchanged and shows it in 13 tabs.
...
## ---- Load the model ---------------------------------------------------------
## pji_mrgsolve_model.R builds the model object pji_mod and the scenario list scen.
if (!exists("pji_mod")) {
...
server <- function(input, output, session) {
  ...
  output$reflink <- renderUI(HTML(
    "<p>For the provenance of the parameters and the verification anchors see <code>pji_references.md</code> (68 papers, every PMID checked) and
     the VERIFICATION section in the header of <code>pji_mrgsolve_model.R</code>.
     The mechanistic map is <code>pji_qsp_model.svg</code>.</p>
