...
| File | Content |
|------|------|
| [`dka_qsp_model.dot`](../../../diabetic-ketoacidosis/dka_qsp_model.dot) | Mechanistic map source — 218 nodes, 297 edges, 19 clusters |
| [`dka_qsp_model.svg`](../../../diabetic-ketoacidosis/dka_qsp_model.svg) · [`dka_qsp_model.png`](../../../diabetic-ketoacidosis/dka_qsp_model.png) | Rendered map (SVG vector / PNG 150 dpi) |
| [`dka_mrgsolve_model.R`](../../../diabetic-ketoacidosis/dka_mrgsolve_model.R) | mrgsolve model — 42 ODEs, a closed-loop protocol executor, 12 scenarios |
| [`dka_shiny_app.R`](../../../diabetic-ketoacidosis/dka_shiny_app.R) | Shiny dashboard — 11 tabs |
| [`dka_references.md`](dka_references.md) | 132 references, classified by section, each noting which part of the model it supports |
| [`dka_reference_check.py`](../../../diabetic-ketoacidosis/dka_reference_check.py) | An independent **reimplementation** of the same 42 equations (pure standard-library Python, RK4) |
| [`dka_reference_output.txt`](../../../diabetic-ketoacidosis/dka_reference_output.txt) | The complete output of the script above — **the source of every number below** |
...
### 1. Bicarbonate Is Not a State Variable — It Is a Residual
...
### 2. Endogenous Insulin Gets Preferential Treatment at the Liver (Portal Privilege)
...
### 3. The Labs at Presentation Are an Output, Not an Input
...
## Results — Computed, Not Asserted

### R1. The Markers We Measure Normalise in Reverse Order of How Much Information They Carry
...
### R2. Where Does the First Hour's Glucose Drop Come From — the Two Levers Are Not Shared
...
### R3. Why Low-Dose Insulin Works — Derived From Nothing But the Ordering of Two IC50s
...
### R4. Hyperglycaemia Is a Volume Disease — the Renal Escape Valve
...
### R5. DKA and HHS Are the Same Equations, Two Knobs
...
### R6. Euglycaemic Ketoacidosis — the Valve Held Open by a Drug
...
### R7. Why Bicarbonate Recovers Later Than the Anion Gap — Two Separable Causes
...
### R8. An Honest Warning About Bicarbonate Itself — Poor Conditioning
...
### R9. Potassium — the Serum Value at Presentation Carries Almost No Information About the Deficit
...
### R10. Cerebral Oedema — the Model Reproduces a Negative Trial (PECARN FLUID 2018)
...
### R11. The Nitroprusside Paradox — the Patient Improves While the Urine Ketone Gets Worse
...
### R12. Giving Bicarbonate — What It Buys
...
### R13. Stopping the IV Infusion — the Overlap Is Not a Formality
...
### R14. Subcutaneous Insulin — the Interesting Number Is the Trough, Not the Peak, and There Are Two Troughs
...
### R15. A Map of Phenotypes — One Model, Many Diseases
...
### R16. Sensitivity (elasticities of the presenting phenotype, ±25%)
...
### R17. Conservation Checks (these must hold or the model is wrong)
...
## Known Limitations and Biases (recorded rather than tuned away)

1. **Bicarbonate's conditioning.** Documented quantitatively in R8. The gap and the ketone
   are the trustworthy outputs; the bicarbonate and pH are not, and the model says
   so with numbers rather than a disclaimer.
2. **The decomposition of cerebral oedema is inference.** The 0.00%/0.94% split between osmotic and
   ...
   calibrated ratio.
3. **The portal:peripheral insulin ratio (PORTF) carries the entire DKA/HHS distinction.** It is taken
...
4. **The split between saturable and non-saturable ketone-oxidation capacity**
   (VMAX_KOX vs KLIN_KOX) is constrained by
...
5. **At 0.05 U/kg/h the ketoacidosis does not resolve within 40 hours.** Reported as-is in
   ...
   measured in.
6. **The precipitating illness is a single scalar** (ILL, decaying with τ 30 h). There
   is no infection dynamics, no source control, no antibiotic effect. A patient whose sepsis is
   worsening cannot be represented.
7. **No coagulation, cardiac electrophysiology, or pharmacogenomics.** Venous thromboembolism and arrhythmia
...
8. **The first-hour glucose drop is somewhat fast**
   (−75 mg/dL/h against the ADA's stated
