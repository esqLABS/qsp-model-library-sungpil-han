...
[`abm_reference_python.py`](abm_reference_python.py), and each is left as a comment at the
...
shiny::runApp("abm_shiny_app.R")     # the 12-tab dashboard
```

In an environment without R, the reference implementation can be run just as it is:

```bash
python3 abm_reference_python.py      # no dependencies. regenerates abm_reference_output.txt
```
