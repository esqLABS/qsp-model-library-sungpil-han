...
##  (abm_mrgsolve_model.R must be in the same directory)
##
...
server <- function(input, output, session) {
  ...
  output$txt_bugs <- renderUI({
    HTML(paste0(
      ...
      "they were first independently implemented in a dependency-free Python RK4 (<code>abm_reference_python.py</code>) ",
      ...
      "the complete run log is <code>abm_reference_output.txt</code>. Carrying over only ",
