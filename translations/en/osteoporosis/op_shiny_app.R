...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("PD Key Biomarkers",     tabName = "pd",        icon = icon("chart-line")),
      menuItem("Clinical Endpoints",  tabName = "endpoints", icon = icon("heartbeat")),
      menuItem("Scenario Comparison",    tabName = "scenarios", icon = icon("sliders-h")),
      menuItem("Biomarker Panel",  tabName = "biomarkers",icon = icon("vial"))
    ),

    hr(),
    h5("  Patient Settings", style="padding-left:10px; font-weight:bold;"),
    ...
    hr(),
    h5("  Treatment Selection", style="padding-left:10px; font-weight:bold;"),
      ...
      tabItem("profile",
        ...
        fluidRow(
          box(title="Disease Overview", width=6, solidHeader=TRUE, status="primary",
            HTML("
              <h4>Osteoporosis</h4>
              <p><b>Definition:</b> A metabolic bone disease in which reduced bone density and deteriorating bone microarchitecture increase fracture risk.
              T-score ≤ −2.5 (WHO criteria).</p>
              <p><b>Prevalence:</b> Over 200 million people worldwide; 30% of women and 8% of men over age 50.</p>
              <p><b>Key Mechanisms:</b></p>
              <ul>
                <li>RANK/RANKL/OPG axis: regulates osteoclast differentiation</li>
                <li>Wnt/β-Catenin + Sclerostin: regulates osteoblasts</li>
                <li>Estrogen deficiency: accelerates postmenopausal bone loss</li>
                <li>PTH · vitamin D · calcium homeostasis</li>
              </ul>
              <p><b>Treatment Goal:</b> Reduce fracture risk (spine 50–70%, hip 30–40%)</p>
            ")
          ),
