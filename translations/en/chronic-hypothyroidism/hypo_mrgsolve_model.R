...
# Chronic hypothyroidism quantitative systems pharmacology model
#
# Author  : Claude Code Routine (CCR)
# Date    : 2026-06-20
# Disease : Chronic Hypothyroidism (primary / central / drug-induced)
#
# Model Scope
# -----------
...
scenario_labels <- c(
  ...
  "3. LT4 standard therapy (LT4 100 μg/day)",
  "4. LT4 low dose (subclinical hypothyroidism, LT4 50 μg/day)",
  "5. LT4 over-treatment (TSH suppression, LT4 175 μg/day)",
  "6. T4+T3 combination (LT4 100+LT3 10 μg/day)",
  "7. Post-thyroidectomy LT4 (125 μg/day)"
)
...
# HPT axis plots
p_tsh <- plot_var(df_all, "A_TSH",   "TSH (mIU/L)",    "Serum TSH", 0.5, 4.5)
p_ft4 <- plot_var(df_all, "fT4_pmol","Free T4 (pmol/L)","Free T4", 12, 22)
p_ft3 <- plot_var(df_all, "fT3_pmol","Free T3 (pmol/L)","Free T3", 3.5, 6.5)
p_rt3 <- plot_var(df_all, "A_rT3",   "rT3 (nmol/L)",   "Reverse T3")

# PD effect plots
p_hr  <- plot_var(df_all, "Eff_HR",  "Heart Rate (bpm)","Heart rate", 60, 100)
p_ldl <- plot_var(df_all, "Eff_LDL", "LDL (mmol/L)",   "LDL cholesterol", NA, 3.4)
p_bmr <- plot_var(df_all, "Eff_BMR", "BMR (%)",        "Basal metabolic rate (BMR)", 85, 115)
p_sym <- plot_var(df_all, "Eff_Sym", "Symptom score (0-100)","Hypothyroid symptom score")

# Combine plots
hpt_panel <- (p_tsh | p_ft4) / (p_ft3 | p_rt3) +
  plot_annotation(title = "HPT axis dynamics — seven treatment scenarios",
                  theme = theme(plot.title = element_text(face="bold")))

pd_panel  <- (p_hr | p_ldl) / (p_bmr | p_sym) +
  plot_annotation(title = "PD effects — seven treatment scenarios",
                  theme = theme(plot.title = element_text(face="bold")))
