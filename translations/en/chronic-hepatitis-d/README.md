...
| File | Contents |
|---|---|
| [`hdv_qsp_model.dot`](hdv_qsp_model.dot) · [`.svg`](hdv_qsp_model.svg) · [`.png`](hdv_qsp_model.png) | Mechanistic map — **18 clusters, 157 nodes** |
| [`hdv_mrgsolve_model.R`](../../../chronic-hepatitis-d/hdv_mrgsolve_model.R) | mrgsolve ODE model — **33 compartments, 14 scenarios** + bile-acid dose-response and floor-sensitivity helpers |
| [`hdv_reference_model.py`](../../../chronic-hepatitis-d/hdv_reference_model.py) | A **dependency-free** Python reference implementation + virtual population (n = 300) + fitting routine |
| [`hdv_model_report.txt`](../../../chronic-hepatitis-d/hdv_model_report.txt) | The computed results behind every figure above (A0–A14). Regenerate with `python3 hdv_reference_model.py` |
| [`hdv_shiny_app.R`](hdv_shiny_app.R) | Shiny dashboard — **9 tabs** (patient/floor · PK and occupancy · virology · endpoints · ALT-RNA dissociation · floor and ceiling · bile acids · scenario comparison · long-term outcomes) |
| [`hdv_references.md`](hdv_references.md) | **62** references, with PMIDs directly confirmed on PubMed marked separately from search links for unconfirmed entries |
...
> actual clinical decision-making, prescribing, or regulatory submission. `hdv_references.md` states
