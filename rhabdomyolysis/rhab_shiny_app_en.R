...
##  Rhabdomyolysis-Induced Acute Kidney Injury — Interactive QSP Dashboard
## ---------------------------------------------------------------------
...
## ---------------------------------------------------------------------
ui <- fluidPage(
  titlePanel("Rhabdomyolysis-Induced AKI QSP Model"),
  ...
  sidebarLayout(
    sidebarPanel(
      width = 3,
      h4("Patient"),
      sliderInput("minj", "Muscle at Risk (kg)",
                  min = 0, max = 14, value = 6, step = 0.5),
      sliderInput("age", "Age (years)", min = 16, max = 90, value = 45),
      radioButtons("sex", "Sex", inline = TRUE,
                   choices = c("Male" = 0, "Female" = 1), selected = 0),
      selectInput("cause", "Cause",
                  choices = c("Crush/Compression" = "crush",
                              "Exertional" = "exert",
                              "Drug (e.g. statin)" = "drug")),
      conditionalPanel(
        "input.cause == 'crush'",
        sliderInput("entrap", "Entrapment Time (h)",
                    min = 0, max = 24, value = 8, step = 1),
        sliderInput("ccomp", "Compartment Compliance (L/mmHg)",
                    min = 0.006, max = 0.040, value = 0.028, step = 0.002),
        checkboxInput("fascio", "Fasciotomy", FALSE),
        conditionalPanel("input.fascio",
                         sliderInput("fasct", "Fasciotomy Timing (h)",
                                     min = 0, max = 48, value = 10, step = 1))
      ),
      hr(),
      h4("Crystalloid  (the FLOW term)"),
      selectInput("fluid", "Type",
                  choices = c("0.9% saline" = "NS", "Ringer lactate" = "LR",
                              "Balanced (Plasma-Lyte-like)" = "PL",
                              "0.45% saline + bicarbonate" = "BIC")),
      sliderInput("flrate", "Rate (mL/h)",
                  min = 0, max = 1200, value = 500, step = 50),
      sliderInput("flon", "Started At (h)",
                  min = 0, max = 48, value = 8, step = 1),
      hr(),
      h4("Alkali  (the pH term)"),
      ...
      hr(),
      h4("Extracorporeal  (the SOURCE term)"),
      sliderInput("crrtq", "Clearance (mL/min)",
                  min = 0, max = 60, value = 0, step = 5),
      sliderInput("crrton", "Started At (h)",
      ...
      sliderInput("eaox", "Antioxidant heme-arm block (fraction)",
                  min = 0, max = 0.95, value = 0, step = 0.05),
      hr(),
      sliderInput("tend", "Follow-up (days)",
                  min = 3, max = 90, value = 28, step = 1),
      checkboxInput("cmp", "Overlay Untreated Control", TRUE)
    ),
    ...
    mainPanel(
      ...
      tabsetPanel(
        ...
        tabPanel(
          "1 · Patient Profile",
          h4("Course at a Glance"),
          fluidRow(column(12, uiOutput("verdict"))),
          plotOutput("p_overview", height = "560px"),
          helpText("The red band is the entrapment period, the blue band is the fluid administration period. ",
                   "Red band = entrapment, blue band = crystalloid infusion.")
        ),

        ## ---- 2 ------------------------------------------------------
        tabPanel(
          "2 · The Two Clocks (CK vs Mb)",
        ...
        ## ---- 4 ------------------------------------------------------
        tabPanel(
          "4 · Tubular Handling (Tm Saturation)",
        ...
        tabPanel(
          ...
          actionButton("run_scen", "Run Selected Scenarios",
                       class = "btn-primary"),
        ...
        tabPanel(
          ...
          h4("Limitations"),
          tags$ul(
            tags$li("This is a single-compartment muscle model, and compartment pressure is consolidated into one value."),
            tags$li("Sepsis, wound infection, and amputation are not included in the model, so the 'fasciotomy not performed' ",
                    "scenario should not be read as clinical guidance."),
            tags$li("Death is represented not by a calibrated survival model but only by cumulative arrhythmia risk, ",
                    "expressed as such."),
            tags$li("For educational and hypothesis-generation purposes only; not a clinical decision-making tool.")
          )
        )
      )
    )
...
server <- function(input, output, session) {
  ...
  sim <- reactive({
    rhab %>% param(pset()) %>%
      ...
      as_tibble() %>% mutate(arm = "Your Regimen")
  })
  ...
  ctrl <- reactive({
    ...
    rhab %>% param(p) %>%
      mrgsim(end = input$tend*24, delta = 0.25, hmax = 0.5) %>%
      as_tibble() %>% mutate(arm = "Untreated")
  })
  ...
  output$p_clocks <- renderPlot({
    d <- dd() %>% filter(arm == "Your Regimen", time <= 168)
  ...
  output$p_arms <- renderPlot({
    d <- dd() %>% filter(arm == "Your Regimen", time <= 336)
    ggplot(d, aes(time)) +
  ...
  output$p_press <- renderPlot({
    d <- dd() %>% filter(arm == "Your Regimen", time <= 240)
    ggplot(d, aes(time)) +
  ...
  scen <- eventReactive(input$run_scen, {
    ...
    validate(need(length(picks) > 0, "Please select a scenario."))
