# Radiation-Induced Lung Injury (RILI) QSP Model — References

# Radiation-Induced Lung Injury — References


> **How these PMIDs were verified.** No entry in this list was written
> from memory. A topic-specific query for each section was sent directly to
> the NCBI E-utilities (`esearch` + `esummary`), and the author, year, journal,
> title and PMID of the records PubMed actually returned were transcribed
> verbatim. Every PMID link therefore resolves. The queries used relevance
> ordering, so some entries are peripheral, and even then the returned record was not altered.
>
> **236 papers** in total. The evidence for each model parameter is in the
> comment on that parameter in `rili_reference_model.py`; the calibration targets are in `rili_calibration.py`.


---

## 1. Dose-Volume Relationships & Radiation Pneumonitis Risk

Clinical evidence for how MLD·V20·V5 relate to pneumonitis risk. The NTCP calibration targets of the model (QUANTEC: MLD 13/20/24 Gy → 10/20/30%) come from here.
...
## 2. NTCP Formalism & the Volume-Effect Exponent

The LKB model · gEUD · the parallel architecture of functional subunits (FSU). The model does not take n as an input but measures it from 300 random DVHs (§2 result: n = 1.43 vs 0.99 in the literature).
...
## 3. The Linear-Quadratic Model & Fractionation Effects

Lung α/β ≈ 3 Gy, NSCLC α/β ≈ 10 Gy. The difference between the two values decides in which direction hypofractionation moves the therapeutic ratio.
...
## 4. Alveolar Epithelium · AT2 Cells · Surfactant

The fact that lethally damaged cells keep functioning until they attempt to divide is what creates the latency (4–12 weeks) in this model. Evidence for AT2 turnover · KL-6 · SP-D.
...
## 5. Microvascular Endothelial Injury & Capillary Rarefaction

Evidence for ASMase–ceramide-mediated endothelial cell death, exhaustion of endothelial progenitor cells, and the permanent repair ceiling EC_max = 1/(1+KECIRR·BED).
...
## 6. DAMP Release & Innate Immune Sensing

HMGB1 · cGAS–STING · NLRP3. In the model the source of DAMPs is not dose but the death flux DTH = KMIT × DOOM.
...
## 7. The Cytokine Network & the Fast Loop

Blood concentrations of TNF-α · IL-6 · IL-1β · TGF-β1 and pneumonitis risk. The gain budget of the fast loop (0.072 < KCE 0.150) is why pneumonitis resolves spontaneously.
...
## 8. TGF-β1 Activation & the Myofibroblast

Latent TGF-β1 is released by the force transmitted through integrin αvβ6 — that activation requires stiffness to cross a threshold is the basis of the bistability in this model.
...
## 9. Bistability · Mechanical Memory · Mathematical Models

Evidence for the three fixed points of the slow loop (normal 1.000 · separatrix 1.586 · fibrosis 2.9) and for the irreversibility created by LOX cross-linking.
...
A drug that reaches only the fast loop. The model predicts that it lowers the symptom index but does not change the fibrotic conversion volume at all.
...
## 11. Amifostine & Radioprotection

WR-1065 plasma half-life ≈ 8 min. The temporal-coincidence requirement written as protection = PFAMI·exp(−ln2·Δt/8min) becomes the mechanistic explanation for the conflicting trial results.
...
## 12. SOD Mimetics & Avasopasem

Superoxide dismutation. It follows the same time-window logic as amifostine, but its half-life is longer at 27 min.
...
## 13. Antifibrotics & ACE Inhibitors

Pirfenidone · nintedanib · ACE inhibitors · pentoxifylline. All of them reach only the slow loop, so none can be detected by a pneumonitis endpoint.
...
## 14. Immune Checkpoint Inhibitors & Radiation Pneumonitis

PACIFIC consolidation therapy. In the model durvalumab does not add a term but raises the gain GIMM of the fast loop, so the excess risk grows with MLD.
...
## 15. Stereotactic Body Radiotherapy (SBRT) Toxicity

The low MLD and steep dose fall-off of peripheral SBRT. This is also the point at which the model under-predicts SBRT pneumonitis (§Limitations).
...
## 16. Key Trials & Comparison of Delivery Techniques

RTOG 0617 · protons vs IMRT · functional lung avoidance · hypofractionated regimens.
...
## 17. Patient Risk Factors & Prediction Models

Pre-existing interstitial lung disease · COPD · genetic polymorphisms. In the model these enter as the baseline reserve RF = √(DLREF/DLCO₀), which turns the same injury into a different grade.
...
## 18. Changes in Pulmonary Function & Imaging Quantification

The fall in DLCO after thoracic radiotherapy, and CT quantification of fibrosis.
...
## 19. QSP & Quantitative Modelling Methodology

mrgsolve and model-informed drug development (MIDD) methodology.
...
## 20. Other Mechanisms & Phenomena

Senescence · NOX4 · macrophage polarisation · lymphopenia · re-irradiation · FLASH · organising pneumonia · out-of-field injury, and others.
...
## Structural choices with no citation

The items below are not taken directly from the literature but are the **structural decisions**
of this model, and should be read as assumptions rather than as evidence.

- **Discretisation into six DVH bins.** A real DVH is continuous; six bins were chosen as
  the minimum resolution at which the volume effect can be interrogated. Increasing the number
  of bins may change the derived value of n.
- **Smoothing of the BED delivery rate.** Fractionated irradiation is written not as a daily
  bolus but as the continuous delivery rate `RB_b = BED_b / T_course`. Total BED and the
  fractionation dependence are preserved exactly, but sub-daily kinetics (sublethal damage
  repair) are not represented. This was adopted because every downstream time constant is ≥ 1 day.
- **PNI50 · PNISL.** Two scale constants that are not mechanistically identifiable; they were
  fitted to the QUANTEC dose-response (`rili_calibration.py`).
- **CTCAE grade thresholds.** The cut-points that convert the continuous index into a grade are
  a convention fitted to the model's output distribution, not an observation.
- **Continuous dose-rate approximation for the oral drugs.** Prednisolone · pirfenidone ·
  nintedanib · lisinopril are written as continuous mg/day inputs. Only durvalumab is a real
  q28d bolus.
- **Algebraic treatment of amifostine.** Instead of integrating the 8-minute half-life as an ODE,
  the protection factor is computed algebraically as a function of the dosing-to-irradiation interval.
