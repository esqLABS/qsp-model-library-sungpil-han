################################################################################
# Graves' Disease QSP Model — mrgsolve Implementation (REFACTORED, pluggable PK)
#
# Refactor of `gd_mrgsolve_model.R` per FORK_WORKFLOW_GUIDE.md Part 2.
# Original untouched (see `gd_refactor_notes.md` for full rationale).
#
# Scope: of the seven rows this file carries in
# `driver-patches/data/compound_perturbation_census.md`, only THREE are
# genuine externally-dosed drugs and were refactored: Methimazole (MMI),
# Propranolol (PROP), Propylthiouracil (PTU). T3, T4, TRAb, and TSH are the
# disease's own endogenous hormone/antibody state variables — none of the
# four has a dosing route (no depot compartment, no absorption/bioavailability
# parameter, no `ev()`/dataset dosing anywhere in the original or this file);
# each is produced solely by the model's own synthesis terms
# (T4_synth/T3_synth/TSH_prod/kprod_TRAb*Bcell_C). They are left completely
# untouched here and the census has been corrected accordingly.
#
# Build-compatibility fix (disclosed, syntax-only, non-numeric — see notes):
# the ORIGINAL does not compile under mrgsolve 2.0.1 for two reasons, both
# fixed here and logged as UPSTREAM_ISSUES.md #93:
#   1. `$PARAM F_MMI` collided with mrgsolve's own reserved `F_<compartment>`
#      bioavailability symbol because a compartment was literally named `MMI`.
#      Renaming the compartment to `CENT_MMI` (which this refactor does anyway,
#      per the naming convention) removes the collision — no separate fix
#      needed beyond the rename itself.
#   2. `$CAPTURE` re-listed eight `$CMT` compartment names directly
#      (`MMI PTU_C RAI_THYR PROP_C TPO_inh ThyMass GO_act BoneResor`), which
#      mrgsolve 2.0.1 rejects ("compartment should not be in $CAPTURE").
#      Compartments are exposed as output columns automatically with no
#      `$CAPTURE` entry needed, so that line is simply dropped (replaced with
#      the refactor's own new `C_<STEM>`/`EFFECT_<STEM>` capture entries).
#
# Compartments (≥15 ODEs, unchanged count = 18):
#   CENT_MMI  = Methimazole plasma-equivalent concentration state (renamed from MMI)
#   CENT_PTU  = PTU plasma-equivalent concentration state (renamed from PTU_C)
#   RAI_SERUM = 131I serum (out of scope — not a census row for this file)
#   RAI_THYR  = 131I thyroid (out of scope)
#   CENT_PROP = Propranolol plasma-equivalent concentration state (renamed from PROP_C)
#   TSH_C     = TSH (endogenous — NOT refactored, see above)
#   T4_C      = total T4 (endogenous — NOT refactored)
#   T3_C      = total T3 (endogenous — NOT refactored)
#   fT4_C     = free T4 (endogenous)
#   fT3_C     = free T3 (endogenous)
#   rT3_C     = reverse T3 (endogenous)
#   TRAb_C    = TRAb (endogenous — NOT refactored)
#   Bcell_C   = disease-specific B cells (endogenous)
#   BoneResor, HR_dev, GO_act, ThyMass, TPO_inh = PD endpoints (endogenous)
################################################################################
...
graves_model <- '
$PROB
Graves Disease QSP Model — HPT Axis, TRAb, Thyroid Hormone PK/PD, Drug Effects (refactored PK)
...
// ── TRAb / B-cell Dynamics (endogenous — untouched) ──
...
CL_MMI    = 4.0,    // elimination rate constant (t half~4h) (was kel_MMI)
...
// ── Drug PK — PTU ──
// Same Archetype-1 bespoke shape as MMI. PTU genuinely has TWO distinct
...
F_PTU        = 0.75,   // [declared, never used in $ODE — preserved as-is]
EC50_PTU     = 1.0,    // PTU for 50% TPO inhibition (ug/mL) (was IC50_PTU)
EMAX_PTU     = 1.0,    // Hill ceiling [new, math-implied]
GAMMA_PTU    = 1.0,    // Hill coefficient [new]
EC50_PTU_D1  = 5.0,    // PTU for 50% D1 inhibition (was IC50_PTU_D1)
EMAX_PTU_D1  = 1.0,    // Hill ceiling [new, math-implied]
...
CL_PROP    = 4.16,   // elimination rate constant (t half~4h) (was kel_PROP)
V1_PROP    = 3.9,    // [declared, never used in $ODE — preserved as-is]
F_PROP     = 0.26,   // [declared, never used in $ODE — preserved as-is]
EC50_PROP  = 0.02,   // propranolol for 50% HR reduction (was EC50_PROP_HR)
EMAX_PROP  = 1.0,    // Hill ceiling [new, math-implied]
GAMMA_PROP = 1.0,    // Hill coefficient [new]

// ── Drug PK — 131I (Radioiodine) [out of scope: not a census row] ──
...
CENT_MMI    // methimazole plasma-equivalent concentration (was MMI)
CENT_PTU    // PTU plasma-equivalent concentration (was PTU_C)
RAI_SERUM   // 131I serum
RAI_THYR    // 131I thyroid
CENT_PROP   // propranolol plasma-equivalent concentration (was PROP_C)
...
$ODE
// ────────────────────────────────────────────────────────────────
// Drug PK (Archetype 1, bespoke: compartment holds concentration directly)
// ────────────────────────────────────────────────────────────────

// Methimazole (administered as dose events via mrgsolve)
double C_MMI  = CENT_MMI;
dxdt_CENT_MMI = -CL_MMI * C_MMI;

// PTU
double C_PTU  = CENT_PTU;
dxdt_CENT_PTU = -CL_PTU * C_PTU;

// Radioiodine — serum → thyroid uptake (out of scope, untouched)
...
// Propranolol
double C_PROP  = CENT_PROP;
dxdt_CENT_PROP = -CL_PROP * C_PROP;

// ────────────────────────────────────────────────────────────────
// Hill interface — one named EFFECT_<STEM> per mechanism
// ────────────────────────────────────────────────────────────────
double EFFECT_MMI    = EMAX_MMI    * pow(C_MMI, GAMMA_MMI)       / (pow(EC50_MMI, GAMMA_MMI)       + pow(C_MMI, GAMMA_MMI));
double EFFECT_PTU     = EMAX_PTU    * pow(C_PTU, GAMMA_PTU)       / (pow(EC50_PTU, GAMMA_PTU)        + pow(C_PTU, GAMMA_PTU));
double EFFECT_PTU_D1  = EMAX_PTU_D1 * pow(C_PTU, GAMMA_PTU_D1)    / (pow(EC50_PTU_D1, GAMMA_PTU_D1)  + pow(C_PTU, GAMMA_PTU_D1));
double EFFECT_PROP    = EMAX_PROP   * pow(C_PROP, GAMMA_PROP)     / (pow(EC50_PROP, GAMMA_PROP)      + pow(C_PROP, GAMMA_PROP));

// ────────────────────────────────────────────────────────────────
// TPO inhibition (0–1 scale, combined MMI + PTU)
// ────────────────────────────────────────────────────────────────
double TPO_inh_MMI = (USE_MMI > 0.5) ? (TPO_inh_max * EFFECT_MMI) : 0.0;
double TPO_inh_PTU = (USE_PTU > 0.5) ? (TPO_inh_max * EFFECT_PTU) : 0.0;
...
// ────────────────────────────────────────────────────────────────
double D1_inh_PTU  = (USE_PTU  > 0.5) ? EFFECT_PTU_D1 : 0.0;
double D1_inh_PROP = (USE_PROP > 0.5) ? EFFECT_PROP   : 0.0;
...
double MMI_immu_eff = (USE_MMI > 0.5) ? MMI_Bcell * EFFECT_MMI : 0.0;
...
$CAPTURE TSH_obs fT4_obs fT3_obs TRAb_obs HR_obs BMD_loss
         C_MMI EFFECT_MMI C_PTU EFFECT_PTU EFFECT_PTU_D1 C_PROP EFFECT_PROP
'

# ─── Compile Model ─────────────────────────────────────────────────────────────
gd_mod <- mrgsolve::mcode("graves_disease_refactored", graves_model)
...
mmi_events <- ev(amt=0.10, ii=0.33, addl=2190-1, cmt=1, time=0)  # q8h for 730 days (cmt 1 = CENT_MMI)
...
rai_events <- ev(amt=15, cmt=3, time=0)  # single RAI dose (mCi → normalized) (cmt 3 = RAI_SERUM)
...
combo_events <- ev(amt=0.10, ii=0.33, addl=365*3-1, cmt=1, time=0) +
                ev(amt=0.06, ii=0.17, addl=365*6-1, cmt=5, time=0)  # prop q6h (cmt 5 = CENT_PROP)
...
# NB: the original's own `br_lt4` event doses cmt=5, which is CENT_PROP, not a
# dedicated LT4 compartment (this model has no LT4 pool despite USE_LT4 and the
# file's header comment implying one) — a pre-existing upstream defect, out of
# scope for this refactor (LT4 is not one of this file's census rows) and left
# exactly as the original had it, so this scenario still reproduces byte-for-byte.
br_mmi   <- ev(amt=0.20, ii=0.33, addl=2190-1, cmt=1, time=0)   # MMI 60mg/day
br_lt4   <- ev(amt=0.10, ii=1.0,  addl=730-1,  cmt=5, time=90)  # LT4 start at day 90 (see NB above)
...
plot_graves_outcomes <- function(data) {
  ...
  layout <- (p_tsh | p_fT4 | p_fT3) /
    ...
    plot_annotation(
      title    = "Graves' Disease QSP Model — Treatment Scenario Comparison (refactored)",
...
# TRAb sensitivity to EC50_MMI (was IC50_MMI)
sa_IC50 <- run_sensitivity("EC50_MMI",
...
vpop_params <- data.frame(
  ...
  EC50_MMI = rlnorm(n_pop, log(0.15), 0.3),     # variability in drug sensitivity (was IC50_MMI)
  ksyn_Bcell = rnorm(n_pop, 0.05, 0.01)
) %>% filter(EC50_MMI > 0.02, ksyn_Bcell > 0.01)
...
# ─── Summary Print ─────────────────────────────────────────────────────────────
cat("\n========================================\n")
cat("Graves' Disease QSP Model Summary (refactored)\n")
cat("========================================\n")
