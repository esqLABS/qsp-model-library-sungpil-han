...
| File | Contents |
|------|------|
| [`icic_qsp_model.dot`](icic_qsp_model.dot) · [`.svg`](icic_qsp_model.svg) · [`.png`](icic_qsp_model.png) | Mechanistic map — **146 nodes, 12 clusters, 238 edges** |
| [`icic_reference_model.py`](icic_reference_model.py) | **The reference implementation — the numerical ground truth.** The file that computes every number below |
| [`icic_mrgsolve_model.R`](icic_mrgsolve_model.R) | mrgsolve 45-state ODE model (a 1:1 translation of the file above) + a 13-scenario driver |
| [`icic_shiny_app_en.R`](icic_shiny_app_en.R) | 8-tab Shiny dashboard |
| [`icic_references_en.md`](icic_references_en.md) | 98 PubMed links, with the source marked for each parameter |
| [`icic_reference_output.txt`](icic_reference_output.txt) | The full analysis output (A-L) |
| [`icic_cross_validation.txt`](icic_cross_validation.txt) | Python ↔ mrgsolve comparison and structural verification |
...
The full table is at the end of `icic_references_en.md`. The three central ones:
...
# Shiny dashboard
R -e 'shiny::runApp("icic_shiny_app_en.R")'
```
