...
# Polyarteritis Nodosa Quantitative Systems Pharmacology Dashboard
# =============================================================================
...
$PROB PAN QSP Model — Polyarteritis Nodosa
          Polyarteritis Nodosa Quantitative Systems Pharmacology Model
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      ...
      menuItem("Scenario Comparison",    tabName = "tab_scenarios", icon = icon("layer-group")),
      menuItem("Biomarker Correlation",  tabName = "tab_corr",      icon = icon("project-diagram"))
    ),
    hr(),
    div(
      style = "padding:10px;",
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(
        ...
        fluidRow(
          box(
            ...
            sliderInput("age",    "Age (years)",         20, 80, 50),
            selectInput("sex",    "Sex",
                        choices = c("Male" = "M", "Female" = "F")),
            sliderInput("wt",     "Weight (kg)",         40, 120, 70),
            selectInput("hbv",    "HBV status",
                        choices = c("Negative" = "0",
                                    "Positive" = "1")),
            selectInput("subtype", "Disease subtype",
                        choices = c("Classic PAN" = "classic",
                                    "HBV-associated PAN" = "hbv",
                                    "Microscopic PAN" = "micro"))
          ),
          box(
            title = "Baseline Disease Activity",
            width = 6, status = "danger", solidHeader = TRUE,
            sliderInput("bvas0",  "Baseline BVAS (0–63)",  0, 63, 24),
            sliderInput("crp0",   "Baseline CRP (mg/L)",               0, 200, 60),
            sliderInput("egfr0",  "Baseline eGFR (mL/min/1.73 m²)",   15, 120, 75),
            sliderInput("n_organs","Number of organs involved", 1, 7, 3)
          )
        ),
        fluidRow(
          box(
            ...
            checkboxGroupInput("organs", "Organs involved",
                               choices = c("Kidney"         = "kidney",
      ...
      tabItem(
        ...
        fluidRow(
          box(
            title = "Treatment Selection & Dosing",
            width = 4, status = "danger", solidHeader = TRUE,
            selectInput("arm_pk", "Treatment arm",
                        choices = c("Prednisolone monotherapy" = "pred_mono",
                                    "Pred + CYC pulse (CYCLOPS)" = "pred_cyc",
                                    "Pred + AZA maintenance" = "pred_aza",
                                    "Antiviral + Pred (HBV-PAN)" = "hbv_pred")),
            hr(),
            h5("Prednisolone"),
            sliderInput("dose_pred",  "Initial dose (mg/day)", 10, 100, 60),
            sliderInput("freq_pred",  "Frequency (/day)", 1, 2, 1),
            h5("Cyclophosphamide (pulse)"),
            sliderInput("dose_cyc",   "Pulse dose (mg)", 200, 1500, 750),
            h5("Azathioprine (maintenance)"),
            sliderInput("dose_aza",   "Daily dose (mg/day)", 50, 200, 150),
            h5("Antiviral (e.g., Tenofovir)"),
            sliderInput("dose_av",    "Daily dose (mg/day)", 100, 400, 300),
            sliderInput("pk_end",     "Simulation horizon (days)", 30, 730, 365)
          ),
          box(
            title = "Prednisolone Plasma Concentration",
      ...
      tabItem(
        ...
        fluidRow(
          box(
            title = "PD Parameter Sensitivity",
            ...
            selectInput("pd_arm",    "Treatment arm",
                        choices = c("pred_mono","pred_cyc","pred_aza","hbv_pred","none"))
          ),
          box(
            title = "Immune Complex & Complement",
      ...
      tabItem(
        ...
        fluidRow(
          box(
            title = "Microaneurysm Burden & Vascular Index",
            width = 12, status = "warning", solidHeader = TRUE,
      ...
      tabItem(
        ...
        fluidRow(
          box(
            title = "Arms to Compare",
            width = 3, status = "danger", solidHeader = TRUE,
            checkboxGroupInput("arms_sel", "Select arms",
                               choices = c(
                                 "No treatment" = "none",
                                 "Pred monotherapy" = "pred_mono",
                                 "Pred + CYC pulse (CYCLOPS)" = "pred_cyc",
                                 "Pred + AZA maintenance" = "pred_aza",
                                 "HBV antiviral + Pred" = "hbv_pred"
                               ),
                               selected = c("none","pred_mono","pred_cyc")),
            sliderInput("scen_end", "Horizon (days)", 180, 730, 365),
            hr(),
            downloadButton("dl_scen", "Download comparison data", class = "btn-sm btn-default")
          ),
          box(
            title = "BVAS Comparison",
            width = 9, status = "danger", solidHeader = TRUE,
      ...
      tabItem(
        ...
        fluidRow(
          ...
          box(
            title = "Biomarker Waterfall",
            width = 6, status = "info", solidHeader = TRUE,
            selectInput("wf_arm", "Arm",
                        choices = c("pred_mono","pred_cyc","pred_aza","hbv_pred","none")),
        ...
        fluidRow(
          box(
            title = "Response Prediction by Subtype",
...
server <- function(input, output, session) {
  ...
  output$risk_output <- renderUI({
    s <- ffs_score()
    risk <- if (s == 0) "Low" else if (s == 1) "Medium" else "High"
  ...
  output$plot_cytokines <- renderPlotly({
    ...
    p <- ggplot(df_long, aes(x = time, y = value, color = marker)) +
      ...
      labs(title = "Inflammatory Cytokines",
  ...
  output$plot_vasc <- renderPlotly({
    ...
    p <- ggplot(df, aes(x = time, y = VASC_norm)) +
      ...
      geom_line(color = "#8B0000", linewidth = 1.2) +
      labs(title = "Vascular Inflammation Index",
  ...
  output$plot_egfr <- renderPlotly({
    ...
    p <- ggplot(df, aes(x = time, y = EGFR_abs)) +
      ...
      labs(title = "Renal Function — eGFR",
  ...
  output$plot_miao <- renderPlotly({
    ...
    p <- ggplot(df_long, aes(x = time, y = value, color = endpoint, fill = endpoint)) +
      ...
      scale_fill_manual(values  = c(`Microaneurysm burden`  = "#8B0000",
                                    `Vascular inflammation` = "#c0392b")) +
      labs(title = "Microaneurysm Burden & Vascular Inflammation",
  ...
  make_scen_plot <- function(df, yvar, ytitle) {
    ...
    p <- ggplot(df, aes_string(x = "time", y = yvar,
      ...
      labs(x = "Time (days)", y = ytitle, color = "Arm") +
      theme_pan()
    g2p(p)
  }
  ...
  output$plot_scatter_egfr_ic <- renderPlotly({
    ...
    p <- ggplot(df, aes(x = IC_norm, y = EGFR_abs, color = time)) +
      ...
      labs(title = "eGFR vs Immune Complex",
  ...
  output$plot_subtype <- renderPlotly({
    ...
    p <- ggplot(df_all, aes(x = time, y = BVAS_abs, color = subtype_label,
      ...
      scale_color_manual(values = c(`Classic PAN` = "#8B0000",
                                    `HBV-PAN`     = "#c0392b",
                                    `Micro-PAN`   = "#e74c3c")) +
      labs(title = "BVAS by disease subtype & treatment",
