## ============================================================
## Alzheimer's Disease — QSP Model (mrgsolve) — REFACTORED PK/PD
## Amyloid Cascade · Tau Pathology · Neuroinflammation
## Cholinergic System · Synaptic Plasticity · Drug PK/PD
##
## This is a fork-owned sibling of `ad_mrgsolve_model.R` (never edit that
## file directly — see FORK_WORKFLOW_GUIDE.md, Part 2). All three modeled
## compounds — Donepezil (DON), Memantine (MEM), Lecanemab (LEC) — have been
## renamed to the fork's pluggable-PK naming convention and each now
## exposes exactly one concentration (`C_<STEM>`) and one named Hill effect
## (`EFFECT_<STEM>`). No PK/PD math was changed for any compound: this is a
## pure structural rename, verified bit-exact against the original via the
## qspserver mrgsolve_api (see ad_refactor_notes.md).
##
## One build-compatibility fix was required for the model to compile at all
## under mrgsolve 2.0.1 (original and refactored alike) -- ten disease-state
## compartment names were removed from $CAPTURE because they duplicated
## $CMT compartment names, which this mrgsolve build refuses to build
## (compartments are already present in every simulation's output
## regardless of $CAPTURE, so nothing about what is reported changes).
## This is a syntax-only fix, unrelated to any compound's own PK/PD; logged
## as translations/UPSTREAM_ISSUES.md #45 and disclosed in full in
## ad_refactor_notes.md. The untouched original still carries the defect.
##
## Compartments (21 ODEs):
##   PK : Donepezil (gut, plasma, peripheral, CNS tissue),
##        Memantine (plasma, CNS tissue),
##        Lecanemab (plasma, peripheral, CNS tissue)
...
$PARAM
  ...
  V1_DON   = 594       // L     central volume [was V2_DON]
  Q_DON    = 40.0      // L/h   intercompartmental clearance
  V2_DON   = 1180      // L     peripheral volume [was V3_DON]
  KP_DON   = 0.18      // -     brain:plasma partition (free) [was Kp_DON]
  F_DON    = 1.0       // -     bioavailability
  EMAX_DON = 1.0       // -     max AChE inhibition [was hardcoded Imax_DON=1.0 in $MAIN]
  EC50_DON = 6.7e-6    // mg/L  AChE EC50 donepezil (Ki ~6.7 nM) [was IC50_AChE_DON]
  GAMMA_DON= 1.2       // -     Hill coefficient donepezil [was HILL_DON]

  // ── MEMANTINE PK (Periclou et al. Clin Pharmacol 2004) ──────
  KA_MEM   = 0.072     // h-1   absorption rate (declared in original but never referenced in
                        //       its $ODE -- dead parameter, preserved unchanged, see notes)
  ...
  GAMMA_MEM= 1.0       // -     Hill coefficient memantine [was HILL_MEM; declared but never
                        ...
                        //       now actually applied, pow(x,1)=x so no numeric change]
...
$CMT
  // Donepezil PK (3-compartment oral + bespoke CNS tissue compartment)
  GUT_DON CENT_DON PERI_DON CNS_DON
  // Memantine PK (1-compartment + bespoke CNS tissue compartment)
  CENT_MEM CNS_MEM
  // Lecanemab PK (2-compartment IV + bespoke CNS tissue compartment)
  CENT_LEC PERI_LEC CNS_LEC
  // Amyloid dynamics
...
$MAIN
  ...
  // ── Effective PK parameters (dead locals, preserved from original;
  //    original never referenced CL2/V2/Q3/V3 again after this point) ──
  double CL2  = CL_DON;
  double V2   = V1_DON;   // [was V2_DON]
  double Q3   = Q_DON;
  double V3   = V2_DON;   // [was V3_DON]

  // ── Drug concentration conversions ──────────────────────────
  // Donepezil plasma conc (mg/L, diagnostic only) + exposed CNS concentration
  double Cp_DON = CENT_DON / V1_DON;
  double C_DON  = CNS_DON;      // exposed concentration -- CNS_DON is already a
                                 // pseudo-equilibrium concentration state (mg/L),
                                 // not an amount, so this is a straight alias

  // Memantine plasma conc (mg/L, diagnostic only) + exposed CNS concentration
  double Cp_MEM = CENT_MEM / V1_MEM;
  double C_MEM  = CNS_MEM;      // exposed concentration, straight alias

  // Lecanemab plasma conc (mg/L, diagnostic only) + exposed CNS concentration
  double Cp_LEC = CENT_LEC / V1_LEC;
  double C_LEC  = CNS_LEC;      // exposed concentration, straight alias

  // ── Drug effect: AChE inhibition by Donepezil ───────────────
  double EFFECT_DON = EMAX_DON * pow(C_DON, GAMMA_DON) /
                     (pow(EC50_DON, GAMMA_DON) + pow(C_DON, GAMMA_DON));

  // ── Drug effect: NMDA block by Memantine ────────────────────
  double EFFECT_MEM = EMAX_MEM * pow(C_MEM, GAMMA_MEM) /
                     (pow(EC50_MEM, GAMMA_MEM) + pow(C_MEM, GAMMA_MEM));

  // ── Drug effect: Lecanemab on protofibrils ───────────────────
  double EFFECT_LEC = EMAX_LEC * pow(C_LEC, GAMMA_LEC) /
                     (pow(EC50_LEC, GAMMA_LEC) + pow(C_LEC, GAMMA_LEC));

  // ── Effective ACh degradation rate ──────────────────────────
  double kdeg_ACh_eff = kdeg_ACh * (1.0 - EFFECT_DON);
...
$ODE
  ...
  // ═══════════════════════════════════════════════════════════
  dxdt_GUT_DON  = -KA_DON * GUT_DON;
  dxdt_CENT_DON =  KA_DON * GUT_DON * F_DON
                 - (CL_DON + Q_DON) / V1_DON * CENT_DON
                 + Q_DON / V2_DON * PERI_DON;
  dxdt_PERI_DON =  Q_DON / V1_DON * CENT_DON
                 - Q_DON / V2_DON * PERI_DON;
  // CNS: pseudo-equilibrium with plasma (Kp ~ 0.18)
  dxdt_CNS_DON  =  0.5 * (KP_DON * CENT_DON / V1_DON - CNS_DON);

  // ═══════════════════════════════════════════════════════════
  // 2. MEMANTINE PK (1-compartment + CNS)
  // ═══════════════════════════════════════════════════════════
  dxdt_CENT_MEM = -CL_MEM / V1_MEM * CENT_MEM;
  dxdt_CNS_MEM  =  0.3 * (KP_MEM * CENT_MEM / V1_MEM - CNS_MEM);

  // ═══════════════════════════════════════════════════════════
  // 3. LECANEMAB PK (2-compartment IV + CNS transfer)
  // ═══════════════════════════════════════════════════════════
  dxdt_CENT_LEC = -CL_LEC / V1_LEC * CENT_LEC
                 - Q_LEC / V1_LEC * CENT_LEC
                 + Q_LEC / V2_LEC * PERI_LEC
                 - KBBB_LEC * CENT_LEC;
  dxdt_PERI_LEC =  Q_LEC / V1_LEC * CENT_LEC
                 - Q_LEC / V2_LEC * PERI_LEC;
  dxdt_CNS_LEC  =  KBBB_LEC * CENT_LEC
                 - KCNS_LEC * CNS_LEC
                 - EFFECT_LEC * 0.05 * CNS_LEC;  // drug bound to target
  ...
  dxdt_AB_PROTO =  ke_oligo * AB_OLIGO
                 - ke_fibril * AB_PROTO
                 - EFFECT_LEC * ke_fibril * AB_PROTO; // lecanemab binding

  // Plaques: deposited, cleared by microglia
  dxdt_AB_PLAQUE=  ke_fibril * (1.0 - EFFECT_LEC) * AB_PROTO
...
$TABLE
  ...
  // ── Drug concentrations ──────────────────────────────────────
  double Cp_Donepezil  = CENT_DON / V1_DON * 1000;    // ng/mL
  double Ccns_Donepezil = CNS_DON * 1000;              // ng/mL
  double Cp_Memantine  = CENT_MEM / V1_MEM * 1000;
  double Cp_Lecanemab  = CENT_LEC / V1_LEC * 1000;    // ug/mL

  // ── Inhibition/occupancy outputs ─────────────────────────────
  double AChE_inhibition = EFFECT_DON * 100;    // % inhibition
  double NMDAR_occupancy = EFFECT_MEM * 100;
  double Proto_neutralized = EFFECT_LEC * 100;
...
$CAPTURE
  ...
  C_DON EFFECT_DON C_MEM EFFECT_MEM C_LEC EFFECT_LEC
'
...
don_dose <- ev(amt = 10, ii = 24, addl = 365*2 - 1, cmt = "GUT_DON")

# Memantine 20 mg QD (oral) — titrated, here simplified as maintenance
mem_dose <- ev(amt = 20, ii = 24, addl = 365*2 - 1, cmt = "CENT_MEM")

# Lecanemab 10 mg/kg biweekly IV (CLARITY-AD regimen; ~700 mg for 70 kg pt)
lec_dose <- ev(amt = 700, ii = 24*14, addl = 26 - 1, cmt = "CENT_LEC",
