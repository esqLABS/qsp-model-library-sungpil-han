...
K_TISSUE_DECAY_LARO: 0.0058 : Tissue-retained enzyme decay rate (1/h)     // functional t1/2 ~5 d
...
CL_GEN   : 3.0   : Genistein clearance (L/h)                 // = original KE_GEN(0.12) x V1_GEN(25); t1/2 ~5-6 h
V1_GEN    : 25    : Genistein apparent Vd (L)
...
// ============================================
EMAX_LARO      : 0.70  : Max systemic (visceral) enzyme-access fraction from laronidase alone
EC50_LARO      : 1.5   : Laronidase tissue-retained enzyme amount for half-max delivery (mg)
GAMMA_LARO     : 1     : Laronidase Hill coefficient (original had no explicit Hill term)
...
// ============================================
EMAX_GEN : 0.25 : Max fractional reduction in GAG synthesis rate from genistein
...
GAMMA_GEN: 1    : Genistein Hill coefficient (original had no explicit Hill term)
...
$CMT @annotated
CENT_LARO   : Laronidase central (plasma) compartment (mg)
TISSUE_LARO : Laronidase tissue-retained active-enzyme pool (mg)
ADA         : Anti-laronidase antibody titer (a.u., 0-1)
GUT_GEN     : Genistein oral gut depot (mg)
CENT_GEN    : Genistein central concentration (mg/L)
...
double LARO_UPTAKE_FLUX = CL_LARO/V1_LARO*CENT_LARO;
dxdt_CENT_LARO   = -LARO_UPTAKE_FLUX;
dxdt_TISSUE_LARO =  LARO_UPTAKE_FLUX - K_TISSUE_DECAY_LARO*TISSUE_LARO;
double C_LARO_PLASMA = CENT_LARO / V1_LARO * 1000.0;   // ng/mL equivalent (plasma, for ADA exposure signal)
double C_LARO = TISSUE_LARO;   // exposed concentration: tissue-retained active-enzyme pool (mg) that PD actually reads

// ---- ADA titer: rises with laronidase exposure, saturating ----
double EXPOSURE_SIGNAL = C_LARO_PLASMA / (10.0 + C_LARO_PLASMA);
...
dxdt_GUT_GEN  = -KA_GEN * GUT_GEN;
dxdt_CENT_GEN =  KA_GEN * GUT_GEN / V1_GEN - CL_GEN/V1_GEN * CENT_GEN;
double C_GEN = CENT_GEN;   // exposed concentration: central compartment already stores concentration directly (mg/L)
...
double EFFECT_LARO = EMAX_LARO * pow(C_LARO, GAMMA_LARO) / (pow(EC50_LARO, GAMMA_LARO) + pow(C_LARO, GAMMA_LARO)) * (1.0 - ADA_INHIB);
...
double ENZ_ACCESS_SYS = EFFECT_LARO + HSCT_EFFECT_SYS + LVGT_EFFECT_SYS
                        - EFFECT_LARO*HSCT_EFFECT_SYS - EFFECT_LARO*LVGT_EFFECT_SYS - HSCT_EFFECT_SYS*LVGT_EFFECT_SYS;
...
double EFFECT_GEN = EMAX_GEN * pow(C_GEN, GAMMA_GEN) / (pow(EC50_GEN, GAMMA_GEN) + pow(C_GEN, GAMMA_GEN));

// ---- GAG burden pools (turnover-to-target) ----
double GAG_SYS_TARGET = GAG_SYS0 * (1.0 - MAXRED_SYS*ENZ_ACCESS_SYS) * (1.0 - EFFECT_GEN);
dxdt_GAG_SYS = K_GAG_SYS * (GAG_SYS_TARGET - GAG_SYS);

double GAG_CNS_TARGET = GAG_CNS0 * (1.0 - MAXRED_CNS*ENZ_ACCESS_CNS) * (1.0 - 0.5*EFFECT_GEN);
dxdt_GAG_CNS = K_GAG_CNS * (GAG_CNS_TARGET - GAG_CNS);

double GAG_CART_TARGET = GAG_CART0 * (1.0 - MAXRED_CART*ENZ_ACCESS_CART) * (1.0 - EFFECT_GEN);
...
$CAPTURE C_LARO_PLASMA C_LARO EFFECT_LARO ADA_INHIB ENZ_ACCESS_SYS ENZ_ACCESS_CNS ENZ_ACCESS_CART C_GEN EFFECT_GEN SURVIVAL
'
...
laro_ev     <- make_ev(laro_dose, 168, 259, "CENT_LARO", rate = laro_dose / 4)   # 5 yr = 260 weekly doses
laro_bridge_ev <- make_ev(laro_dose, 168, 14,  "CENT_LARO", rate = laro_dose / 4)   # ~15 wk bridging, stopped at engraftment
...
scenarios <- list(
  ...
  "8_Genistein_SRT_Adjunct_to_ERT"            = list(ev = c(laro_ev, make_ev(10*WT_CHILD, 24, 5*365, "GUT_GEN")),
                                                       param = list(PHENOTYPE = 0, HSCT_FLAG = 0)),
