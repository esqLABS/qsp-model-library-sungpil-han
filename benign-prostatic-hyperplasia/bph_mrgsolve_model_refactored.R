...
## bph_refactor_notes.md for full detail):
##   All four compounds refactored -- Dutasteride, Finasteride, Tadalafil,
##   Tamsulosin -- per their rows in
##   driver-patches/data/compound_perturbation_census.md ("Redirect
##   concentration, clean single site"). Archetype 3 (depot+central+
##   peripheral, linear) for all four; naming: GUT_<STEM>/CENT_<STEM>/
##   PERI_<STEM>/KA_<STEM>/F_<STEM>/CL_<STEM>/V1_<STEM>/V2_<STEM>/Q_<STEM>/
##   C_<STEM>/EC50_<STEM>/EMAX_<STEM>/GAMMA_<STEM>/EFFECT_<STEM>.
##   Every compound's own effect term was already a bare Emax/Imax ratio
##   (Emax*C/(EC50+C) for tamsulosin, C/(EC50+C) for the three 5AR/PDE5
##   drugs) -- this is a RENAME to the guide's explicit Hill form
##   (EMAX*C^GAMMA/(EC50^GAMMA+C^GAMMA), GAMMA=1), not a refit. EMAX=1 and
##   GAMMA=1 are new named parameters for finasteride/dutasteride/tadalafil
##   (the original had them only implicitly); tamsulosin's EMAX_TAMS=0.95
##   is the original's own explicit value, renamed.
##   The multi-drug combination arithmetic (SRD5A2/SRD5A1 additive 5AR
##   inhibition, PDE5-driven cGMP relaxation) is untouched -- only the
##   inputs feeding it (EFFECT_FINA/EFFECT_DUT/EFFECT_TAD) were renamed
##   from inh_fina/inh_dut/inh_pde5.
##
## TWO PRE-EXISTING BUILD DEFECTS (mrgsolve 2.0.1), unrelated to any
## compound's own PK -- see bph_refactor_notes.md and
## translations/UPSTREAM_ISSUES.md for full detail. Both are syntax-only
## fixes applied directly in this file; the untouched bph_mrgsolve_model.R
## still carries both, unfixed:
##   1. `_init_<CMT> = value;` is not accepted at all ("was not declared in
##      this scope") -- converted to the modern `<CMT>_0 = value;` idiom
##      throughout $MAIN. No compartment, index, or starting value changed.
##   2. $TABLE declared `double NAME = expr;` and *also* used the identical
##      NAME as an old-style `capture NAME = expr;` target elsewhere
##      (8 pre-existing cases: DHT_inhibition_pct, DHT_PROST_inh_pct,
##      PV_change_pct, IPSS_change, QMAX_change, PVR_change,
##      PSA_change_pct, Alpha1_block_pct -- plus this refactor's own new
##      C_<STEM>/EFFECT_<STEM> ODE locals, which hit the same collision
##      class against their own $TABLE captures). mrgsolve 2.0.1 auto-
##      promotes any $CAPTURE'd name to a class member, which collides
##      with a same-named `double` declared elsewhere ("redefinition of
##      capture ..."). Fixed by renaming the intermediate `double` local
##      (append `_calc`/`_tbl`, exposed capture name unchanged) for the
##      8 pre-existing cases, and by dropping `double` from the ODE-local
##      C_<STEM>/EFFECT_<STEM> declarations (bare assignment to the member
##      mrgsolve already promotes from the later `capture` statement) for
##      this refactor's own new names. Zero numeric values changed either
##      way; confirmed empirically via POST /model_manifest.
##
## C_<STEM>/EFFECT_<STEM> are $ODE-local (and independently recomputed
## $TABLE-local, for $CAPTURE) `double`s, not $PARAM -- mrgsolve 2.0.1 does
## not allow a name to be simultaneously a settable $PARAM default and a
## value freshly assigned every $ODE/$TABLE step. All are $CAPTURE'd via
## `capture NAME = expr;`, discoverable through POST /model_manifest's
## outputPaths (not its parameter list) -- same documented constraint as
## several other *_refactor_notes.md in this repo (autoimmune-
## polyendocrinopathy, chronic-lymphocytic-leukemia, rheumatoid-arthritis).
##
## Verified against the original (rerun via the qspserver mrgsolve_api
## service, http://localhost:8007) on the original file's own 6 treatment
## scenarios (Watchful Waiting, Tamsulosin, Finasteride, Dutasteride, Combo
## DUT+TAMS, Tadalafil), full 730-day/732-point daily grid (no shortening
## needed): every $CAPTURE'd output matched the original EXACTLY (max abs
## diff 0.0) in every scenario -- see bph_refactor_notes.md for detail.
## =============================================================================
...
EC50_TAMS = 0.8   // ng/mL, alpha1A-AR 50% occupancy (Foglar 1995)
EMAX_TAMS = 0.95  // maximum alpha1A occupancy fraction
GAMMA_TAMS = 1.0  // New -- original had no explicit Hill exponent (bare Emax*C/(EC50+C) ratio)
...
EC50_FINA = 0.3   // ng/mL, SRD5A2 IC50 (Gormley 1992, J Urol 147:1562)
EMAX_FINA = 1.0   // New -- original had no explicit Emax; its ratio C/(EC50+C) is Emax=1 implicitly
GAMMA_FINA = 1.0  // New -- original had no explicit Hill exponent
...
EC50_DUT = 0.05   // ng/mL, dual SRD5A1+2 (Bramson 1997, J Steroid Biochem 64:189)
EMAX_DUT = 1.0    // New -- original had no explicit Emax; its ratio C/(EC50+C) is Emax=1 implicitly
GAMMA_DUT = 1.0   // New -- original had no explicit Hill exponent
...
EC50_TAD = 2.0    // ng/mL, PDE5 IC50 (Gresser 2002, World J Urol 20:360)
EMAX_TAD = 1.0    // New -- original had no explicit Emax; its ratio C/(EC50+C) is Emax=1 implicitly
GAMMA_TAD = 1.0   // New -- original had no explicit Hill exponent
...
GUT_TAMS CENT_TAMS PERI_TAMS
// Finasteride 3-compartment PK
GUT_FINA CENT_FINA PERI_FINA
// Dutasteride 3-compartment PK
GUT_DUT CENT_DUT PERI_DUT
// Tadalafil 3-compartment PK
GUT_TAD CENT_TAD PERI_TAD
...
if(NEWIND <= 1) {
  ...
  TEST_P_0    = TEST0 * 50.0;   // nmol (amount)
  DHT_P_0     = DHT0 * 30.0;    // nmol (Vd ~30 L for DHT)
  DHT_PROST_0 = DHT_PROST0;     // nmol/L (tissue concentration)
  AR_ACT_0    = AR_ACT0;        // normalized

  // Structural/functional baseline
  PV_0        = PV0;
  CGMP_0      = CGMP0;
  ALPHA1_OCC_0 = 0.0;           // no drug → zero alpha1 blockade
  IPSS_0      = IPSS0;
  QMAX_0      = QMAX0;
  PVR_0       = PVR0;
  PSA_0       = PSA0;
...
// =========================================================================
// Dose is added to GUT_TAMS via event records (see R simulation code)
// GI absorption → central → peripheral
dxdt_GUT_TAMS = -KA_TAMS * GUT_TAMS;
// Central: input from gut (F already accounted for in dose), distribution, elimination
// Note: GUT_TAMS units = ug (dose); CENT_TAMS = ug; concentration = CENT_TAMS/V1_TAMS ng/mL
dxdt_CENT_TAMS   =  F_TAMS * KA_TAMS * GUT_TAMS
                - (CL_TAMS / V1_TAMS) * CENT_TAMS
                - (Q_TAMS  / V1_TAMS) * CENT_TAMS
                + (Q_TAMS  / V2_TAMS) * PERI_TAMS;
dxdt_PERI_TAMS   =  (Q_TAMS  / V1_TAMS) * CENT_TAMS
                - (Q_TAMS  / V2_TAMS) * PERI_TAMS;

// Plasma concentration (ng/mL = ug/L)
C_TAMS = (V1_TAMS > 0) ? CENT_TAMS / V1_TAMS : 0.0;

// Alpha1-AR occupancy from tamsulosin (Emax model)
EFFECT_TAMS = (USE_TAMS > 0.5) ?
                  EMAX_TAMS * pow(C_TAMS, GAMMA_TAMS) / (pow(EC50_TAMS, GAMMA_TAMS) + pow(C_TAMS, GAMMA_TAMS)) : 0.0;

// =========================================================================
// SECTION 2: FINASTERIDE PK (3-compartment oral, 5 mg QD)
// =========================================================================
dxdt_GUT_FINA = -KA_FINA * GUT_FINA;
dxdt_CENT_FINA   =  F_FINA * KA_FINA * GUT_FINA
                - (CL_FINA / V1_FINA) * CENT_FINA
                - (Q_FINA  / V1_FINA) * CENT_FINA
                + (Q_FINA  / V2_FINA) * PERI_FINA;
dxdt_PERI_FINA   =  (Q_FINA  / V1_FINA) * CENT_FINA
                - (Q_FINA  / V2_FINA) * PERI_FINA;

C_FINA = (V1_FINA > 0) ? CENT_FINA / V1_FINA : 0.0;  // ng/mL
...
EFFECT_FINA = (USE_FINA > 0.5) ?
                  EMAX_FINA * pow(C_FINA, GAMMA_FINA) / (pow(EC50_FINA, GAMMA_FINA) + pow(C_FINA, GAMMA_FINA)) : 0.0;

// =========================================================================
// SECTION 3: DUTASTERIDE PK (3-compartment oral, 0.5 mg QD)
// =========================================================================
dxdt_GUT_DUT = -KA_DUT * GUT_DUT;
dxdt_CENT_DUT   =  F_DUT * KA_DUT * GUT_DUT
               - (CL_DUT / V1_DUT) * CENT_DUT
               - (Q_DUT  / V1_DUT) * CENT_DUT
               + (Q_DUT  / V2_DUT) * PERI_DUT;
dxdt_PERI_DUT   =  (Q_DUT  / V1_DUT) * CENT_DUT
               - (Q_DUT  / V2_DUT) * PERI_DUT;

C_DUT = (V1_DUT > 0) ? CENT_DUT / V1_DUT : 0.0;  // ng/mL
...
EFFECT_DUT = (USE_DUT > 0.5) ?
                 EMAX_DUT * pow(C_DUT, GAMMA_DUT) / (pow(EC50_DUT, GAMMA_DUT) + pow(C_DUT, GAMMA_DUT)) : 0.0;
...
double inh_SRD5A2 = EFFECT_FINA * (1.0 - EFFECT_DUT) + EFFECT_DUT;  // combined type 2 inhibition
double inh_SRD5A1 = EFFECT_DUT;                                   // type 1 inhibition (dutasteride only)
double total_5AR_inh = f2_fraction * inh_SRD5A2 + f1_fraction * inh_SRD5A1;

// =========================================================================
// SECTION 4: TADALAFIL PK (3-compartment oral, 5 mg QD)
// =========================================================================
dxdt_GUT_TAD = -KA_TAD * GUT_TAD;
dxdt_CENT_TAD   =  F_TAD * KA_TAD * GUT_TAD
               - (CL_TAD / V1_TAD) * CENT_TAD
               - (Q_TAD  / V1_TAD) * CENT_TAD
               + (Q_TAD  / V2_TAD) * PERI_TAD;
dxdt_PERI_TAD   =  (Q_TAD  / V1_TAD) * CENT_TAD
               - (Q_TAD  / V2_TAD) * PERI_TAD;

C_TAD = (V1_TAD > 0) ? CENT_TAD / V1_TAD : 0.0;  // ng/mL

// PDE5 inhibition fraction by tadalafil
EFFECT_TAD = (USE_TAD > 0.5) ?
                  EMAX_TAD * pow(C_TAD, GAMMA_TAD) / (pow(EC50_TAD, GAMMA_TAD) + pow(C_TAD, GAMMA_TAD)) : 0.0;
...
double pde5_activity = 1.0 - EFFECT_TAD;  // fraction of PDE5 remaining active
...
dxdt_ALPHA1_OCC = k_alpha1_eq * (EFFECT_TAMS - ALPHA1_OCC);
...
$TABLE
// Drug concentrations (ng/mL) -- recomputed from state, same convention this
// file already used before the refactor (each $TABLE quantity is derived
// fresh from compartment state rather than re-using an $ODE-local variable)
double C_TAMS_tbl = CENT_TAMS / V1_TAMS;
double C_FINA_tbl = CENT_FINA / V1_FINA;
...
double EFFECT_FINA_tbl = (USE_FINA > 0.5) ?
    EMAX_FINA * pow(C_FINA_tbl, GAMMA_FINA) / (pow(EC50_FINA, GAMMA_FINA) + pow(C_FINA_tbl, GAMMA_FINA)) : 0.0;
double EFFECT_DUT_tbl  = (USE_DUT > 0.5) ?
    EMAX_DUT  * pow(C_DUT_tbl,  GAMMA_DUT)  / (pow(EC50_DUT,  GAMMA_DUT)  + pow(C_DUT_tbl,  GAMMA_DUT))  : 0.0;
double EFFECT_TAD_tbl  = (USE_TAD > 0.5) ?
    EMAX_TAD  * pow(C_TAD_tbl,  GAMMA_TAD)  / (pow(EC50_TAD,  GAMMA_TAD)  + pow(C_TAD_tbl,  GAMMA_TAD))  : 0.0;

// DHT inhibition percentage (vs baseline)
double DHT_P_conc = DHT_P / 30.0;   // nmol/L
double DHT_inhibition_pct_calc = (DHT_P_conc < DHT0) ?
                              (1.0 - DHT_P_conc / DHT0) * 100.0 : 0.0;

// Prostatic DHT inhibition
double DHT_PROST_inh_pct_calc = (DHT_PROST < DHT_PROST0) ?
                             (1.0 - DHT_PROST / DHT_PROST0) * 100.0 : 0.0;

// Prostate volume change (cc and %)
double PV_change_cc  = PV - PV0;
double PV_change_pct_calc = PV_change_cc / PV0 * 100.0;

// IPSS change from baseline
double IPSS_change_calc = IPSS - IPSS0;

// Qmax change from baseline
double QMAX_change_calc = QMAX - QMAX0;

// PVR change
double PVR_change_calc = PVR - PVR0;

// PSA change %
double PSA_change_pct_calc = (PSA - PSA0) / PSA0 * 100.0;

// Alpha1-AR blockade %
double Alpha1_block_pct_calc = ALPHA1_OCC * 100.0;
...
capture EFFECT_TAD  = EFFECT_TAD_tbl;
capture DHT_inhibition_pct = DHT_inhibition_pct_calc;
capture DHT_PROST_inh_pct = DHT_PROST_inh_pct_calc;
capture PV_change_pct = PV_change_pct_calc;
capture PV_cc                = PV;
capture IPSS_score           = IPSS;
capture IPSS_change = IPSS_change_calc;
capture QMAX_mL_s            = QMAX;
capture QMAX_change = QMAX_change_calc;
capture PVR_mL               = PVR;
capture PVR_change = PVR_change_calc;
capture PSA_ngmL             = PSA;
capture PSA_change_pct = PSA_change_pct_calc;
capture Alpha1_block_pct = Alpha1_block_pct_calc;
...
scenario_list <- list(
  ...
  list(
    ...
    events = make_ev(0.4, sim_duration_days, "GUT_TAMS")
  ),
  ...
  list(
    ...
    events = make_ev(5, sim_duration_days, "GUT_FINA")
  ),
  ...
  list(
    ...
    events = make_ev(0.5, sim_duration_days, "GUT_DUT")
  ),
  ...
  list(
    ...
    events = ev_c(
      make_ev(0.5, sim_duration_days, "GUT_DUT"),
      make_ev(0.4, sim_duration_days, "GUT_TAMS")
    )
  ),
  ...
  list(
    ...
    events = make_ev(5, sim_duration_days, "GUT_TAD")
  )
)
...
out_pk_tams <- mrgsim(
  mod_pk_tams,
  events  = make_ev(0.4, pk_sim_days + 1, "GUT_TAMS"),
...
out_pk_tad <- mrgsim(
  mod_pk_tad,
  events  = make_ev(5, pk_sim_days + 1, "GUT_TAD"),
  end     = (pk_sim_days + 1) * 24,
  delta   = 0.5,
  obsonly = TRUE
)
...
p6_pk <- ggplot() +
  geom_line(data = day14_tams,
            aes(x = time_h_ss, y = C_TAMS, color = "Tamsulosin 0.4mg"),
            linewidth = 1.2) +
  geom_line(data = day14_tad,
            aes(x = time_h_ss, y = C_TAD, color = "Tadalafil 5mg"),
            linewidth = 1.2) +
