...
The list of literature used to build the mechanistic map of anthracycline cardiotoxicity (`aic_qsp_model_en.dot`), the mrgsolve ODE model
(`aic_mrgsolve_model_en.R`), the Shiny dashboard (`aic_shiny_app_en.R`) and the independent numerical
...
> different paper.
>
> **Important**: every figure marked "model:" in `README_en.md` and in the model files is a
...
> set out in the CALIBRATION ANCHORS section at the top of `aic_mrgsolve_model_en.R`.
