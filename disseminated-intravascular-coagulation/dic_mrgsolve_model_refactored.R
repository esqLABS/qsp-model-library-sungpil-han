...
##  dic_refactor_notes.md): only two compounds' PK/PD blocks were renamed to
##  the fork's pluggable-PK convention -- ATRA (stem ATR: KAATR/VATR/KELATR/
##  EC50ATRA/EMAXATRA/ATRAD/ATRAC/ATRAE -> KA_ATR/V1_ATR/CL_ATR/EC50_ATR/
##  EMAX_ATR/GAMMA_ATR/GUT_ATR/CENT_ATR/C_ATR/EFFECT_ATR) and tranexamic acid
##  (stem TXA: KELTX/VTX/IC50TXA/TXAC/CTXA/TXAI -> CL_TXA/V1_TXA/EC50_TXA/
##  EMAX_TXA/GAMMA_TXA/CENT_TXA/C_TXA/EFFECT_TXA). Every other compound
##  (heparin, enoxaparin, thrombomodulin alfa, drotrecogin alfa, argatroban)
##  and every disease-side equation is byte-identical to dic_mrgsolve_model.R.
## =====================================================================
...
EC50_ATR : 0.05    : ATRA EC50 for differentiation (mg/L), was EC50ATRA
EMAX_ATR : 1.0     : ATRA maximal differentiation effect (-), was EMAXATRA
GAMMA_ATR: 1        : ATRA Hill coefficient (-), new -- original had no explicit Hill exponent
...
CL_TXA   : 6.96    : tranexamic acid clearance (L/h), = KELTX x VTX
V1_TXA   : 12.0    : tranexamic acid volume of distribution (L)
...
CL_ATR   : 50.0    : ATRA clearance (L/h), = KELATR x VATR
V1_ATR   : 100     : ATRA volume of distribution (L)
...
CENT_TXA : 0 : tranexamic acid central amount (mg), was TXAC
GUT_ATR  : 0 : ATRA gut depot (mg), was ATRAD
CENT_ATR : 0 : ATRA central amount (mg), was ATRAC
...
// ATRA (ATR) exposed concentration + Hill effect -- fork PK/PD refactor.
// Rename only: EMAXATRA*C/(EC50ATRA+C) was already the canonical Hill ratio
// with an implicit exponent of 1, so GAMMA_ATR = 1 reproduces it exactly.
double C_ATR      = CENT_ATR / V1_ATR;
double EFFECT_ATR = EMAX_ATR * pow(C_ATR, GAMMA_ATR)
                   / (pow(EC50_ATR, GAMMA_ATR) + pow(C_ATR, GAMMA_ATR));
dxdt_PATH  = -KCLRP * SRCCTL * PATHv * (1.0 - 0.5 * OLIV);
dxdt_BLAST =  KGB * BLASTv * (1.0 - BLASTv / BMAX) - KDIFF * EFFECT_ATR * BLASTv;
...
double C_TXA = CENT_TXA / V1_TXA;                // ug/mL -- was CTXA = TXAC/VTX
...
double FGNLYS = KFGN * PLNv * (1.0 - A2v) * (1.0 - A2v) * (1.0 - EFFECT_TXA);
...
double vPLN  = (KPG * FREETPA * pos(PLG) * FSURF
             +  KUPA * BLASTv / (KBL50 + BLASTv) * pos(PLG)) * (1.0 - EFFECT_TXA);
...
dxdt_CENT_TXA = RATE_TXA - CL_TXA / V1_TXA * CENT_TXA;         // was dxdt_TXAC = RATE_TXA - KELTX*TXAC
dxdt_GUT_ATR  = RATE_ATRA - KA_ATR * GUT_ATR;                  // was dxdt_ATRAD = RATE_ATRA - KAATR*ATRAD
dxdt_CENT_ATR = KA_ATR * GUT_ATR - CL_ATR / V1_ATR * CENT_ATR; // was dxdt_ATRAC = KAATR*ATRAD - KELATR*ATRAC
...
// ---- fork PK/PD refactor: discoverability captures for C_<STEM>/EFFECT_<STEM> ----
// Additive only -- no counterpart in the original, excluded from the verification
// diff. Recomputed from compartment state under o-prefixed locals because mrgsolve
// hoists every $ODE `double` into the same anonymous-namespace scope as $TABLE
// `capture`s, so reusing the bare C_ATR/EFFECT_ATR/C_TXA/EFFECT_TXA names here
// would collide with their $ODE declarations (same class of error as FREETPA
// above; see dic_refactor_notes.md and UPSTREAM_ISSUES.md).
double oC_ATR      = CENT_ATR / V1_ATR;
double oEFFECT_ATR = EMAX_ATR * pow(oC_ATR, GAMMA_ATR)
                    / (pow(EC50_ATR, GAMMA_ATR) + pow(oC_ATR, GAMMA_ATR));
capture ATR_conc   = oC_ATR;
capture ATR_effect = oEFFECT_ATR;
double oC_TXA      = CENT_TXA / V1_TXA;
double oEFFECT_TXA = EMAX_TXA * pow(oC_TXA, GAMMA_TXA)
                    / (pow(EC50_TXA, GAMMA_TXA) + pow(oC_TXA, GAMMA_TXA));
capture TXA_conc   = oC_TXA;
capture TXA_effect = oEFFECT_TXA;
'
...
SCEN <- list(
  ...
  L_TXA_sepsis = list(aet = "sepsis",
    ev  = bolus("CENT_TXA", 1000, 0),          # was "TXAC"
