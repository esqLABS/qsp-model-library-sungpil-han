## =============================================================================
##  ros_mrgsolve_model_refactored.R
...
##  ros_refactor_notes.md for full detail). Only ISOTRETINOIN (stem ISO,
##  the compound this file's row in
##  driver-patches/data/compound_perturbation_census.md classifies as
##  "Redirect concentration (clean single site)") is in scope. Every other
##  compound modelled here (ivermectin IVM, metronidazole MTZ, azelaic acid
##  AZA, brimonidine BRM, oxymetazoline OXY, minocycline MIN, doxycycline
##  DOX) is completely untouched -- same names, same parameters, same ODEs.
##
##    - Archetype 3 minus peripheral (depot + central, linear elimination,
##      no bioavailability term in the original): GUT_ISO/CENT_ISO renamed
##      from ISOG/ISOP, KA_ISO/V1_ISO/CL_ISO renamed from KAISO/VISO/CLISO.
##    - C_ISO = CENT_ISO / V1_ISO is the ONE exposed concentration read by
##      both of isotretinoin's disease-facing effects (this compound
##      genuinely has two independent mechanisms in the original: sebum
##      suppression and innate/TLR2-KLK5 damping -- kept as two separate
##      named EFFECT_ISO_* terms per compound, not collapsed into one,
##      since the guide's "not buried inside a combined multi-drug
##      expression" concern is about mixing DIFFERENT drugs, not a single
##      drug's own distinct mechanisms; c.f. doxycycline's own five FDOX*
##      terms elsewhere in this same file).
##    - EFFECT_ISO_SEB = EMAX_ISO_SEB*C_ISO^GAMMA/(EC50_ISO_SEB^GAMMA+
##      C_ISO^GAMMA), GAMMA_ISO_SEB=1: a rename of the original's
##      ISOEMAX*CISO/(ISOEC50+CISO), already a plain Hill(gamma=1) ratio.
##    - EFFECT_ISO_INN = EMAX_ISO_INN*C_ISO^GAMMA/(EC50_ISO_INN^GAMMA+
##      C_ISO^GAMMA), EMAX_ISO_INN=1 (new, but not a free choice -- forced
##      by the original's own algebra), GAMMA_ISO_INN=1: downstream code
##      uses FISOA = 1 - EFFECT_ISO_INN, algebraically identical to the
##      original's FISOA = 1/(1+CISO/ISOAIC50). Rename, not a refit.
##    - C_ISO/EFFECT_ISO_SEB/EFFECT_ISO_INN are predeclared as `double`s in
##      $GLOBAL (not $PARAM): mrgsolve 2.0.1 compiles $PARAM members as
##      read-only references inside $ODE, so a value recomputed every
##      timestep from state cannot also live in $PARAM (same constraint
##      already documented in several sibling refactors, e.g.
##      copd/copd_mrgsolve_model_refactored.R, breast-cancer/
##      bc_refactor_notes.md). Exposed via $CAPTURE instead, so all three
##      are discoverable in /model_manifest's outputPaths and in every
##      simulation's output columns.
##    - The original compiled cleanly under mrgsolve 2.0.1 as-is (confirmed
##      via qspserver /model_manifest) -- no build-compatibility fix was
##      needed, and none is logged to UPSTREAM_ISSUES.md for this file.
##
##  Verified against the original via the qspserver mrgsolve_api service --
##  see ros_refactor_notes.md for the full result.
## -----------------------------------------------------------------------------
##  THE MODELLING IDEA
## -----------------------------------------------------------------------------
...
// ---- PK/PD refactor (isotretinoin, stem ISO) -----------------------------
//  C_<STEM>/EFFECT_<STEM> must be recomputed every timestep from state, so
//  they cannot live in $PARAM: mrgsolve 2.0.1 compiles $PARAM members as
//  read-only references inside $ODE, and reassigning one there fails with
//  "assignment of read-only reference" (same constraint documented in
...
KA_ISO  :  2.00 : isotretinoin absorption (1/day) (renamed from KAISO)
V1_ISO  : 40.0  : isotretinoin apparent volume (L) (renamed from VISO)
CL_ISO  : 33.0  : isotretinoin apparent clearance (L/day) (renamed from CLISO)
EC50_ISO_SEB : 0.30 : isotretinoin sebosuppression EC50, mg/L (renamed from ISOEC50)
...
GUT_ISO  : 0 : isotretinoin gut depot (mg) (renamed from ISOG)
CENT_ISO : 0 : isotretinoin central amount (mg) (renamed from ISOP)
// ---- device signals -----------------------------------------------------
...
C_ISO = CENT_ISO / V1_ISO;                  // mg/L (pluggable-PK exposed concentration)
...
double FISOA = 1.0 - EFFECT_ISO_INN;                  // retinoid -> innate arm
...
// ===========================================================================
// EFFECT_ISO_SEB = EMAX_ISO_SEB*C_ISO^GAMMA/(EC50_ISO_SEB^GAMMA+C_ISO^GAMMA);
// GAMMA_ISO_SEB=1 makes this the exact original ISOEMAX*CISO/(ISOEC50+CISO)
// ratio -- a rename, not a refit (see ros_refactor_notes.md).
EFFECT_ISO_SEB = EMAX_ISO_SEB * pow(C_ISO, GAMMA_ISO_SEB)
                 / (pow(EC50_ISO_SEB, GAMMA_ISO_SEB) + pow(C_ISO, GAMMA_ISO_SEB));
double SEBT = SEB0 * (1.0 + ASEBAND * (ANDROG - 1.0));
dxdt_SEB = KSEB * (SEBT * (1.0 - EFFECT_ISO_SEB) - SEB);
...
double CISOO  = C_ISO;                              // unchanged output name, now an alias of C_ISO
...
C_ISO  : isotretinoin plasma concentration, pluggable-PK alias of CISOO (mg/L)
EFFECT_ISO_SEB : isotretinoin sebosuppression effect (0-1)
EFFECT_ISO_INN : isotretinoin innate (TLR2/KLK5) damping effect (0-1)
'
...
dose_isotretinoin <- function(mg = 20, start = 0, weeks = 20) {
  ev(amt = mg, cmt = "GUT_ISO", time = start, ii = 1,     # renamed from ISOG
     addl = round(weeks * 7) - 1)
}
