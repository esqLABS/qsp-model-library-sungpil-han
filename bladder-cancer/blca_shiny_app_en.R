...
ui <- dashboardPage(
  skin = "blue",
  dashboardHeader(title = "Bladder Cancer QSP Dashboard (BLCA)"),

  dashboardSidebar(
    sidebarMenu(
      menuItem("Patient profile", tabName = "patient", icon = icon("user")),
      menuItem("Pharmacokinetics (PK)",   tabName = "pk",      icon = icon("flask")),
      menuItem("Immune biomarkers", tabName = "immune", icon = icon("shield-alt")),
      menuItem("Tumour dynamics",     tabName = "tumor",   icon = icon("viruses")),
  ...
  dashboardBody(
    tabItems(
      ...
      tabItem("patient",
        fluidRow(
          box(title="Patient settings", status="primary", solidHeader=TRUE, width=4,
            sliderInput("age", "Age (years)", 40, 85, 65),
            radioButtons("sex", "Sex", c("Male"="M","Female"="F"), inline=TRUE),
            sliderInput("wt", "Weight (kg)", 40, 120, 70),
            radioButtons("stage", "Stage", c("NMIBC"="NMIBC","MIBC/metastatic"="MIBC"), inline=TRUE),
            checkboxInput("fgfr3", "FGFR3 mutation positive (erdafitinib candidate)", FALSE),
            checkboxInput("pdl1", "PD-L1 CPS ≥ 10 (pembrolizumab preferred)", FALSE),
            selectInput("drug_main", "Treatment selection", drug_choices),
            sliderInput("dur", "Simulation period (days)", 90, 730, 365),
            actionButton("run", "Run simulation", class="btn-success btn-block")
          ),
          box(title="Patient summary", status="info", solidHeader=TRUE, width=8,
      ...
      tabItem("pk",
        fluidRow(
          box(title="Drug plasma concentration", status="primary", solidHeader=TRUE, width=12,
            plotlyOutput("pk_conc", height="350px"))
        ),
        fluidRow(
          box(title="Receptor occupancy (IO)", status="warning", solidHeader=TRUE, width=6,
            plotlyOutput("pk_ro", height="280px")),
          box(title="BCG bladder-wall effect concentration", status="danger", solidHeader=TRUE, width=6,
      ...
      tabItem("immune",
        fluidRow(
          box(title="CD8+ cytotoxic T cells (CTL)", status="success", solidHeader=TRUE, width=6,
            plotlyOutput("imm_cd8", height="280px")),
          box(title="Regulatory T cells (Treg) & MDSC", status="danger", solidHeader=TRUE, width=6,
            plotlyOutput("imm_treg_mdsc", height="280px"))
        ),
        fluidRow(
          box(title="IFN-γ dynamics", status="info", solidHeader=TRUE, width=6,
            plotlyOutput("imm_ifng", height="280px")),
          box(title="PD-L1 expression level", status="warning", solidHeader=TRUE, width=6,
      ...
      tabItem("tumor",
        fluidRow(
          box(title="Tumour burden (log scale)", status="primary", solidHeader=TRUE, width=6,
            plotlyOutput("tum_burden", height="300px")),
          box(title="Tumour size change — SLD (mm)", status="warning", solidHeader=TRUE, width=6,
            plotlyOutput("tum_sld", height="300px"))
        ),
        fluidRow(
          box(title="Tumour reduction (%)", status="success", solidHeader=TRUE, width=6,
            plotlyOutput("tum_red", height="280px")),
          box(title="FGFR3 signalling activity", status="info", solidHeader=TRUE, width=6,
      ...
      tabItem("endpoints",
        fluidRow(
          box(title="SLD change — waterfall plot (final)", status="primary", solidHeader=TRUE, width=6,
            plotlyOutput("ep_waterfall", height="320px")),
          box(title="Clinical outcome summary table", status="info", solidHeader=TRUE, width=6,
            DTOutput("ep_table"))
        ),
        fluidRow(
          box(title="RECIST-based tumour response", status="success", solidHeader=TRUE, width=12,
      ...
      tabItem("compare",
        fluidRow(
          box(title="Select comparison scenarios", status="primary", solidHeader=TRUE, width=3,
            checkboxGroupInput("drugs_comp", "Treatment selection (multiple allowed)",
              choices=drug_choices, selected=c("Untreated","BCG","Pembrolizumab")),
            checkboxInput("fgfr3_comp", "FGFR3 mutation positive", FALSE),
            actionButton("run_comp", "Run scenario comparison", class="btn-warning btn-block")
          ),
          box(title="Tumour reduction comparison", status="info", solidHeader=TRUE, width=9,
            plotlyOutput("comp_tumor", height="380px"))
        ),
        fluidRow(
          box(title="CD8+ CTL comparison", status="success", solidHeader=TRUE, width=6,
            plotlyOutput("comp_cd8", height="280px")),
          box(title="NMP22 urine marker comparison", status="warning", solidHeader=TRUE, width=6,
      ...
      tabItem("biomarkers",
        fluidRow(
          box(title="NMP22 urine biomarker", status="primary", solidHeader=TRUE, width=6,
            plotlyOutput("bio_nmp22", height="280px")),
          box(title="PD-L1 expression vs IFN-γ", status="warning", solidHeader=TRUE, width=6,
            plotlyOutput("bio_pdl1_ifng", height="280px"))
        ),
        fluidRow(
          box(title="Biomarker summary table", status="info", solidHeader=TRUE, width=12,
            DTOutput("bio_table"))
        )
      )
...
server <- function(input, output, session) {
  ...
  output$patient_summary <- renderDT({
    data.frame(
      Item=c("Age","Sex","Weight","Stage","FGFR3 mutation","PD-L1 CPS","Selected treatment","Simulation period"),
      Value=c(paste0(input$age," years"), input$sex, paste0(input$wt,"kg"),
           input$stage, ifelse(input$fgfr3,"Positive (+)","Negative (-)"),
           ifelse(input$pdl1,"CPS≥10","CPS<10"),
           input$drug_main, paste0(input$dur," days"))
  ...
  output$pk_conc <- renderPlotly({
    ...
    if (d=="Pembrolizumab") {
      p <- ggplot(df, aes(time_d, pc_ug)) + geom_line(color="#4DAF4A",size=1) +
        labs(x="Time (days)", y="Concentration (μg/mL)", title="Pembrolizumab plasma concentration")
    } else if (d=="Atezolizumab") {
      p <- ggplot(df, aes(time_d, ac_ug)) + geom_line(color="#984EA3",size=1) +
        labs(x="Time (days)", y="Concentration (μg/mL)", title="Atezolizumab plasma concentration")
    } else if (d=="Erdafitinib") {
      p <- ggplot(df, aes(time_d, Erda_plasm)) + geom_line(color="#FF7F00",size=1) +
        labs(x="Time (days)", y="Concentration (μg/mL)", title="Erdafitinib plasma concentration")
    } else if (d %in% c("GC (Cis+Gem)")) {
      p <- ggplot(df) +
        geom_line(aes(time_d, Cis_plasm, color="Cisplatin"), size=1) +
        geom_line(aes(time_d, Gem_plasm, color="Gemcitabine"), size=1) +
        scale_color_manual(values=c("Cisplatin"="#377EB8","Gemcitabine"="#E41A1C")) +
        labs(x="Time (days)", y="Concentration (μg/mL)", title="GC chemotherapy plasma concentration", color="")
    } else if (d=="BCG") {
      p <- ggplot(df, aes(time_d, BCG_eff)) + geom_line(color="#E41A1C",size=1) +
        labs(x="Time (days)", y="BCG effect concentration (a.u.)", title="BCG bladder-wall effect concentration")
    } else if (d=="Enfortumab Vedotin") {
      p <- ggplot(df, aes(time_d, EnFV_c)) + geom_line(color="#A65628",size=1) +
        labs(x="Time (days)", y="Concentration (μg/mL)", title="Enfortumab vedotin plasma concentration")
    } else {
      p <- ggplot(df, aes(time_d, TumBurd)) + geom_line(color="#999999",size=1) +
        labs(x="Time (days)", y="Tumour cell count", title="Untreated — tumour cell count")
    }
    ggplotly(p + theme_bw())
  })
  ...
  output$pk_ro <- renderPlotly({
    ...
    p <- ggplot(df) +
      ...
      labs(x="Time (days)", y="Receptor occupancy (0–1)", color="") + theme_bw()
    ggplotly(p)
  })
  ...
  output$pk_bcg <- renderPlotly({
    df <- sim_data()
    p <- ggplot(df, aes(time_d, BCG_eff)) + geom_line(color="#E41A1C",size=1) +
      labs(x="Time (days)", y="BCG bladder-wall concentration (a.u.)") + theme_bw()
    ggplotly(p)
  })
  ...
  output$imm_cd8 <- renderPlotly({
    ...
    p <- ggplot(df, aes(time_d, CD8_eff)) + geom_line(color="#4DAF4A",size=1) +
      labs(x="Time (days)", y="CD8+ CTL (relative units)") + theme_bw()
    ggplotly(p)
  })
  ...
  output$imm_treg_mdsc <- renderPlotly({
    ...
    p <- ggplot(df) +
      ...
      labs(x="Time (days)", y="Cell count (relative units)", color="") + theme_bw()
    ggplotly(p)
  })
  ...
  output$imm_ifng <- renderPlotly({
    ...
    p <- ggplot(df, aes(time_d, IFNg_cmt)) + geom_line(color="#9C27B0",size=1) +
      labs(x="Time (days)", y="IFN-γ concentration (relative units)") + theme_bw()
    ggplotly(p)
  })
  ...
  output$imm_pdl1 <- renderPlotly({
    ...
    p <- ggplot(df, aes(time_d, PDL1_lvl)) + geom_line(color="#FF7F00",size=1) +
      labs(x="Time (days)", y="PD-L1 expression level (relative units)") + theme_bw()
    ggplotly(p)
  })
  ...
  output$tum_burden <- renderPlotly({
    ...
    p <- ggplot(df, aes(time_d, log10(TumBurd+1))) + geom_line(color="#E41A1C",size=1) +
      labs(x="Time (days)", y="log₁₀ tumour cell count") + theme_bw()
    ggplotly(p)
  })
  ...
  output$tum_sld <- renderPlotly({
    ...
    p <- ggplot(df, aes(time_d, SLD_cmt)) + geom_line(color="#377EB8",size=1) +
      labs(x="Time (days)", y="SLD (mm)") + theme_bw()
    ggplotly(p)
  })
  ...
  output$tum_red <- renderPlotly({
    ...
    p <- ggplot(df, aes(time_d, TumorRed)) + geom_line(color="#4DAF4A",size=1) +
      ...
      labs(x="Time (days)", y="Tumour reduction (%)") + theme_bw()
    ggplotly(p)
  })
  ...
  output$tum_fgfr3 <- renderPlotly({
    ...
    p <- ggplot(df, aes(time_d, FGFR3act)) + geom_line(color="#FF7F00",size=1) +
      labs(x="Time (days)", y="FGFR3 signalling activity (a.u.)") + theme_bw()
    ggplotly(p)
  })
  ...
  output$ep_waterfall <- renderPlotly({
    ...
    p <- ggplot(bar_df, aes(x=Drug, y=SLD_chg, fill=SLD_chg>0)) +
      ...
      labs(x="", y="SLD change (%)", title="Waterfall plot (RECIST baseline)") +
      theme_bw()
    ggplotly(p)
  })
  ...
  output$ep_table <- renderDT({
    ...
    data.frame(
      Metric=c("SLD change (%)","Tumour reduction (%)","CD8+ CTL (final)","NMP22 (final)","PD-1 RO (final)","PD-L1 RO (final)"),
      Value=round(c(final$SLD_chg, final$TumorRed, final$CD8_eff,
  ...
  output$ep_recist <- renderPlotly({
    ...
    recist_df <- df |>
      mutate(Response = case_when(
        SLD_chg <= -30 ~ "Partial response (PR)",
        SLD_chg >= 20  ~ "Progressive disease (PD)",
        TRUE           ~ "Stable disease (SD)"
      ))
    p <- ggplot(recist_df, aes(time_d, SLD_chg, color=Response)) +
      geom_line(size=1) +
      scale_color_manual(values=c("Partial response (PR)"="#4DAF4A","Progressive disease (PD)"="#E41A1C","Stable disease (SD)"="#FF7F00")) +
      geom_hline(yintercept=c(20,-30), linetype="dashed", alpha=0.5) +
      labs(x="Time (days)", y="SLD change (%)", color="RECIST") + theme_bw()
    ggplotly(p)
  })
  ...
  output$comp_tumor <- renderPlotly({
    ...
    p <- ggplot(df, aes(time_d, TumorRed, color=Drug)) + geom_line(size=1) +
      ...
      labs(x="Time (days)", y="Tumour reduction (%)", color="Treatment") + theme_bw()
    ggplotly(p)
  })
  ...
  output$comp_cd8 <- renderPlotly({
    ...
    p <- ggplot(df, aes(time_d, CD8_eff, color=Drug)) + geom_line(size=1) +
      scale_color_manual(values=drug_colors) +
      labs(x="Time (days)", y="CD8+ CTL", color="Treatment") + theme_bw()
    ggplotly(p)
  })
  ...
  output$comp_nmp22 <- renderPlotly({
    ...
    p <- ggplot(df, aes(time_d, NMP22_cmt, color=Drug)) + geom_line(size=1) +
      scale_color_manual(values=drug_colors) +
      labs(x="Time (days)", y="NMP22 (relative units)", color="Treatment") + theme_bw()
    ggplotly(p)
  })
  ...
  output$bio_nmp22 <- renderPlotly({
    ...
    p <- ggplot(df, aes(time_d, NMP22_cmt)) + geom_line(color="#E41A1C",size=1) +
      labs(x="Time (days)", y="NMP22 (relative units)", title="Urine NMP22 dynamics") + theme_bw()
    ggplotly(p)
  })
  ...
  output$bio_pdl1_ifng <- renderPlotly({
    ...
    p <- ggplot(df, aes(IFNg_cmt, PDL1_lvl, color=time_d)) +
      ...
      scale_color_viridis_c(name="Time (days)") +
      labs(x="IFN-γ (a.u.)", y="PD-L1 expression (a.u.)", title="PD-L1 vs IFN-γ phase space") +
      theme_bw()
    ggplotly(p)
  })
  ...
  output$bio_table <- renderDT({
    ...
    colnames(bio_sum) <- c("Time (days)","NMP22","PD-L1","IFN-γ","FGFR3act")
