...
##  3. Calcium / PTH / Vitamin D Dynamics
##  4. Bone Remodeling & BMD
##  5. Scenario Comparison
##  6. Biomarker Dashboard
## ============================================================
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      ...
      menuItem("Ca/PTH/VitD Dynamics", tabName = "tab3", icon = icon("chart-line")),
      menuItem("Bone Remodeling & BMD", tabName = "tab4", icon = icon("bone")),
      menuItem("Scenario Comparison", tabName = "tab5", icon = icon("balance-scale")),
      menuItem("Biomarker Dashboard", tabName = "tab6", icon = icon("vials"))
    ),
    hr(),
    h5("Disease Settings", style = "padding-left:15px; color:#ccc"),
    selectInput("severity", "PHPT Severity",
      choices = c("Normal" = 0,
                  "Mild PHPT" = 1,
                  "Severe PHPT" = 2),
      selected = 1),
    numericInput("gfr0", "Baseline eGFR (mL/min)", value = 85, min = 10, max = 120, step = 5),
    numericInput("duration_yr", "Simulation duration (years)", value = 5, min = 1, max = 10, step = 1),
    hr(),
    h5("Treatment Settings", style = "padding-left:15px; color:#ccc"),
    numericInput("ptx_day", "Parathyroidectomy timing (day; 0=none)", value = 0, min = 0, max = 3650),
    numericInput("cin_dose", "Cinacalcet dose (mg/day; 0=none)", value = 0, min = 0, max = 180, step = 30),
    numericInput("deno_dose", "Denosumab dose (mg; 0=none)", value = 0, min = 0, max = 120, step = 60),
    numericInput("deno_int", "Denosumab dosing interval (days)", value = 180, min = 90, max = 360, step = 90),
    actionButton("run_btn", "Run simulation", class = "btn-primary", width = "100%")
  ),
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "tab1",
        fluidRow(
          box(title = "Primary Hyperparathyroidism (PHPT) Overview", width = 12,
              status = "primary", solidHeader = TRUE,
              tabsetPanel(
                tabPanel("Disease Overview",
                  h4("Pathophysiology"),
                  p("PHPT arises from autonomous PTH secretion due to parathyroid adenoma (85%), multiple
                    adenomas (3%), hyperplasia (12%), or carcinoma (1%). Excess PTH causes hypercalcaemia via
                    increased bone resorption↑, increased renal Ca reabsorption↑, and increased 1,25(OH)₂D synthesis↑."),
                  h4("Clinical Classification"),
                  tableOutput("classification_tbl"),
                  h4("NIH 2022 Surgical Indications"),
                  tableOutput("nih_criteria_tbl")
                ),
                tabPanel("CaSR-PTH Feedback",
                  plotlyOutput("casr_curve", height = "400px"),
                  p("Normal: rising serum Ca activates CaSR → suppresses PTH secretion.
                    PHPT: suppression is not possible because of autonomous secretion.")
                ),
      ...
      tabItem(tabName = "tab2",
        fluidRow(
          box(title = "Cinacalcet PK — Plasma Concentration Over Time", width = 6,
              status = "info", solidHeader = TRUE,
              plotlyOutput("cin_pk_plot", height = "320px")),
          box(title = "Denosumab PK — Plasma Concentration Over Time (TMDD)", width = 6,
              status = "success", solidHeader = TRUE,
              plotlyOutput("deno_pk_plot", height = "320px"))
        ),
        fluidRow(
          box(title = "Cinacalcet Dose-Response (EC50 Model)", width = 6,
              status = "info", solidHeader = TRUE,
      ...
      tabItem(tabName = "tab3",
        fluidRow(
          box(title = "iPTH Plasma Concentration", width = 6,
              status = "danger", solidHeader = TRUE,
              plotlyOutput("pth_plot", height = "300px")),
          box(title = "Serum Calcium (Total)", width = 6,
              status = "warning", solidHeader = TRUE,
              plotlyOutput("ca_plot", height = "300px"))
        ),
        fluidRow(
          box(title = "Serum Phosphate", width = 6,
              status = "primary", solidHeader = TRUE,
        ...
        fluidRow(
          box(title = "Urinary Calcium Excretion", width = 6,
              status = "warning", solidHeader = TRUE,
              plotlyOutput("ucal_plot", height = "300px")),
          box(title = "eGFR Over Time", width = 6,
              status = "primary", solidHeader = TRUE,
      ...
      tabItem(tabName = "tab4",
        fluidRow(
          box(title = "Lumbar Spine BMD", width = 6,
              status = "purple", solidHeader = TRUE,
              plotlyOutput("bmd_ls_plot", height = "300px")),
          box(title = "Femoral Neck BMD", width = 6,
              status = "purple", solidHeader = TRUE,
              plotlyOutput("bmd_fn_plot", height = "300px"))
        ),
        fluidRow(
          box(title = "OB/OC Ratio & Bone Remodeling Markers", width = 6,
              status = "danger", solidHeader = TRUE,
              plotlyOutput("oc_ob_plot", height = "300px")),
          box(title = "T-score Over Time (L1-L4)", width = 6,
              status = "warning", solidHeader = TRUE,
              plotlyOutput("tscore_plot", height = "300px"))
        ),
        fluidRow(
          box(title = "Serum CTX & P1NP (Bone Turnover Markers)", width = 12,
              status = "info", solidHeader = TRUE,
      ...
      tabItem(tabName = "tab5",
        fluidRow(
          box(title = "Scenario Selection (up to 4)", width = 4,
              status = "primary", solidHeader = TRUE,
              checkboxGroupInput("sc_select",
                label = "Select scenarios to compare",
                choices = c(
                  "Normal" = "sc0",
                  "Untreated PHPT (mild)" = "sc1",
                  "Untreated PHPT (severe)" = "sc2",
                  "Cinacalcet 60mg" = "sc3",
                  "Denosumab q6mo" = "sc4",
                  "Parathyroidectomy (day 90)" = "sc5",
                  "Cin + Deno combination" = "sc6"
                ),
                selected = c("sc1", "sc3", "sc5")
              ),
              actionButton("run_compare", "Run comparison", class = "btn-primary")
          ),
          box(title = "Year-1 Summary Table", width = 8,
              status = "info", solidHeader = TRUE,
              DTOutput("compare_tbl"))
        ),
        fluidRow(
          box(title = "PTH Comparison", width = 6, status = "danger", solidHeader = TRUE,
              plotlyOutput("cmp_pth", height = "280px")),
          box(title = "Serum Ca Comparison", width = 6, status = "warning", solidHeader = TRUE,
              plotlyOutput("cmp_ca", height = "280px"))
        ),
        fluidRow(
          box(title = "Lumbar Spine BMD Comparison", width = 6, status = "success", solidHeader = TRUE,
              plotlyOutput("cmp_bmd", height = "280px")),
          box(title = "Urinary Ca Comparison", width = 6, status = "info", solidHeader = TRUE,
      ...
      tabItem(tabName = "tab6",
        ...
        fluidRow(
          box(title = "Simulation Results — Full Biomarker Table", width = 12,
              status = "primary", solidHeader = TRUE,
              DTOutput("biomarker_tbl"))
        ),
        fluidRow(
          box(title = "Biomarker Time Series (Heatmap)", width = 12,
              status = "info", solidHeader = TRUE,
...
server <- function(input, output, session) {
  ...
  output$classification_tbl <- renderTable({
    data.frame(
      Classification = c("Asymptomatic PHPT", "Normocalcaemic PHPT", "Mild PHPT", "Severe PHPT", "Hypercalcaemic crisis"),
      `Serum Ca` = c("Within upper limit of normal", "Normal", "ULN + <1 mg/dL", "> ULN + 1 mg/dL", "> 14 mg/dL"),
      `Serum PTH` = c("↑", "↑↑", "↑", "↑↑↑", "↑↑↑"),
      Frequency = c("75-80%", "15%", "10%", "5%", "<1%"),
  ...
  output$nih_criteria_tbl <- renderTable({
    data.frame(
      `Surgical Indications (NIH 2022)` = c(
        "Serum Ca > upper limit of normal + 1 mg/dL (> 0.25 mmol/L)",
        "eGFR < 60 mL/min/1.73m²",
        "Urolithiasis or nephrocalcinosis",
        "Urinary Ca > 400 mg/day + high-risk stone profile",
        "DXA: T-score < -2.5 (lumbar spine/hip/radius)",
        "Vertebral fracture (radiograph/VFA)",
        "Age < 50 years",
        "Patient preference (surveillance not feasible)"
      ),
      stringsAsFactors = FALSE
    )
  })
  ...
  output$drug_summary_tbl <- renderTable({
    data.frame(
      Drug = c("Cinacalcet (Sensipar)", "Denosumab (Prolia)", "Alendronate (Fosamax)"),
      Mechanism = c("CaSR allosteric potentiator → PTH↓",
               "Anti-RANKL mAb → OC inhibition → BMD↑",
               "Farnesyl-PP synthase inhibition → OC apoptosis"),
      Dose = c("30-180 mg/day po", "60 mg SC q6mo", "70 mg/wk po"),
      MainEffects = c("Ca↓ ~0.5 mg/dL, PTH↓ 30-50%",
                  "CTX↓ 70-80%, BMD↑ 5%/yr",
                  "CTX↓ 50-70%, BMD stable"),
      stringsAsFactors = FALSE
    )
  })
  ...
  output$vbox_nih_ca <- renderValueBox({
    valueBox("> 1 mg/dL ULN", "Ca surgical criterion", icon = icon("tint"), color = "red")
  })
  output$vbox_nih_epi <- renderValueBox({
    valueBox("< 60 mL/min", "eGFR criterion", icon = icon("kidney"), color = "yellow")
  })
  output$vbox_nih_bone <- renderValueBox({
    valueBox("T < -2.5", "BMD criterion", icon = icon("bone"), color = "orange")
  })
  output$vbox_nih_age <- renderValueBox({
    valueBox("< 50 years", "Age criterion", icon = icon("user"), color = "blue")
  })
  ...
  output$cin_pk_plot <- renderPlotly({
    ...
    plot_ly(d, x = ~time, y = ~Cin_ng_mL, type = "scatter", mode = "lines",
      ...
      layout(title = "Cinacalcet Plasma Concentration",
  ...
  output$deno_pk_plot <- renderPlotly({
    ...
    plot_ly(d, x = ~time, y = ~Deno_ng_mL, type = "scatter", mode = "lines",
      ...
      layout(title = "Denosumab Plasma Concentration (TMDD)",
             xaxis = list(title = "Time (days)"),
             yaxis = list(title = "Concentration (ng/mL)"))
  })
  ...
  output$cin_ec50_plot <- renderPlotly({
    ...
    plot_ly(df_ec, x = ~conc, y = ~eff, type = "scatter", mode = "lines",
      ...
      layout(title = "Cinacalcet Emax Model (CaSR Set-point Shift)",
             xaxis = list(title = "Cinacalcet concentration (ng/mL)"),
  ...
  output$pk_params_tbl <- renderDT({
    df <- data.frame(
      ...
      Parameter = c("F (oral bioavailability)", "Vc (L)", "CL (L/day)", "t½",
                 "F (SC)", "Vd (L)", "t½",
                 "F (oral)", "Bone-binding t½"),
      Value = c("22% (fasting)", "55", "125", "6-8h",
             "62%", "3.1", "~28 days",
             "0.6%", "~10 years"),
      stringsAsFactors = FALSE
    )
  ...
  output$pth_plot <- renderPlotly({
    make_line_plot(sim_data(), "PTH_pg_mL", "iPTH Plasma Concentration", "PTH (pg/mL)",
      hlines = list(list(y = 15, label = "LLN 15", color = "gray"),
                    list(y = 65, label = "ULN 65", color = "orange")))
  })

  output$ca_plot <- renderPlotly({
    make_line_plot(sim_data(), "Ca_total", "Total Serum Calcium", "Ca (mM)",
      hlines = list(list(y = 2.2, label = "Lower limit of normal", color = "blue"),
                    list(y = 2.6, label = "Upper limit of normal", color = "red")),
      colors = "#FF9800")
  })

  output$po4_plot <- renderPlotly({
    make_line_plot(sim_data(), "PO4", "Serum Phosphate", "PO4 (mM)",
      hlines = list(list(y = 0.8, label = "Lower limit of normal", color = "blue"),
                    list(y = 1.5, label = "Upper limit of normal", color = "red")),
      colors = "#2196F3")
  })
  ...
  output$vitd_plot <- renderPlotly({
    ...
    plot_ly(d, x = ~time) %>%
      ...
      layout(title = "Vitamin D Metabolites", xaxis = list(title = "Time (days)"),
             yaxis = list(title = "Concentration"), legend = list(orientation = "h"))
  })

  output$ucal_plot <- renderPlotly({
    make_line_plot(sim_data(), "Ca_urine", "Urinary Calcium Excretion", "Urinary Ca (mg/day)",
      hlines = list(list(y = 300, label = "Hypercalciuria threshold", color = "red")),
      colors = "#9C27B0")
  })

  output$gfr_plot <- renderPlotly({
    make_line_plot(sim_data(), "GFR", "eGFR Over Time", "eGFR (mL/min/1.73m²)",
      hlines = list(list(y = 60, label = "CKD G2/G3 boundary", color = "orange"),
                    list(y = 30, label = "CKD G4 boundary", color = "red")),
      colors = "#00838F")
  })
  ...
  output$bmd_ls_plot <- renderPlotly({
    make_line_plot(sim_data(), "BMD_LS", "Lumbar Spine BMD (L1-L4)", "BMD (g/cm²)",
      hlines = list(list(y = 0.84, label = "Osteopenia threshold", color = "orange"),
                    list(y = 0.72, label = "Osteoporosis threshold", color = "red")),
      colors = "#7B1FA2")
  })

  output$bmd_fn_plot <- renderPlotly({
    make_line_plot(sim_data(), "BMD_FN", "Femoral Neck BMD", "BMD (g/cm²)",
      hlines = list(list(y = 0.63, label = "Osteopenia threshold", color = "orange"),
                    list(y = 0.53, label = "Osteoporosis threshold", color = "red")),
      colors = "#6A1B9A")
  })
  ...
  output$oc_ob_plot <- renderPlotly({
    d <- sim_data()
    plot_ly(d, x = ~time) %>%
      add_lines(y = ~OC, name = "OC (osteoclast)", line = list(color = "red")) %>%
      add_lines(y = ~OB, name = "OB (osteoblast)", line = list(color = "blue")) %>%
      add_lines(y = ~OC_OB_ratio, name = "OC/OB ratio", line = list(color = "purple", dash = "dash")) %>%
      layout(title = "OB/OC Cell Dynamics", xaxis = list(title = "Time (days)"),
             yaxis = list(title = "Relative cell count"), legend = list(orientation = "h"))
  })

  output$tscore_plot <- renderPlotly({
    d <- sim_data()
    plot_ly(d, x = ~time) %>%
      add_lines(y = ~T_score_LS, name = "Lumbar spine T-score", line = list(color = "#7B1FA2")) %>%
      add_lines(y = ~T_score_FN, name = "Femoral neck T-score", line = list(color = "#AB47BC")) %>%
      add_lines(y = rep(-1, nrow(d)), name = "Osteopenia threshold (-1)",
                line = list(dash = "dash", color = "orange")) %>%
      add_lines(y = rep(-2.5, nrow(d)), name = "Osteoporosis threshold (-2.5)",
                line = list(dash = "dash", color = "red")) %>%
      layout(title = "T-score Over Time", xaxis = list(title = "Time (days)"),
  ...
  output$btm_plot <- renderPlotly({
    d <- sim_data()
    plot_ly(d, x = ~time) %>%
      add_lines(y = ~CTX_index, name = "CTX index (ng/L)", line = list(color = "red")) %>%
      add_lines(y = ~P1NP_index, name = "P1NP index (µg/L)", line = list(color = "green")) %>%
      layout(title = "Bone Turnover Marker Trends",
             xaxis = list(title = "Time (days)"),
             yaxis = list(title = "Marker level"),
             legend = list(orientation = "h"))
  })
  ...
  sc_labels <- c(
    sc0 = "Normal", sc1 = "Untreated (mild)", sc2 = "Untreated (severe)",
  ...
  output$compare_tbl <- renderDT({
    ...
    tbl <- d %>%
      ...
      summarise(
        ...
        `Urinary Ca (mg/d)` = round(mean(Ca_urine), 0),
  ...
  output$cmp_pth <- renderPlotly({
    ...
    plot_ly(d, x = ~time, y = ~PTH_pg_mL, color = ~scenario,
      ...
      layout(title = "PTH Comparison", xaxis = list(title = "Day"),
  ...
  output$cmp_ca <- renderPlotly({
    ...
    plot_ly(d, x = ~time, y = ~Ca_total, color = ~scenario,
            type = "scatter", mode = "lines") %>%
      layout(title = "Serum Ca Comparison", xaxis = list(title = "Day"),
  ...
  output$cmp_bmd <- renderPlotly({
    ...
    plot_ly(d, x = ~time, y = ~BMD_LS, color = ~scenario,
            type = "scatter", mode = "lines") %>%
      layout(title = "Lumbar Spine BMD Comparison", xaxis = list(title = "Day"),
  ...
  output$cmp_ucal <- renderPlotly({
    ...
    plot_ly(d, x = ~time, y = ~Ca_urine, color = ~scenario,
            type = "scatter", mode = "lines") %>%
      layout(title = "Urinary Ca Comparison", xaxis = list(title = "Day"),
  ...
  output$vb_pth_now <- renderValueBox({
    ...
    valueBox(paste0(val, " pg/mL"), "Current iPTH", icon = icon("syringe"), color = color)
  })
  ...
  output$vb_ca_now <- renderValueBox({
    ...
    valueBox(paste0(val, " mM"), "Total Serum Ca", icon = icon("tint"), color = color)
  })
  ...
  output$vb_bmd_now <- renderValueBox({
    ...
    valueBox(paste0(val, " (T=", ts, ")"), "Lumbar Spine BMD", icon = icon("bone"), color = color)
  })
  ...
  output$heatmap_plot <- renderPlotly({
    ...
    plot_ly(d_long, x = ~time, y = ~Marker, z = ~ZScore, type = "heatmap",
      ...
      layout(title = "Biomarker Z-Score Heatmap",
             xaxis = list(title = "Time (days)"),
             yaxis = list(title = ""))
  })
}
