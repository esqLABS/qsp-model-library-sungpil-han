#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Build cmv_references_en.md from PubMed.
...
# ---------------------------------------------------------------------------
S1 = "1 · Overview · epidemiology · guidelines"
S2 = "2 · Viral kinetics in vivo: doubling time · decline half-life"
S3 = "3 · Latency and reactivation, myeloid reservoir"
S4 = "4 · CMV-specific cellular immunity and the protective threshold"
S5 = "5 · Immunosuppressants and CMV risk: ATG, mTOR inhibitors"
S6 = "6 · Ganciclovir · valganciclovir PK/PD"
S7 = "7 · Letermovir: terminase, PK, DDI, trials"
S8 = "8 · Maribavir: pUL97, antagonism with ganciclovir, SOLSTICE"
S9 = "9 · Foscarnet · cidofovir · salvage therapy"
S10 = "10 · Resistance mechanisms and genotypes: UL97, UL54, UL56, UL27"
S11 = "11 · Prophylaxis vs pre-emptive therapy · monitoring interval"
S12 = "12 · Late-onset CMV and immune-guided stopping strategies"
S13 = "13 · Myelosuppression · modelling neutropenia (Friberg model)"
S14 = "14 · Nephrotoxicity · renal dose adjustment"
S15 = "15 · Indirect effects · rejection · graft outcome"
S16 = "16 · End-organ disease · tissue compartmentalisation"
S17 = "17 · Assay standardisation · IU/mL (WHO standard)"
S18 = "18 · Cell therapy · CMV-Ig · vaccines"
S19 = "19 · QSP · viral-dynamics modelling methodology"
...
def main():
    ...
    L = []
    L.append("# Cytomegalovirus (CMV) infection after transplantation — references")
    L.append("### Cytomegalovirus in Transplant Recipients · References")
    L.append("")
    L.append("**%d papers** · %d sections · every PMID, title, journal and year was "
             "**queried live** through the NCBI E-utilities by `mkrefs.py`; none of them "
             "was written from memory." % (n, len(secs)))
    L.append("")
    L.append("The `intent` field on each entry records **what that citation was "
             "asked to support**. The point is that where a search result does not "
             "match the intent, the mismatch is visible at a glance rather than hidden.")
    L.append("")
    L.append("To regenerate:")
    L.append("```bash")
    L.append("python3 mkrefs.py --refresh    # re-query PubMed")
    L.append("```")
    L.append("")
    L.append("---")
    L.append("")
    for sec, entries in secs.items():
        ...
        for intent, pid, rec in entries:
            if not pid:
                L.append("- *(unresolved query — no PubMed hit)* — intent: %s" % intent)
                continue
    ...
    if dupes:
        L.append("---")
        L.append("")
        L.append("## Duplicate query log (queries that resolved to an already-cited paper)")
        L.append("")
        L.append("These are cases where searches with different intents converged on "
                 "the same paper. They are left here rather than hidden.")
        L.append("")
    ...
    L.append("---")
    L.append("")
    L.append("## Where these are used in the model")
    L.append("")
    L.append("| Model component | Evidence section |")
    L.append("|---|---|")
    L.append("| `DBL0` 1.2 d · `THALFTX` 2.4 d → `KPROD`, e\\* = 0.667 | 2 |")
    L.append("| Latent reservoir · `KREACT`·`AREACT` reactivation terms | 3 |")
    L.append("| `KE8`·`E8MAX`·`KMEM` → E8\\* = 4.81 /µL (threshold 2) | 4 |")
    L.append("| `KAG` antigen-dependent expansion = the mechanism of late-onset CMV | 4, 12 |")
    L.append("| `ISI` weighting · ATG·mTORi terms | 5 |")
    L.append("| `FVGCV`·`CLGCV`·`FRENGCV`·`KPHOS`·`EC50GTP` | 6 |")
    L.append("| `FULTV`·`EC50LTV`·`DDITLTV` 2.4-fold interaction | 7 |")
    L.append("| `KIMBVK` (blockade of pUL97 activation = antagonism) | 8 |")
    L.append("| `EC50FOS`·`AFOS`·`FMG` · cidofovir `KPPCDV` | 9, 14 |")
    L.append("| `FK_A` 0.125 · `RESBLTV` 3000 · `FIT_A`·`FIT_B` | 10 |")
    L.append("| Monitoring-interval clock 2^(Δt/1.2 d) · threshold setting | 11, 17 |")
    L.append("| `GAM`·`EMAXMYE`·`EMPA`·`KGCSF` (Friberg chain) | 13 |")
    L.append("| `H0REJ`·`CREJV`·`CREJIS` · eGFR trajectory | 15 |")
    L.append("| `KDIS`·`KD50`·`PVIRT` · sanctuary penetration coefficient | 16 |")
    L.append("| `ACT`·`CMVIG` inputs | 18 |")
    L.append("| Model structure · the critical-efficacy concept · validation methodology | 19 |")
    L.append("")
    open(OUT, "w").write("\n".join(L) + "\n")
