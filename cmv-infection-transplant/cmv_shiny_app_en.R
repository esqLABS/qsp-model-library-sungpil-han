...
##  Post-Transplant CMV Infection QSP Model — Shiny Dashboard
##
##  DESIGN PRINCIPLE
##  ---------------------------------------------------------------------------
...
##
##  TABS
##    1  Patient · Transplant Profile   patient / transplant profile and risk stratification
##    2  Drug PK              antiviral PK, GCV-TP activation, free fractions
##    3  Viral Kinetics          the two DNA species, log10 DNAemia, doubling time
##    4  Two Thresholds (e* / E8*)     THE CORE TAB: both margins on one screen
##    5  Immune Reconstitution            CD8 effector/memory, CD4, NK, antibody, ISI
##    6  Resistance                  strain competition, selection window, cross-resistance
##    7  Toxicity (Marrow · Kidney)      Friberg ANC chain, eGFR, magnesium, dose log
##    8  Strategy Comparison              prophylaxis vs pre-emptive, monitoring interval
##    9  Clinical Endpoints         disease / rejection hazards, cost, PCR count
##   10  Biomarker Panel         everything a clinic would actually order
##   11  Sensitivity · Training-Dose Curve   parameter sweeps incl. the training-dose U-curve
##   12  Model Documentation · Limitations        equations, calibration ledger, stated omissions
##
##  RUN
##    shiny::runApp("cmv_shiny_app.R")
##  Requires: shiny, mrgsolve, dplyr, tidyr, ggplot2, DT
##  and cmv_mrgsolve_model_en.R in the same directory (the driver functions in its
...
##  ENGINE
## ---------------------------------------------------------------------------
MODEL_FILE <- "cmv_mrgsolve_model_en.R"
...
ui <- fluidPage(
  titlePanel(HTML(paste0(
    "<b>Post-Transplant Cytomegalovirus (CMV) Infection — QSP Dashboard</b>",
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      h4("Patient"),
      selectInput("sero", "D/R serostatus",
                  c("D+/R−  (highest risk, primary infection)" = "D+/R-",
                    "D+/R+" = "D+/R+", "D−/R+" = "D-/R+",
                    "D−/R−  (lowest risk)" = "D-/R-"), "D+/R-"),
      selectInput("induction", "Induction",
                  c("rabbit ATG" = "ATG", "alemtuzumab" = "alemtuzumab",
                    "basiliximab / none" = "basiliximab"), "ATG"),
      sliderInput("crcl", "Allograft function, CrCl (mL/min)", 15, 120, 95, 5),
      checkboxInput("mpa", "mycophenolate", TRUE),
      checkboxInput("mtor", "Switch to mTOR inhibitor (protective)", FALSE),
      ...
      conditionalPanel(
        ...
        selectInput("proph_drug", "Prophylactic agent",
                    c("valganciclovir 900 mg od" = "VGCV",
                      "letermovir 480 mg od" = "LTV"), "VGCV"),
        sliderInput("proph_days", "Prophylaxis duration (days)", 0, 365, 200, 10),
        sliderInput("adhere", "Adherence / effective dose fraction",
                    0.2, 1.0, 1.0, 0.05),
        checkboxInput("post_monitor", "PCR surveillance after prophylaxis ends", TRUE),
        checkboxInput("tac_cut_ltv",
                      "Halve tacrolimus dose when starting letermovir", TRUE)),
      sliderInput("monitor_int", "PCR surveillance interval, Δt (days)", 2, 21, 14, 0.5),
      numericInput("trigger", "Treatment initiation threshold (IU/mL)", 1000, 137, 100000, 100),

      hr(), h4("Treatment · Salvage"),
      selectInput("tx_drug", "First-line treatment",
                  c("valganciclovir" = "VGCV", "ganciclovir IV" = "VGCV",
                    "maribavir" = "MBV", "foscarnet" = "FOS"), "VGCV"),
      selectInput("tx_drug2", "Switch on refractory disease",
      ...
      sliderInput("resist_seed",
                  "UL97 minority-variant fraction at treatment start",
                  0, 0.5, 0, 0.05),
      sliderInput("tx_min_days", "Minimum secondary-prophylaxis duration (d)",
                  0, 90, 0, 7),
      checkboxInput("renal_adjust", "Renal-function-based dose adjustment (renal dose bands)",
                    TRUE),
      checkboxInput("gcsf", "G-CSF for neutropenia", TRUE),

      hr(),
      sliderInput("tend", "Observation period (days)", 100, 730, 365, 5),
      actionButton("go", "Run simulation", class = "btn-primary"),
      br(), br(),
      helpText(HTML(paste0(
        "<b>e* = 0.667</b> — a regimen that cannot exceed this value produces ",
        "breakthrough infection regardless of its label.<br><b>E8* = 4.81 /µL</b> — the ",
        "CMV-specific CD8 count at which control is achieved without drugs. No drug can shift this value.")))
    ),

    mainPanel(
      width = 9,
      fluidRow(
        column(3, wellPanel(strong("e (total suppression fraction)"), br(),
                            textOutput("kpi_eps", inline = TRUE))),
        column(3, wellPanel(strong("e − e* (margin)"), br(),
                            textOutput("kpi_margin", inline = TRUE))),
        column(3, wellPanel(strong("Effective trigger"), br(),
                            textOutput("kpi_trigger", inline = TRUE))),
        column(3, wellPanel(strong("E8 − E8* (immune margin)"), br(),
      ...
      tabsetPanel(
        ...
        tabPanel("1 · Patient Profile",
                 ...
                 h4("Initial conditions actually used"),
        ...
        tabPanel("2 · Drug PK",
                 ...
                 h4("Ganciclovir activation step (the pUL97-dependent activation step)"),
                 plotOutput("p_gtp", height = 280),
                 helpText(HTML(paste0(
                   "GCV-TP is made by the <b>viral</b> kinase pUL97. Therefore ",
                   "maribavir (a pUL97 inhibitor) enters not ganciclovir's <b>effect term</b> but its ",
                   "<b>activation term</b>, and the two drugs are structurally antagonistic."))),
                 h4("Free fraction matters: 99% / 98% bound"),
                 DTOutput("tbl_free")),

        tabPanel("3 · Viral Kinetics",
                 h4("Measured DNAemia is the sum of two species"),
                 plotOutput("p_species", height = 330),
                 helpText(HTML(paste0(
                   "Vv = virion DNA packaged in the capsid (the only infectious species). ",
                   "Vl = free DNA released from dying infected cells (about ",
                   "67% of what PCR sees). Polymerase inhibitors reduce both, whereas terminase/kinase ",
                   "inhibitors reduce only Vv, and because the concatemer is not cleaved, DNA per cell is ",
                   "actually increased ⇒ <b>even if letermovir works perfectly, plasma CMV ",
                   "DNA barely falls.</b>"))),
                 h4("The two measured rates: doubling time and decay half-life"),
                 plotOutput("p_rates", height = 260)),

        tabPanel("4 · Two Thresholds (e* / E8*)",
                 h4("Threshold 1 — drug: is e above e*?"),
                 plotOutput("p_eps", height = 300),
                 h4("Threshold 2 — immunity: has E8 reached E8*?"),
                 plotOutput("p_e8", height = 300),
                 h4("Table scoring every regimen on e* alone"),
                 DTOutput("tbl_eps_table")),

        tabPanel("5 · Immune Reconstitution",
                 h4("CMV-specific T cells · NK · antibody"),
                 plotOutput("p_imm", height = 330),
                 h4("Total immunosuppression, ISI, and its components"),
        ...
        tabPanel("6 · Resistance",
                 h4("Strain competition in virion DNA"),
                 plotOutput("p_strain", height = 300),
                 h4("Days inside the selection window"),
                 plotOutput("p_selwin", height = 250),
                 h4("Cross-resistance table"),
                 DTOutput("tbl_xres"),
                 helpText(HTML(paste0(
                   "Resistance is a matter of <b>shape</b>, not <b>strength</b>. The single UL56 C325Y ",
                   "mutation drives letermovir's e from 0.969 → 0.010 (leaving only ",
                   "1.1% of the effect), and UL97 M460V drives ganciclovir's e ",
                   "from 0.890 → 0.112 (12.6% remaining). Both are 'single ",
                   "mutations', but only the latter leaves room to compensate by increasing the dose, ",
                   "and even then the required GCV-TP is 2.0-fold the licensed treatment dose.")))),

        tabPanel("7 · Toxicity (Marrow · Kidney)",
                 h4("Friberg 5-compartment neutrophil chain"),
                 plotOutput("p_anc", height = 300),
                 h4("Renal function · Magnesium"),
                 plotOutput("p_kidney", height = 280),
                 h4("What the ANC rule actually did"),
                 verbatimTextOutput("txt_log"),
                 h4("CrCl → exposure → effect · toxicity (the loop, tabulated)"),
                 DTOutput("tbl_renal")),

        tabPanel("8 · Strategy Comparison",
                 h4("Monitoring interval as a dose"),
                 DTOutput("tbl_clock"),
                 h4("Run the standard arms side by side"),
                 actionButton("run_arms", "Run the 16 standard scenarios",
                              class = "btn-default"),
                 br(), br(),
                 DTOutput("tbl_arms"),
        ...
        tabPanel("9 · Clinical Endpoints",
                 ...
                 h4("Endpoint summary for this run"),
                 DTOutput("tbl_end"),
                 helpText(paste(
        ...
        tabPanel("10 · Biomarker Panel",
                 h4("What a clinic would actually order"),
                 plotOutput("p_biomk", height = 520),
                 DTOutput("tbl_biomk")),

        tabPanel("11 · Sensitivity · Training-Dose Curve",
                 h4("The training-dose U-curve"),
                 helpText(HTML(paste0(
                   "Is complete suppression optimal? If E8 proliferation is antigen-dependent, prophylaxis that ",
                   "suppresses completely leaves nothing durable behind. ",
                   "In a 200-day prophylaxis run, the model finds its minimum near an adherence of 0.40, ",
                   "which is <b>below e*</b>. <b>Do not read this as a prescribing recommendation</b> — it is a single deterministic ",
                   "patient, and the depth of the minimum depends heavily on KAG."))),
                 actionButton("run_ucurve", "Calculate U-curve (slow)",
                              class = "btn-default"),
                 br(), br(),
                 plotOutput("p_ucurve", height = 340),
                 h4("One-at-a-time sensitivity"),
                 selectInput("sens_par", "Parameter",
                             c("DBL0", "THALFTX", "KE8", "KAG", "KISE", "RHO8",
                               "EC50GTP", "FK_A", "RESBLTV", "AMPPK", "GAM",
                               "EMAXMYE", "KDIS", "LAMT"), "KAG"),
                 plotOutput("p_sens", height = 320)),

        tabPanel("12 · Model Documentation · Limitations",
                 h4("Organising thesis"),
                 verbatimTextOutput("txt_thesis"),
                 h4("Calibration ledger: what was fitted, to what"),
                 DTOutput("tbl_ledger"),
                 h4("Stated omissions"),
                 verbatimTextOutput("txt_gaps"),
                 h4("Defects found by running it"),
