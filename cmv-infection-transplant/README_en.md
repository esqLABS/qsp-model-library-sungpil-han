...
<a href="cmv_qsp_model_en.svg"><img src="cmv_qsp_model_en.png" width="880" alt="CMV QSP mechanistic map"></a>

| File | Contents |
|---|---|
| [`cmv_qsp_model_en.dot`](cmv_qsp_model_en.dot) · [`.svg`](cmv_qsp_model_en.svg) · [`.png`](cmv_qsp_model_en.png) | Mechanistic map — 171 nodes · 19 clusters · 219 edges |
| [`cmv_mrgsolve_model_en.R`](cmv_mrgsolve_model_en.R) | 48-ODE mrgsolve model + clinical decision-rule R driver + 19 scenarios |
| [`cmv_python_reference.py`](cmv_python_reference.py) | Independent Python/scipy re-implementation of the same 48 ODEs (**the verification original**) |
| [`cmv_reference_output.txt`](cmv_reference_output.txt) | The output of **actually running** the above file — the source of every number in this README |
| [`cmv_shiny_app_en.R`](cmv_shiny_app_en.R) | 12-tab Shiny dashboard |
| [`cmv_references_en.md`](cmv_references_en.md) | 116 papers · 19 sections · live PubMed lookup |
| [`mkrefs_en.py`](mkrefs_en.py) | Reference generator (a device for not writing PMIDs from memory) |
...
The full table is in [`cmv_reference_output.txt`](cmv_reference_output.txt) §F. Extract:
...
```bash
# mechanistic map
dot -Tsvg cmv_qsp_model_en.dot -o cmv_qsp_model_en.svg
dot -Tpng -Gdpi=150 cmv_qsp_model_en.dot -o cmv_qsp_model_en.png
...
python3 mkrefs_en.py --refresh

# mrgsolve / Shiny  (no R toolchain in this environment, so these were not run — see the caveat below)
Rscript -e 'library(mrgsolve); mod <- mread("cmv_mrgsolve_model_en.R")'
Rscript -e 'shiny::runApp("cmv_shiny_app_en.R")'
```
...
> so `cmv_mrgsolve_model_en.R` and `cmv_shiny_app_en.R` are equation-by-equation transcriptions of
