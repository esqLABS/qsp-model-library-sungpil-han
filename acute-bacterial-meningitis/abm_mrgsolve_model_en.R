...
##  Acute bacterial meningitis — 63-ODE QSP model (mrgsolve)
##
##  -----------------------------------------------------------------------------
##  Provenance / how this file came to be
##  -----------------------------------------------------------------------------
##  This repository environment had no R runtime.  So the 63 equations below were
##  first independently implemented in dependency-free pure Python RK4 (`abm_reference_python_en.py`)
##  and actually integrated; every number quoted in README_en.md is the output of that
##  integration, and `abm_reference_output_en.txt` is the full log of that run.
##
##  Doing so exposed **32 real defects**.  Wherever one was fixed, a "BUG FOUND"
##  comment was left behind — someone reading a QSP model has the right to know
##  which lines were hard.  The full list is [F1]-[F32] in the preamble of
##  abm_reference_python_en.py; only the ones that changed equation structure are carried over here:
##
##    F1   The glucose consumption term had no substrate dependence, so even after CSF glucose
##         had collapsed to 0 the neutrophils and bacteria kept eating "absent glucose" → lactate 243 mmol/L.
##         Lactate comes only from glucose.  Michaelis-Menten in the consumption term is mandatory.
##    F14  Blood bacteria had no carrying capacity, so N_b diverged to 2e10 CFU/mL over 336 h
##         and back-contaminated the CSF.  Logistic carrying capacity added.
##    F15  The osmolality conversion used 18.2 instead of mannitol MW 182, so 0.5 g/kg raised
##         plasma osmolality by 113 mOsm/kg (really 11.3).  A 10-fold unit error.
##    F16  The hazard function integrated permanent damage (1 − N_cort) over all 14 days, so even a
##         recovered patient ended at a 99 % death probability.  Structural damage is scored once, at the end.
##    F17  Below CPP<45 the autoregulation formula made a patient with autoregulation intact more
##         ischaemic than a pressure-passive one.  Autoregulation can never be a disadvantage.
##    F28  If the ICP·CPP hazard terms are linear, "CPP 45 for 100 h" and "CPP 5 for 10 h" carry
##         the same risk.  A mild but sustained excursion came to dominate the hazard function.
##         Replaced with a quadratic.
##    F33  A dose scheduled at t < 0 vanished silently - and that was making the model's
##         central claim untestable.  Write "dexamethasone 20 minutes before the first
##         antibiotic" as time = -0.33 and, because the integration starts at t = 0, that
##         dose is never evaluated.  So the real first dose of the "given earlier" arm was the second
##         dose going in at +5.67 h, and the observation that "timing does not matter" was not
##         physiology but a scheduling bug.  The evidence was in the output: peak
##         cell wall/pneumolysin/TNF in the rifampicin-2-h-ahead scenario matched the no-rifampicin scenario to two decimals.
##         -> zero_shift() moves the whole schedule so that the earliest event lands at t=0.
##    F29  A floor on bacterial extinction must not be written inside the derivative block — it is
##         a discontinuous switch and it stalls the integrator.  The Python reference applies the
##         floor **between** integration steps, and the mrgsolve version does not touch the state
##         at all, judging sterilisation solely by the reporting threshold in $TABLE
##         (N_c < 10 CFU/mL = culture limit of detection).  LSODA converges stably to 0 without a floor.
##
##  -----------------------------------------------------------------------------
##  What the model is for / the structural commitment of this model
##  -----------------------------------------------------------------------------
##  A single structural commitment: **what injures the brain is not the bacterial count but a product.**
##
##      injury flux  =  k_kill(t) · N(t) · Y_lysis · (1 − E_dex(t))
##
##  All three factors are functions of time, and k_kill jumps from 0 → E_max at the first
##  antibiotic dose, exactly when N is at its maximum.  So this product is **largest in the
##  first few hours of treatment.**  That one fact orders the whole treatment map:
##
##    · The steroid has to be on already before that peak (hence "before or with the
##      antibiotic").  The asymmetry of the transcriptional-effect compartment (on t½ 1.5 h / off 17 h) makes that window.
##    · Delay the antibiotic and N has grown exponentially, so the peak grows too.
##      "give it early" and "kill it slowly" do not conflict — the optimum is
##      **as early as possible with the shield already raised.**
##    · At the same log-kill, a different Y gives a different injury integral (β-lactam vs rifampicin).
##
##  And the two doors are really one door — the blood-CSF barrier Pb.  Inflammation opens it
##  and dexamethasone closes it.  Open, it lets hydrophilic antibiotics in (cluster 13) and at the
##  same time albumin·neutrophils·water (cluster 6-9).  So the **sign of dexamethasone
##  splits by drug**: for ceftriaxone against a susceptible strain the C/MIC margin is 200-fold, so
##  it is a net benefit; for vancomycin against a cephalosporin-resistant strain there is no margin and the same
##  manoeuvre doubles the time to CSF sterilisation.
##
##  -----------------------------------------------------------------------------
...
##  (identical to the Python reference; abm_reference_output_en.txt holds the
...
scenarios <- list(
  S01 = list(lab = "untreated natural course",     ev = NULL,                                     p = list()),
  S02 = list(lab = "CEF alone (t=0)",              ev = ev_cef(),                                 p = list()),
  S03 = list(lab = "CEF + DEX 20 min before",      ev = ev_cef() + ev_dex(-0.33),                 p = list()),
  S04 = list(lab = "CEF + DEX together (0 h)",     ev = ev_cef() + ev_dex(0),                     p = list()),
  ...
  S08 = list(lab = "CEF + DEX 1 day only",         ev = ev_cef() + ev_dex(0, days = 1),           p = list()),
  S09 = list(lab = "antibiotic delayed 3 h",       ev = ev_cef(3) + ev_dex(3),                    p = list()),
  S10 = list(lab = "antibiotic delayed 6 h",       ev = ev_cef(6) + ev_dex(6),                    p = list()),
  S11 = list(lab = "antibiotic delayed 12 h",      ev = ev_cef(12) + ev_dex(12),                  p = list()),
  S12 = list(lab = "resistant (MIC 4) CEF alone",  ev = ev_cef(),                                 p = list(MIC_cef = 4)),
  S13 = list(lab = "resistant + VAN, no DEX",      ev = ev_cef() + ev_van(),                      p = list(MIC_cef = 4)),
  S14 = list(lab = "resistant + VAN + DEX",        ev = ev_cef() + ev_van() + ev_dex(0),          p = list(MIC_cef = 4)),
  S15 = list(lab = "resistant + VAN + RIF + DEX",  ev = ev_cef() + ev_van() + ev_rif() + ev_dex(0), p = list(MIC_cef = 4)),
  S16 = list(lab = "RIF 2 h ahead → CEF",          ev = ev_cef() + ev_rif(-2),                    p = list()),
  S17 = list(lab = "CEF + DEX + mannitol",         ev = ev_cef() + ev_dex(0) + ev_mann(),         p = list()),
  S18 = list(lab = "CEF + DEX + glycerol",         ev = ev_cef() + ev_dex(0) + ev_gly(),          p = list()),
  S19 = list(lab = "high inoculum 1e8 + DEX",      ev = ev_cef() + ev_dex(0),                     p = list(N0 = 1e8)),
  S20 = list(lab = "early low inoculum 1e5 + DEX", ev = ev_cef() + ev_dex(0),                     p = list(N0 = 1e5)),
  S21 = list(lab = "aged/immunosuppressed + DEX",  ev = ev_cef() + ev_dex(0),                     p = list(HOST_DEF = 0.4, MU_SCALE = 1.3)),
  S22 = list(lab = "CEF cont. infusion + DEX",     ev = ev_cef_ci() + ev_dex(0),                  p = list()),
  S23 = list(lab = "antipyretic only (no ABX)",    ev = NULL,                                     p = list(ANTIPYR = 0.7)),
  S24 = list(lab = "standard care CEF+VAN+DEX",    ev = ev_cef() + ev_van() + ev_dex(-0.33),      p = list()),
  S25 = list(lab = "resistant CEF-CI+VAN+DEX",     ev = ev_cef_ci() + ev_van() + ev_dex(-0.33),   p = list(MIC_cef = 4)),
  S26 = list(lab = "standard care + EVD drainage", ev = ev_cef() + ev_van() + ev_dex(-0.33),      p = list(CSF_DRAIN = 12))
)
