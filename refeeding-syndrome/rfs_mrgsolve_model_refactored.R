## ============================================================================
##  rfs_mrgsolve_model_refactored.R
##  REFEEDING SYNDROME  -  Quantitative Systems Pharmacology model (mrgsolve)
##  REFACTORED sibling of rfs_mrgsolve_model.R -- pluggable-PK naming
##  convention applied to the two compounds carrying rows in
##  driver-patches/data/compound_perturbation_census.md that are actually
##  dosed in this file: Magnesium (MG) and Potassium (K). Both are
##  dual-purpose electrolytes (endogenous AND a genuine IV-infusion dosing
##  route, MG_DOSE/K_DOSE) with no separate PK compartment of their own --
##  MGE/KE *are* the disease state, so this refactor renames the exposed
##  concentration to C_MG/C_K (unchanged calculation, same $ODE lines)
##  and pulls out the already-distinct named physiological terms each
##  electrolyte feeds as EFFECT_<STEM>_* (EFFECT_MG_PTH, EFFECT_MG_KWASTE,
##  EFFECT_MG_QTC, EFFECT_K_QTC, EFFECT_K_ALD), per the guide's "None of
##  these fit"/bespoke handling -- see rfs_refactor_notes.md for the full
##  account, including the six-line mrgsolve-2.0.1 multi-declarator build
##  fix this file also carries (UPSTREAM_ISSUES.md #124), a fix that
##  touches neither electrolyte's own math. All parameter values are
##  copied verbatim from the original; phosphate, thiamine, and every
##  other compound/state in the file are completely untouched. The
##  original (rfs_mrgsolve_model.R) is untouched.
## ============================================================================
##
##  THE THESIS
##  ----------
...
EC50_MG_PTH = 0.22    // [RENAMED, see refactor notes] magnesium for half-maximal PTH secretion (mmol/L)
GAMMA_MG_PTH = 1.0    // [NEW, disclosed] Hill coeff. for EFFECT_MG_PTH; no explicit coefficient in the original
...
double EMAX_MG_PTH;  // [NEW, derived] see $MAIN
...
// identical to the original -- see rfs_refactor_notes.md).
EMAX_MG_PTH = (EC50_MG_PTH + MGSER0)/MGSER0;
...
double C_K  = posv(KE)/Vecf;   // [RENAMED, see refactor notes] the exposed potassium concentration
double C_MG = posv(MGE)/Vecf;  // [RENAMED, see refactor notes] the exposed magnesium concentration
...
double g_cho = kcal_h*cho_f/4.0;
double g_fat = kcal_h*fat_f/9.0;
double g_pro = kcal_h*pro_f/4.0;
...
double qP = 0.40 + 0.60*quality;
double qK = 0.70 + 0.30*quality;
double qMg = 0.45 + 0.55*quality;
double qTh = quality;
...
if(PHASE > 1.5){
  ...
  if(RESCUE > 0.5){
    ...
    ivK  += 1.00*BW/24.0*swbelow(C_K , 3.30, 0.100);
    ivMg += 0.20*BW/24.0*swbelow(C_MG, 0.65, 0.030);
  }
}
...
if(Ebal > 0.0){
  ...
  double a2 = C_K/3.50;  if(a2 > 1.0) a2 = 1.0;
  dFFM_d *= a1*a2;
}
...
double JKin = KKP*pumpm(X, EMAXK, TAUX)*C_K*(1.0 + 0.8*depK);
double JKout = KKLEAK*posv(KI)*(1.0 + 0.9*cat);
double JKnew = dxdt_FFM*RHOK;
double EFFECT_MG_KWASTE = 1.0 + BETAMGK*posv(1.0 - C_MG/MGSER0);  // [RENAMED from fMgK]
...
double ExK = KOBL + KKSEC*C_K*ALD*EFFECT_MG_KWASTE*(1.0 + 1.4*DIURETIC)*romk;
...
double JMgin = KMGP*pumpm(X, EMAXMG, TAUX)*C_MG*(1.0 + 0.7*depMg);
double JMgout = KMGLEAK*posv(MGI)*(1.0 + 0.6*cat);
double JMgnew = dxdt_FFM*RHOMG;
double TmMg = TMMG*(1.0 - ETOHMG*alc);
double ExMg = (GFR0*posv(FUFMG*C_MG - TmMg) + 0.5/24.0*C_MG/MGSER0)
            * (1.0 + 1.8*DIURETIC);
...
double EFFECT_MG_PTH = EMAX_MG_PTH * pow(C_MG, GAMMA_MG_PTH)
                      ...
                      // [RENAMED+rewritten from fMgPTH; algebraically identical, see notes]
if(EFFECT_MG_PTH > 1.05) EFFECT_MG_PTH = 1.05;
double secPTH = (PTHMIN + (PTHMAX - PTHMIN)/(1.0 + pow(Caser/CA50, HCA)))*EFFECT_MG_PTH;
...
double EFFECT_K_QTC  = AK*posv(KSER0 - C_K);   // [NEW, extracted] linear QTc contribution of hypokalaemia
double EFFECT_MG_QTC = AMG*posv(MGSER0 - C_MG); // [NEW, extracted] linear QTc contribution of hypomagnesaemia
double QTc = QTC0 + EFFECT_K_QTC + EFFECT_MG_QTC
           + ACA*posv(CA0 - Caser);
...
$CAPTURE
// [REFACTORED, NEW] the naming-convention interface variables. C_MG/C_K/
// EFFECT_MG_*/EFFECT_K_* are $ODE-local doubles (mrgsolve 2.0.1 treats
// $PARAM values as read-only inside $ODE, so declaring them there
// instead is a compile error -- same constraint documented across this
// refactor batch, e.g. wilsons-disease/wd_refactor_notes.md). Exposed
// here so /model_manifest lists them under outputPaths.
C_MG C_K EFFECT_MG_PTH EFFECT_MG_KWASTE EFFECT_MG_QTC EFFECT_K_QTC EFFECT_K_ALD
'
