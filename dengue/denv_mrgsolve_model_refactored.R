...
#  dengue/denv_refactor_notes.md for full detail):
#    Only the antiviral (AV) compound's PK block and its effect on viral
#    production/NS1 secretion were touched. Paracetamol (APAP) and
#    methylprednisolone (STER) PK are byte-for-byte identical to
#    denv_mrgsolve_model.R.
#    - AVD/AVC/kaAV/kelAV/VdAV/EMAXAV/EC50AV renamed to convention:
#      GUT_AV/CENT_AV/KA_AV/KEL_AV/V1_AV/EMAX_AV/EC50_AV, with an explicit
#      GAMMA_AV = 1 added (the original had no Hill exponent term).
#    - Bespoke depot+single-compartment structure, kept exactly as the
#      original has it: CENT_AV (was AVC) is integrated directly as a
#      CONCENTRATION (mg/L) by its own $ODE line, not as an amount that PD
#      divides by volume; V1_AV only ever appears inside the depot-to-
#      concentration conversion KA_AV*GUT_AV/V1_AV, exactly as VdAV did in
#      the original. C_AV = CENT_AV, undivided -- preserved unchanged for
#      exact numerical equivalence with the original's av_block. This is the
#      same quirk documented for tocilizumab in sepsis/sep_refactor_notes.md,
#      here combined with a depot (the guide's archetype 3 minus the
#      peripheral compartment, with archetype 1's "compartment IS
#      concentration" behavior instead of an amount/volume division).
#    - EFFECT_AV is a RENAME of the original's av_block, not a fit: the
#      original term `EMAXAV * AVC / (AVC + EC50AV)` is already an exact
#      Hill ratio with an implicit gamma = 1, so EMAX_AV/EC50_AV carry the
#      original's exact values and GAMMA_AV = 1 is added explicitly.
#      EFFECT_AV replaces av_block at both its use sites (the viral
#      production block `p_eff` and the NS1 secretion block `dxdt_NS1`).
#    - C_AV and EFFECT_AV were added to $CAPTURE (additive only) purely so
#      they are discoverable as outputs via the qspserver mrgsolve_api
#      /model_manifest and /run_simulation endpoints, per FORK_WORKFLOW_GUIDE.md's
#      qspserver compatibility requirements (state-dependent quantities like
#      these cannot themselves be $PARAM values, per the precedent set in
#      thyroid-eye-disease/ted_refactor_notes.md).
#
#  Verified against the original (rerun via the qspserver mrgsolve_api
#  service) on the original file's own S10/S11 antiviral dosing scenarios:
#  see denv_refactor_notes.md for the full result.
# =============================================================================
...
// ===========================================================================
// C_AV is the single exposed antiviral concentration PD reads (fork PK/PD
...
dxdt_NS1 = pNS1 * (1.0 - EFFECT_AV) * INF - (cNS1 + NS1MAB) * NS1
         - kNS1ab * ABN * NS1;
...
$CAPTURE ABH0 V0 RCRYS RCOLL C_AV EFFECT_AV
'
...
##   24 = PLT, 38 = GUT_AV (antiviral depot, was AVD), 40 = APAP, 41 = STER
##   Compartment order/position is unchanged by the PK/PD refactor, only names.
