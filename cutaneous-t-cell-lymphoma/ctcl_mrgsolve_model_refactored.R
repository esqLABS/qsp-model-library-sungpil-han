...
// Fork PK/PD refactor: MM = free MMAE payload liberated from brentuximab
// vedotin (BV); renamed to convention, Archetype 1 (no depot, single
// compartment, linear elimination) per FORK_WORKFLOW_GUIDE.md Part 2.
// KDEC/FMMAE/BV1 remain BV’s own names (untouched, out of scope) -- MM’s
// generation term is a cross-compound source, not a dose, and is preserved
// verbatim.
V1_MM  :  1200.: MMAE central volume of distribution (L)  [was VMM]
CL_MM  :  60.  : MMAE clearance (L/day)                   [was CLMM]
...
// MM Hill interface (EFFECT_MM). Original effect term (`byst`) was already a
// plain Emax ratio (KBYST*C/(C+K50)) -- this is a rename, not a refit.
EMAX_MM :  0.020  : MMAE bystander kill Emax (1/day)       [was KBYST]
EC50_MM :  0.0020 : MMAE bystander kill K50 (ug/mL)        [was KMMAE50]
GAMMA_MM:  1       : Hill coefficient (original had no explicit Hill exponent)
...
CENT_MM : Free MMAE central (mg)                            [was MMAE1]
PERI_MM : MMAE placeholder, zero dynamics in the original (mg) [was MMAE2]
...
double C_MM = CENT_MM / V1_MM;                      // [was CMM = MMAE1/VMM]
dxdt_CENT_MM = KDEC * BV1 * FMMAE - CL_MM * C_MM;   // KDEC/FMMAE/BV1 are BV’s
                                                     // own PK (untouched); MM
                                                     // is generated by BV’s
                                                     // deconjugation, not dosed
...
// MM Hill interface: EFFECT_MM is a rename of `byst`, not a refit --
// KBYST*C/(C+K50) is already exactly this shape with GAMMA_MM = 1.
...
double CBVP    = BV1 / VB1;
double CMMP    = CENT_MM / V1_MM * 1000.0;              // ng/mL [was MMAE1/VMM]
