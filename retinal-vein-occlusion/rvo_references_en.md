# Retinal Vein Occlusion (RVO) QSP Model — References
# Retinal Vein Occlusion QSP Model — Annotated References

Every PMID was looked up via PubMed E-utilities on 2026-08-05 to confirm the title, author, journal, and year.
...
## 2. The evidence for the pressure term — retinal capillary hydrostatic pressure
...
## 3. Thrombus, recanalisation · the collateral clock
...
## 4. Blood-retina barrier and permeability
...
## 7. Binding affinity and suppression duration
...
## 8. Anti-VEGF trials: the calibration targets
...
## 10. Ischaemia, neovascularisation, NVG
...
## 11. Structural biomarkers and visual function
...
## 12. Systemic risk and blood pressure
...
## 13. Real-world outcomes and treatment intensity
...
## Where each key parameter comes from
...
- **Individual prediction.** The parameters are population-level
  and the calibration targets are trial means. Nothing here is fitted to any
  individual eye.
- **`ALPHA` is a calibration constant, not a mechanism.** The 120-fold gap between an SPR
...
- **Absolute thickness.** `KF`, `VMAX_PUMP` and `KM_PUMP` are fitted
...
- **The `SIGMA = SIGMAX·TJ^0.55` relationship.** The functional coupling of the reflection
