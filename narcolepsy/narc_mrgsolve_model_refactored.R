...
# REFACTORED PK sibling of narc_mrgsolve_model.R
# =============================================================================
# Author  : Claude Code (CCR auto-generated)
# Date    : 2026-06-28
# Disease : Narcolepsy Type 1 (NT1, hypocretin-deficient)
#
# THIS FILE vs. the original (narc_mrgsolve_model.R):
# Per the fork's PK/PD refactor guide (see FORK_WORKFLOW_GUIDE.md, Part 2),
# all five compounds this model gives their own PK compartments — Sodium
# Oxybate (OXY), Modafinil (MOD), Pitolisant (PIT), Solriamfetol (SOL), and
# Venlafaxine (VEN) — have been renamed to the guide's naming convention
# (KA_/F_/CL_/V1_/V2_/Q_<STEM>, C_<STEM> exposed concentration, EFFECT_<STEM>
# Hill interface). The original already used GUT_<STEM>/CENT_<STEM>/PERI_<STEM>
# compartment names and EMAX_<STEM>_<TARGET>/EC50_<STEM>_<TARGET> parameter
# names matching this fork's convention almost exactly, so no compartment was
# renamed and compartment order is unchanged; only the volume parameters
# (VD_<STEM> -> V1_<STEM>, VP_OXY -> V2_OXY), the plasma-concentration globals
# (CP_<STEM> -> C_<STEM>), and the shared Hill-coefficient parameters
# (H_EDS/H_CAT/H_WAKE/H_TMN/H_SOL, each shared across several compounds'
# effect terms) needed changing. No equation and no parameter *value* was
# changed anywhere in the disease/neuroscience PD core or in any of the five
# compounds' own PK/PD arithmetic — every rename is a 1:1 identifier swap.
# See narc_refactor_notes.md for the full per-compound account and the
# verification results.
#
# This file ALSO carries one syntax-only, non-numeric, non-behavioral
# build-compatibility fix that the original needs to compile under mrgsolve
# 2.0.1 at all: the original's $CAPTURE block lists ten names
# (WAKE_STATE, REM_STATE, NREM_STATE, VLPO_ACT, SLEEP_P, ADENOSINE, WAKE_LC,
# WAKE_TMN, WAKE_VTA, WAKE_DRN) that duplicate $CMT compartment names, which
# mrgsolve 2.0.1 refuses to build ("compartment should not be in $CAPTURE").
# The ten names are dropped from $CAPTURE here (compartments are always
# present in mrgsolve's output regardless of $CAPTURE, so this changes
# nothing about what is reported) — see narc_refactor_notes.md and
# translations/UPSTREAM_ISSUES.md #78 for the exact defect and fix, and
# confirmation this changes no simulated value.
#
...
KA_PIT   = 0.8    // /hr
F_PIT    = 1.00
V1_PIT   = 4.5    // central volume (L/kg)
...
$MAIN
// ---- Volumes (L) ----
double VC_OXY  = V1_OXY * BW;
double VP_OXY_ = V2_OXY * BW;
double VC_MOD  = V1_MOD * BW;
double VC_PIT  = V1_PIT * BW;
double VC_SOL  = V1_SOL * BW;
double VC_VEN  = V1_VEN * BW;

// ---- Plasma concentrations (ug/mL = mg/L) ----
C_OXY = CENT_OXY / VC_OXY;
C_MOD = CENT_MOD / VC_MOD;
C_PIT = CENT_PIT / VC_PIT;
C_SOL = CENT_SOL / VC_SOL;
C_VEN = CENT_VEN / VC_VEN;
...
// ---- Drug Emax effects ----
// Sodium oxybate
EFFECT_OXY_EDS = EMAX_OXY_EDS * pow(C_OXY, GAMMA_OXY_EDS) /
                 (pow(EC50_OXY_EDS, GAMMA_OXY_EDS) + pow(C_OXY, GAMMA_OXY_EDS));
EFFECT_OXY_CAT = EMAX_OXY_CAT * pow(C_OXY, GAMMA_OXY_CAT) /
                 (pow(EC50_OXY_CAT, GAMMA_OXY_CAT) + pow(C_OXY, GAMMA_OXY_CAT));

// Modafinil
EFFECT_MOD_WAKE = EMAX_MOD_WAKE * pow(C_MOD, GAMMA_MOD_WAKE) /
                  (pow(EC50_MOD_WAKE, GAMMA_MOD_WAKE) + pow(C_MOD, GAMMA_MOD_WAKE));
EFFECT_MOD_EDS  = EMAX_MOD_EDS * pow(C_MOD, GAMMA_MOD_EDS) /
                  (pow(EC50_MOD_EDS, GAMMA_MOD_EDS) + pow(C_MOD, GAMMA_MOD_EDS));

// Pitolisant
EFFECT_PIT_TMN = EMAX_PIT_TMN * pow(C_PIT, GAMMA_PIT_TMN) /
                 (pow(EC50_PIT_TMN, GAMMA_PIT_TMN) + pow(C_PIT, GAMMA_PIT_TMN));
EFFECT_PIT_EDS = EMAX_PIT_EDS * pow(C_PIT, GAMMA_PIT_EDS) /
                 (pow(EC50_PIT_EDS, GAMMA_PIT_EDS) + pow(C_PIT, GAMMA_PIT_EDS));

// Solriamfetol
EFFECT_SOL_WAKE = EMAX_SOL_WAKE * pow(C_SOL, GAMMA_SOL_WAKE) /
                  (pow(EC50_SOL_WAKE, GAMMA_SOL_WAKE) + pow(C_SOL, GAMMA_SOL_WAKE));
EFFECT_SOL_EDS  = EMAX_SOL_EDS * pow(C_SOL, GAMMA_SOL_EDS) /
                  (pow(EC50_SOL_EDS, GAMMA_SOL_EDS) + pow(C_SOL, GAMMA_SOL_EDS));

// Venlafaxine
EFFECT_VEN_CAT  = EMAX_VEN_CAT * pow(C_VEN, GAMMA_VEN_CAT) /
                  (pow(EC50_VEN_CAT, GAMMA_VEN_CAT) + pow(C_VEN, GAMMA_VEN_CAT));
...
// --- A. SODIUM OXYBATE PK ---
// ===========================================================================
double VC_OXY_  = V1_OXY * BW;
double VP_OXY__ = V2_OXY * BW;
...
double C_OXY_out  = CENT_OXY / (V1_OXY * BW);
double C_MOD_out  = CENT_MOD / (V1_MOD * BW);
double C_PIT_out  = CENT_PIT / (V1_PIT * BW);
double C_SOL_out  = CENT_SOL / (V1_SOL * BW);
double C_VEN_out  = CENT_VEN / (V1_VEN * BW);
...
$CAPTURE
C_OXY_out C_MOD_out C_PIT_out C_SOL_out C_VEN_out
CSF_OREXIN OREXIN_NEURONS
EDS_NOW CAT_NOW SLEEP_LATENCY REM_PCT SLEEP_EFF CIRC_OUT
C_OXY C_MOD C_PIT C_SOL C_VEN
EFFECT_OXY_EDS EFFECT_OXY_CAT EFFECT_MOD_WAKE EFFECT_MOD_EDS EFFECT_PIT_TMN EFFECT_PIT_EDS EFFECT_SOL_WAKE EFFECT_SOL_EDS EFFECT_VEN_CAT
'
...
message("Compiling Narcolepsy QSP mrgsolve model (refactored)...")
mod <- mcode("narcolepsy_qsp_refactored", model_text)
...
daily_summary <- results_df %>%
  ...
  summarise(
    ...
    CP_OXY_peak     = max(C_OXY_out, na.rm = TRUE),
    CP_MOD_peak     = max(C_MOD_out, na.rm = TRUE),
    CP_PIT_peak     = max(C_PIT_out, na.rm = TRUE),
    CP_SOL_peak     = max(C_SOL_out, na.rm = TRUE),
    .groups = "drop"
  ) %>%
...
pk_day1 <- results_df %>%
  ...
  mutate(
    drug = recode(drug,
                  C_OXY_out = "Sodium Oxybate",
                  C_MOD_out = "Modafinil",
                  C_PIT_out = "Pitolisant",
                  C_SOL_out = "Solriamfetol",
                  C_VEN_out = "Venlafaxine")
  ) %>%
...
# Safer path
plot_path <- "/home/user/qsp/narcolepsy/narc_simulation_results_refactored.png"
...
sleep_plot_path <- "/home/user/qsp/narcolepsy/narc_sleep_architecture_refactored.png"
