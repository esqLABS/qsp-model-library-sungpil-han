## =============================================================================
## NAFLD/MASLD QSP Model — mrgsolve Implementation — REFACTORED (pluggable PK)
## Non-Alcoholic Fatty Liver Disease / Metabolic-Associated Steatotic Liver Disease
##
## Derived from nafld_mrgsolve_model.R (untouched, upstream-tracked). Both
## compounds' PK are reorganized into their own clearly-delimited blocks,
## renamed to the fork's `<ROLE>_<STEM>` convention (FORK_WORKFLOW_GUIDE.md
## Part 2), and each exposes exactly one `C_<STEM>` concentration plus its own
## named `EFFECT_<STEM>_*` disease-effect term(s). See nafld_refactor_notes.md
## for the full account (census correction, naming map, build-compat fixes,
## verification result).
##
## Model structure (unchanged from the original):
##   - Drug PK (2 compounds: FXR agonist [OCA-like] + GLP-1 RA [semaglutide-like])
...
## ─────────────────────────────────────────────────────────────
## MODEL CODE (refactored: pluggable-PK naming convention)
## ─────────────────────────────────────────────────────────────

nafld_refactored_code <- '
$PROB NAFLD/MASLD QSP Model v2.0 — 20 compartments, mrgsolve [refactored: pluggable-PK naming convention]

$PARAM
// --- FXR agonist (OCA-like) PK (renamed to FXR stem) ---
KA_FXR    = 0.8       // absorption rate constant (h^-1)
CL_FXR    = 4.5       // clearance (L/h)
V1_FXR    = 15.0      // central volume (L)
V2_FXR    = 30.0      // peripheral volume (L)
...
// FXR agonist (OCA-like) — renamed to EMAX_FXR_*/EC50_FXR/GAMMA_FXR_* convention.
// EC50_FXR already matched the naming convention in the original (unchanged).
// One EC50_FXR is shared across all four FXR effects, exactly as the original did.
EC50_FXR  = 0.5       // EC50 for FXR agonism (µg/mL)
EMAX_FXR_DNL = 0.40   // max DNL reduction by FXR
GAMMA_FXR_DNL = 1      // Hill exponent [original had no explicit Hill coefficient]
EMAX_FXR_TGF = 0.35   // max TGF-β reduction by FXR
GAMMA_FXR_TGF = 1      // Hill exponent [original had no explicit Hill coefficient]
EMAX_FXR_LPS = 0.25   // max LPS reduction by FXR (gut FXR)
GAMMA_FXR_LPS = 1      // Hill exponent [original had no explicit Hill coefficient]
EMAX_FXR_FGF19 = 1.5   // max FGF19 induction by FXR [was a hardcoded 1.5 literal in the original, now a named param, same value]
GAMMA_FXR_FGF19 = 1    // Hill exponent [original had no explicit Hill coefficient]

// GLP-1 RA (semaglutide-like) — renamed to EMAX_GLP1_*/EC50_GLP1/GAMMA_GLP1_* convention.
// EC50_GLP1 already matched the naming convention in the original (unchanged).
EC50_GLP1 = 0.3       // EC50 for GLP-1R agonism (µg/mL)
EMAX_GLP1_IR = 0.45   // max IR reduction
GAMMA_GLP1_IR = 1      // Hill exponent [original had no explicit Hill coefficient]
EMAX_GLP1_DNL = 0.30  // max DNL reduction (hepatic)
GAMMA_GLP1_DNL = 1     // Hill exponent [original had no explicit Hill coefficient]
EMAX_GLP1_Kup = 0.35  // max Kupffer activation reduction
GAMMA_GLP1_Kup = 1     // Hill exponent [original had no explicit Hill coefficient]

// Resmetirom (THRβ agonist) and Pioglitazone (PPARγ) parameters below are carried over
// UNCHANGED from the original -- see refactor notes: neither is wired to any PK
// compartment, $MAIN expression, $ODE expression, or $TABLE/$CAPTURE output anywhere
// in the original file (confirmed by exact-name grep); the R-side "Resmetirom" scenario
// instead reuses the FXR/OCA PK compartment with overridden Emax_FXR_* parameters. Dead
// $PARAM entries, left exactly as in the original -- out of scope for this refactor
// (neither compound has a PK block to reorganize).
...
$CMT
// PK compartments
GUT_FXR   // FXR agonist oral drug absorption depot (mg)
CENT_FXR  // FXR agonist central PK compartment (mg)
PERI_FXR  // FXR agonist peripheral PK compartment (mg)
...
GUT_GLP1_0 = 0;
CENT_GLP1_0 = 0;
LFFA_0 = 120.0;      // elevated baseline (NAFLD patient: ~3x normal)
LTAG_0 = 80.0;       // elevated baseline steatosis
LDAG_0 = 15.0;       // elevated DAG (lipotoxic)
ROS_LVR_0 = 0.60;    // baseline oxidative stress
NRF2_ACT_0 = 0.25;   // partially active Nrf2
ER_STRESS_0 = 0.40;  // baseline ER stress
KUP_ACT_0 = 0.30;    // mild baseline Kupffer activation
TNF_0 = 0.25;
IL6_0 = 0.20;
IL1B_0 = 0.15;
MCP1_0 = 0.10;
NEUTRO_0 = 0.10;
HEPATO_APOP_0 = 0.30;
HSC_ACTIV_0 = 0.20;  // early stellate cell activation
TGF_B1_0 = 0.30;
COLLAGEN_0 = 2.0;    // F1–F2 baseline fibrosis

// Plasma drug concentrations (µg/mL) -- the single exposed, PD-facing concentration
// per compound. Computed here (not $ODE) to match exactly where the original placed
// this (FORK_WORKFLOW_GUIDE.md: "keep a calculation in the block the original used it
// in" -- mrgsolve evaluates $MAIN once per reporting/dosing interval using
// start-of-interval state; moving this into $ODE would change the simulated
// trajectory, not just where the code lives).
double C_FXR  = CENT_FXR / V1_FXR;
double C_GLP1 = CENT_GLP1 / V1_GLP1;
...
double EFFECT_FXR_DNL = EMAX_FXR_DNL * pow(C_FXR, GAMMA_FXR_DNL)
                        / (pow(EC50_FXR, GAMMA_FXR_DNL) + pow(C_FXR, GAMMA_FXR_DNL));
double EFFECT_FXR_TGF = EMAX_FXR_TGF * pow(C_FXR, GAMMA_FXR_TGF)
                        / (pow(EC50_FXR, GAMMA_FXR_TGF) + pow(C_FXR, GAMMA_FXR_TGF));
double EFFECT_FXR_LPS = EMAX_FXR_LPS * pow(C_FXR, GAMMA_FXR_LPS)
                        / (pow(EC50_FXR, GAMMA_FXR_LPS) + pow(C_FXR, GAMMA_FXR_LPS));
double EFFECT_FXR_FGF19 = EMAX_FXR_FGF19 * pow(C_FXR, GAMMA_FXR_FGF19)
                        / (pow(EC50_FXR, GAMMA_FXR_FGF19) + pow(C_FXR, GAMMA_FXR_FGF19)); // FGF19 induction

// GLP-1 RA effects
double EFFECT_GLP1_IR  = EMAX_GLP1_IR  * pow(C_GLP1, GAMMA_GLP1_IR)
                        / (pow(EC50_GLP1, GAMMA_GLP1_IR) + pow(C_GLP1, GAMMA_GLP1_IR));
double EFFECT_GLP1_DNL = EMAX_GLP1_DNL * pow(C_GLP1, GAMMA_GLP1_DNL)
                        / (pow(EC50_GLP1, GAMMA_GLP1_DNL) + pow(C_GLP1, GAMMA_GLP1_DNL));
double EFFECT_GLP1_Kup = EMAX_GLP1_Kup * pow(C_GLP1, GAMMA_GLP1_Kup)
                        / (pow(EC50_GLP1, GAMMA_GLP1_Kup) + pow(C_GLP1, GAMMA_GLP1_Kup));

// Effective FFA uptake (amplified by IR, attenuated by GLP-1 RA)
double kLFFA_eff = kLFFA_in * IR_index * (1 - EFFECT_GLP1_IR * 0.4);

// Effective DNL (amplified by IR, attenuated by FXR & GLP-1)
double kDNL_eff = kDNL_base * IR_index * (1 - EFFECT_FXR_DNL) * (1 - EFFECT_GLP1_DNL);
...
double FGF19_level = kFGF19_prod * (1 + EFFECT_FXR_FGF19) / kFGF19_deg;
double BA_synth_eff = kBA_synth / (1 + 2.0 * FGF19_level);

// Kupffer activation (LPS, ROS, lipid perox → activation; adiponectin → inhibition)
double KUP_drive = kKup_act * (1 + 0.5 * ROS_LVR) * (1 - EFFECT_GLP1_Kup)
                 * (1 - EFFECT_FXR_LPS * 0.3);
...
dxdt_CENT_FXR  = KA_FXR * F_FXR * GUT_FXR - (CL_FXR/V1_FXR) * CENT_FXR - (Q_FXR/V1_FXR) * CENT_FXR + (Q_FXR/V2_FXR) * PERI_FXR;
dxdt_PERI_FXR  = (Q_FXR/V1_FXR) * CENT_FXR - (Q_FXR/V2_FXR) * PERI_FXR;
...
// NOTE: the original wrote this dxdt with a trailing "where NFKB_drive = ...;"
// clause -- not valid mrgsolve/C++ syntax (mrgsolve has no "where" keyword; see
// refactor notes for the confirmed compile failure and this fix). Rewritten as a
// standard local-variable-then-use, same formula, same value, syntax-only.
...
double Hepatic_TG = LTAG;                          // hepatic TAG (µmol/g)
double Hepatic_FFA = LFFA;                         // hepatic FFA (µmol/g)
double ROS_level  = ROS_LVR;                       // oxidative stress index
double ER_stress  = ER_STRESS;                     // ER stress index
double Kupffer_activation = KUP_ACT;               // Kupffer activity (0–1)
double TNFalpha   = TNF;                           // TNF-α (AU)
double IL6_level  = IL6;                           // IL-6 (AU)
double IL1beta    = IL1B;                          // IL-1β (AU)
double Apoptosis  = HEPATO_APOP;                   // apoptosis index
double HSC_act    = HSC_ACTIV;                     // HSC activation (0–1)
double TGFbeta1   = TGF_B1;                        // TGF-β1 (AU)
double Collagen   = COLLAGEN;                      // collagen (normalized)
double ALT        = ALT_base + kALT_lysis * HEPATO_APOP;
double AST        = (ALT_base + kALT_lysis * HEPATO_APOP) * 0.7;

$CAPTURE Cp_FXR_ug Cp_GLP1_ug Hepatic_TG Hepatic_FFA ROS_level ER_stress Kupffer_activation TNFalpha IL6_level IL1beta Apoptosis HSC_act TGFbeta1 Collagen ALT AST C_FXR C_GLP1 EFFECT_FXR_DNL EFFECT_FXR_TGF EFFECT_FXR_LPS EFFECT_FXR_FGF19 EFFECT_GLP1_IR EFFECT_GLP1_DNL EFFECT_GLP1_Kup
'

## ─────────────────────────────────────────────────────────────
## COMPILE MODEL
## ─────────────────────────────────────────────────────────────
nafld_mod <- mcode("nafld_masld_qsp_refactored", nafld_refactored_code)

## ─────────────────────────────────────────────────────────────
## SCENARIO DEFINITIONS (dosing unchanged; compartments addressed by their
## renamed GUT_FXR / GUT_GLP1 depot names instead of the original's bare
## numeric indices — same 1-based positions, 1 and 4 respectively)
## ─────────────────────────────────────────────────────────────
...
scen1_nodrug <- ev(time = 0, cmt = "GUT_FXR", amt = 0)  # dummy event

# Scenario 2: FXR Agonist (OCA 25mg/day)
# OCA 25mg daily: ~0.25 µg/mL Cp at steady state
scen2_oca <- ev(time = 0, cmt = "GUT_FXR", amt = 25, ii = 24, addl = 729)

# Scenario 3: GLP-1 RA (Semaglutide 2.4mg/week sc)
# Weekly subcutaneous dose, ka slow (half-life ~7d)
scen3_sema <- ev(time = 0, cmt = "GUT_GLP1", amt = 2.4, ii = 168, addl = 103)

# Scenario 4: Combination FXR + GLP-1 RA
scen4_combo <- ev(time = 0, cmt = "GUT_FXR", amt = 25, ii = 24, addl = 729) +
               ev(time = 0, cmt = "GUT_GLP1", amt = 2.4, ii = 168, addl = 103)

# Scenario 5: Resmetirom (THRβ agonist, 80mg/day)
# Approximate PK: ~0.8 µg/mL; use FXR agonist PK but update Emax params
# (unchanged from the original: resmetirom has no PK block of its own in this
# file — it is simulated by reusing the FXR/OCA depot with overridden FXR
# Emax/EC50 parameters. See refactor notes.)
scen5_resmet <- ev(time = 0, cmt = "GUT_FXR", amt = 80, ii = 24, addl = 729) %>%
  mrgsolve::ev() %>%
  {
