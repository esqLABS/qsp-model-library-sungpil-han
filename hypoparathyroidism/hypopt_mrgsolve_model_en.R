...
##
##  Calibration anchors (see hypopt_references_en.md for the citations)
...
HYPOPT_simulate_scenarios <- function(mod = HYPOPT_build(), which = 1:15) {
  ...
  S[[1]] <- function() run("01 Healthy control", P_HEALTHY)
  ...
  S[[2]] <- function() run("02 Post-surgical HypoPT — untreated", P_SURG)
  ...
  S[[3]] <- function() run(
    "03 Conventional therapy (Ca 1000 mg/day + calcitriol 0.5 ug/day)",
  ...
  S[[4]] <- function() run(
    "04 Conventional therapy + thiazide (calcium sparing: Ca 500 mg/day + HCTZ 25 mg BID)",
  ...
  S[[5]] <- function() run(
    "05 Over-treatment (Ca 3000 mg/day + calcitriol 1.0 ug/day)",
  ...
  S[[6]] <- function() run(
    "06 rhPTH(1-84) 50→100 ug SC QD + conventional therapy tapered",
  ...
  S[[8]] <- function() run(
    "08 Palopegteriparatide 18 ug QD (conventional therapy stopped)",
  ...
  S[[9]] <- function() run(
    "09 Palopegteriparatide 30 ug QD (dose escalation)",
    utils::modifyList(c(P_SURG, HYPOPT_pth_params$transcon),
                      list(DCASUP = 0, DCTR = 0)),
    base_pars = P_SURG,
  ...
  S[[10]] <- function() run("10 ADH1 (CASR gain of function) — untreated", P_ADH1)
  ...
  S[[12]] <- function() run(
    "12 Hypomagnesaemic functional HypoPT → Mg repletion (30 days)",
  ...
  S[[13]] <- function() run(
    "13 Acute hypocalcaemic crisis — IV calcium gluconate",
  ...
  S[[14]] <- function() run(
    "14 Long term 10 years — conventional therapy (renal outcomes)",
    utils::modifyList(P_SURG, list(DCASUP = 1000, DCTR = 0.5)),
    base_pars = P_SURG,
  ...
  S[[15]] <- function() run(
    "15 Long term 10 years — palopegteriparatide (renal outcomes)",
    utils::modifyList(c(P_SURG, HYPOPT_pth_params$transcon),
                      list(DCASUP = 0, DCTR = 0)),
    base_pars = P_SURG,
