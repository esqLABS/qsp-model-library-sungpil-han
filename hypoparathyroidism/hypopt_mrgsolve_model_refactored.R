...
// ------------------------- thiazide ---------------------------------------
// PK/PD refactor (see hypopt_refactor_notes.md): renamed to the
// C_<STEM>/EFFECT_<STEM> convention; archetype 3 without a peripheral
// compartment (depot + central, linear elimination). Values unchanged from
// the original KATHZ/FTHZ/VTHZ/CLTHZ/EMXTHZ/EC50THZ.
KA_THZ   :   1.20  : Hydrochlorothiazide absorption rate constant (1/h)
F_THZ    :   0.70  : Hydrochlorothiazide bioavailability
V1_THZ   : 200.0   : Hydrochlorothiazide V/F (L)
CL_THZ   :  20.0   : Hydrochlorothiazide CL/F (L/h)
EMAX_THZ :   0.012 : Maximal thiazide increment in fractional Ca reabsorption
EC50_THZ :  25.0   : Thiazide EC50 (ng/mL)
GAMMA_THZ:   1.0   : Thiazide Hill coefficient (no exponent in original -> 1)

// ------------------------- encaleret (CaSR NAM) ---------------------------
// PK/PD refactor (see hypopt_refactor_notes.md): renamed to the
// C_<STEM>/EFFECT_<STEM> convention; archetype 3 without a peripheral
// compartment. Encaleret genuinely has TWO distinct pharmacological actions
// on the same concentration (parathyroid CaSR set-point shift and TAL
// fractional Ca-reabsorption increment), so it gets two named effect terms,
// EFFECT_ENC_SETPT and EFFECT_ENC_RENAL, sharing one C_ENC and one EC50_ENC
// (both potencies were already identical in the original). Values unchanged
// from the original KAENC/FENC/VENC/CLENC/EMXENC/EMXENCK/EC50ENC.
KA_ENC   :   1.00  : Encaleret absorption rate constant (1/h)
F_ENC    :   0.60  : Encaleret bioavailability
V1_ENC   : 300.0   : Encaleret V/F (L)
CL_ENC   :  30.0   : Encaleret CL/F (L/h)
EMAX_ENC_SETPT :   0.40  : Maximal rightward shift of the CaSR set-point (fractional)
EMAX_ENC_RENAL :   0.022 : Maximal renal (TAL) increment in fractional Ca reabsorption
EC50_ENC : 100.0   : Encaleret EC50 (ng/mL), shared by both effect sites
GAMMA_ENC:   1.0   : Encaleret Hill coefficient (no exponent in original -> 1)
...
GUT_THZ  : Hydrochlorothiazide in gut (mg)
CENT_THZ : Hydrochlorothiazide, central (mg)
GUT_ENC  : Encaleret in gut (mg)
CENT_ENC : Encaleret, central (mg)
...
// EFFECT_ENC_SETPT is computed in $ODE/$TABLE (see thiazide/encaleret PK
// block); this macro just reads it, same as it read the old CENC-based
// ratio inline before the refactor.
#define SETPTE   (SETPT*(1.0 + EFFECT_ENC_SETPT))
...
// ---- renal calcium
// EFFECT_THZ and EFFECT_ENC_RENAL are computed in $ODE/$TABLE (see
// thiazide/encaleret PK block); this macro just reads them, same as it
// read the old FRTHZM/FRENCM ratios inline before the refactor.
#define FRRAW    (FR0 + FRPTH*EPCA + EFFECT_THZ + EFFECT_ENC_RENAL - CASROFF - CASRSL*CASRGN*(CAIONX - CAION0))
...
dxdt_GUT_THZ  = -KA_THZ*GUT_THZ;
dxdt_CENT_THZ =  KA_THZ*GUT_THZ - (CL_THZ/V1_THZ)*CENT_THZ;
C_THZ      = CENT_THZ/V1_THZ*1000.0;
EFFECT_THZ = EMAX_THZ*pow(C_THZ, GAMMA_THZ)/(pow(EC50_THZ, GAMMA_THZ) + pow(C_THZ, GAMMA_THZ));

// ------------------------------------------------------------- encaleret (PK/PD refactor)
// Own clearly-delimited PK block (archetype 3, no peripheral compartment).
// One concentration (C_ENC), two named effect terms because encaleret has
// two distinct real actions on this concentration -- the parathyroid CaSR
// set-point shift (consumed by SETPTE, used only in dxdt_PTHE below) and the
// TAL fractional Ca-reabsorption increment (consumed by FRRAW/FRCA, used by
// dxdt_CAE next) -- sharing one EC50/GAMMA, exactly as the original did.
dxdt_GUT_ENC  = -KA_ENC*GUT_ENC;
dxdt_CENT_ENC =  KA_ENC*GUT_ENC - (CL_ENC/V1_ENC)*CENT_ENC;
C_ENC            = CENT_ENC/V1_ENC*1000.0;
EFFECT_ENC_SETPT = EMAX_ENC_SETPT*pow(C_ENC, GAMMA_ENC)/(pow(EC50_ENC, GAMMA_ENC) + pow(C_ENC, GAMMA_ENC));
EFFECT_ENC_RENAL = EMAX_ENC_RENAL*pow(C_ENC, GAMMA_ENC)/(pow(EC50_ENC, GAMMA_ENC) + pow(C_ENC, GAMMA_ENC));
...
double EFFECT_THZ       = EMAX_THZ*pow(C_THZ, GAMMA_THZ)/(pow(EC50_THZ, GAMMA_THZ) + pow(C_THZ, GAMMA_THZ));
double C_ENC            = CENT_ENC/V1_ENC*1000.0;
double EFFECT_ENC_SETPT = EMAX_ENC_SETPT*pow(C_ENC, GAMMA_ENC)/(pow(EC50_ENC, GAMMA_ENC) + pow(C_ENC, GAMMA_ENC));
double EFFECT_ENC_RENAL = EMAX_ENC_RENAL*pow(C_ENC, GAMMA_ENC)/(pow(EC50_ENC, GAMMA_ENC) + pow(C_ENC, GAMMA_ENC));
...
$CAPTURE @annotated
C_THZ   : Hydrochlorothiazide concentration (ng/mL) -- pluggable PK/PD hook
EFFECT_THZ : Thiazide effect on fractional tubular Ca reabsorption (0-EMAX_THZ)
C_ENC   : Encaleret concentration (ng/mL) -- pluggable PK/PD hook
EFFECT_ENC_SETPT : Encaleret effect, fractional CaSR set-point shift (0-EMAX_ENC_SETPT)
EFFECT_ENC_RENAL : Encaleret effect, fractional TAL Ca-reabsorption increment (0-EMAX_ENC_RENAL)
...
hypopt_thiazide <- function(mg_per_dose = 25, ii = 12, days = 180, start = 0) {
  mrgsolve::ev(time = start, amt = mg_per_dose, cmt = "GUT_THZ",
               ii = ii, addl = hypopt_addl(days, ii))
}
...
hypopt_encaleret <- function(mg_per_dose = 60, ii = 12, days = 180, start = 0) {
  mrgsolve::ev(time = start, amt = mg_per_dose, cmt = "GUT_ENC",
               ii = ii, addl = hypopt_addl(days, ii))
}
