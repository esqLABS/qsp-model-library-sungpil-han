...
| Deliverable | File |
|---|---|
| 🗺️ Mechanistic map (DOT, 20 clusters · 200 nodes · 272 edges) | [`td_qsp_model_en.dot`](td_qsp_model_en.dot) |
| 🖼️ Map (SVG) | [`td_qsp_model_en.svg`](td_qsp_model_en.svg) |
| 🖼️ Map (PNG, 150 dpi) | [`td_qsp_model_en.png`](td_qsp_model_en.png) |
| ⚙️ mrgsolve ODE model (40 ODEs · 10 scenarios · 11 analysis functions) | [`td_mrgsolve_model_en.R`](td_mrgsolve_model_en.R) |
| 📊 Shiny dashboard (9 tabs) | [`td_shiny_app_en.R`](td_shiny_app_en.R) |
| 🔬 Independent implementation for verification (numpy/scipy) | [`td_reference_check_en.py`](td_reference_check_en.py) |
| 📚 References (72 items, PubMed links) | [`td_references_en.md`](td_references_en.md) |
...
[`td_reference_check_en.py`](td_reference_check_en.py), an independent implementation
...
dot -Tsvg td_qsp_model_en.dot -o td_qsp_model_en.svg
dot -Tpng -Gdpi=150 td_qsp_model_en.dot -o td_qsp_model_en.png

# 2) mrgsolve model — runs all 11 analyses and prints the tables
Rscript td_mrgsolve_model_en.R

# 3) Shiny dashboard (9 tabs)
R -e 'shiny::runApp("td_shiny_app_en.R", port = 8080)'

# 4) reproduce the same figures without R / mrgsolve (only numpy + scipy needed)
python3 td_reference_check_en.py          # everything
python3 td_reference_check_en.py --quick  # scenarios only
```

`td_reference_check_en.py` is an independent implementation that transcribes the
`$ODE` block of `td_mrgsolve_model_en.R` as it stands, and it regenerates every
