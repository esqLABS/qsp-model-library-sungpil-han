...
that went into `op_qsp_model_en.dot` (the mechanistic map), `op_mrgsolve_model_en.R` (the 51-state ODE
model), `op_reference_model.py` (the Python re-implementation used for verification) and
`op_shiny_app_en.R` (the dashboard). The brackets at the end of each entry show which number in the
