...
IL6_SS0  = 12      // baseline IL-6 composite cytokine [pg/mL]
...
// Archetype 2 (two-compartment, linear, bidirectional Q/V exchange),
// with a bespoke twist: PD reads the peripheral/tissue pool (M6P-receptor
// cellular uptake site), not the central plasma compartment -- see notes.
...
KM6P     = 0.006      // M6P receptor Km [U/mL] -- unused in original $ODE, preserved as-is (see notes)
UPTK_ERT = 0.45       // tissue uptake rate [1/h] -- unused in original $ODE, preserved as-is (see notes)
EMAX_ERT = 0.85       // maximal GBA activity restoration [fraction]
EC50_ERT = 0.6        // EC50 for GBA restoration [U/mL in macrophage]
GAMMA_ERT = 1         // Hill coefficient (original had no explicit Hill term)
...
// Archetype 3 minus peripheral compartment (oral depot + central, linear).
KA_ELIS  = 0.80       // absorption rate [1/h]
F_ELIS   = 0.20       // bioavailability (EM)
CL_ELIS  = 38.0       // clearance [L/h]
V1_ELIS  = 106        // volume of distribution [L]
EC50_ELIS = 0.010     // GCS EC50 [μg/mL]
EMAX_ELIS = 0.95      // max GCS inhibition
GAMMA_ELIS = 1        // Hill coefficient (original had no explicit Hill term)
...
V1_MIGS  = 28         // volume [L]
EC50_MIGS = 50.0      // GCS EC50 [μg/mL] (low selectivity)
EMAX_MIGS = 0.80      // max inhibition
GAMMA_MIGS = 1        // Hill coefficient (original had no explicit Hill term)
...
V1_VENG  = 480        // high Vd (CNS penetration) [L]
Kp_CNS   = 0.30       // brain/plasma ratio -- unused in original $ODE, preserved as-is (see notes)
EC50_VENG = 0.015     // GCS EC50 (lower than miglustat)
EMAX_VENG = 0.92
GAMMA_VENG = 1        // Hill coefficient (original had no explicit Hill term)
...
$CMT
// ERT
CENT_ERT   // ERT central plasma [U/kg]
PERI_ERT   // ERT tissue / macrophage

// Eliglustat
GUT_ELIS   // eliglustat gut
CENT_ELIS  // eliglustat plasma [μg/mL]

// Miglustat
GUT_MIGS   // miglustat gut
CENT_MIGS  // miglustat plasma [μg/mL]

// Venglustat
GUT_VENG   // venglustat gut
CENT_VENG  // venglustat plasma [μg/mL]
...
$MAIN
CENT_ERT_0 = 0;    PERI_ERT_0 = 0;
GUT_ELIS_0 = 0;    CENT_ELIS_0 = 0;
GUT_MIGS_0 = 0;    CENT_MIGS_0 = 0;
GUT_VENG_0 = 0;    CENT_VENG_0 = 0;
GBA_0      = 5;
GC_MAC_0   = 100;
GC_SP_0    = 100;
GC_LV_0    = 100;
...
$ODE
// ────────────────────────────────────────────────────────────
// ERT PK (Archetype 2: two-compartment, linear, bidirectional Q/V)
// ────────────────────────────────────────────────────────────
double k10_ERT = (CL_ERT * BW) / (V1_ERT * BW);
double k12_ERT = Q_ERT / (V1_ERT * BW);
double k21_ERT = Q_ERT / (V2_ERT * BW);

dxdt_CENT_ERT = -(k10_ERT + k12_ERT) * CENT_ERT + k21_ERT * PERI_ERT;
dxdt_PERI_ERT = k12_ERT * CENT_ERT - k21_ERT * PERI_ERT;

// ────────────────────────────────────────────────────────────
// Eliglustat PK (Archetype 3 minus peripheral: oral depot + central, linear)
// ────────────────────────────────────────────────────────────
dxdt_GUT_ELIS  = -KA_ELIS * GUT_ELIS;
dxdt_CENT_ELIS = KA_ELIS * GUT_ELIS * F_ELIS / V1_ELIS - (CL_ELIS / V1_ELIS) * CENT_ELIS;

// ────────────────────────────────────────────────────────────
// Miglustat PK (Archetype 3 minus peripheral)
// ────────────────────────────────────────────────────────────
dxdt_GUT_MIGS  = -KA_MIGS * GUT_MIGS;
dxdt_CENT_MIGS = KA_MIGS * GUT_MIGS * F_MIGS / V1_MIGS - (CL_MIGS / V1_MIGS) * CENT_MIGS;

// ────────────────────────────────────────────────────────────
// Venglustat PK (Archetype 3 minus peripheral)
// ────────────────────────────────────────────────────────────
dxdt_GUT_VENG  = -KA_VENG * GUT_VENG;
dxdt_CENT_VENG = KA_VENG * GUT_VENG * F_VENG / V1_VENG - (CL_VENG / V1_VENG) * CENT_VENG;
...
// concentration directly (no volume division needed downstream), same
// quirk documented for CENT_NIM/TOCI_C/CENT_GEN elsewhere in this corpus.
double C_ELIS = CENT_ELIS;
double C_MIGS = CENT_MIGS;
double C_VENG = CENT_VENG;

// ────────────────────────────────────────────────────────────
// Drug effects on GBA and GCS
// ────────────────────────────────────────────────────────────
// ERT effect: restores GBA activity via M6P-receptor uptake
double EFFECT_ERT = USE_ERT * EMAX_ERT * VELA_MOD * pow(C_ERT, GAMMA_ERT) / (pow(EC50_ERT, GAMMA_ERT) + pow(C_ERT, GAMMA_ERT));

// SRT effects: inhibit GCS → reduce substrate synthesis
double EFFECT_ELIS = USE_ELIS * EMAX_ELIS * pow(C_ELIS, GAMMA_ELIS) / (pow(EC50_ELIS, GAMMA_ELIS) + pow(C_ELIS, GAMMA_ELIS));
double EFFECT_MIGS = USE_MIGS * EMAX_MIGS * pow(C_MIGS, GAMMA_MIGS) / (pow(EC50_MIGS, GAMMA_MIGS) + pow(C_MIGS, GAMMA_MIGS));
double EFFECT_VENG = USE_VENG * EMAX_VENG * pow(C_VENG, GAMMA_VENG) / (pow(EC50_VENG, GAMMA_VENG) + pow(C_VENG, GAMMA_VENG));
double SRT_inh  = 1.0 - (1.0 - EFFECT_ELIS) * (1.0 - EFFECT_MIGS) * (1.0 - EFFECT_VENG);

// Effective GBA: baseline mutant activity + ERT restoration
double GBA_eff = GBA + EFFECT_ERT * (30.0 - GBA); // 30 = normal GBA

// ────────────────────────────────────────────────────────────
// GBA enzyme dynamics
// ────────────────────────────────────────────────────────────
dxdt_GBA = k_gba_syn - k_gba_deg * GBA + EFFECT_ERT * 0.005;

// ────────────────────────────────────────────────────────────
// Glucocerebroside dynamics
// ────────────────────────────────────────────────────────────
double GCS_activity = k_gc_syn * (1.0 - SRT_inh);
double GC_clearance = (k_gc_deg * GBA_eff / GBA0 + EFFECT_ERT * 0.025);
...
double ERT_Cplasma = CENT_ERT * BW;           // U total (informational/diagnostic only, not read by any PD equation)
double SRT_GCS_inh = SRT_inh * 100.0;        // % GCS inhibition

// NOTE (build-compatibility fix, see refactor notes): mrgsolve 2.0.1
// rejects a $CMT compartment name listed directly in $CAPTURE
// ("compartment should not be in $CAPTURE") -- every state
// (GBA, GC_MAC, ..., CENT_ERT, PERI_ERT, CENT_ELIS, CENT_MIGS, CENT_VENG,
// GUT_ELIS, GUT_MIGS, GUT_VENG, etc.) is captured automatically by
// mrgsolve without being listed here; only the $TABLE-derived doubles
// and the new named C_<STEM>/EFFECT_<STEM> covariates need to be listed.
$CAPTURE
GC_TOTAL GBA_PCT_NORMAL ERT_Cplasma SRT_GCS_inh
C_ERT EFFECT_ERT C_ELIS EFFECT_ELIS C_MIGS EFFECT_MIGS C_VENG EFFECT_VENG
'
...
make_ert_doses <- function(bw = 70, dose_ukg = 60, n_doses = 52, interval_h = 336) {
  ...
  ev(cmt = "CENT_ERT", amt = dose_ukg * bw / (0.18 * bw), # normalised to conc
...
scenarios <- list(
  ...
  S4_Eliglustat_EM = list(
    ...
    events = make_oral_doses("GUT_ELIS", 84, 12, 730)
  ),
  S5_Eliglustat_PM = list(
    ...
    events = make_oral_doses("GUT_ELIS", 84, 24, 730)
  ),
  S6_CombinationLowERT_ELIS = list(
    ...
    events = c(make_ert_doses(70, 30, 52, 336),
               make_oral_doses("GUT_ELIS", 84, 12, 730))
  )
)
...
vp_params <- tibble(
  ...
  GL1_SS0  = pmax(rnorm(n_vp, 8.5, 2.5), 1),  # renamed from GL1_0, see $PARAM note above (dead param, no effect on simulation)
...
run_vp <- function(id, vp_row, sc_param, events) {
  ...
  init(m, list(
    ...
    GL1 = vp_row$GL1_SS0, SV = vp_row$SV0,
