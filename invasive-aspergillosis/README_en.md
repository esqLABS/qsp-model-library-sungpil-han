...
| File | Contents |
|---|---|
| [`ipa_qsp_model.dot`](ipa_qsp_model.dot) · [SVG](ipa_qsp_model.svg) · [PNG](ipa_qsp_model.png) | Mechanistic map — 171 nodes, 17 clusters, 259 edges |
| [`ipa_mrgsolve_model.R`](ipa_mrgsolve_model.R) | mrgsolve ODE model (53 compartments, 5 antifungals, 2 concomitant drugs) + 23 scenarios |
| [`ipa_shiny_app_en.R`](ipa_shiny_app_en.R) | Shiny dashboard (11 tabs) |
| [`ipa_references_en.md`](ipa_references_en.md) | 99 references — every PMID looked up and verified through NCBI E-utilities |
| [`ipa_reference_model.py`](ipa_reference_model.py) | Independent Python/scipy re-implementation — for verification |
| [`ipa_reference_output.txt`](ipa_reference_output.txt) · [`ipa_scenario_results.json`](ipa_scenario_results.json) | The computed output behind every number below |
...
# Dashboard
R -e 'shiny::runApp("ipa_shiny_app_en.R")'
```
...
- **`PERF` could not be calibrated against human data.** The evidence is set out
  in the last section of [`ipa_references_en.md`](ipa_references_en.md),
