# =============================================================================
# acute-pancreatitis/ap_mrgsolve_model_refactored.R
# -----------------------------------------------------------------------------
# PK/PD REFACTOR of ap_mrgsolve_model.R -- pluggable-PK rewrite per
# FORK_WORKFLOW_GUIDE.md Part 2. Original file is untouched; this is a new
# sibling. Compounds refactored: indomethacin (IND), octreotide (OCT),
# gabexate (GAB), nafamostat (NAF), ulinastatin (ULI), meropenem (MER),
# anakinra (AKR), fentanyl (FEN) -- the eight rows in
# driver-patches/data/compound_perturbation_census.md classified
# "Redirect concentration inside #define macro" for acute-pancreatitis. Every
# disease-side compartment/equation is byte-for-byte identical to
# ap_mrgsolve_model.R; only each compound's own PK compartment names, its
# exposed concentration, and its Hill effect term were pulled out of #define
# macros and renamed to this fork's convention (CENT_<STEM>/GUT_<STEM>/
# C_<STEM>/EFFECT_<STEM>/EMAX_<STEM>/EC50_<STEM>/GAMMA_<STEM>). See
# ap_refactor_notes.md for full detail, including a pre-existing upstream
# $CAPTURE build defect (logged as UPSTREAM_ISSUES.md #101) fixed
# syntax-only in this sibling.
# -----------------------------------------------------------------------------
...
EC50_IND    : 1.2   : EC50 for NF-κB / COX inhibition (mg/L)
GAMMA_IND   : 1     : Hill coefficient (no exponent in original -> 1)
...
EC50_OCT    : 1.5   : EC50 for secretion / cytokine inhibition (ng/mL)
GAMMA_OCT   : 1     : Hill coefficient (no exponent in original -> 1)

// Gabexate -- archetype 1 (single compartment, linear)
CL_GAB      : 30.0  : CL (L/h)
V1_GAB      : 20.0  : Central volume of distribution (L)
EMAX_GAB    : 1     : Max fractional protease-inhibition effect (no explicit Emax in original -> 1)
EC50_GAB    : 3.0   : EC50 for protease inhibition (mg/L)
GAMMA_GAB   : 1     : Hill coefficient (no exponent in original -> 1)

// Nafamostat -- archetype 1 (single compartment, linear)
CL_NAF      : 60.0  : CL (L/h)
V1_NAF      : 16.0  : Central volume of distribution (L)
EMAX_NAF    : 1     : Max fractional effect (no explicit Emax in original -> 1)
EC50_NAF    : 0.5   : EC50 (mg/L)
GAMMA_NAF   : 1     : Hill coefficient (no exponent in original -> 1)

// Ulinastatin -- archetype 1 (single compartment, linear)
CL_ULI      : 5.0   : CL (L/h)
V1_ULI      : 8.0   : Central volume of distribution (L)
EMAX_ULI    : 1     : Max fractional effect (no explicit Emax in original -> 1)
EC50_ULI    : 50.0  : EC50 (U/L scaled)
GAMMA_ULI   : 1     : Hill coefficient (no exponent in original -> 1)

// Meropenem -- archetype 1 (single compartment, linear)
CL_MER      : 12.0  : CL (L/h)
V1_MER      : 18.0  : Central volume of distribution (L)
EMAX_MER    : 1     : Max fractional bactericidal effect (no explicit Emax in original -> 1)
EC50_MER    : 8.0   : EC50 for bactericidal effect (mg/L)
GAMMA_MER   : 1     : Hill coefficient (no exponent in original -> 1)
...
EC50_AKR    : 1.0   : Competitive inhibitor constant vs IL-1, used as EC50 (mg/L) (renamed from AKR_KI)
GAMMA_AKR   : 1     : Hill coefficient (no exponent in original -> 1)

// Fentanyl PCA -- archetype 1 (single compartment, linear)
CL_FEN      : 50.0  : CL (L/h)
V1_FEN      : 250   : Central volume of distribution (L)
EMAX_FEN    : 1     : Max fractional analgesic effect (no explicit Emax in original -> 1)
EC50_FEN    : 1.5   : Analgesia EC50 (ng/mL)
GAMMA_FEN   : 1     : Hill coefficient (no exponent in original -> 1)

$CMT @annotated
// PK depots / centrals (11 compartments)
GUT_IND      : Indomethacin PR depot (mg)
CENT_IND     : Indomethacin central amount (mg)
CENT_OCT     : Octreotide central amount (mg)
GUT_OCT      : Octreotide SC depot (mg)
CENT_GAB     : Gabexate central amount (mg)
CENT_NAF     : Nafamostat central amount (mg)
CENT_ULI     : Ulinastatin central amount (U)
CENT_MER     : Meropenem central amount (mg)
GUT_AKR      : Anakinra SC depot (mg)
CENT_AKR     : Anakinra central amount (mg)
CENT_FEN     : Fentanyl central amount (mg)
...
$ODE
// ================= PK (8 compounds, each own block) =================
// ---- Indomethacin (archetype 3 minus peripheral: depot + central) ----
dxdt_GUT_IND  = -KA_IND*GUT_IND;
dxdt_CENT_IND =  KA_IND*GUT_IND - (CL_IND/V1_IND)*CENT_IND;
double C_IND = CENT_IND/V1_IND;
double EFFECT_IND = EMAX_IND*pow(C_IND, GAMMA_IND)
                    / (pow(EC50_IND, GAMMA_IND) + pow(C_IND, GAMMA_IND));

// ---- Octreotide (archetype 3 minus peripheral: depot + central) ----
dxdt_GUT_OCT  = -KA_OCT*GUT_OCT;
dxdt_CENT_OCT =  KA_OCT*GUT_OCT - (CL_OCT/V1_OCT)*CENT_OCT;
double C_OCT = CENT_OCT/V1_OCT;
double EFFECT_OCT = EMAX_OCT*pow(C_OCT, GAMMA_OCT)
                    / (pow(EC50_OCT, GAMMA_OCT) + pow(C_OCT, GAMMA_OCT));

// ---- Gabexate (archetype 1: single compartment, linear) ----
dxdt_CENT_GAB = -(CL_GAB/V1_GAB)*CENT_GAB;
double C_GAB = CENT_GAB/V1_GAB;
double EFFECT_GAB = EMAX_GAB*pow(C_GAB, GAMMA_GAB)
                    / (pow(EC50_GAB, GAMMA_GAB) + pow(C_GAB, GAMMA_GAB));

// ---- Nafamostat (archetype 1: single compartment, linear) ----
dxdt_CENT_NAF = -(CL_NAF/V1_NAF)*CENT_NAF;
double C_NAF = CENT_NAF/V1_NAF;
double EFFECT_NAF = EMAX_NAF*pow(C_NAF, GAMMA_NAF)
                    / (pow(EC50_NAF, GAMMA_NAF) + pow(C_NAF, GAMMA_NAF));

// ---- Ulinastatin (archetype 1: single compartment, linear) ----
dxdt_CENT_ULI = -(CL_ULI/V1_ULI)*CENT_ULI;
double C_ULI = CENT_ULI/V1_ULI;
double EFFECT_ULI = EMAX_ULI*pow(C_ULI, GAMMA_ULI)
                    / (pow(EC50_ULI, GAMMA_ULI) + pow(C_ULI, GAMMA_ULI));

// ---- Meropenem (archetype 1: single compartment, linear) ----
dxdt_CENT_MER = -(CL_MER/V1_MER)*CENT_MER;
double C_MER = CENT_MER/V1_MER;
double EFFECT_MER = EMAX_MER*pow(C_MER, GAMMA_MER)
                    / (pow(EC50_MER, GAMMA_MER) + pow(C_MER, GAMMA_MER));

// ---- Anakinra (archetype 3 minus peripheral: depot + central) ----
dxdt_GUT_AKR  = -KA_AKR*GUT_AKR;
dxdt_CENT_AKR =  KA_AKR*GUT_AKR - (CL_AKR/V1_AKR)*CENT_AKR;
double C_AKR = CENT_AKR/V1_AKR;
double EFFECT_AKR = EMAX_AKR*pow(C_AKR, GAMMA_AKR)
                    / (pow(EC50_AKR, GAMMA_AKR) + pow(C_AKR, GAMMA_AKR));

// ---- Fentanyl (archetype 1: single compartment, linear) ----
dxdt_CENT_FEN = -(CL_FEN/V1_FEN)*CENT_FEN;
double C_FEN = CENT_FEN/V1_FEN;
double EFFECT_FEN = EMAX_FEN*pow(C_FEN, GAMMA_FEN)
                    / (pow(EC50_FEN, GAMMA_FEN) + pow(C_FEN, GAMMA_FEN));

// ================= DISEASE (unchanged apart from EFF_* -> EFFECT_*) =================
...
double act = kTRACT*TRYPG*Ca*etio_drive*prss1_mult
           + kTAUTO*TRYP*TRYPG
           - (1.0 - EFFECT_GAB) * (1.0 - EFFECT_NAF) * (1.0 - EFFECT_ULI) * 0.0; // protease inhibitors block
double inh = (kSPINK*spink1_loss + kCTRC) * TRYP
           * (1.0 + 5.0*EFFECT_GAB + 5.0*EFFECT_NAF + 3.0*EFFECT_ULI);
...
// ----- NF-κB & cytokines -----
double nfkb_drive = kNFkB_g*(0.5*TRYP + DAMP + 0.5*BT) * (1.0 - 0.7*EFFECT_IND - 0.5*EFFECT_NAF);
dxdt_NFKB  = nfkb_drive - kNFkB_d*NFKB;

dxdt_TNF   = kTNF_g*NFKB*(1.0 - 0.4*EFFECT_OCT) - kTNF_clr*TNF;
double il1_drive = kIL1_g*NFKB*(1.0 - 0.4*EFFECT_IND);
dxdt_IL1   = il1_drive - kIL1_clr*IL1*(1.0 + 3.0*EFFECT_AKR);
...
dxdt_PERM  = perm_drive*(1.0 - 0.4*EFFECT_OCT - 0.5*EFFECT_ULI) - kPerm_clr*PERM;

// ----- Gut barrier -----
double gut_protect = (ENteral > 0.5) ? EN_eff : 0.0;
dxdt_GUT   = kGut_g*PERM*(1.0 - gut_protect) - kGut_clr*GUT;
dxdt_BT    = kBT_g*GUT - kBT_clr*BT*(1.0 + 5.0*EFFECT_MER);
...
dxdt_VAS   = pain_drive*(1.0 - 0.7*EFFECT_FEN - 0.3*EFFECT_IND) - 0.05*VAS;
...
$CAPTURE
C_IND EFFECT_IND C_OCT EFFECT_OCT C_GAB EFFECT_GAB C_NAF EFFECT_NAF
C_ULI EFFECT_ULI C_MER EFFECT_MER C_AKR EFFECT_AKR C_FEN EFFECT_FEN
SURVPROB BISAP
'

# ===== BUILD MODEL =====
ap_model <- mcode("acute_pancreatitis_refactored", ap_code)
...
# NOTE: compartment names below are the refactored ones (GUT_IND/GUT_OCT/
# GUT_AKR were DEPOT_IND/DEPOT_OCT/DEPOT_AKR in the original; CENT_OCT/
# CENT_GAB/CENT_NAF/CENT_ULI/CENT_MER/CENT_FEN were C_OCT/C_GAB/C_NAF/C_ULI/
# C_MER/C_FEN in the original) -- same 1-based compartment index in both
# files, this is a rename of the dosing target only, not a behavioral change.
...
ev_indomethacin_PR  <- function() ev(time = 0, amt = 100, cmt = "GUT_IND", ID = 1)
ev_octreotide_SC    <- function() {
  # 100 µg SC q8h × 14 d -> 0.1 mg
  ev(time = seq(0, TEND - 8, by = 8), amt = 0.1, cmt = "GUT_OCT", ID = 1)
}
ev_octreotide_IV    <- function() {
  # 50 µg/h continuous IV (approx via q1h)
  ev(time = seq(0, TEND - 1, by = 1), amt = 0.05, cmt = "CENT_OCT", ID = 1)
}
ev_gabexate         <- function() {
  # 2400 mg/d divided q6h -> 600 mg q6h IV
  ev(time = seq(0, TEND - 6, by = 6), amt = 600, cmt = "CENT_GAB", ID = 1)
}
ev_nafamostat       <- function() {
  # 40 mg/d as 24-h infusion -> 1.67 mg/h
  ev(time = seq(0, TEND - 1, by = 1), amt = 1.67, cmt = "CENT_NAF", ID = 1)
}
ev_ulinastatin      <- function() {
  # 200 000 U IV q8h
  ev(time = seq(0, TEND - 8, by = 8), amt = 2e5, cmt = "CENT_ULI", ID = 1)
}
ev_meropenem        <- function() {
  # 1 g IV q8h × 14 d for infected necrosis
  ev(time = seq(0, TEND - 8, by = 8), amt = 1000, cmt = "CENT_MER", ID = 1)
}
ev_anakinra         <- function() {
  # 100 mg SC q24h × 14 d
  ev(time = seq(0, TEND - 24, by = 24), amt = 100, cmt = "GUT_AKR", ID = 1)
}
ev_fentanyl_PCA     <- function() {
  # 50 µg q1h PCA equivalent
  ev(time = seq(0, TEND - 1, by = 1), amt = 0.05, cmt = "CENT_FEN", ID = 1)
}
