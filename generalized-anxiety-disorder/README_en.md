...
| File | Contents |
|---|---|
| [`gad_qsp_model.dot`](gad_qsp_model.dot) · [`.svg`](gad_qsp_model.svg) · [`.png`](gad_qsp_model.png) | Mechanistic map — **206 nodes · 19 clusters · 301 edges** |
| [`gad_python_reference.py`](gad_python_reference.py) | **The runnable reference implementation** (49 ODEs). Every number in the README, in mrgsolve and in Shiny comes from here |
| [`gad_calibrate.py`](gad_calibrate.py) | Calibration. **Five parameters, six numbers.** Records even which attempt failed and why |
| [`gad_analysis.py`](gad_analysis.py) → [`gad_reference_output.txt`](gad_reference_output.txt) | The complete run of all 12 scenario blocks (472 lines) |
| [`gad_mrgsolve_model.R`](gad_mrgsolve_model.R) | mrgsolve model (50 compartments). The reference implementation carried across equation by equation |
| [`mkmrgsolve.py`](mkmrgsolve.py) | The generator for the R file above — makes it impossible for the calibrated values and the initial conditions to diverge |
| [`gad_shiny_app.R`](gad_shiny_app.R) | Shiny dashboard (**12 tabs**) |
| [`gad_references.md`](gad_references.md) · [`mkrefs.py`](mkrefs.py) | **118 references / 113 unique PMIDs**, all looked up live on PubMed |
...
docstring of [`gad_calibrate.py`](gad_calibrate.py)
and [`calib_stage1_joint.log`](calib_stage1_joint.log).
...
- **The mrgsolve port**: the initial conditions in `$MAIN` are a fit of the
  ...
  error of **5.7e-03** (SLEEPD). Because the generator ([`mkmrgsolve.py`](mkmrgsolve.py))
...
[`gad_mrgsolve_model.R`](gad_mrgsolve_model.R) and
[`gad_shiny_app.R`](gad_shiny_app.R) **have not been run.** The R files were
...
**Seventeen scenarios** are in [`gad_reference_output.txt`](gad_reference_output.txt):
...
[`gad_references.md`](gad_references.md) — **118 entries / 113 unique PMIDs**, in 20 sections.
Every title, journal, year, author and PMID was looked up live by [`mkrefs.py`](mkrefs.py) through the
