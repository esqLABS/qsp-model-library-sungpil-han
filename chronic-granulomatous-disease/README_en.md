...
| File | Contents |
|---|---|
| [`cgd_qsp_model_en.dot`](cgd_qsp_model_en.dot) · [`.svg`](cgd_qsp_model_en.svg) · [`.png`](cgd_qsp_model_en.png) | Mechanistic map. **152 nodes, 17 clusters, 228 edges** |
| [`cgd_kernel.py`](cgd_kernel.py) | **Layer 1** — the 16-state kernel of phagosomal oxidative chemistry. Calibrated on the normal neutrophil only |
| [`cgd_python_reference.py`](cgd_python_reference.py) | **Layers 2 and 3** — infectious focus + whole patient, **53 ODEs**. The implementation that was actually run |
| [`cgd_analysis.py`](cgd_analysis.py) | The complete calibration, verification and scenario runner (sections A–L) |
| [`cgd_reference_output.txt`](cgd_reference_output.txt) | The full output of that run. The source of every number in this README |
| [`cgd_calibration.json`](cgd_calibration.json) | The calibrated constants (machine-injected into the R file) |
| [`cgd_mrgsolve_model_en.R`](cgd_mrgsolve_model_en.R) | mrgsolve 53-ODE model (an equation-by-equation mirror of the Python implementation) |
| [`cgd_shiny_app_en.R`](cgd_shiny_app_en.R) | Shiny dashboard, **12 tabs** |
| [`sync_r_params.py`](sync_r_params.py) | Machine injection of the parameters from JSON into R (`--check` verifies against drift) |
| [`cgd_references_en.md`](cgd_references_en.md) | **95 papers**, all looked up live through the PubMed E-utilities |
| [`mkrefs_en.py`](mkrefs_en.py) | The reference generator |
...
dot -Tsvg cgd_qsp_model_en.dot -o cgd_qsp_model_en.svg
python3 mkrefs_en.py --refresh        # re-query the references
```
...
- **The autosomal recessive genotypes were not tested as separate entities.** The fact
  ...
  model cannot see it.
- **There was no R toolchain.** `cgd_mrgsolve_model_en.R` and `cgd_shiny_app_en.R` mirror
...
[`cgd_references_en.md`](cgd_references_en.md) — **95 papers**, in 12 sections. Not one of the
