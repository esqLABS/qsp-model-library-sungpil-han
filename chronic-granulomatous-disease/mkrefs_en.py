#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Build cgd_references_en.md from PubMed.
...
Q = [
 ...
 # ------------------------------------------------------------------- 6
 ("6. The catalase dogma in question",
 ...
 # ------------------------------------------------------------------- 8
 ("8. Why CGD inflames without an organism",
...
def main():
    ...
    out = []
    out.append("# Chronic Granulomatous Disease — references\n")
    ...
    out.append("**%d papers** organised into %d sections.\n"
               % (n_tot, len(secs)))
    out.extend(body)
    open("cgd_references_en.md", "w").write("".join(out))
    print("wrote cgd_references_en.md with %d resolved references" % n_tot)
