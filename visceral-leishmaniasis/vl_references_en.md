...
This list is the basis for the structural choices and parameters that went into
`vl_qsp_model_en.dot` (mechanistic map), `vl_mrgsolve_model_en.R` (73-ODE model), and `vl_reference_model.py` (independent verification implementation).

**Every entry is a real record retrieved directly from PubMed E-utilities** — the title, author,
journal, year, and PMID were carried over exactly as retrieved, and no citation was written from memory.
The retrieval script called esearch/esummary across 41 queries (by topic) and removed duplicates
to build the list below.
...
**Taken directly from the literature (sourced):** drug-specific plasma pharmacokinetic parameters (clearance,
volume of distribution, absorption rate, half-life), allometric exponents (0.75/1.0), the paediatric exposure gap for miltefosine,
the frequency of antimony resistance in Bihar, the clinical cure rate and adverse-event frequency of each regimen, the fact that IL-10 is a marker of
active VL, the fact that the leishmanin skin-test response reverts after treatment, and the fact that PKDL
appears after treatment.

**Model-level hypotheses, and NOT observations:**
the EC50 values in the intracellular macrophage compartment (calibrated parameters fitted to
the clinical cure rate, since free intracellular concentration cannot be measured), the MPS partition ratios for liposomal clearance (FSP_A/FLI_A/FBM_A/FSK_A),
the size of the quiescent population and its EC50 multiplier, all rate constants
of the TMEM, IL-10, and MACT modules, and the absolute position of the CD4-dependent separatrix. These exist
to make the structural claims testable, and the reported ratios, ordering, and windows are far
more robust than the absolute values.

---

## 1. Epidemiology, burden and the elimination programme
...
## 2. Vector, transmission and inoculum
...
## 3. The intracellular niche and macrophage biology
...
## 4. Immunopathogenesis: IL-10, Th1 and the priming race
...
## 5. Haematology, splenomegaly and HLH
...
## 6. Diagnosis, test of cure and the serology trap
...
## 7. Liposomal amphotericin B: PK and tissue targeting
...
## 8. Amphotericin B deoxycholate and nephrotoxicity
...
## 9. Miltefosine: PK, paediatric exposure and resistance
...
## 10. Paromomycin: efficacy, PK and ototoxicity
...
## 11. Pentavalent antimony: activation, resistance and cardiotoxicity
...
## 12. Combination and short-course therapy
...
## 13. HIV co-infection and relapse
...
## 16. PK-PD modelling and QSP methodology
...
## 17. Pipeline compounds and immunotherapy
...
## Guidelines and primary sources, consulted directly
...
- PubMed records: **223** (all retrieved directly, duplicates removed)
- Category sections: **17**
