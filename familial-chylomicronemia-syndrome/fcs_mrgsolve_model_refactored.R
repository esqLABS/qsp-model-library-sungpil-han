...
##  fcs_mrgsolve_model_refactored.R
## =============================================================================
##
##  PK/PD REFACTOR (per FORK_WORKFLOW_GUIDE.md Part 2)
##  ---------------------------------------------------------------------------
##  Refactored sibling of fcs_mrgsolve_model.R. The original is left completely
##  untouched; every disease-side equation (two-limb TRL clearance, pancreatitis
##  hazard, hepatic VLDL, endpoints, safety) is copied verbatim. Only the five
##  drugs' PK/PD blocks are reorganized into the guide's pluggable naming
##  convention (GUT_/CENT_/PERI_/LIV_/SYS_/C_/EFFECT_<STEM>):
##
##    VOL  volanesorsen  (naked 2'-MOE ApoC-III antisense oligonucleotide)
##    OLE  olezarsen     (GalNAc3-conjugated ApoC-III antisense oligonucleotide)
##    PLO  plozasiran    (GalNAc-conjugated ApoC-III siRNA)
##    EVI  evinacumab    (anti-ANGPTL3 monoclonal antibody)
##    FIB  fenofibrate   (oral PPAR-alpha agonist, fibrate class)
##
##  Every one of these is confirmed against the actual code and README, not
##  assumed from the census's parenthetical short codes (which name TARGETS,
##  not drugs): "ANGPTL3 (EVI)" is evinacumab; "ApoC-III (OLE)" is olezarsen;
##  "ApoC-III (VOL)" is volanesorsen -- a genuinely different, older ApoC-III
##  ASO from OLE, kept as its own scenario/arm, not a duplicate; "LPL protein
##  (FIB)" is fenofibrate (its target is the LPL protein pool, but the
##  compound itself is a real, approved oral small-molecule fibrate, not a
##  gene-therapy or protein construct). A fifth real compound, plozasiran
##  (PLO), is fully modeled here (its own SC-depot -> plasma -> hepatic-uptake
##  -> RISC-loading PK, dosed in FCS_scenario_plozasiran() and used in three
##  analysis functions) but has NO row at all in
##  driver-patches/data/compound_perturbation_census.md for this disease
##  directory -- a census omission, not a "this compound is out of scope"
##  decision. It is refactored alongside the other four for naming
##  consistency across the three ApoC-III-directed arms (VOL/OLE/PLO all
##  feed the same dxdt_APOC3_M term) and reported as a finding in
##  fcs_refactor_notes.md and as a new census row.
##
##  None of the five is TMDD. Evinacumab is a monoclonal antibody but the
##  original models its action as a plain algebraic neutralisation fraction
##  on ANGPTL3 (no free-receptor pool, no complex compartment, no
##  target-mediated elimination term) -- Archetype 2 (2-compartment linear
##  PK, no depot) with a plain Hill effect, not Archetype 4. Every compound's
##  Hill/effect term below is an exact rename of the original's algebra
##  (gamma = 1 where the original had no explicit Hill exponent), never a
##  refit. Volanesorsen, olezarsen and plozasiran have no plasma volume
##  parameter anywhere in the original -- their disease-facing effect is
##  driven directly off a hepatic-tissue (or RISC-loaded) compartment AMOUNT,
##  not a concentration -- so C_VOL/C_OLE/C_PLO below are identity renames of
##  that amount, never an invented concentration. Full per-compound archetype
##  classification, the complete renaming map, and the qspserver
##  mrgsolve_api verification results are in fcs_refactor_notes.md.
## =============================================================================
##
...
# Refactored: pluggable PK naming convention for VOL/OLE/PLO/EVI/FIB
# (FORK_WORKFLOW_GUIDE.md Part 2). Disease-side equations unchanged.
...
// ----------------------------------------------------------------- apoC-III
// VOL/OLE/PLO PK-derived effect fractions renamed to EMAX_/EC50_/GAMMA_<STEM>
// (per-stem split of the shared EMAX_ASO/EMAX_SI, all same original value,
// so each ApoC-III drug is independently driveable; see refactor notes).
...
EMAX_VOL  :    6.0    : Max fold-increase in mRNA degradation, volanesorsen (-) [was EMAX_ASO]
EC50_VOL  :  200      : Hepatic volanesorsen for half-max effect (mg)
GAMMA_VOL :    1      : Volanesorsen Hill coefficient (-) [new; original implicit 1]
EMAX_OLE  :    6.0    : Max fold-increase in mRNA degradation, olezarsen (-) [was EMAX_ASO]
EC50_OLE  :   88      : Hepatic olezarsen for half-max effect (mg)
GAMMA_OLE :    1      : Olezarsen Hill coefficient (-) [new; original implicit 1]
EMAX_PLO  :    6.0    : Max fold-increase in mRNA degradation, plozasiran (-) [was EMAX_SI]
EC50_PLO  :   18      : RISC-loaded plozasiran for half-max effect (AU) [was EC50_SI]
GAMMA_PLO :    1      : Plozasiran Hill coefficient (-) [new; original implicit 1]
EMAX_FIB_APOC3 : 0.20 : Fenofibrate suppression of APOC3 transcription (-) [was IFIB_C3]
A_CKD_C3  :    0.00   : Nephrotic or CKD apoC-III up-drive (-)

// ------------------------------------------------------------------ ANGPTL3
KOUT_ANG  :    0.030  : ANGPTL3 turnover (1/h)
EMAX_EVI  :    0.95   : Max evinacumab neutralisation of ANGPTL3 (-) [was IMAX_EVI]
EC50_EVI  :    4.0    : Evinacumab plasma concentration for half-max (mg/L) [was IC50_EVI]
GAMMA_EVI :    1      : Evinacumab Hill coefficient (-) [new; original implicit 1]

// -------------------------------------------------------------- LPL protein
KOUT_LPL  :    0.050  : Functional endothelial LPL pool turnover (1/h)
EMAX_FIB_LPL : 0.50   : Max fenofibrate induction of LPL synthesis (-) [was EFIB_LPL]
EC50_FIB  :    8.0    : Fenofibric acid for half-max PPAR-alpha effect (mg/L)
GAMMA_FIB :    1      : Fenofibrate Hill coefficient (-) [new; original implicit 1]
...
V1_FIB    :   18      : Volume of distribution of fenofibric acid (L) [was V_FIB]
CL_FIB    :    0.6246 : Clearance of fenofibric acid (L/h) [was KE_FIB=0.0347/h; CL_FIB = KE_FIB*V_FIB exactly, CL_FIB/V1_FIB = 0.0347 = original KE_FIB]
...
ALT        : Serum ALT (U/L)
GUT_VOL    : Volanesorsen SC depot (mg) [was VOL_SC]
CENT_VOL   : Volanesorsen plasma (mg) [was VOL_CP]
LIV_VOL    : Volanesorsen hepatic tissue (mg) [was VOL_LIV]
SYS_VOL    : Volanesorsen non-hepatic tissue (mg) [was VOL_SYS]
GUT_OLE    : Olezarsen SC depot (mg) [was OLE_SC]
CENT_OLE   : Olezarsen plasma (mg) [was OLE_CP]
LIV_OLE    : Olezarsen hepatic tissue (mg) [was OLE_LIV]
SYS_OLE    : Olezarsen non-hepatic tissue (mg) [was OLE_SYS]
GUT_PLO    : Plozasiran SC depot (mg) [was PLO_SC]
CENT_PLO   : Plozasiran plasma (mg) [was PLO_CP]
LIV_PLO    : Plozasiran hepatic tissue (mg) [was PLO_LIV]
RISC_PLO   : RISC-loaded plozasiran guide strand (AU) [was PLO_RISC]
CENT_EVI   : Evinacumab central (mg) [was EVI_C]
PERI_EVI   : Evinacumab peripheral (mg) [was EVI_P]
GUT_FIB    : Fenofibrate in gut (mg) [was FIB_GUT]
CENT_FIB   : Fenofibric acid in plasma (mg) [was FIB_C]
...
$ODE
// ============================ drug exposures ================================
// Each compound below exposes exactly one canonical C_<STEM> (the single
// point an external covariate could later be substituted in) and one named
// EFFECT_<STEM> Hill term. VOL/OLE/PLO have no plasma volume anywhere in the
// original -- C_VOL/C_OLE/C_PLO are therefore identity renames of the
// hepatic-tissue (or RISC-loaded) compartment AMOUNT the original effect was
// already computed from, not an invented concentration.
// Evinacumab is dosed as a direct bolus into CENT_EVI itself (no absorption
// depot), so a concentration computed from CENT_EVI as an $ODE-local double
// reads the PRE-dose value on the duplicate report row /run_simulation
// emits at the exact instant of a dose (confirmed live: C_EVI/EFFECT_EVI
// read 0 at t = dose time, jumping to their true value only on the next
// output row). VOL/OLE/PLO/FIB are all dosed into an upstream depot/gut
// compartment that builds their C_<STEM>-defining state gradually, so they
// never see this discontinuity (confirmed live, no artifact) and are
// captured directly as $ODE locals below. For EVI only, the disease-facing
// calculation below (ANGFREE, needed every solver substep) keeps its own
// internal variables; the convention-named C_EVI/EFFECT_EVI that get
// captured are instead recomputed fresh in $TABLE from post-dose state,
// mirroring the original file’s own CEVI/CEVI2 split for exactly this
// reason.
double C_EVI_ODE  = CENT_EVI / V1_EVI;          // mg/L -- internal, ODE-substep use only
double C_FIB  = CENT_FIB / V1_FIB;              // mg/L -- true concentration
double C_VOL  = LIV_VOL;                        // mg -- hepatic ASO amount (no volume in original)
double C_OLE  = LIV_OLE;                        // mg -- hepatic ASO amount (no volume in original)
double C_PLO  = RISC_PLO;                       // AU -- RISC-loaded guide-strand amount
double ASOSYS = SYS_VOL + SYS_OLE;              // systemic PS-ASO burden (mg); combined only at point of use (platelet safety term)
double ASOLIV = LIV_VOL + LIV_OLE;              // hepatic ASO burden (mg); combined only at point of use (ALT safety term)

double EFFECT_VOL = EMAX_VOL * pow(C_VOL, GAMMA_VOL) / (pow(EC50_VOL, GAMMA_VOL) + pow(C_VOL, GAMMA_VOL));
double EFFECT_OLE = EMAX_OLE * pow(C_OLE, GAMMA_OLE) / (pow(EC50_OLE, GAMMA_OLE) + pow(C_OLE, GAMMA_OLE));
double EFFECT_PLO = EMAX_PLO * pow(C_PLO, GAMMA_PLO) / (pow(EC50_PLO, GAMMA_PLO) + pow(C_PLO, GAMMA_PLO));
double EFFECT_EVI_ODE = EMAX_EVI * pow(C_EVI_ODE, GAMMA_EVI) / (pow(EC50_EVI, GAMMA_EVI) + pow(C_EVI_ODE, GAMMA_EVI));
double EFFECT_FIB_FRAC   = pow(C_FIB, GAMMA_FIB) / (pow(EC50_FIB, GAMMA_FIB) + pow(C_FIB, GAMMA_FIB));
double EFFECT_FIB_LPL    = EMAX_FIB_LPL   * EFFECT_FIB_FRAC;
double EFFECT_FIB_APOC3  = EMAX_FIB_APOC3 * EFFECT_FIB_FRAC;

// ============================ apoC-III axis =================================
double KIN_M  = KOUT_M;
double KSYN_P = KDEG_P * APOC3_REF;
dxdt_APOC3_M  = KIN_M * (1.0 - EFFECT_FIB_APOC3)
                - KOUT_M * APOC3_M * (1.0 + EFFECT_VOL + EFFECT_OLE + EFFECT_PLO);
...
double ANGFREE = ANGPTL3 * (1.0 - EFFECT_EVI_ODE);

double KIN_LPL = KOUT_LPL;
double PREGF   = 1.0 - 0.35*PREG;                       // pregnancy lowers LPL
dxdt_LPL_ACT   = KIN_LPL * F_GENO * (1.0 + EFFECT_FIB_LPL) * PREGF
                 - KOUT_LPL * LPL_ACT;
...
// ============================ drug PK =======================================
dxdt_GUT_VOL  = -KA_VOL*GUT_VOL;
dxdt_CENT_VOL =  KA_VOL*GUT_VOL*F_VOL - (KL_VOL + KO_VOL + KE_VOL)*CENT_VOL;
dxdt_LIV_VOL  =  KL_VOL*CENT_VOL - KOUTL_VOL*LIV_VOL;
dxdt_SYS_VOL  =  KO_VOL*CENT_VOL - KOUTS_VOL*SYS_VOL;

dxdt_GUT_OLE  = -KA_OLE*GUT_OLE;
dxdt_CENT_OLE =  KA_OLE*GUT_OLE*F_OLE - (KL_OLE + KO_OLE + KE_OLE)*CENT_OLE;
dxdt_LIV_OLE  =  KL_OLE*CENT_OLE - KOUTL_OLE*LIV_OLE;
dxdt_SYS_OLE  =  KO_OLE*CENT_OLE - KOUTS_OLE*SYS_OLE;

dxdt_GUT_PLO   = -KA_PLO*GUT_PLO;
dxdt_CENT_PLO  =  KA_PLO*GUT_PLO*F_PLO - (KL_PLO + KO_PLO + KE_PLO)*CENT_PLO;
dxdt_LIV_PLO   =  KL_PLO*CENT_PLO - KOUTL_PLO*LIV_PLO - KRISC_IN*LIV_PLO;
dxdt_RISC_PLO  =  KRISC_IN*LIV_PLO - KRISC_OUT*RISC_PLO;

dxdt_CENT_EVI = -(CL_EVI/V1_EVI)*CENT_EVI - (Q_EVI/V1_EVI)*CENT_EVI + (Q_EVI/V2_EVI)*PERI_EVI;
dxdt_PERI_EVI =  (Q_EVI/V1_EVI)*CENT_EVI - (Q_EVI/V2_EVI)*PERI_EVI;

dxdt_GUT_FIB  = -KA_FIB*GUT_FIB;
dxdt_CENT_FIB =  KA_FIB*GUT_FIB*F_FIB - (CL_FIB/V1_FIB)*CENT_FIB;
...
// C_FIB/C_VOL/C_OLE/C_PLO and EFFECT_VOL/EFFECT_OLE/EFFECT_PLO/
// EFFECT_FIB_LPL/EFFECT_FIB_APOC3 are already declared once, in $ODE
// (mrgsolve compiles $ODE and $TABLE locals into one shared scope --
// confirmed empirically: a second same-named "double" declaration here
// fails to compile as a redefinition). Reused directly, not recomputed,
// exactly the way the original reused CL_LPL/GC3/SATFRAC under the
// TABLE-only names CLLPL/GC3O/SATO to avoid the same collision. C_EVI/
// EFFECT_EVI are the one exception: evinacumab is dosed as a direct bolus
// into CENT_EVI, so its $ODE-local (C_EVI_ODE/EFFECT_EVI_ODE) reads a
// stale pre-dose value on the exact dose-instant report row (confirmed
// live). Recomputing fresh here, under the convention names, mirrors the
// original’s own CEVI/CEVI2 split and eliminates the artifact for the
// captured covariate (verified: no discontinuity at the dose row).
double C_EVI   = CENT_EVI/V1_EVI;
double EFFECT_EVI = EMAX_EVI*pow(C_EVI, GAMMA_EVI)/(pow(EC50_EVI, GAMMA_EVI) + pow(C_EVI, GAMMA_EVI));
double ANGF    = ANGPTL3*(1.0 - EFFECT_EVI);
...
C_EVI   : Evinacumab plasma concentration, exposed pluggable PK covariate (mg/L) [was CEVI2 in the original]
C_FIB   : Fenofibric acid plasma concentration, exposed pluggable PK covariate (mg/L)
C_VOL   : Volanesorsen hepatic tissue amount, exposed pluggable PK covariate (mg)
C_OLE   : Olezarsen hepatic tissue amount, exposed pluggable PK covariate (mg)
C_PLO   : RISC-loaded plozasiran amount, exposed pluggable PK covariate (AU)
EFFECT_EVI       : Evinacumab fractional ANGPTL3 neutralisation (-)
EFFECT_VOL       : Volanesorsen fold-increase in apoC-III mRNA degradation (-)
EFFECT_OLE       : Olezarsen fold-increase in apoC-III mRNA degradation (-)
EFFECT_PLO       : Plozasiran fold-increase in apoC-III mRNA degradation (-)
EFFECT_FIB_LPL   : Fenofibrate fractional induction of LPL synthesis (-)
EFFECT_FIB_APOC3 : Fenofibrate fractional suppression of APOC3 transcription (-)
'

mod <- mcode("fcs_refactored", fcs_code)
...
##  (Unchanged from the original -- these are disease-side calibration targets,
##  not affected by the PK naming refactor.)
##
...
volanesorsen <- function(start_day, weeks, mg = 300)
  ev(amt = mg, cmt = "GUT_VOL", time = start_day*24, ii = 168, addl = weeks - 1)

olezarsen <- function(start_day, months, mg = 80)
  ev(amt = mg, cmt = "GUT_OLE", time = start_day*24, ii = 28*24, addl = months - 1)

plozasiran <- function(start_day, n_doses, mg = 25)
  ev(amt = mg, cmt = "GUT_PLO", time = start_day*24, ii = 90*24, addl = n_doses - 1)

evinacumab <- function(start_day, n_doses, mg_per_kg = 15, wt = 70)
  ev(amt = mg_per_kg*wt, cmt = "CENT_EVI", time = start_day*24, ii = 28*24,
     addl = n_doses - 1)

fenofibrate <- function(start_day, days, mg = 145)
  ev(amt = mg, cmt = "GUT_FIB", time = start_day*24, ii = 24, addl = days - 1)
