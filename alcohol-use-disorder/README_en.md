...
| File | Contents |
|------|------|
| [`aud_qsp_model.dot`](aud_qsp_model.dot) | Graphviz mechanistic map source — **21 modules · 248 nodes · 403 edges** |
| [`aud_qsp_model.svg`](aud_qsp_model.svg) / [`.png`](aud_qsp_model.png) | The rendered map (the PNG at 150 dpi) |
| [`aud_mrgsolve_model.R`](aud_mrgsolve_model.R) | mrgsolve model with **72 ODE compartments · 271 parameters · 21 scenarios** + validation report (actually run) |
| [`aud_shiny_app.R`](aud_shiny_app.R) | **Nine-tab** interactive dashboard (including a 13-regimen builder) |
| [`aud_references_en.md`](aud_references_en.md) | **132 papers** (21 sections) — every PMID looked up through the PubMed E-utilities and cross-checked against the title. The 15 that failed the check were excluded rather than guessed at, and are noted at the end of the list |
...
Full citation list: [`aud_references_en.md`](aud_references_en.md) (132 papers, PubMed-verified).
