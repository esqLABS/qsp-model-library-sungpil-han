...
| Deliverable | File |
|---|---|
| 🗺️ Mechanistic map (DOT) | [`ins_qsp_model_en.dot`](ins_qsp_model_en.dot) |
| 🖼️ Map (SVG, full detail) | [`ins_qsp_model_en.svg`](ins_qsp_model_en.svg) |
| 🖼️ Map (PNG)              | [`ins_qsp_model_en.png`](ins_qsp_model_en.png) |
| ⚙️ mrgsolve ODE model     | [`ins_mrgsolve_model.R`](ins_mrgsolve_model.R) |
| 📊 Shiny dashboard        | [`ins_shiny_app_en.R`](ins_shiny_app_en.R) |
| 📚 References             | [`ins_references_en.md`](ins_references_en.md) |
...
[![CID QSP map](ins_qsp_model_en.png)](ins_qsp_model_en.svg)
...
```r
shiny::runApp("ins_shiny_app_en.R")
```
...
├── ins_qsp_model_en.dot          mechanistic map source (Graphviz)
├── ins_qsp_model_en.svg          map (vector, full resolution)
├── ins_qsp_model_en.png          map (150 dpi)
├── ins_mrgsolve_model.R       63-ODE QSP model + 19 scenarios + validation block
├── ins_shiny_app_en.R            9-tab interactive dashboard
└── ins_references_en.md          105 references (PubMed-verified)
```

Rebuild the map with:

```bash
dot -Tsvg ins_qsp_model_en.dot -o ins_qsp_model_en.svg
dot -Tpng -Gdpi=150 ins_qsp_model_en.dot -o ins_qsp_model_en.png
```
...
purposes. It was built based on published literature and clinical trial data
but has not been independently validated or certified, and **must not be used
directly for actual clinical decision-making, prescribing, or regulatory
submission.** In particular, always consult a physician on the choice, dose,
and discontinuation of any hypnotic.
