# Chronic Insomnia Disorder — References

This is the evidence base for the Chronic Insomnia Disorder QSP model (`ins_qsp_model_en.dot` · `ins_mrgsolve_model.R` · `ins_shiny_app_en.R`).
All **105** references below were looked up via PubMed E-utilities to confirm PMID, journal, and year,
and the links connect directly to the corresponding PMID.

The literature is arranged following the model's construction order:
disease definition → two-process model → homeostatic/circadian mechanisms → flip-flop switch → hyperarousal →
3-P perpetuating loop → CBT-I → drug-specific PK/PD → exogenous modulators → outcomes and quantitative modelling.

> ⚠️ This list is intended for model documentation for educational and research purposes, and does not replace clinical practice guidelines.

---

## A. Epidemiology · Definition · Diagnosis
...
## B. Two-process model (Process S x C)
...
## C. Homeostatic sleep pressure · Adenosine
...
## E. Arousal system · Flip-flop switch · Orexin
...
## F. Hyperarousal hypothesis (core pathophysiology)
...
## G. 3-P model · Cognitive-behavioural theory
...
## H. CBT-I clinical trials · Meta-analyses
...
## K. Melatonergic · H1 · 5-HT2A drugs
...
## L. Caffeine · Alcohol · Light · Shift work
...
## M. Outcomes · Comorbidities · Quantitative modelling
...
## Mapping of literature to model parameters

| Model component | Parameter | Key evidence |
|---|---|---|
| Process S rise/decay time constants | `TAUR` 18.2 h · `TAUD` 4.2 h | Daan/Beersma/Borbély 1984 · Borbély 2016 |
| Circadian alertness signal amplitude · phase | `AMPC` · `TPKC` | Dijk & Czeisler 1995 |
| Endogenous period | `TAUINT` 24.18 h | Czeisler 1999 |
| Light phase-response curve | `KLIGHT` · `prcL` | Khalsa/St Hilaire PRC studies |
| Melatonin phase-response curve | `KMELPH` · `prcM` | Lewy melatonin PRC |
| Nocturnal melatonin light suppression | `LUX50` 90 lux | Zeitzer 2000 |
| Adenosine-caffeine competition | `KICAF` 5 mg/L | Porkka-Heiskanen 1997 · Landolt |
| Flip-flop mutual inhibition | `WNE` · `WHA` · `WOX` | Saper 2005 · Scammell 2017 |
| Hyperarousal state variable | `A0` · `AROU` | Riemann 2010 · Bonnet & Arand 2010 |
| Nocturnal HPA rise | `GCORT` · `AMPCOR` | Vgontzas 2001 |
| 3-P perpetuating loop | `COND` · `SEFF` · `DBAS` · `TIBS` | Spielman 1987 · Harvey 2002 · Espie 2006 |
| Sleep restriction therapy | `KTIBDN` · `SETGT` | Spielman 1987 · Miller 2014 |
| Zolpidem PK | `KAZOL` · `VZOL` · `CLZOL` | zolpidem PK literature · Greenblatt |
| DORA half-life comparison | `CLSUV` · `CLLEM` · `CLDAR` | Herring 2016 · Rosenberg 2019 · Mignot 2022 |
| Ramelteon + M-II | `FMM2` · `CLRM2` | Karim ramelteon dose-escalation study |
| Low-dose doxepin H1 | `EC50DOX` · `KADOX` | Krystal doxepin 35-day sleep laboratory trial |
| BzRA tolerance · rebound | `KTOLIN` · `KTOLOUT` · `GREB` | Roehrs & Roth · Soyka |
| Glymphatic clearance | `KGIN` · `N3REF` | Xie 2013 series |
| Insomnia-depression bidirectionality | `GDEPISI` | Baglioni 2011 |
| Markov sleep-architecture modelling reference | Entire architecture | Bizzotto · Karlsson sleep Markov models |
