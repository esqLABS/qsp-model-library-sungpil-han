# Essential Tremor — QSP model references

**Essential Tremor QSP model — verified reference list**

**126 references** in total. Every entry in this file was not typed by hand but is a **record actually returned**
when `et_reference_check_en.py` queried PubMed through NCBI E-utilities. The title, author, journal,
year, and PMID are all transferred exactly as PubMed reported them, and not one citation
was written from memory. To reproduce it:

```bash
python3 et_reference_check_en.py            # pass 1: title-based verification
python3 et_reference_check_en.py --harvest   # pass 2: collection by topic query
python3 et_reference_check_en.py --emit      # pass 3: generate this file
```

Only 39 of the 91 candidate titles were confirmed on the first pass (the rest were because the author's memory
was a paraphrase). Rather than printing an approximate citation as it stood, topic queries were
run by section and **the papers PubMed actually returned** were cited instead.
The entries carrying a `query` field are the ones collected that way.

---

## 1. Epidemiology · clinical phenomenology · diagnostic criteria

*Epidemiology, phenomenology and diagnostic criteria* — prevalence, the ET plus controversy, natural history. The basis for the model's patient phenotypes (G0 · HDG · VXG) and for the baseline TETRAS values.
...
## 2. The oscillator theory of tremor · measurement and rating scales

*Tremor as an oscillator; measurement and rating scales* — separating loop gain from delay, mechanical resonance, and Elble's log transformation — the basis for oscillator core ⑧ of the model and for the mapping to TETRAS.
...
*Cerebellar pathology* — the Purkinje torpedo, climbing fibre-PC synaptic pathology, the loss of dentate GABA receptors — the basis for the PCINT and DNDIS state variables.
...
## 4. The inferior olivary oscillator · T-type calcium · harmaline

*Inferior olive, T-type calcium, harmaline* — Cav3.1 · Cx36 · subthreshold oscillation. The basis for the a_O (olivary fraction) parameter and for the cross-species discrepancy result.
...
*Genetics* — LINGO1 · FUS · NOTCH2NLC · GWAS · harmane exposure.
...
*Propranolol and beta-blockade* — the peripheral β₂ site, the failure of β₁ selectivity, the efficacy of nadolol — the basis for the Gaddum competition equation of φ_spindle and for the dose ceiling.
...
## 7. Primidone and phenobarbital

*Primidone and phenobarbital* — the contributions of parent and metabolite, the PK, the clinical effect — the basis for the result that 'the parent is the active molecule'.
...
*Other pharmacotherapy* — topiramate · gabapentin · zonisamide · the benzodiazepines.
...
*Ethanol and 1-octanol* — acute suppression, rebound, self-medication — the basis for the asymmetric ADAPTF/ADAPTS kinetics.
...
## 10. T-type calcium channel blockers in the clinic

*T-type calcium channel blockers in the clinic* — CX-8998 · ulixacaltamide. The data that will validate (or refute) the ceiling the model derived.
...
*Botulinum toxin* — the grip strength problem in hand tremor trials, guided injection, SNAP-25 recovery — the basis for the f_spill result.
...
*Surgery and neuromodulation* — MRgFUS · Vim DBS · stimulation frequency · lesion volume against ataxia · habituation — the basis for φ_thal (the series factor).
...
## 13. Endpoints and quality of life

*Endpoints and quality of life* — TETRAS · FTM · QUEST · accelerometry.
...
*QSP methodology* — the model structure and the validation methodology.
...
## Where the model leans on this literature

| Model element | Section with the evidence | Notes |
|---|---|---|
| `rating = 2 + 2·log10(A_cm)` (the Elble log transformation) | 2 | the whole conclusion that the 'discrepancy' between accelerometry and TETRAS is logarithmic comes from here |
| `f = 1/tau_loop` and the mass loading discriminant test | 2 | the ET frequency is a central delay, the EPT frequency a mechanical resonance |
| `PCINT`, `DNDIS` | 3 | torpedoes, climbing fibre pathology, the loss of dentate GABA receptors |
| `a_O = 0.35` (the olivary fraction) | 4, 10 | in the harmaline rat a_O=1, in humans <0.62 — the latter is the upper bound the failed trial gave us |
| `KI_PRP_B2 = 0.6 nM`, `FB2 = 0.60` | 6 | the peripheral β₂ site; nadolol works, atenolol does not |
| `EMAX_PRM/EC50_PRM` against `EMAX_PB/EC50_PB` | 7 | decomposing the contribution of the parent against phenobarbital |
| `TAUF_ON = 1 h`, `TAUF_OFF = 5 h` | 9 | the asymmetry of Mellanby acute tolerance and rebound |
| `f_spill` | 11 | the observation that guided injection preserves grip strength |
| `F50D = 80 Hz`, `HDBS = 4`, `V50L`, `V50A` | 12 | the >100 Hz rule, and the lesion volume-ataxia trade-off |

## Disclaimer

This model is a semi-quantitative QSP model for educational and research purposes. The literature above is the
**starting point** for the structure and parameters of the model; the model has not been fitted to or validated against patient data. It must not be used
