...
# cd_refactor_notes.md for the full account). Summary:
#
#   The original's single administered-drug PK block (DrugGut/DrugPlasma,
#   generic "Drug_type" switch) is, per this file's own $PARAM comments
#   ("Drug PK parameters (Larazotide example)", "molecular weight
#   (larazotide)"), modelled on LARAZOTIDE (AT-1001) specifically. It is
#   renamed to the guide's convention with stem LARA: GUT_LARA/CENT_LARA/
#   KA_LARA/F_LARA/CL_LARA/V1_LARA/MW_LARA/EC50_LARA/EMAX_LARA/GAMMA_LARA/
#   C_LARA/EFFECT_LARA. The original's two duplicate concentration-reporting
#   sites (a local `Cp` inside $ODE and a second `Drug_Cp` inside $TABLE,
#   both merely re-reading DrugPlasma) are normalized into the single
#   `C_LARA` variable and every read site is redirected at it.
#
#   The Drug_type==2/3 branches (E_ZED, E_AMG) are NOT a separate compound
#   with its own PK -- the original never gives ZED1227 or AMG714 their own
#   dosing/absorption/clearance; they are alternative, mutually-exclusive
#   disease-effect hypotheses that reuse the *same* generic PK output. They
#   have no census row of their own and are therefore left completely
#   untouched in every respect except that they now read from the renamed
#   C_LARA/EMAX_LARA (a forced, purely mechanical consequence of removing
#   the duplicate `Cp`/`Emax_drug` names they used to share with larazotide
#   -- not a re-scoping of a second compound). See cd_refactor_notes.md.
# ==============================================================================
...
// --- Drug PK parameters (Larazotide / AT-1001; stem LARA -- pluggable-PK refactor) ---
F_LARA         : 0.5    : Bioavailability (gut-local drug; systemic F~0.01)
KA_LARA        : 0.6    : h-1, first-order absorption rate
CL_LARA        : 8.0    : L/h, total drug clearance
V1_LARA        : 6.0    : L, volume of distribution
MW_LARA        : 406    : g/mol, molecular weight (larazotide)

// --- Drug PD parameters ---
Drug_type      : 0      : 0=none, 1=Larazotide (IP), 2=ZED1227 (tTG2), 3=AMG714 (IL-15)
EC50_LARA      : 50     : ng/mL, EC50 of larazotide for IP reduction
EC50_ZED_tTG   : 80     : ng/mL, EC50 of ZED1227 for tTG2 inhibition (not this census row -- see notes)
EC50_AMG_IL15  : 15     : ng/mL, EC50 of AMG714 for IL-15 neutralization (not this census row -- see notes)
EMAX_LARA      : 0.85   : Maximum drug efficacy (85% of pathway inhibition); shared verbatim by the original across all three Drug_type branches -- see refactor notes
GAMMA_LARA     : 1      : Hill coefficient (implicit=1 in the original; made explicit by this refactor)
...
GUT_LARA     : 0    : Larazotide in GI tract (mg) [was DrugGut]
CENT_LARA    : 0    : Larazotide in plasma (ng/mL) [was DrugPlasma]

$GLOBAL
// Pluggable-PK refactor: the compounds exposed concentration and Hill
// effect are declared once at file scope (mrgsolve 2.0.1 hoists every
// block-local "double NAME" into one shared anonymous namespace, so the
// same name declared as a local in two different DSL blocks, e.g. once in
...
// ---- Drug concentration (single, normalized site) ----
// The original had two duplicate re-readings of DrugPlasma under two
// different names ("Cp" here in $ODE, "Drug_Cp" in $TABLE) -- normalized
// into this one file-scope C_LARA, redirected at every read site below.
// CENT_LARA is itself a concentration-valued state in the original
// (dC/dt = in/V - k*C, not amount/V), so C_LARA is an identity alias, not
// a division -- same bespoke deviation from the literal Archetype-3
// template as documented for CENT_SNMP/CENT_PB/CENT_UDCA in
// neonatal-hyperbilirubinemia/nhb_refactor_notes.md.
C_LARA = CENT_LARA;  // ng/mL

// ---- Drug effect functions ----
// EFFECT_LARA is the canonical named Hill interface for this census
// row compound (larazotide); a rename, not a refit -- the original
// ratio was already exactly this shape (implicit gamma=1).
EFFECT_LARA = (Drug_type == 1) ?
    EMAX_LARA*pow(C_LARA, GAMMA_LARA) / (pow(EC50_LARA, GAMMA_LARA) + pow(C_LARA, GAMMA_LARA)) :
    0.0;
// E_ZED / E_AMG are NOT part of this census row (no separate PK of their
// own in the original -- see file header and refactor notes); left with
// their original names and shapes, unchanged, only redirected onto the
// renamed C_LARA / EMAX_LARA (the same shared variables they always read,
// merely renamed).
double E_ZED   = (Drug_type == 2) ? EMAX_LARA * C_LARA / (C_LARA + EC50_ZED_tTG)  : 0.0;
double E_AMG   = (Drug_type == 3) ? EMAX_LARA * C_LARA / (C_LARA + EC50_AMG_IL15) : 0.0;
...
dxdt_IP = k_IP_IL15 * h_IL15_IP * (1.0 - EFFECT_LARA)
...
// [19] Larazotide in GI tract (mg): cleared by absorption [was dxdt_DrugGut]
dxdt_GUT_LARA = -KA_LARA * GUT_LARA;

// [20] Larazotide in plasma (ng/mL) [was dxdt_DrugPlasma]
dxdt_CENT_LARA = F_LARA * KA_LARA * GUT_LARA * 1000.0 / V1_LARA
    - (CL_LARA / V1_LARA) * CENT_LARA;
...
// Drug_Cp (the second of the two duplicate concentration sites in the
// original) removed -- C_LARA (file-scope, single site) is captured
// directly below.
//
// Build-compat fix (see cd_refactor_notes.md / UPSTREAM_ISSUES.md): the
// original also listed the compartments AntiTTG, VH, CrD, AbsArea in
// $CAPTURE below, which mrgsolve 2.0.1 rejects ("compartment should not
// be in $CAPTURE") since they are already $INIT compartments. Removed
...
run_scenario <- function(scen_name, scen_params) {
  ...
  if (scen_params$Drug_dose > 0) {
    ...
    ev <- ev(time = 0, amt = 0, cmt = "GUT_LARA")
  }
