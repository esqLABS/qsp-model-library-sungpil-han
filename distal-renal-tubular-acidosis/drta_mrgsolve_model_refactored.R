...
KA_HCTZ :   1.10: thiazide absorption rate constant (/h) (was ka_hctz)
...
GUT_HCTZ : thiazide depot, gut (mg) (was AG_hctz)
CENT_HCTZ: thiazide central compartment, amount (mg-equivalent; C_HCTZ = CENT_HCTZ/V1_HCTZ) (was C_hctz, which held concentration directly)
...
// --- PK/PD refactor: HCTZ named covariates, exposed via $TABLE/$CAPTURE ---
// (state-dependent, so cannot themselves be $PARAM defaults -- see
// drta_refactor_notes.md, "qspserver /model_manifest discoverability")
double o_C_HCTZ, o_EFFECT_HCTZ;
...
// --- HCTZ PK (PK/PD refactor, archetype 3 without peripheral: depot +
//     central, linear; C_HCTZ is the single exposed concentration, computed
...
dxdt_CENT_HCTZ =  KA_HCTZ * GUT_HCTZ - CL_HCTZ * C_HCTZ;
...
double Ksec = kK_sec * K_pl0 * pow(K_pl / K_pl0, nK_sec) * BW / 24.0
            ...
            + kK_H * Hdef
            + kK_Na * Na_alk_h
            + kK_hctz * EFFECT_HCTZ * BW / 24.0 * 0.05;
...
o_C_HCTZ = C_HCTZ;  o_EFFECT_HCTZ = EFFECT_HCTZ;
...
CMT <- c(AG_bicIR = 1, AG_citIR = 2, AG_bicPR = 3, AG_citPR = 4,
         AG_KCl = 5, AG_K = 6, GUT_HCTZ = 8, AG_vitD = 10, AG_acid = 56)
...
make_regimen <- function(kind = "none", mEq_kg_day = 0, times = c(7, 13, 19),
  ...
  if (hctz_mg > 0)      add(CMT["GUT_HCTZ"], hctz_mg, times[1])
