...
| File | Purpose |
|------|---------|
| `ach_qsp_model_en.dot` / `.svg` / `.png` | Graphviz mechanistic map — **14 clusters, 110+ nodes** |
| `ach_mrgsolve_model_en.R` | **23-ODE** mrgsolve model (10 PK + 12 disease/clinical + 2 AE) with 10 scenarios |
| `ach_shiny_app_en.R` | Interactive **8-tab** Shiny dashboard |
| `ach_references_en.md` | **82** PubMed citations grouped by section |
...
dot -Tsvg ach_qsp_model_en.dot -o ach_qsp_model_en.svg
dot -Tpng -Gdpi=110 ach_qsp_model_en.dot -o ach_qsp_model_en.png
```

```r
# Run a scenario in R
library(mrgsolve); library(dplyr); library(ggplot2)
source("ach_mrgsolve_model_en.R")           # builds `mod_ach` + `scenarios`
...
shiny::runApp("ach_shiny_app_en.R")
```

---

## Disease-specific quick-reference terminology
