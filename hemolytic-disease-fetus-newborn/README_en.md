...
| File | Content |
|------|------|
| [`hdfn_qsp_model.dot`](hdfn_qsp_model.dot) · [`.svg`](hdfn_qsp_model.svg) · [`.png`](hdfn_qsp_model.png) | Mechanistic map — **201 nodes / 18 clusters**. Boxes = state variables (ODEs), ovals = mediators/fluxes, hexagons = drugs/procedures, diamonds = clinical endpoints, octagons = **derived thresholds or arithmetic identities** |
| [`hdfn_mrgsolve_model.R`](hdfn_mrgsolve_model.R) | **45-ODE** mrgsolve model + an R driver that implements the obstetric protocols as a discrete-time controller + **16 scenarios** |
| [`hdfn_shiny_app.R`](hdfn_shiny_app.R) | **12-tab** dashboard. Tab 4 shows A x f_ag x M as separate trajectories, and every treatment control in the sidebar is labelled with which factor it moves |
| [`hdfn_references.md`](hdfn_references.md) | **96** references. Every one resolved live against NCBI esearch/esummary, with the *intent* — what the model took from that paper — recorded alongside each entry so a mismatch between the retrieved paper and the intent is visible at a glance |
| [`hdfn_python_reference.py`](hdfn_python_reference.py) | **The executed reference implementation.** Integrates the 45 ODEs with scipy. Where the mrgsolve file and this file disagree, this file is correct |
| [`hdfn_calibrate.py`](hdfn_calibrate.py) · [`calib.log`](calib.log) · [`hdfn_calibration.json`](hdfn_calibration.json) | The four-stage calibration and its full log |
| [`hdfn_analysis.py`](hdfn_analysis.py) · [`hdfn_reference_output.txt`](hdfn_reference_output.txt) | The script that produces every number above, and its output |
| [`mkrefs.py`](mkrefs.py) · [`refs_raw.json`](refs_raw.json) · [`refs_meta.json`](refs_meta.json) | The reference-resolution script and its raw payload — every citation can be re-derived |
