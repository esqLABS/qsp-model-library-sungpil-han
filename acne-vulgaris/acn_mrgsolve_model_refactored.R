...
## ============================================================================
##
##  PK/PD REFACTOR (see FORK_WORKFLOW_GUIDE.md Part 2 and acn_refactor_notes.md
##  for the full account). Per the census row for this file
##  (driver-patches/data/compound_perturbation_census.md), 5 compounds were
##  renamed to the fork's pluggable-PK convention: EE (ethinylestradiol),
##  ISO (isotretinoin, parent), OXO (4-oxo-isotretinoin, the active
##  metabolite of ISO), SPI (spironolactone/canrenone), TET (tetracycline
##  class). Every other compound in this file (BPO, topical retinoid,
##  clindamycin, azelaic acid, dapsone, clascoterone) is completely
##  untouched -- not in scope for this pass.
##    - EE:  archetype 3 minus peripheral (depot GUT_EE + central CENT_EE).
##      Two named effects: EFFECT_EE_SHBG (hepatic SHBG induction),
##      EFFECT_EE_LH (LH suppression) -- both plain Hill ratios in the
##      original, pulled out as EMAX_EE_*/EC50_EE_*/GAMMA_EE_*=1 (rename,
##      not a fit).
##    - TET: archetype 3 minus peripheral (GUT_TET + CENT_TET). Two named
##      effects matching the file's own documented "TWO separate
##      concentration-effect relationships": EFFECT_TET_AMR (antimicrobial,
##      SUBANTI-gated) and EFFECT_TET_AI (non-antimicrobial
##      anti-inflammatory). A third internal use (MMP-degradation
##      enhancement) and the antibiotic-selection-pressure term both reuse
##      C_TET directly with the renamed EC50_TET_AI/EC50_TET_AMR constants,
##      exactly as the original did -- not wrapped in a separate EFFECT_ name
##      since the original never treated them as independent effects.
##    - ISO: archetype 3 (GUT_ISO + CENT_ISO + PERI_ISO), unchanged shape.
##      X_ISO = C_ISO + POT_OXO*C_OXO (parent + potency-weighted active
##      metabolite) feeds ONE combined Hill, EFFECT_ISO -- this is a
##      disclosed bespoke choice: parent and active metabolite share one
##      Hill denominator in the original (this is real pharmacology, not a
##      multi-drug shortcut), so splitting it into two independent Hills
##      would require an actual refit, not a rename. See refactor notes.
##    - OXO: bespoke archetype-1-like single compartment (CENT_OXO), but
##      fed by metabolism from CENT_ISO (KMET_ISO) rather than its own
##      dose/depot -- there is no GUT_OXO. No independent EFFECT_OXO exists
##      in the original (its only route to the disease is through the
##      shared X_ISO/EFFECT_ISO above); C_OXO is still exposed as its own
##      clean, independently redirectable concentration.
##    - SPI: archetype 3 minus peripheral (GUT_SPI + CENT_SPI). Two named
##      Hill effects: EFFECT_SPI_LYASE (17,20-lyase inhibition) and
##      EFFECT_SPI_KAL (serum-potassium safety effect). A third action, AR
##      competitive antagonism, is a linear C_SPI/KI_SPI_AR term (raises
##      the apparent AR dissociation constant) -- NOT a saturating Hill in
##      the original, so it is renamed and left inline rather than forced
##      into a fabricated EFFECT_SPI_AR Hill shape.
##  All PK/PD parameter VALUES are copied verbatim from the original; only
##  names, the effect-term shape (explicit pow(...,GAMMA) with GAMMA=1
##  where the original had an implicit linear ratio), and $TABLE/$CAPTURE
##  additions changed. Verified against the original via the qspserver
##  mrgsolve_api service -- see acn_refactor_notes.md for the full
...
EMAX_EE_SHBG : 1.70  : Emax EE on hepatic SHBG synthesis
EC50_EE_SHBG : 2.5e-5: EC50 EE on hepatic SHBG synthesis (mg/L)
GAMMA_EE_SHBG : 1.00 : EE SHBG-induction Hill coefficient (none in original; =1, rename not a fit)
HINSSH  : 0.35  : Insulin exponent suppressing SHBG
EMAX_EE_LH   : 0.42  : Emax EE on LH-driven gonadal androgen output
EC50_EE_LH   : 1.8e-5: EC50 EE on LH suppression (mg/L)
GAMMA_EE_LH   : 1.00 : EE LH-suppression Hill coefficient (none in original; =1, rename not a fit)
EMAX_SPI_LYASE  : 0.40  : Emax spironolactone 17,20-lyase inhibition
EC50_SPI_LYASE   : 1.50  : Spironolactone conc for half-max lyase inhibition (mg/L)
GAMMA_SPI_LYASE  : 1.00  : Spironolactone 17,20-lyase Hill coefficient (none in original; =1, rename not a fit)
...
KI_SPI_AR   : 0.95  : Canrenone Ki at the AR (mg/L)
...
EMAX_TET_MMP    : 1.40   : Emax tetracycline on MMP degradation
...
EMAX_TET_AI   : 0.22   : Emax tetracycline NON-antimicrobial anti-inflammatory effect
EC50_TET_AI   : 0.35   : EC50 tetracycline anti-inflammatory (mg/L)
GAMMA_TET_AI  : 1.00   : Tetracycline anti-inflammatory Hill coefficient (none in original; =1, rename not a fit)
...
KA_TET   : 0.90   : Tetracycline absorption rate (1/h)
F_TET    : 0.90   : Tetracycline bioavailability
V1_TET    : 52.0   : Tetracycline volume (L)
CL_TET   : 2.20   : Tetracycline clearance (L/h)
EMAX_TET_AMR  : 0.050  : Emax tetracycline bactericidal rate (1/h)
EC50_TET_AMR  : 2.50   : EC50 tetracycline antimicrobial (mg/L)
GAMMA_TET_AMR : 1.00   : Tetracycline antimicrobial Hill coefficient (none in original; =1, rename not a fit)
...
KA_ISO   : 0.45   : Isotretinoin absorption rate (1/h)
F0_ISO   : 0.25   : Isotretinoin bioavailability fasted
FOODEF_ISO  : 1.20   : Fractional increase in F with a high-fat meal
FOOD_ISO    : 1      : 1 = taken with food 0 = fasted
LIDOSE_ISO  : 0      : 1 = lidose/micronised formulation (food effect abolished)
V1_ISO    : 75.0   : Isotretinoin central volume (L)
CL_ISO   : 2.00   : Isotretinoin clearance (L/h)
Q_ISO    : 3.00   : Isotretinoin intercompartmental clearance (L/h)
V2_ISO   : 90.0   : Isotretinoin peripheral volume (L)
KMET_ISO    : 0.018  : Isotretinoin -> 4-oxo metabolic rate (1/h)
V1_OXO    : 45.0   : 4-oxo-isotretinoin volume (L)
CL_OXO   : 1.10   : 4-oxo-isotretinoin clearance (L/h)
POT_OXO  : 0.35   : Potency of 4-oxo relative to parent
EC50_ISO  : 0.30   : EC50 isotretinoin effect (mg/L parent-equivalents)
EMAX_ISO  : 1.00   : Emax isotretinoin effect scaling
GAMMA_ISO : 1.00   : Isotretinoin composite-effect Hill coefficient (none in original; =1, rename not a fit)

// ---- systemic PK: spironolactone and EE ---------------------------------
KA_SPI   : 1.00   : Spironolactone absorption (1/h)
F_SPI    : 0.70   : Spironolactone bioavailability (canrenone equivalents)
V1_SPI    : 60.0   : Spironolactone volume (L)
CL_SPI   : 2.60   : Spironolactone clearance (L/h)
KA_EE    : 1.20   : Ethinylestradiol absorption (1/h)
F_EE     : 0.43   : Ethinylestradiol bioavailability
V1_EE     : 260    : Ethinylestradiol volume (L)
CL_EE    : 12.0   : Ethinylestradiol clearance (L/h)
...
EMAX_SPI_KAL    : 0.13   : Emax spironolactone on serum potassium
EC50_SPI_KAL   : 1.20   : Spironolactone conc for half-max potassium effect (mg/L)
GAMMA_SPI_KAL  : 1.00   : Spironolactone potassium-effect Hill coefficient (none in original; =1, rename not a fit)
...
GUT_TET   :   0   : Tetracycline gut depot (mg)
CENT_TET   :   0   : Tetracycline central (mg)
GUT_ISO   :   0   : Isotretinoin gut depot (mg)
CENT_ISO   :   0   : Isotretinoin central (mg)
PERI_ISO   :   0   : Isotretinoin peripheral (mg)
CENT_OXO   :   0   : 4-oxo-isotretinoin central (mg)
GUT_SPI   :   0   : Spironolactone gut depot (mg)
CENT_SPI   :   0   : Canrenone central (mg)
GUT_EE    :   0   : Ethinylestradiol gut depot (mg)
CENT_EE    :   0   : Ethinylestradiol central (mg)
CUM_ISO :   0   : Cumulative isotretinoin dose (mg/kg)
...
// =========================================================================
double C_TET  = FLOOR(CENT_TET, 0.0) / V1_TET;                       // mg/L
double C_ISO  = FLOOR(CENT_ISO, 0.0) / V1_ISO;                       // mg/L
double C_OXO  = FLOOR(CENT_OXO, 0.0) / V1_OXO;                       // mg/L
double C_SPI  = FLOOR(CENT_SPI, 0.0) / V1_SPI;                       // mg/L
double C_EE   = FLOOR(CENT_EE , 0.0) / V1_EE;                        // mg/L
...
double X_ISO  = C_ISO + POT_OXO * C_OXO;
double EFFECT_ISO = EMAX_ISO * pow(X_ISO, GAMMA_ISO) / (pow(EC50_ISO, GAMMA_ISO) + pow(X_ISO, GAMMA_ISO));

// retinoid receptor effect: topical depot + systemic isotretinoin
double RETTOP = EMXRET * SRET / (EC5RET + SRET);
double RETEFF = RETTOP + 0.85 * EFFECT_ISO;
if (RETEFF > 2.0) RETEFF = 2.0;

// tetracycline: TWO separate concentration-effect arms
double EFFECT_TET_AI  = EMAX_TET_AI * pow(C_TET, GAMMA_TET_AI) / (pow(EC50_TET_AI, GAMMA_TET_AI) + pow(C_TET, GAMMA_TET_AI));                // anti-inflammatory
double EFFECT_TET_AMR = (SUBANTI > 0.5 ? 0.0 : EMAX_TET_AMR * pow(C_TET, GAMMA_TET_AMR) / (pow(EC50_TET_AMR, GAMMA_TET_AMR) + pow(C_TET, GAMMA_TET_AMR)));
...
//  1. Hormonal axis
// =========================================================================
double EFFECT_EE_SHBG = EMAX_EE_SHBG * pow(C_EE, GAMMA_EE_SHBG) / (pow(EC50_EE_SHBG, GAMMA_EE_SHBG) + pow(C_EE, GAMMA_EE_SHBG));
double EFFECT_EE_LH   = EMAX_EE_LH   * pow(C_EE, GAMMA_EE_LH)   / (pow(EC50_EE_LH,   GAMMA_EE_LH)   + pow(C_EE, GAMMA_EE_LH));
double EFFECT_SPI_LYASE = EMAX_SPI_LYASE * pow(C_SPI, GAMMA_SPI_LYASE) / (pow(EC50_SPI_LYASE, GAMMA_SPI_LYASE) + pow(C_SPI, GAMMA_SPI_LYASE));

double TTIN = KOTT * TT0 * (1.0 + 0.9 * PCOSF) * (1.0 - EFFECT_EE_LH) * (1.0 - EFFECT_SPI_LYASE);
dxdt_TT   = TTIN - KOTT * TT;

double INSN  = FLOOR(INS, 0.1) / INSREF;
double SHBGIN = KOSHBG * SHBGB * (1.0 + EFFECT_EE_SHBG) * pow(FLOOR(1.0/INSN, 0.05), HINSSH);
...
double KDEFF  = KDAR * (1.0 + SCLA / KICLAS + C_SPI / KI_SPI_AR);
...
dxdt_LIP = KLIP * LIPDRV / (1.0 + EISOLIP * EFFECT_ISO) - KLIP * LIP;
...
dxdt_SGM = KPROL * ARSG * SGM * (1.0 - SGM / SGMcap)
           - KAPO * SGM * (1.0 + EISOAP * EFFECT_ISO);
...
// CUM_ISO is what turns the empirical 120-150 mg/kg rule into an emergent
...
double TETKL2 = EFFECT_TET_AMR * (1.0 - RESFn);
...
double PRESS = SCLI / (EC5CLI + SCLI)
             + (SUBANTI > 0.5 ? 0.0 : (1.0 - 0.45 * NARROW) * C_TET / (EC50_TET_AMR + C_TET));
...
double AI_TET = EFFECT_TET_AI;
if (AI_TET > 0.70) AI_TET = 0.70;

dxdt_TLR = KTLR * PAMPn * (1.0 - AI_TET) - KTLR * TLR;
...
dxdt_MMP = KMMP * 0.5 * (NEUn + TNFn) - KMMP * MMP * (1.0 + EMAX_TET_MMP * C_TET / (EC50_TET_AI + C_TET));
...
double F_ISO = F0_ISO * (1.0 + FOODEF_ISO * (LIDOSE_ISO > 0.5 ? 1.0 : FOOD_ISO));
dxdt_GUT_ISO  = -KA_ISO * GUT_ISO;
dxdt_CENT_ISO =  KA_ISO * GUT_ISO * F_ISO
             - (CL_ISO / V1_ISO) * CENT_ISO
             - KMET_ISO * CENT_ISO
             - Q_ISO * (CENT_ISO / V1_ISO - PERI_ISO / V2_ISO);
dxdt_PERI_ISO =  Q_ISO * (CENT_ISO / V1_ISO - PERI_ISO / V2_ISO);
dxdt_CENT_OXO =  KMET_ISO * CENT_ISO - (CL_OXO / V1_OXO) * CENT_OXO;
dxdt_CUM_ISO = KA_ISO * GUT_ISO / FLOOR(WT, 1.0);
...
// 10. Safety trackers
// =========================================================================
dxdt_TG   = KTG  * TGB  * (1.0 + ETG  * EFFECT_ISO) - KTG  * TG;
dxdt_ALT  = KALT * ALTB * (1.0 + EALT * EFFECT_ISO) - KALT * ALT;
double EFFECT_SPI_KAL = EMAX_SPI_KAL * pow(C_SPI, GAMMA_SPI_KAL) / (pow(EC50_SPI_KAL, GAMMA_SPI_KAL) + pow(C_SPI, GAMMA_SPI_KAL));
dxdt_KSER = KKAL * KALB * (1.0 + EFFECT_SPI_KAL) - KKAL * KSER;
dxdt_MUCO = KMUC * (10.0 * EFFECT_ISO * MUCSENS + EMUCT * RETTOP
...
double SERO  = POS(SER);
double CTETO = POS(CENT_TET) / V1_TET;
double CISOO = POS(CENT_ISO) / V1_ISO;
double COXOO = POS(CENT_OXO) / V1_OXO;
double CSPIO = POS(CENT_SPI) / V1_SPI;
double CEEO  = POS(CENT_EE)  / V1_EE * 1e6;          // pg/mL
double CACNT = POS(CAP) + POS(CAB);

double ISOEQT  = CISOO + POT_OXO * COXOO;
double ISOEFFT = EMAX_ISO * ISOEQT / (EC50_ISO + ISOEQT);
double AIEFFT  = EMAX_TET_AI * CTETO / (EC50_TET_AI + CTETO);

double FAIO = (POS(TT) / 28.8) / (POS(SHBG) + 1.0) * 100.0;

// [refactor] C_<STEM>/EFFECT_<STEM> are NOT redeclared here: mrgsolve
// compiles $ODE and $TABLE into the SAME function (confirmed empirically
...
ISOEFFT : Isotretinoin composite effect (0-1) -- table-side recompute, matches EFFECT_ISO
AIEFFT  : Tetracycline anti-inflammatory effect (0-1) -- table-side recompute, matches EFFECT_TET_AI
C_TET   : Tetracycline concentration (mg/L) -- redirectable single site ($ODE-scope, canonical name)
C_ISO   : Isotretinoin (parent) concentration (mg/L) -- redirectable single site ($ODE-scope, canonical name)
C_OXO   : 4-oxo-isotretinoin (metabolite) concentration (mg/L) -- redirectable single site ($ODE-scope, canonical name)
C_SPI   : Canrenone (spironolactone-equivalent) concentration (mg/L) -- redirectable single site ($ODE-scope, canonical name)
C_EE    : Ethinylestradiol concentration (mg/L) -- redirectable single site ($ODE-scope, canonical name)
EFFECT_ISO      : Isotretinoin composite (parent + 4-oxo) effect (0-1)
EFFECT_TET_AI   : Tetracycline non-antimicrobial anti-inflammatory effect (0-1)
EFFECT_TET_AMR  : Tetracycline antimicrobial bactericidal-rate effect (1/h)
EFFECT_EE_SHBG  : Ethinylestradiol hepatic SHBG-induction effect (0-1)
EFFECT_EE_LH    : Ethinylestradiol LH-suppression effect (0-1)
EFFECT_SPI_LYASE: Spironolactone 17,20-lyase inhibition effect (0-1)
EFFECT_SPI_KAL  : Spironolactone serum-potassium effect (0-1)
...
acn_tetracycline<- function(dose = 100, ii = 24, days = 84, start = 0)
  mrgsolve::ev(time = start, amt = dose, cmt = "GUT_TET", ii = ii,
               addl = ceiling(days * 24 / ii) - 1)

acn_isotretinoin<- function(mgkg = 0.5, wt = 62, ii = 24, days = 140, start = 0)
  mrgsolve::ev(time = start, amt = mgkg * wt, cmt = "GUT_ISO", ii = ii,
               addl = ceiling(days * 24 / ii) - 1)

acn_spironolactone <- function(dose = 100, ii = 24, days = 168, start = 0)
  mrgsolve::ev(time = start, amt = dose, cmt = "GUT_SPI", ii = ii,
               addl = ceiling(days * 24 / ii) - 1)
...
acn_coc <- function(ee_ug = 30, cycles = 6, start = 0) {
  do.call(c, lapply(seq_len(cycles) - 1, function(k) {
    mrgsolve::ev(time = start + k * 28 * 24, amt = ee_ug / 1000, cmt = "GUT_EE",
                 ii = 24, addl = 20)
  }))
}
...
ACN_scenarios <- function(wt = 62) list(
  ...
  list(id = 10, label = "10. Sarecycline 1.5 mg/kg + adapalene 0.1% (12 wk)",
       ...
       param = list(NARROW = 1, CL_TET = 1.75, V1_TET = 55),
...
ACN_relapse <- function(sim, stop_week) {
  ...
  do.call(rbind, lapply(sp, function(d) {
    ...
    data.frame(
      ...
      cum_mgkg   = round(max(d$CUM_ISO), 1),
