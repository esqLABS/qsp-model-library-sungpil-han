...
| Deliverable | File |
|--------|------|
| 🗺️ Mechanistic map | [`tap_qsp_model_en.dot`](tap_qsp_model_en.dot) · [SVG](tap_qsp_model_en.svg) · [PNG](tap_qsp_model_en.png) — 162 nodes / 246 edges / 20 clusters |
| ⚙️ mrgsolve model | [`tap_mrgsolve_model_en.R`](tap_mrgsolve_model_en.R) — 50 ODEs, time unit = hours, 25 scenarios |
| 🐍 Independent verification implementation | [`tap_python_reference.py`](tap_python_reference.py) · [full output](tap_reference_output.txt) — 15 verification sections |
| 📊 Shiny dashboard | [`tap_shiny_app_en.R`](tap_shiny_app_en.R) — 12 tabs |
| 📚 References | [`tap_references_en.md`](tap_references_en.md) — 88 PubMed entries (`mkrefs_en.py` queries them live; no PMID written from memory) |
...
[`tap_python_reference.py`](tap_python_reference.py),
and the full text is in
[`tap_reference_output.txt`](tap_reference_output.txt).
...
[`tap_mrgsolve_model_en.R`](tap_mrgsolve_model_en.R)
...
```r
source("tap_mrgsolve_model_en.R")
...
```bash
# map
dot -Tsvg tap_qsp_model_en.dot -o tap_qsp_model_en.svg
dot -Tpng -Gdpi=150 tap_qsp_model_en.dot -o tap_qsp_model_en.png
...
Rscript -e 'source("tap_mrgsolve_model_en.R"); tap_verify(); print(tap_all())'
Rscript -e 'shiny::runApp("tap_shiny_app_en.R")'
```
