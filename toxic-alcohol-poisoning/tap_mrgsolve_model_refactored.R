...
KA_FOM   :   2.5 : first-order absorption of oral fomepizole (1/h)      [was KAFOM]
...
Q_FOM    :  25.0 : fomepizole inter-compartmental clearance (L/h)  [was hardcoded 25.0 in q_FOM]
...
// ------------------------------------------------------------- antidotes ----
GUT_FOM  : fomepizole in the gut lumen (mmol)               [was A_GUTF]
CENT_FOM : fomepizole, central (mmol)                        [was A_FOM1]
PERI_FOM : fomepizole, peripheral (mmol)                     [was A_FOM2]
...
double V1_FOM= 0.42 * WT;      double V2_FOM= 0.28 * WT;   // total 0.70 L/kg [was V1FOM/V2FOM]
...
capture C_FOM = CENT_FOM / V1_FOM; double C_FOM2 = PERI_FOM / V2_FOM;   // C_FOM: the exposed concentration (redirect site)
...
double abs_F  = KA_FOM * GUT_FOM;
double q_FOM  = Q_FOM * (C_FOM - C_FOM2);
dxdt_GUT_FOM  = -abs_F;
dxdt_CENT_FOM = abs_F + fom_inf - q_FOM - el_fom - cl_hd_fom * C_FOM;
dxdt_PERI_FOM = q_FOM;
...
double bic_z  = (pH - PH_TARGET) / PH_TAU;   // [build-compat rename: was _e; a
                                              // leading underscore is rejected by
                                              // this mrgsolve build, see
                                              // UPSTREAM_ISSUES.md]
if (bic_z >  50.0) bic_z =  50.0;
if (bic_z < -50.0) bic_z = -50.0;
double bic_eff = bic_inf / (1.0 + exp(bic_z));         // titrated, not poured
...
double cFM = CENT_FOM / V1FM_;   // was A_FOM1
...
fom_events <- function(start, tend, hd = list(), boost = TRUE, wt = 70,
  ...
  ev <- data.frame(time = start, amt = mmol_fom(load_mgkg, wt), cmt = "CENT_FOM")
  on_hd <- function(t) any(vapply(hd, function(w) t >= w[1] && t < w[2], TRUE))
  tt <- start + q; n <- 0
  while (tt < tend) {
    d <- mmol_fom(maint_mgkg, wt) * if (n >= 4) 1.5 else 1.0
    ev <- rbind(ev, data.frame(time = tt, amt = d, cmt = "CENT_FOM"))
    n <- n + 1
