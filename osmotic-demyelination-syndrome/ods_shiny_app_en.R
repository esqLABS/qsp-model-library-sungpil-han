...
source_model <- function() {
  e <- new.env()
  sys.source("ods_mrgsolve_model_en.R", envir = e)
  e
}
E   <- source_model()
mod <- E$mod
...
<tr><th>Known limitations</th><td>R is not available in the build container, so this model has been <b>verified by equation but not by compilation</b>. Every ODE was independently reimplemented and integrated in Python/scipy (ods_verify_python_en.py). Ω has never been measured in a living human brain.</td></tr>
</table>"))
}
