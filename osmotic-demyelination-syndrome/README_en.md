...
[![ODS QSP mechanistic map](ods_qsp_model_en.png)](ods_qsp_model_en.svg)
...
| File | Contents |
|---|---|
| [`ods_qsp_model_en.dot`](ods_qsp_model_en.dot) / [`.svg`](ods_qsp_model_en.svg) / [`.png`](ods_qsp_model_en.png) | Mechanistic map — **154 nodes · 20 clusters · 224 edges** |
| [`ods_mrgsolve_model_en.R`](ods_mrgsolve_model_en.R) | mrgsolve model — **40 ODEs · 20 scenarios** |
| [`ods_shiny_app_en.R`](ods_shiny_app_en.R) | Shiny dashboard — **11 tabs** (including the safety map, the counterfactual experiment and the deadline) |
| [`ods_references_en.md`](ods_references_en.md) | **123 references**, every PMID looked up and checked against the PubMed E-utilities |
| [`ods_verify_python_en.py`](ods_verify_python_en.py) | Independent Python/scipy reimplementation of the 40 ODEs, and the scenario runs |
| [`ods_verification_output.txt`](ods_verification_output.txt) | The run output of the file above — **the source of every number in this document** |
...
```r
source("ods_mrgsolve_model_en.R")
S <- run_scenarios(mod)
plot_central_experiment(S)          # the counterfactual experiment of §2
plot(S$S06, SODIUM + UOSMOL + EFWCL + OMEGAc + STRESSc + DEFICIT ~ time)

shiny::runApp("ods_shiny_app_en.R")    # the 11-tab dashboard
```

```bash
python3 ods_verify_python_en.py        # regenerates every number
```
