...
<a href="tbi_qsp_model_en.svg"><img src="tbi_qsp_model_en.png" width="640" alt="sTBI QSP mechanistic map"></a>
...
| File | Contents |
|---|---|
| [`tbi_qsp_model_en.dot`](tbi_qsp_model_en.dot) | Mechanistic map — **236 nodes · 335 edges · 20 clusters** |
| [`tbi_qsp_model_en.svg`](tbi_qsp_model_en.svg) / [`.png`](tbi_qsp_model_en.png) | Rendered output (150 dpi) |
| [`tbi_reference_model_en.py`](tbi_reference_model_en.py) | **Reference implementation.** Pure standard library, 47 ODEs, fixed-step RK4 |
| [`tbi_reference_output_en.txt`](tbi_reference_output_en.txt) | The complete stdout of the file above — **the sole source of every number below** |
| [`tbi_mrgsolve_model_en.R`](tbi_mrgsolve_model_en.R) | mrgsolve model (a 1:1 port of the Python implementation) + 12 scenarios |
| [`tbi_shiny_app_en.R`](tbi_shiny_app_en.R) | Shiny dashboard, 11 tabs |
| [`tbi_references_en.md`](tbi_references_en.md) | 132 references + parameter-provenance table + **a table of where the model is wrong** |

```bash
python3 tbi_reference_model_en.py           # regenerates every number below
dot -Tsvg tbi_qsp_model_en.dot -o tbi_qsp_model_en.svg
Rscript -e 'shiny::runApp("tbi_shiny_app_en.R")'
```

> **Provenance rule.** Every number in this README was obtained by running
> `tbi_reference_model_en.py` and is contained verbatim in
> `tbi_reference_output_en.txt`. Not one number was written from memory. Where the
...
- Full results and commentary: [`tbi_reference_output_en.txt`](tbi_reference_output_en.txt)
- 132 references and the parameter-provenance table: [`tbi_references_en.md`](tbi_references_en.md)
