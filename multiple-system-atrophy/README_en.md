# Multiple System Atrophy (MSA) — QSP Model
...
| Deliverable | File |
|---|---|
| 🗺️ Mechanistic map (DOT) | [`msa_qsp_model_en.dot`](msa_qsp_model_en.dot) |
| 🖼️ Map (SVG, zoomable)   | [`msa_qsp_model_en.svg`](msa_qsp_model_en.svg) |
| 🖼️ Map (PNG)             | [`msa_qsp_model_en.png`](msa_qsp_model_en.png) |
| ⚙️ mrgsolve ODE model     | [`msa_mrgsolve_model.R`](msa_mrgsolve_model.R) |
| 📊 Shiny dashboard        | [`msa_shiny_app_en.R`](msa_shiny_app_en.R) |
| 📚 References (139, PubMed-verified) | [`msa_references_en.md`](msa_references_en.md) |
...
MSA_SHINY_RUN=1 Rscript -e 'shiny::runApp("msa_shiny_app_en.R", port = 8080)'

# Head-less test of all 10 dashboard panels — needs mrgsolve, not a browser
Rscript -e 'source("msa_shiny_app_en.R"); msa_shiny_selftest()'

# Re-render the mechanistic map
dot -Tsvg msa_qsp_model_en.dot -o msa_qsp_model_en.svg
dot -Tpng -Gdpi=150 -Gsize=72,36 msa_qsp_model_en.dot -o msa_qsp_model_en.png
```
...
| Tab | Content |
|---|---|
| 1 Patient profile | the **seven multiplicands** shown separately, with the drug implication stated |
| 2 Orthostatic tilt test | tilt BP/HR/NE/cerebral-autoregulation margin + the diagnostic metric table |
| 3 24-hour BP & nocturnal loop | 48-h SBP with night shading, natriuresis, urine output, ECF |
| 4 Drug PK | plasma concentrations plus α1 / NET / AADC target occupancy |
| 5 Autonomic axis | SNA, NE handling, RAAS, AVP–AQP2, and α1 up-regulation over years |
| 6 Neuropathological progression | glial cascade, myelin/mitochondria/inflammation, regional neuronal loss |
| 7 Motor symptoms & levodopa | phenotype composition and the waning levodopa response vs PD |
| 8 Clinical endpoints | UMSARS I/II/IV, SCOPA-AUT, PVR, survival |
| 9 Scenario comparison | 13 regimens ranked by morning standing SBP against supine hypertension |
| 10 Biomarkers & differential diagnosis | NfL, seed, MIBG correlate (`POSTG`), four-phenotype discrimination panel |
...
This model is a **Quantitative Systems Pharmacology (QSP) model intended for
educational and research purposes**. It has been calibrated against published
literature and clinical trial summary values, but it has not been
