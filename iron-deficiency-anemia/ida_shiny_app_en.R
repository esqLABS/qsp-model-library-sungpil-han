...
## ---------------------------------------------------------------------
model_file <- "ida_mrgsolve_model_en.R"
if (!file.exists(model_file))
  stop("ida_mrgsolve_model_en.R must sit next to this app.")
...
## =====================================================================
ui <- fluidPage(
  titlePanel("Iron Deficiency Anaemia QSP Dashboard"),
  ...
  sidebarLayout(
    sidebarPanel(
      width = 3,
      h4("Patient"),
      sliderInput("vbleed", "Ongoing Blood Loss (mL/day)",
                  min = 0.2, max = 12, value = 6.375, step = 0.125),
      helpText("6.375 mL/day ~ 178 mL/cycle. 0.6 = replete reference."),
      sliderInput("diet", "Dietary Iron (mg/day)",
                  min = 4, max = 30, value = 16, step = 1),
      sliderInput("il6", "IL-6 input (pg/mL/h) — inflammation",
                  min = 0, max = 4, value = 0, step = 0.25),
      helpText("IL-6 steady state = 20 x this value."),
      sliderInput("tmprss6", "TMPRSS6 Activity (1 = normal, <1 = IRIDA)",
                  min = 0.15, max = 1, value = 1, step = 0.05),
      hr(),
      h4("Therapy"),
      radioButtons("route", "Route",
                   c("None" = "none", "Oral" = "oral",
                     "IV" = "iv"), selected = "oral"),
      conditionalPanel(
        "input.route == 'oral'",
        sliderInput("odose", "Elemental Iron per Dose (mg)",
                    min = 15, max = 200, value = 65, step = 5),
        radioButtons("ointerval", "Dosing Interval",
                     c("q12h (BID)" = "12", "q24h (daily)" = "24",
                       "q48h (alternate day)" = "48", "q72h" = "72"),
                     selected = "24"),
        checkboxInput("adhere", "Account for reduced adherence due to GI side effects", TRUE)
      ),
      conditionalPanel(
        "input.route == 'iv'",
        sliderInput("ivdose", "IV Dose per Administration (mg)",
                    min = 100, max = 1500, value = 1000, step = 50),
        sliderInput("ivn", "Number of Doses (weekly)", min = 1, max = 5, value = 1),
        radioButtons("ivprod", "Product",
      ...
      hr(),
      sliderInput("weeks", "Follow-up (weeks)",
                  min = 4, max = 24, value = 12, step = 1),
      checkboxInput("reeq", "Re-equilibrate baseline state from patient parameters (slow, ~20 s)", FALSE),
    ...
    mainPanel(
      ...
      tabsetPanel(
        ...
        tabPanel("1 · Patient Profile",
                 ...
                 hr(), h4("Iron Balance at Baseline"),
        ...
        tabPanel("2 · Oral Iron PK",
                 br(), h4("Single Oral Dose — the gate opening and closing"),
        ...
        tabPanel("3 · Dosing Interval (Key)",
        ...
        tabPanel("4 · Hematologic Response",
                 br(), h4("Hemoglobin and Red Cell Indices"),
                 plotOutput("hb_plot", height = "620px"),
                 tableOutput("hb_tbl")),
        tabPanel("5 · Iron Distribution",
                 ...
                 hr(), h4("Tissue Iron vs Hemoglobin Recovery (fatigue / restless legs)"),
                 plotOutput("tissue_plot", height = "320px")),
        tabPanel("6 · Biomarkers",
                 br(), h4("Diagnostic Read-outs"),
        ...
        tabPanel("7 · Safety: Phosphate",
                 br(), h4("FGF23 — Phosphate — Vitamin D Axis"),
        ...
        tabPanel("8 · Scenario Comparison",
                 br(), h4("Direct Comparison of Two Regimens"),
                 fluidRow(
