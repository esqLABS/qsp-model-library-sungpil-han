...
All PMIDs were looked up via the PubMed E-utilities API to confirm the title, authors, journal, and year.
...
> flagged as such in `README_en.md`.
...
> on the numbers; `README_en.md` states the measurement that would falsify them.
...
> first-order structural assumption in `README_en.md`.
...
This reference list and model are for **research and educational purposes**. They
must not be used for clinical practice or dosing decisions. Some parameter values
are literature-based, but a substantial number are assumptions, and each section's *Parameter note* states this fact
along with the limits of the supporting evidence.
...
such case is flagged in a *Parameter note* above and in `README_en.md`.
