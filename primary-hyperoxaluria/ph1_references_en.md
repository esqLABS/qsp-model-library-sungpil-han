# Primary Hyperoxaluria QSP model — references
...
**Every PMID in this list was actually looked up and confirmed through NCBI E-utilities (esearch +
esummary)**, and the title, journal, and year were taken exactly from the original record PubMed returned.
Not one citation was written from memory
(because in a past session four of five PMIDs written from memory pointed at entirely unrelated
papers). The links are all of the form
...
### Honest provenance of the parameters

The credibility of a QSP model comes from distinguishing "which numbers came from the literature and which are
estimates". In this model, each is as follows.

**Literature-anchored values**

| Value | Where it sits in the model | Section with the evidence |
|---|---|---|
| Normal 24-hour urinary oxalate < 0.46 mmol/1.73 m²/day | the `D06` anchor | 1, 7 |
| PH1 urinary oxalate 1.0–2.5 mmol/1.73 m²/day | the phenotype range of the `FAGT` scan | 1, 3 |
| Normal plasma oxalate 1–6 µmol/L, ESKD-PH1 60–120 µmol/L | `POX_OUT` | 6 |
| The plasma CaOx solubility limit ≈ 30 µmol/L | `POX_CRIT` | 6 |
| Renal clearance of oxalate / creatinine clearance ≈ 1.0–1.5 (tubular secretion) | `F_SEC_OX` | 6, 9 |
| ILLUMINATE-A lumasiran urinary oxalate −65.4 % | the calibration target for `EMAX_HAO1` | 10 |
| Lumasiran regimen 3 mg/kg monthly ×3 → then quarterly | `lum_ev()` | 10 |
| Lumasiran plasma t½ ≈ 5 hours, Tmax ≈ 4 hours | `KA_LUM`, `CL_LUM` | 10, 12 |
| Nedosiran 160 mg SC monthly | `ned_ev()` | 11 |
| No response in PH2 in PHYOX2 | `D07` (reproduced structurally) | 11 |
| A −30 to −50 % pyridoxine response in G170R homozygotes | `B6RESC` | 3 |
| Pyridoxine 5–20 mg/kg/day, with sensory neuropathy at high dose | `PN_MGKG`, `B6_NEURO` | 3 |
| Hyperhydration 2–3 L/m²/day | `U_VOL_TGT` | 14 |
| The formula for the Tiselius AP(CaOx) index | `ap_index` | 7 |
| About 20–50 % of PH1 reaches ESKD by age 25 | the calibration target for `K_LOSS` | 1, 15 |
| Hydroxyproline contributes about 15 % of the oxalate | `J_HYP`, `FR_M_*` | 5 |
| ASGPR-mediated hepatocyte-selective uptake of a GalNAc-siRNA | `CLUP_LUM`, the `ASGPR` node | 12 |

**Estimates — not verified against the literature, and flagged as such**

There is no single verifiable citation for the following parameters. They were chosen for internal consistency of the model (so that the
normal state is an exact fixed point) and to reproduce the clinical anchors, and they are
**not presented as literature values.**

- `PHI_GOOX` (the fraction of glyoxylate that glycolate oxidase oxidises to oxalate,
  0.15) — diagnostic `D12` back-calculates 0.077 from the PHYOX2 result and offers that value itself
  as a prediction refutable by a hepatocyte experiment.
- `FR_LDH` / `FR_GRH` / `FR_GXSP` (the branching fractions of cytosolic glyoxylate)
- `GXP_REF`, `KM_AGT` (the degree of saturation of AGT — it determines the size of the non-linearity in `D03` and is the
  assumption with the greatest sensitivity)
- `CLDEP_B/S/K`, `POX_CRIT_B/S` (the tissue deposition clearances and thresholds)
- `BMAX_BONE` = 600 mmol (the bone oxalate storage capacity) — a value close to the physically possible
  upper limit; the range of bone oxalate quantification in the literature is wide and it could not be pinned down.
- `K_RES` / `K_BURY` / `K_RES_D` (the two-compartment kinetics of a bone-surface exchange pool and a deeply crystal-bound
  pool) — borrowed from the standard structure of calcium bone kinetics, and not values
  measured directly for oxalate.
- `K_NUC` … `K_LOSS` (every rate constant of the crystal → NLRP3 → IL-1β → TGF-β → fibrosis → nephron loss
  cascade). The individual constants are arbitrary, and only the **composite result**
  (the age at which PH1 reaches ESKD, and the eGFR slope) was calibrated to the literature.
- `OX_XHEP2` = 600 µmol/day (the extrahepatic oxalate production that PH2 exposes) — `D07` shows that the
  conclusion holds even with this value set to zero, so the headline result does not depend on this
  estimate.
- The effect size of the `IL1_BLOCK` scenario (S33) — a mechanistic prediction for which there is no approved treatment and
  no clinical evidence whatever.

---


## 1. Overview, epidemiology, diagnosis
...
## 2. Glyoxylate metabolism and AGT enzymology
...
## 3. Genotype-phenotype and pyridoxine responsiveness
...
## 4. PH2 / PH3 — a different enzyme, a different pharmacology
...
## 5. Hydroxyproline · ascorbate · ethylene glycol — substrate supply
...
## 6. Distribution, plasma oxalate, systemic oxalosis
...
## 7. Crystallisation physical chemistry and supersaturation
...
## 8. Crystal-induced injury, inflammation, fibrosis
...
## 9. The gut-kidney oxalate axis and Oxalobacter
...
## 11. Nedosiran and LDHA-directed therapy
...
## 12. GalNAc-siRNA delivery and PK/PD
...
## 13. Gene therapy and chaperone rescue
...
## 14. Supportive care: fluids and citrate
...
## 15. Dialysis and transplantation
...
**294 references** in total. Every PMID was actually looked up and confirmed with NCBI E-utilities.
...
- OMIM: PH1 259900 (AGXT) · PH2 260000 (GRHPR) · PH3 613616 (HOGA1) —
  <https://www.omim.org/>
- OxalEurope / Rare Kidney Stone Consortium registries —
