...
## Citation convention -- must read

Entries in this list are of **two kinds**. The distinction is deliberate, made to avoid inventing unconfirmed PMIDs
out of thin air.

| Form | Meaning |
|---|---|
| `**Author** Title. *Journal* Year. [PMID 12345](.)` | Bibliographic data **actually verified by querying** PubMed E-utilities. Title, journal, year, and PMID are all exactly as returned by PubMed. |
| `Query — [PubMed search](.)` | An entry for which a title-field search could not automatically resolve a single paper. **Rather than guessing a PMID**, a live PubMed search link is always provided instead. |
...
> Which reference each of the model's quantitative anchors comes from is laid out in the
> `psp_validate()` table in `psp_mrgsolve_model.R` and in the validation section of `README_en.md`. Below,
> a comment starting with `—` indicates **which structure and which parameter** of the model
...
## The Five Papers This Model Is Built On

1. **PASSPORT (gosuranemab).** CSF unbound N-terminal tau -98%, PSPRS change at week 52
   10.4 vs 10.6 (p = 0.85), no change in CSF NfL. This is the model's central anchor, and at the same time
   it is the quantitative basis for the statement that "target engagement is not
   the same as target validity."
2. **ARISE (tilavonemab).** The same N-terminal strategy failed a second time, independently
   (meeting futility criteria at the second interim analysis). Once could be chance; twice is structure.
3. **NIO752 phase 1.** CSF total tau / p-tau181 -20% (one-fifth of PASSPORT's),
   yet CSF NfL stabilised (placebo +40%). The only downstream signal in this disease's history
   is attached to the smallest biomarker change.
4. **AL-108-231 (davunetide).** Microtubule stabilisation. No effect. Crucially, the paper's authors
   stated that "the expected annual change was observed in both arms, so the study was adequately
   powered" -- meaning the failure is a mechanistic problem, not a statistical one.
5. **Golbe & Ohman-Strickland 2007 (PSPRS).** 11.3 +/- 11.0 points per year. This is the slope
   this model must reproduce, and the very fact that the standard deviation equals the mean
   is itself a constraint on clinical trial design.
