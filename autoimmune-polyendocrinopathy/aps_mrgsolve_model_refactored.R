...
##
## PK/PD REFACTOR (this file): Cyclosporine A (CSA), Hydrocortisone (HC),
## and the JAK inhibitor tofacitinib (JAKI) were renamed to the fork's
## PK/PD refactor convention (see FORK_WORKFLOW_GUIDE.md, Part 2) so each
## exposes a single named concentration (C_CSA / C_HC / C_JAKI) and, where
## the original had a saturable effect, a single named Hill term
## (EFFECT_CSA / EFFECT_JAKI). Abatacept and Rituximab are untouched.
## See aps_refactor_notes.md for the full account, including a pre-existing
## $CMT/$INIT compartment-redeclaration build defect (unrelated to any
## compound's PK) that is fixed here with a syntax-only change (removing
## the redundant $CMT block) and logged as UPSTREAM_ISSUES.md #51.
##
## Compartments (20):
...
## 19. CENT_CSA      – Cyclosporine A plasma (ng/mL) [was Drug_CsA]
## 20. Drug_Aba      – Abatacept plasma (µg/mL) [untouched]
## 21. Drug_RTX      – Rituximab plasma (µg/mL) [untouched]
## 22. CENT_JAKI     – Tofacitinib plasma (ng/mL) [was Drug_JAKi]
## 23. CENT_HC       – Hydrocortisone plasma (µg/dL equivalent) [was Drug_HC]
##
## Treatment scenarios:
...
V1_HC         = 35.0  // Vd HC (L) [was Vd_HC]
HC_dose       = 0.0   // HC dose (mg/day) — off by default (dosing driver, unchanged name)

// Cyclosporine A (CSA) — refactored PK naming
F_CSA         = 0.30  // Bioavailability CsA (highly variable) [was F_CsA]
...
EMAX_CSA      = 1.0   // New — original had no explicit Emax; its ratio C/(IC50+C) is Emax=1 implicitly
GAMMA_CSA     = 1.0   // New — original had no explicit Hill exponent

// Abatacept — out of scope for this refactor pass, untouched
...
// Rituximab — out of scope for this refactor pass, untouched
...
// Tofacitinib (JAKI) — refactored PK naming
F_JAKI        = 0.74  // Bioavailability [was F_JAKi]
KA_JAKI       = 6.0   // Absorption rate (/day) [was ka_JAKi]
CL_JAKI       = 2.8   // CL, t1/2~3h (/day) [was k_JAKi_clear]
V1_JAKI       = 87.0  // Vd (L) [was Vd_JAKi]
JAKi_dose     = 0.0   // Dose (mg/day) — off by default (dosing driver, unchanged name)
EC50_JAKI     = 8.0   // JAKI for 50% JAK-STAT suppression (ng/mL) [was IC50_JAKi]
EMAX_JAKI     = 1.0   // New — original had no explicit Emax; its ratio C/(IC50+C) is Emax=1 implicitly
GAMMA_JAKI    = 1.0   // New — original had no explicit Hill exponent
...
C_CSA = CENT_CSA;   // state already IS a concentration (ng/mL); no volume division needed
EFFECT_CSA = (C_CSA > 0) ? EMAX_CSA*pow(C_CSA, GAMMA_CSA) / (pow(EC50_CSA, GAMMA_CSA) + pow(C_CSA, GAMMA_CSA)) : 0;

// ── JAK inhibitor (tofacitinib): exposed concentration + Hill effect ────
C_JAKI = CENT_JAKI;
EFFECT_JAKI = (C_JAKI > 0) ? EMAX_JAKI*pow(C_JAKI, GAMMA_JAKI) / (pow(EC50_JAKI, GAMMA_JAKI) + pow(C_JAKI, GAMMA_JAKI)) : 0;

// Abatacept / Rituximab — out of scope for this refactor pass, untouched
...
double Immuno_suppress = (1 - 0.85*EFFECT_CSA) * (1 - 0.80*E_Aba) * (1 - 0.80*E_RTX) * (1 - 0.70*EFFECT_JAKI);
...
C_HC = CENT_HC;   // bare assignment, same reason as C_CSA/C_JAKI above; state already IS a concentration (µg/dL)
double HC_exogenous = C_HC;   // renamed source only; formula unchanged
...
double adrenal_attack = k_adren_dest * AutoAb_adren * AutoT_pool * (1 - E_Aba*0.5) * (1 - EFFECT_JAKI*0.4);
...
double beta_attack = k_beta_dest * AutoAb_beta * AutoT_pool * (1 - E_Aba*0.5) * (1 - EFFECT_CSA*0.6);
...
// ── THYROID GLAND / T4 ─────────────────────────────────────────
double thy_attack = k_thy_dest * AutoAb_thy * AutoT_pool * (1 - EFFECT_JAKI*0.4);
...
double CsA_elim   = CL_CSA * CENT_CSA;
dxdt_CENT_CSA = CsA_input - CsA_elim;
...
double JAKi_input = (JAKi_dose > 0) ? KA_JAKI * F_JAKI * JAKi_dose / V1_JAKI : 0;  // ng/mL/day
double JAKi_elim  = CL_JAKI * CENT_JAKI;
dxdt_CENT_JAKI = JAKi_input - JAKi_elim;

// HC: oral absorption → plasma cortisol equivalent
double HC_input = (HC_dose > 0) ? KA_HC * F_HC * HC_dose / V1_HC : 0;  // µg/dL/day
double HC_elim  = CL_HC * CENT_HC;
dxdt_CENT_HC = HC_input - HC_elim;
...
## ── Compile model ─────────────────────────────────────────────
mod <- mcode("APS_QSP_refactored", code, quiet = TRUE)
...
simulate_scenario <- function(sc) {
  ...
  if(sc$HC_dose > 0) {
    ev_list[["HC"]] <- ev(cmt="CENT_HC", amt=sc$HC_dose, ii=1, addl=364*5-1, rate=-2)
  }
  # CsA oral daily (if prescribed)
  if(sc$CsA_dose > 0) {
    ev_list[["CsA"]] <- ev(cmt="CENT_CSA", amt=sc$CsA_dose*70, ii=1, addl=364*5-1, rate=-2)
  }
  ...
  if(sc$JAKi_dose > 0) {
    ev_list[["JAKi"]] <- ev(cmt="CENT_JAKI", amt=sc$JAKi_dose/2, ii=0.5, addl=365*5*2-1, rate=-2)
  }
