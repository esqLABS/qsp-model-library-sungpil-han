...
## Mechanism: Tfh2-driven IgG4+ plasmablasts, M2-TGF-beta fibrosis
##
## Drugs modelled:
##   1. Rituximab (anti-CD20, IV)
##   2. Prednisone (glucocorticoid, oral)
##   3. Dupilumab (anti-IL-4Ra, SC)  [investigational]
##
## ODE Compartments (23 total):
##   PK: CENT_RTX, PERI_RTX, REC_FREE_RTX, COMPLEX_RTX   (4)
##       GUT_PRED, CENT_PRED                             (2)
##       GUT_DUP, CENT_DUP, REC_FREE_DUP, COMPLEX_DUP    (4)
...
## igg4rd_refactor_notes.md for full detail). All three modelled compounds
## (RTX, PRED, DUP) are in scope -- there is no fourth compound left
## untouched in this file.
##
##   - Archetype 4 (TMDD) for RTX and DUP: CD20_FREE/RTX_CD20 and
##     IL4RA_FREE/DUP_IL4RA renamed to REC_FREE_RTX/COMPLEX_RTX and
##     REC_FREE_DUP/COMPLEX_DUP; SC_DUP renamed to GUT_DUP (depot-naming
##     convention). CENT_RTX/PERI_RTX/CENT_DUP and every PK parameter kept
##     its role, renamed only where the convention requires uppercase
##     (kon_RTX -> KON_RTX, CD20_ss -> RTOT_RTX, etc). Receptor-binding ODEs
##     are UNCHANGED pharmacology, renamed only -- nothing was flattened.
##   - Archetype 3-minus-peripheral (depot+central, linear) for PRED, plain
##     rename (ka_PRED -> KA_PRED, V_PRED -> V1_PRED, Emax_PRED -> EMAX_PRED).
##   - All three central PK compartments are the corpus's recurring
##     "compartment IS concentration" idiom (confirmed from the original's
##     own $TABLE, which reads CENT_RTX/CENT_PRED/CENT_DUP directly as nM
##     with no /V division) -- so C_RTX/C_PRED/C_DUP = CENT_RTX/CENT_PRED/
##     CENT_DUP, undivided, exactly matching the original's own convention.
##   - EFFECT_RTX/EFFECT_PRED/EFFECT_DUP are RENAMES of Ekill_RTX/Eimmu_PRED/
##     IL4RA_occ, not refits -- the originals' own formulas kept verbatim
##     (aliased), zero added approximation. Analytically, RTX and DUP
##     occupancy vs. concentration is exactly Hill(gamma=1) at quasi-steady-
##     state (Kd_RTX=(KOFF_RTX+KINT_RTX)/KON_RTX=0.549091 nM,
##     Kd_DUP=(KOFF_DUP+KINT_DUP)/KON_DUP=0.503333 nM); documented as new
##     GAMMA_RTX/GAMMA_PRED/EC50_DUP/GAMMA_DUP parameters (EC50_RTX left
##     alone -- see notes for why the original's own dead EC50_RTX=50 was not
##     overwritten with the derived value).
##   - C_RTX/EFFECT_RTX/C_PRED/EFFECT_PRED/C_DUP/EFFECT_DUP are computed once
##     in $ODE (used by the disease equations there) and again, under an
##     "_OUT" suffix, in $TABLE for discoverability via /model_manifest +
##     /run_simulation -- mrgsolve 2.0.1 shares one C++ scope between $ODE
##     and $TABLE for any $CAPTURE'd name, so declaring the identical name
##     via "double X = ..." in both blocks is a redefinition error (confirmed
##     empirically); same fix as eosinophilic-esophagitis's C_MEPO_OUT.
##   - Build-compat fix (unrelated to any of the 3 compounds, see notes and
##     translations/UPSTREAM_ISSUES.md): the original's fibrosis term
##     `TGFB^2` uses R's `^` power operator, invalid in the C++ mrgsolve
##     compiles this DSL to (`invalid operands ... to binary 'operator^'`).
##     Rewritten as `pow(TGFB,2)` (and `pow(TGFB_EC50_FIB,2)`) here only --
##     the tracked original is untouched and still fails to compile as-is.
##
## Verified against the original via the qspserver mrgsolve_api service
## on the original file's own dosing scenarios -- see igg4rd_refactor_notes.md
## for the full result.
## ============================================================
...
RTOT_RTX   = 180.0   // nM, baseline CD20 on B cells
KSYN_RTX = 90.0    // nM/day, CD20 synthesis rate (= kd × RTOT_RTX)
KDEG_RTX = 0.5     // 1/day, CD20 baseline degradation
...
RTOT_DUP  = 10.0    // nM, baseline IL-4Rα
KSYN_DUP = 0.5    // nM/day
KDEG_DUP = 0.05   // 1/day
...
// ── Refactor-added Hill-interface parameters (pluggable-PK naming
...
CENT_RTX  = 0
PERI_RTX  = 0
REC_FREE_RTX = 180.0   // nM (= RTOT_RTX)
COMPLEX_RTX  = 0
...
$ODE
// ─── Rituximab PK (TMDD) ─────────────────────────────────────────────────
double kon_r = KON_RTX * CENT_RTX * REC_FREE_RTX;
double koff_r = KOFF_RTX * COMPLEX_RTX;
double kint_r = KINT_RTX * COMPLEX_RTX;
...
dxdt_REC_FREE_RTX = KSYN_RTX - KDEG_RTX * REC_FREE_RTX - kon_r + koff_r;
dxdt_COMPLEX_RTX  = kon_r - koff_r - kint_r;
...
// ─── Dupilumab PK (TMDD) ──────────────────────────────────────────────────
double kon_d = KON_DUP * CENT_DUP * REC_FREE_DUP;
double koff_d = KOFF_DUP * COMPLEX_DUP;
double kint_d = KINT_DUP * COMPLEX_DUP;

dxdt_GUT_DUP    = - KA_DUP * GUT_DUP;
dxdt_CENT_DUP  = KA_DUP * GUT_DUP
                 - CL_DUP/V1_DUP * CENT_DUP
                 - kon_d + koff_d;
dxdt_REC_FREE_DUP = KSYN_DUP - KDEG_DUP * REC_FREE_DUP - kon_d + koff_d;
dxdt_COMPLEX_DUP  = kon_d - koff_d - kint_d;

// ─── Drug effect calculations ─────────────────────────────────────────────
// RTX: fraction of B cells killed (TMDD-driven; CD20 occupancy)
double CD20_occ = COMPLEX_RTX / (COMPLEX_RTX + REC_FREE_RTX + 0.001);
...
double IL4RA_occ = COMPLEX_DUP / (COMPLEX_DUP + REC_FREE_DUP + 0.001);

// ── Pluggable-PK interface (NEW, additive only): exactly one exposed
// concentration and one named effect per compound, per the refactor
// naming convention. These are pure aliases of the calculations above --
// zero numeric change, see igg4rd_refactor_notes.md.
double C_RTX = CENT_RTX;         // exposed RTX concentration (compartment IS concentration; CL/Q/V act as rate constants -- see notes)
double EFFECT_RTX = Ekill_RTX;   // exposed RTX disease-effect (alias)
double C_PRED = CENT_PRED;       // exposed PRED concentration (compartment IS concentration)
double EFFECT_PRED = Eimmu_PRED; // exposed PRED disease-effect (alias)
double C_DUP = CENT_DUP;         // exposed DUP concentration (compartment IS concentration)
double EFFECT_DUP = IL4RA_occ;   // exposed DUP disease-effect (alias)
...
double TGFB_eff = pow(TGFB,2) / (pow(TGFB_EC50_FIB,2) + pow(TGFB,2));
...
double CD20_occ_pct   = COMPLEX_RTX / (COMPLEX_RTX + REC_FREE_RTX + 0.001) * 100;
double IL4RA_occ_pct  = COMPLEX_DUP / (COMPLEX_DUP + REC_FREE_DUP + 0.001) * 100;
...
// eosinophilic-esophagitis/eoe_mrgsolve_model_refactored.R), purely to make
// them discoverable/retrievable via /model_manifest + /run_simulation.
double C_RTX_OUT      = CENT_RTX;
double EFFECT_RTX_OUT  = EMAX_RTX * (COMPLEX_RTX / (COMPLEX_RTX + REC_FREE_RTX + 0.001));
double C_PRED_OUT      = CENT_PRED;
double EFFECT_PRED_OUT = EMAX_PRED * CENT_PRED / (EC50_PRED + CENT_PRED);
double C_DUP_OUT       = CENT_DUP;
double EFFECT_DUP_OUT  = COMPLEX_DUP / (COMPLEX_DUP + REC_FREE_DUP + 0.001);
...
$CAPTURE RTX_conc_ug_mL PRED_conc_nM DUP_conc_ug_mL Bcell_pct
         ...
         C_RTX_OUT EFFECT_RTX_OUT C_PRED_OUT EFFECT_PRED_OUT C_DUP_OUT EFFECT_DUP_OUT
'
...
e6 <- ev(time = dup_times, cmt = "GUT_DUP", amt = DUP_300mg_nM)  # renamed from SC_DUP
out6 <- mod %>%
