################################################################################
# Alagille Syndrome (ALGS) QSP Model -- mrgsolve Implementation (REFACTORED, pluggable PK)
#
# Refactor of `algs_mrgsolve_model.R` per FORK_WORKFLOW_GUIDE.md Part 2.
# Original untouched (see `algs_refactor_notes.md` for full rationale).
#
# Scope: of the two rows this file carried in
# `driver-patches/data/compound_perturbation_census.md` -- "BA" and
# "Terminal ileum" -- NEITHER is a drug. "BA" (bile acid) is the disease's
# own endogenous, hepatically-synthesised metabolite (BAHEP/BABIL/BADUO/
# BAILE/BAPOR/BASYS/BACOL -- no dosing route anywhere); "Terminal ileum" is
# an anatomical site name lifted from a $CMT comment, not a compound at all.
# Both are corrected below rather than force-fitted with a PK block.
#
# The census had UNDER-counted this file the same way it did
# prostate-cancer and hypercalcemia-of-malignancy: reading the model's own
# $PARAM/$CMT/$ODE blocks turned up ONE genuine externally-dosed compound
# the census listed no row for at all -- a generic IBAT (ileal bile acid
# transporter / ASBT) inhibitor, representing BOTH maralixibat and
# odevixibat (the file's own MRX/ODEV dosing shorthands), which share one
# potency-scaled PK/PD block differentiated only by a POT_IBAT multiplier
# and DOSEKG_IBAT dose. Refactored below, stem `IBAT`.
#
# UDCA, cholestyramine, rifampicin, naltrexone, PEBD (surgery) and MCT
# formula were also checked and are NOT genuine PK compounds in this file:
# each is a bare on/off flag or a fixed, dose-independent intensity
# (UDCAF/CHOLB/RIFF/NALF/PEBDF/MCTF) with no depot, no concentration state,
# and no timecourse -- there is no PK to redirect. Left untouched, not
# added as new rows (see notes).
#
# No build-compatibility fix needed -- the ORIGINAL compiles cleanly under
# mrgsolve 2.0.1 (`POST /model_manifest` on the untouched original: HTTP
# 200, 130 parameters, 61 outputs, no error). No UPSTREAM_ISSUES.md entry
# was needed.
#
# Compartments (44 total, unchanged count):
#   GUT_IBAT    = IBAT inhibitor, upper-gut depot (renamed from DEPOT)
#   MIDGUT_IBAT = IBAT inhibitor, mid-gut transit (renamed from DGUT, bespoke -- no
#                 role in the guide's naming table; pure transit, no PD reads it)
#   ILE_IBAT    = IBAT inhibitor, terminal ileum -- the compound's site of
#                 action, the ONLY site PD reads (renamed from DILE)
#   SYS_IBAT    = IBAT inhibitor, systemic (renamed from DSYS) -- off-target,
#                 informational only, never read by PD (same "lung vs
#                 plasma" pattern as this guide's copd/aortic-aneurysm rows)
#   all 40 disease/PD/clinical compartments -- UNCHANGED (endogenous state,
#   not this refactor's scope)
#
# PK archetype: BESPOKE. None of the guide's four reference archetypes fit
# a linear three-compartment GI transit chain (depot -> mid-gut -> ileum)
# feeding a small, PD-inert systemic branch, all in raw mass units (ug) --
# there is no volume of distribution or plasma concentration anywhere in
# the original for this compound, so CL/Q/V (Archetype 2/3) cannot be
# constructed without inventing a parameter the original never had. Kept
# as disclosed micro-rate-constant transit (KTR1_IBAT/KTR2_IBAT/KTR3_IBAT/
# KSYSD_IBAT), same rationale as this guide's ZOL (zoledronate) precedent
# in hypercalcemia-of-malignancy.
#
# The exposed covariate: C_IBAT = POT_IBAT * ILE_IBAT (potency-scaled
# ileal drug amount, ug -- an amount, not a concentration; the original's
# own effect term, CEFF = POT*DILE, was already amount-based, never
# converted to a concentration by any volume term, so none is invented
# here either). EFFECT_IBAT is a pure rename of the original's INH
# (fractional ASBT blockade): EMAX_IBAT (was IMAXA), EC50_IBAT (was the
# local IC50 = IC50KG*WT, kept as a WT-scaled local, not a $PARAM, since
# WT is a per-patient covariate the original never fixed into a constant),
# GAMMA_IBAT = 1 (new, explicit -- math-implied by the original's plain
# Emax-style ratio, no fit).
#
# Verified via qspserver mrgsolve_api against the original's own ICONIC
# (maralixibat) and ASSERT (odevixibat, both arms) replication scenarios,
# full native windows (438.3d and 180d respectively, delta=1d, no
# shortening needed): exact match, max abs diff 0.0 across all 61 shared
# outputs (57 unchanged disease-side names + the 4 renamed PK-compartment
# pairs), every scenario -- see `algs_refactor_notes.md` for the full
# comparison table. This is the expected result for a pure structural
# reorganization with an implicit-gamma=1 rename, not a fit.
################################################################################
...
// =====================================================================
DOSEKG_IBAT  :   0    : IBAT inhibitor dose (ug/kg/day)
POT_IBAT     :   1.0  : Potency vs odevixibat reference (odev 1.0, maralixibat 0.28)
TSTART_IBAT  :   0    : Treatment start (day)
TSTOP_IBAT   :   0    : Treatment stop (day)
KTR1_IBAT    :   6.0  : Drug depot -> mid gut (1/day)
KTR2_IBAT    :   6.0  : Drug mid gut -> terminal ileum (1/day)
KTR3_IBAT    :   6.0  : Drug ileum -> faeces (1/day)
FSYS_IBAT    :   0.005: Fraction of drug absorbed systemically (-)
KSYSD_IBAT   :   3.0  : Systemic drug elimination (1/day)
EMAX_IBAT   :   0.92 : Maximum achievable fractional ASBT blockade (-)
IC50KG_IBAT  :   6.3  : ASBT half-maximal ileal drug amount, per kg body weight (ug/kg) -- EC50_IBAT = IC50KG_IBAT*WT
GAMMA_IBAT   :   1.0  : Hill coefficient, ASBT blockade vs ileal drug [new, math-implied =1, no explicit exponent in original] (-)
...
$CMT @annotated
// ---- drug PK -------------------------------------------------------
GUT_IBAT   : IBAT inhibitor, upper gut depot (ug)
MIDGUT_IBAT    : IBAT inhibitor, mid gut (ug)
ILE_IBAT    : IBAT inhibitor, terminal ileum - site of action (ug)
SYS_IBAT    : IBAT inhibitor, systemic (ug)
// ---- bile acid system ----------------------------------------------
...
ONTRT_IBAT = (SOLVERTIME >= TSTART_IBAT && SOLVERTIME < TSTOP_IBAT) ? 1.0 : 0.0;
RIN_IBAT   = ONTRT_IBAT * DOSEKG_IBAT * WT;                       // ug/day

dxdt_GUT_IBAT    = RIN_IBAT - KTR1_IBAT * GUT_IBAT;
dxdt_MIDGUT_IBAT = KTR1_IBAT * GUT_IBAT - KTR2_IBAT * MIDGUT_IBAT;
dxdt_ILE_IBAT    = KTR2_IBAT * MIDGUT_IBAT * (1.0 - FSYS_IBAT) - KTR3_IBAT * ILE_IBAT;
dxdt_SYS_IBAT    = KTR2_IBAT * MIDGUT_IBAT * FSYS_IBAT - KSYSD_IBAT * SYS_IBAT;

C_IBAT  = POT_IBAT * ILE_IBAT;                                // potency-scaled ileal drug, the exposed PD-driving covariate
EC50_IBAT  = IC50KG_IBAT * WT;
EFFECT_IBAT = EMAX_IBAT * pow(C_IBAT, GAMMA_IBAT) / (pow(EC50_IBAT, GAMMA_IBAT) + pow(C_IBAT, GAMMA_IBAT));  // fractional ASBT blockade -- named Hill interface

// PEBD is a surgical version of the same lesion: it removes the ileal
// return entirely rather than inhibiting the transporter.
FREAB_D  = EPS0 * (1.0 - EFFECT_IBAT);
...
C_IBAT  = POT_IBAT * ILE_IBAT;
EC50_IBAT  = IC50KG_IBAT * WT;
EFFECT_IBAT   = EMAX_IBAT * C_IBAT / (EC50_IBAT + C_IBAT);
FREAB   = PEBDF > 0.5 ? EPS0 * PEBDEPS : EPS0 * (1.0 - EFFECT_IBAT);
FREAB_B  = FREAB * (1.0 - CHOLB);
...
FATREL_O CDUO_OUT JCAP_OUT PHI_OUT INH_OUT SIG_OUT EXPO_OUT PORTP_O INR C_IBAT EFFECT_IBAT
'
...
eq_init <- function(burn = 400, ...) {
  ...
  frozen <- list(KDR = 0, KDLOSS = 0, KFF = 0, KFR = 0,
                 DOSEKG_IBAT = 0, TSTART_IBAT = 0, TSTOP_IBAT = 0,
...
sim_eq <- function(end = yr(15), delta = 7, events = NULL, burn = 400, ...) {
  ...
  struct <- p[intersect(names(p),
              c("WT", "AGE0", "DPR0", "DPRN", "JCAPN", "GAMD", "SYNKG",
                "NEXP", "HL0", "HQ0", "EMAX_IBAT", "KBILC", "KBILF", "BILMIN",
                "EPS0", "SIGMAX", "KSIG"))]
...
# Drug shorthands -----------------------------------------------------
#   Odevixibat is the potency reference (POT_IBAT = 1.0).  Maralixibat needs
#   ~3x the dose for the same ileal effect, hence POT_IBAT = 0.28 at
#   380 ug/kg/day.
ODEV <- list(DOSEKG_IBAT = 120, POT_IBAT = 1.00)
MRX  <- list(DOSEKG_IBAT = 380, POT_IBAT = 0.28)
...
sc02_iconic <- function() {
  d <- sim(end = yr(1.2), delta = 1,
           DOSEKG_IBAT = MRX$DOSEKG_IBAT, POT_IBAT = MRX$POT_IBAT,
           TSTART_IBAT = 0, TSTOP_IBAT = yr(1.2), PBOMAX = 0.7)
  data.frame(
...
sc02b_bolus_equivalence <- function() {
  # Sampled at delta = 0.05 d: with KTR3_IBAT = 6/day the ileal drug amount is
  ...
  cont <- sim(end = 168, delta = 0.05, DOSEKG_IBAT = ODEV$DOSEKG_IBAT, POT_IBAT = ODEV$POT_IBAT,
              TSTART_IBAT = 0, TSTOP_IBAT = 168)
  amt  <- ODEV$DOSEKG_IBAT * 15
  ev   <- ev(amt = amt, cmt = "GUT_IBAT", ii = 1, addl = 167)
  bol  <- sim(end = 168, delta = 0.05, events = ev, POT_IBAT = ODEV$POT_IBAT,
              DOSEKG_IBAT = 0, TSTART_IBAT = 0, TSTOP_IBAT = 0)
  data.frame(
...
sc03_assert <- function() {
  drug <- sim(end = 180, delta = 1, DOSEKG_IBAT = ODEV$DOSEKG_IBAT, POT_IBAT = ODEV$POT_IBAT,
              TSTART_IBAT = 0, TSTOP_IBAT = 180, PBOMAX = 0.8)
  pbo  <- sim(end = 180, delta = 1, DOSEKG_IBAT = 0, TSTART_IBAT = 0, TSTOP_IBAT = 0,
              PBOMAX = 0.8)
...
sc05_duct_sweep <- function(dpr = seq(0.10, 0.45, by = 0.01)) {
  do.call(rbind, lapply(dpr, function(x) {
    base <- sim_eq(end = 180, delta = 30, DPR0 = x, DOSEKG_IBAT = 0)
    drug <- sim_eq(end = 180, delta = 1, DPR0 = x,
                   DOSEKG_IBAT = ODEV$DOSEKG_IBAT, POT_IBAT = ODEV$POT_IBAT,
                   TSTART_IBAT = 0, TSTOP_IBAT = 180)
...
sc06_phi_identifiability <- function(imax = seq(0.50, 0.92, by = 0.07)) {
  ...
  do.call(rbind, lapply(imax, function(im) {
    ...
    err <- function(dpr) {
      b <- sim_eq(end = 1, delta = 1, DPR0 = dpr, EMAX_IBAT = im)
      d <- sim_eq(end = 180, delta = 3, DPR0 = dpr, EMAX_IBAT = im,
                  DOSEKG_IBAT = ODEV$DOSEKG_IBAT, POT_IBAT = ODEV$POT_IBAT,
                  TSTART_IBAT = 0, TSTOP_IBAT = 180)
    ...
    b <- sim_eq(end = 1, delta = 1, DPR0 = r$root, EMAX_IBAT = im)
    d <- sim_eq(end = 180, delta = 3, DPR0 = r$root, EMAX_IBAT = im,
                DOSEKG_IBAT = ODEV$DOSEKG_IBAT, POT_IBAT = ODEV$POT_IBAT,
                TSTART_IBAT = 0, TSTOP_IBAT = 180)
    data.frame(
      EMAX_IBAT_assumed = im,
...
sc05_duct_sweep_k <- function(kbilc, dpr = seq(0.10, 0.45, by = 0.02)) {
  do.call(rbind, lapply(dpr, function(x) {
    base <- sim_eq(end = 180, delta = 30, DPR0 = x, KBILC = kbilc, DOSEKG_IBAT = 0)
...
sc07_pebd <- function(dpr = c(0.32, 0.236, 0.16)) {
  do.call(rbind, lapply(dpr, function(x) {
    b <- sim_eq(end = 1,   delta = 1, DPR0 = x, DOSEKG_IBAT = 0)
    p <- sim_eq(end = 365, delta = 7, DPR0 = x, PEBDF = 1)
    m <- sim_eq(end = 365, delta = 7, DPR0 = x,
                DOSEKG_IBAT = ODEV$DOSEKG_IBAT, POT_IBAT = ODEV$POT_IBAT,
                TSTART_IBAT = 0, TSTOP_IBAT = 365)
    data.frame(
      DPR0        = x,
...
sc08_drug_panel <- function() {
  arms <- list(
    ...
    "maralixibat 380 ug/kg/d"  = c(MRX,  list(TSTART_IBAT = 0, TSTOP_IBAT = 180)),
    "odevixibat 120 ug/kg/d"   = c(ODEV, list(TSTART_IBAT = 0, TSTOP_IBAT = 180)),
    "PEBD"                     = list(PEBDF = 1),
    "odevixibat + MCT formula" = c(ODEV, list(TSTART_IBAT = 0, TSTOP_IBAT = 180, MCTF = 1))
  )
...
sc09_micellar <- function(dpr = c(0.236, 0.60)) {
  do.call(rbind, lapply(dpr, function(x) {
    arms <- list(
      untreated      = list(),
      odevixibat     = c(ODEV, list(TSTART_IBAT = 0, TSTOP_IBAT = yr(2))),
      cholestyramine = list(CHOLB = 0.55)
    )
    do.call(rbind, lapply(names(arms), function(nm) {
...
sc10_timing <- function(start_ages = c(1, 3, 5, 8, 12), to_age = 18) {
  do.call(rbind, lapply(start_ages, function(a) {
    d <- sim(end = yr(to_age - 1), delta = 30.4375, AGE0 = 1,
             DOSEKG_IBAT = MRX$DOSEKG_IBAT, POT_IBAT = MRX$POT_IBAT,
             TSTART_IBAT = yr(a - 1), TSTOP_IBAT = yr(to_age - 1))
...
sc11_efs <- function(nexp = 1.60, years = 6) {
  ...
  trt <- sim(end = yr(years), delta = 30.4375, AGE0 = 3, NEXP = nexp,
             DOSEKG_IBAT = MRX$DOSEKG_IBAT, POT_IBAT = MRX$POT_IBAT,
             TSTART_IBAT = 0, TSTOP_IBAT = yr(years))
...
sc14_nonresponder <- function() {
  arms <- list(
    ...
    "responder (DPR 0.32), odevixibat" = c(list(DPR0 = 0.32), ODEV,
                                           list(TSTART_IBAT = 0, TSTOP_IBAT = yr(3))),
