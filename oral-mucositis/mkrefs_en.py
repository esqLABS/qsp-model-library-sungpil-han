#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Build om_references_en.md from PubMed.
...
def main():
    ...
    out = []
    out.append("# Oral Mucositis — QSP model references\n")
    ...
    for sec, (title, blurb) in SECTIONS.items():
        ...
        for intent, q, pid in rows:
            ...
            out.append("   PMID [%s](https://pubmed.ncbi.nlm.nih.gov/%s/)  "
                       % (pid, pid))
            out.append("   *role in the model:* %s  " % intent)
            out.append("")
        out.append("")

    out.append("---\n")
    out.append("## Citation rules (how this file was built)\n")
    out.append("- Each entry is defined by a `(section, intent, query)` triple; "
               "the top non-duplicate hit from PubMed `esearch` "
               "(`sort=relevance`) is taken and its bibliography resolved by `esummary`.\n")
    out.append("- Without `sort=relevance` the E-utilities return results "
               "**by date**, so a recent paper unrelated to the query rises to "
               "the top. This option must not be removed.\n")
    out.append("- Duplicate PMIDs are removed automatically and replaced by the next candidate.\n")
    out.append("- Caches: `refs_raw.json` (query→PMID), `refs_meta.json` "
               "(PMID→bibliography). Delete them and re-run to re-query everything.\n")
    if BROADENED:
        out.append("")
        out.append("## Queries that had to be broadened\n")
        out.append("PubMed combines every term with AND, so an over-specified "
                   "query returns nothing. For the queries below, trailing terms "
                   "were dropped until results appeared, and the query actually "
                   "used is recorded alongside.\n")
        for orig, used in BROADENED:
            out.append("- `%s`  \n  → actually used: `%s`" % (orig, used))
        out.append("")
    open("om_references_en.md", "w").write("\n".join(out))
    sys.stderr.write("wrote om_references_en.md with %d refs\n" % n)
