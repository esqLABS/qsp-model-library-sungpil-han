...
<a href="om_qsp_model_en.svg"><img src="om_qsp_model_en.png" width="820" alt="Oral mucositis QSP mechanistic map"></a>
...
is in [`om_reference_output.txt`](om_reference_output.txt).
...
| File | Contents |
|---|---|
| [`om_qsp_model_en.dot`](om_qsp_model_en.dot) | Mechanistic map source — **165 nodes · 18 clusters · 237 edges** |
| [`om_qsp_model_en.svg`](om_qsp_model_en.svg) / [`.png`](om_qsp_model_en.png) | Rendering (`dot -Tsvg` / `dot -Tpng -Gdpi=150`) |
| [`om_python_reference.py`](om_python_reference.py) | **The reference implementation that actually runs** — 50 ODEs, interval-wise integrator |
| [`om_calibrate.py`](om_calibrate.py) | Four-stage calibration (nested bisection + parallel virtual population) |
| [`om_analysis.py`](om_analysis.py) | The experiments of 12 sections — produces every number above |
| [`om_reference_output.txt`](om_reference_output.txt) | The full text of that run's output |
| [`calib.log`](calib.log) | Calibration log (including the failed attempts) |
| [`om_calibration.json`](om_calibration.json) | The eight fitted values — both R and Python read them from here |
| [`om_mrgsolve_model_en.R`](om_mrgsolve_model_en.R) | mrgsolve model — 50 ODEs, 17 scenarios |
| [`om_mrgsolve_template_en.R`](om_mrgsolve_template_en.R) · [`mkmrgsolve.py`](mkmrgsolve.py) | Generation template and generator (prevents R/Python parameter drift) |
| [`om_shiny_app_en.R`](om_shiny_app_en.R) | Shiny dashboard — **15 tabs** |
| [`om_references_en.md`](om_references_en.md) | **115 papers**, every one resolved live against PubMed |
| [`mkrefs_en.py`](mkrefs_en.py) | The generator for that reference list |
...
python3 mkmrgsolve.py           # generate om_mrgsolve_model_en.R
python3 mkrefs_en.py               # re-resolve the reference list (network required)
dot -Tsvg om_qsp_model_en.dot -o om_qsp_model_en.svg
dot -Tpng -Gdpi=150 om_qsp_model_en.dot -o om_qsp_model_en.png
```

```r
library(mrgsolve); library(dplyr)
mod <- mread("om_mrgsolve_model_en.R")
out <- mod %>% data_set(scen_hdm_cryo(6)) %>% mrgsim(end = 45, delta = 0.05)
plot(out, ULC + WHO + VAS + ANC ~ time)
shiny::runApp("om_shiny_app_en.R")
```
...
- **This was written in an environment without an R toolchain.**
  `om_mrgsolve_model_en.R` and `om_shiny_app_en.R` mirror the executed Python reference
...
- The oral microbiota, pain, and myelosuppression axes are each present only as
  minimal structure. The corresponding sections of `om_references_en.md` give the basis
