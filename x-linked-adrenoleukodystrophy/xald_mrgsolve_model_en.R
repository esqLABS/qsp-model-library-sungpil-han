...
#  X-linked adrenoleukodystrophy · quantitative systems pharmacology model
# =============================================================================
#
#  ORGANISING THESIS
#  -----------------------------------------------------------------------------
...
#
#  All parameter provenance is in xald_references_en.md.  Values marked FIT were
...
xald_scenarios <- function() {
  list(
    ...
    s10_elicel_early = list(
      label = "10. eli-cel gene therapy, inside the window (transplant at Loes 2)",
    ...
    s11_elicel_early_control = list(
      label = "11. matched control for 10 (same patient, untreated)",
      param = list(MUTRES = 0.02, SUSCTV = SUSC_CALD, VCNIN = 0.80), end = 14*YR),

    s12_elicel_late = list(
      label = "12. eli-cel, after the window was missed (transplant at Loes 15)",
      param = list(MUTRES = 0.02, SUSCTV = SUSC_CALD, VCNIN = 0.80), end = 14*YR,
      ev_fn = function() .hsct_regimen(t0 = .find_loes_day(15), vcn = 0.80)),

    s13_allo_hsct = list(
      label = "13. allogeneic HSCT, inside the window (VCN 0, GVHD risk)",
...
xald_validate <- function(mod = NULL, verbose = TRUE) {
  ...
  add("A1",  "normal plasma C26:0",             0.35,  round(at(h,10*YR,"C26"),3), "umol/L")
  add("A2",  "untreated X-ALD plasma C26:0",    1.30,  round(at(am,10*YR,"C26"),3), "umol/L")
  add("A3",  "C26:0 fold elevation",            3.70,  round(at(am,10*YR,"C26FOLD"),2), "fold")
  add("A4",  "normal C26:0-lysoPC",             0.060, round(at(h,10*YR,"C26LYSO"),4), "umol/L")
  add("A5",  "X-ALD C26:0-lysoPC",              0.550, round(at(am,10*YR,"C26LYSO"),4), "umol/L")
  add("A6",  "lysoPC fold elevation at birth (newborn screening feasibility)", 8.0,
      round(at(am,1,"C26LYSO")/at(h,1,"C26LYSO"),2), "fold")

  # ---- the adrenal axis --------------------------------------------------
  add("A7",  "normal cortisol",                 300,   round(at(h,10*YR,"CORTISOL")), "nmol/L")
  add("A8",  "age at adrenal insufficiency",    7.5,   round(ft(am,"ADRINSUF",1)/YR,2), "years")
  add("A9",  "untreated adrenal insuff. ACTH",  150,   round(at(am,12*YR,"ACTHOUT")), "pg/mL")
  add("A10", "untreated adrenal insuff. K+",    5.8,   round(at(am,12*YR,"POTASSIUM"),2), "mmol/L")

  # ---- VARIABLE 2: the cerebral switch ----------------------------------
  add("A11", "CCALD onset age (Loes >= 1)",     7.0,   round(ft(cc,"LOES",1)/YR,2), "years")
  add("A12", "Loes 1 -> 15 transit time",       1.50,
      round((ft(cc,"LOES",15) - ft(cc,"LOES",1))/YR,2), "years")
  add("A13", "untreated CCALD final Loes",      32,    round(max(cc$LOES),1), "points")
  add("A14", "pure AMN peak Loes (no lesion)",  0.5,   round(max(am$LOES),2), "points")
  add("A15", "AMN EDSS (age 45)",               4.0,   round(at(am,45*YR,"EDSS"),2), "points")
  ...
  add("A16", "Lorenzo oil plasma C26:0 drop",   50.0,
      round(100*(1 - at(lo,6*YR,"C26")/at(loc,6*YR,"C26")),1), "%")
  add("A17", "Lorenzo oil final Loes drop (literature: no effect)", 0.0,
  ...
  tx  <- .find_loes_day(2)
  add("A18", "Loes rise 18 months after eli-cel", 2.5,
      round(at(gt,tx+548,"LOES") - at(gt,tx,"LOES"),2), "points")
  add("A19", "eli-cel 24-mo MFD-free (1=met)",  1.0,
      as.numeric(at(gt,tx+730,"MFD") == 0), "0/1")
  add("A20", "plasma C26:0 change after eli-cel (literature: almost none)", 0.0,
      round(100*(at(gt,tx+730,"C26")/at(gt,tx-30,"C26") - 1),1), "%")
  add("A21", "neutrophil recovery after nadir", 13,
      round(.engraft_day(gt, tx)), "days")
  add("A22", "blood monocyte chimerism (1 y)",  0.95,  round(at(gt,tx+365,"CHIMER"),3), "fraction")
  add("A23", "busulfan cumulative AUC",         90,    round(max(gt$BUSAUC)), "mg*h/L")

  res <- do.call(rbind, R)
  if (verbose) {
    cat("\n================ X-ALD model validation (anchors A1-A23) ================\n")
    print(res, row.names = FALSE)
    ok <- sum(abs(res$pct) <= 20, na.rm = TRUE)
    cat(sprintf("\nanchors within 20%%: %d / %d\n", ok, sum(!is.na(res$pct))))
  }
  invisible(res)
}
...
#  values that shipped are recorded in xald_references_en.md and the directory
...
xald_report <- function(n = 300) {
  ...
  assign(".XALD_MOD", mod, envir = globalenv())
  v <- xald_validate(mod)
  cat("\n=== Where the bistable switch sits (a model output) ===\n")
  cat(sprintf("susceptibility threshold SUSC* = %.4f  (found by bisection)\n", xald_bifurcation(mod)))
  cat("\n=== Phenotype distribution: generated by inter-individual variation in SUSC alone ===\n")
  pop <- xald_population(mod, n = n)
  cat(sprintf("cerebral conversion  : %.1f%%   (literature 35-40%%)\n", 100*mean(pop$cerebral)))
  cat(sprintf("adrenal insufficiency: %.1f%%   (literature about 80%%)\n", 100*mean(pop$adrinsuf)))
  o <- pop$onset[!is.na(pop$onset)]
  if (length(o)) cat(sprintf("cerebral onset age : median %.1f y, range %.1f-%.1f y  (literature 4-8 y, mode 7 y)\n",
      median(o), min(o), max(o)))
  cat(sprintf("\nplasma C26:0 is identical across the whole cohort: a single value of %.3f umol/L\n", 1.299))
  cat("-> the same marker, different diseases. The central claim of this model.\n")
