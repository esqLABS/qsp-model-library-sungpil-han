...
// ---- insulin pharmacokinetics (PK/PD refactor, see dka_refactor_notes.md) -
// Renamed to the FORK_WORKFLOW_GUIDE.md Part 2 convention, stem INS.
V1_INS  :   6.0  : insulin distribution volume (L) -- was VINS
CL_INS  :  50.0  : insulin clearance (L/h) -- was CLINS; 1 U/h gives 20 uU/mL at steady state
KA_INS  :   1.50 : s.c. absorption, rapid-acting analogue (1/h) -- was KA_SC
KA_REG  :   0.50 : s.c. absorption, regular human insulin (1/h) -- unchanged; never referenced in $ODE in the original (see refactor notes, dead parameter)
...
TAU_PERI_INS : 0.35 : peripheral insulin effect-site time constant (h) -- was TINSE_F
TAU_HEP_INS  : 1.50 : hepatic insulin effect-site time constant (h) -- was TINSE_S
...
EMAX_INS_LIP  : 0.92 : maximal fractional suppression of lipolysis -- was EMAX_LIP
EC50_INS_LIP  : 15.0 : half-maximal suppression of lipolysis (uU/mL) -- was IC50_LIP
GAMMA_INS_LIP : 1.0  : Hill coefficient (original had no explicit Hill term)
...
GUT_INS      : subcutaneous insulin depot (U) -- was INSSC
CENT_INS     : plasma insulin (uU/mL) -- was INSP
EFF_PERI_INS : peripheral insulin effect site (uU/mL) -- was INSEF
EFF_HEP_INS  : hepatic insulin effect site (uU/mL) -- was INSES
GCG     : glucagon (pmol/L)
CORT    : cortisol (ug/dL)
...
double X_INS_PERI = EFF_PERI_INS / IRs;   // peripheral effect site, IR-scaled -- was Ieff
double X_INS_HEP  = EFF_HEP_INS  / IRs;   // hepatic effect site, IR-scaled -- was IeffS
...
double EFFECT_INS_HGP = EMAX_INS_HGP * pow(X_INS_HEP, GAMMA_INS_HGP)
               / (pow(EC50_INS_HGP, GAMMA_INS_HGP) + pow(X_INS_HEP, GAMMA_INS_HGP));
double EFFECT_INS_UP  = EMAX_INS_UP * pow(X_INS_PERI, GAMMA_INS_UP)
               / (pow(EC50_INS_UP, GAMMA_INS_UP) + pow(X_INS_PERI, GAMMA_INS_UP));
double EFFECT_INS_KSH = EMAX_INS_KSH * pow(X_INS_PERI, GAMMA_INS_KSH)
               / (pow(EC50_INS_KSH, GAMMA_INS_KSH) + pow(X_INS_PERI, GAMMA_INS_KSH));

double fLIP  = 1.0 - EFFECT_INS_LIP;
double fHGP  = 1.0 - EFFECT_INS_HGP;
double fUP   = EFFECT_INS_UP;
double fKSH  = EFFECT_INS_KSH;
double fKSH0 = INSB / (INSB + EC50_INS_KSH);

double sec = SECMAX * BETA * BETAF * Gp * Gp / (Gp * Gp + KGSEC * KGSEC);
double Riv = INS_IV + sec;                                       // U/h
double abs_sc = KA_INS * GUT_INS;
dxdt_GUT_INS      = -KA_INS * GUT_INS;
dxdt_CENT_INS     = ((Riv + abs_sc) * 1.0e6 - CL_INS * 1000.0 * CENT_INS) / (V1_INS * 1000.0);
dxdt_EFF_PERI_INS = (CENT_INS - EFF_PERI_INS) / TAU_PERI_INS;
dxdt_EFF_HEP_INS  = (CENT_INS - EFF_HEP_INS)  / TAU_HEP_INS;
...
double gcg_t = GCG0 * (1.0 + 2.2 * (1.0 - X_INS_PERI / (X_INS_PERI + 40.0))
                       + 0.8 * ILL + 1.5 * hypo);
...
double IeffP = (EFF_PERI_INS + PORTF * 20.0 * sec) / IRs;
...
capture C_INS_report          = C_INS;
capture EFFECT_INS_LIP_report = EFFECT_INS_LIP;
capture EFFECT_INS_HGP_report = EFFECT_INS_HGP;
capture EFFECT_INS_UP_report  = EFFECT_INS_UP;
capture EFFECT_INS_KSH_report = EFFECT_INS_KSH;
...
SIZE_PARAMS <- c("SECMAX", "V1_INS", "CL_INS", "HGP0", "GUCNS", "CLNI", "VMI_UP",
...
run_protocol <- function(p, init, hours = 24, step = 0.25,
  ...
  out <- list()
  t <- 0
  if (bolus > 0) st$CENT_INS <- st$CENT_INS + bolus * 1e6 / (p$V1_INS * 1000)
  while (t < hours - 1e-9) {
    ## --- read the patient ------------------------------------------------
...
sc9_subcutaneous <- function() {
  p <- patient(ILL0 = 0.30)
  pr <- present(p, 20)
  st <- pr$init; st$GUT_INS <- st$GUT_INS + 0.3 * p$BW
  out <- list(); t <- 0
  for (k in 1:12) {
    ...
    st <- start_from(seg)
    st$GUT_INS <- st$GUT_INS + if (tail(seg$GLUmM, 1) >= 13.9) 0.2 * p$BW else 0.1 * p$BW
    t <- t + 2
  }
