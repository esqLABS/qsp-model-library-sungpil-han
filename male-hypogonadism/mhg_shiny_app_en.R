## =============================================================================
##  Male Hypogonadism — QSP Shiny dashboard
##  mhg_shiny_app.R
##
##  Run:
##    setwd("male-hypogonadism"); shiny::runApp("mhg_shiny_app.R")
##
##  Requires mhg_mrgsolve_model_en.R in the same directory (it is sourced below,
...
source("mhg_mrgsolve_model_en.R", local = FALSE)
...
## -------------------------------------------------------------------- UI ----
ui <- fluidPage(
  titlePanel("Male Hypogonadism QSP Dashboard"),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("reg1", "Regimen A", choices = REGIMENS, selected = "gel"),
      selectInput("reg2", "Regimen B", choices = REGIMENS, selected = "im_q2wk"),
      selectInput("reg3", "Regimen C", choices = REGIMENS, selected = "none"),
      sliderInput("days", "Simulation Duration (days)", 90, 1095, 365, step = 30),
      hr(),
      h5("Physiological Parameters"),
      sliderInput("age", "Age (years)", 20, 85, 50, step = 1),
      sliderInput("shbg0", "Baseline SHBG (nmol/L)", 8, 90, 35, step = 1),
      sliderInput("ins", "Insulin Resistance (1 = lean)", 0.6, 4.0, 1.0, step = 0.1),
      sliderInput("fat0", "Baseline Fat Mass (kg)", 10, 60, 26, step = 1),
      hr(),
      h5("Model Assumptions"),
      sliderInput("ec50h", "EC50_HEPC — Erythropoietic Response (pg/mL)",
                  100, 700, 300, step = 25),
      helpText(style = "font-size:11px;",
               "This value must be higher than the physiological range for the erythropoietic response to become ",
               "convex and peak-dominated within the treatment range. It is a calibrated value, not a measured one. ",
               "(This single calibrated parameter is what makes waveform matter ",
               "for haematocrit — slide it down and the effect disappears.)"),
      sliderInput("aiflag", "Anastrozole Co-administration (1 mg/day, 0 = none)",
                  0, 1, 0, step = 1),
      hr(),
    ...
    mainPanel(
      ...
      tabsetPanel(
        ...
        tabPanel("② Binding Equilibrium (SHBG)",
          br(),
          tags$p("The relationship between the measured total testosterone and the acting free testosterone is a ",
                 "nonlinear mass-action equilibrium governed by SHBG. Because SHBG buffering ",
                 "saturates, the free fraction rises together with total T as it increases ",
                 "— in other words, FT is a convex function of TT."),
          plotOutput("nomogram", height = "420px"),
          hr(), verbatimTextOutput("nomo_txt")),
        tabPanel("③ Frame Dependence of Diagnostic Thresholds",
          br(),
          tags$p("A single total T threshold (300 ng/dL) is not equivalent to a single free T threshold (65 pg/mL). ",
                 "Depending on SHBG, the same threshold shifts from 237 ng/dL to 635 ng/dL."),
          plotOutput("framing", height = "400px"),
          hr(), tableOutput("frame_tbl")),
        tabPanel("④ PK Waveform",
          br(),
          tags$p("Even at the same weekly dose, the waveform differs by formulation. In the free T curve, ",
                 "the peak is amplified more than in total T — a direct consequence of convex binding."),
          plotOutput("pk_tt", height = "300px"),
          plotOutput("pk_ft", height = "300px"),
          hr(), tableOutput("pk_tbl")),
        tabPanel("⑤ Erythrocytosis (The Convexity Ledger)",
          br(),
          tags$p("Decomposes the erythrocytosis gap between formulations. (a) Convex binding is real and ",
                 "scales with amplitude, but it is small. (b) The convex erythropoietic response does not compound ",
                 "— because supraphysiological peaks cross onto the concave top of the Hill curve, ",
                 "which is the point where the model falsifies its own hypothesis. Once dose is matched, the waveform effect ",
                 "amounts to only about 0.1 point of Hct, and most of the gap in the literature comes from dose differences and ",
                 "the amplification that arises when a fixed threshold (Hct>54%) is applied to a population distribution. ",
                 "In other words, it is a property of the decision rule, not of the drug."),
          plotOutput("hct_plot", height = "340px"),
          hr(), tableOutput("convex_tbl"),
          hr(), h5("EC50_HEPC Sensitivity"), tableOutput("sens_tbl")),
        tabPanel("⑥ Intratesticular T · Fertility",
          br(),
          tags$p("Normalizing serum T and maintaining intratesticular T (ITT) are different problems. ",
                 "Exogenous T switches off LH and collapses ITT to a few percent of normal."),
          fluidRow(column(6, plotOutput("itt_plot", height = "320px")),
                   column(6, plotOutput("sperm_plot", height = "320px"))),
          hr(), tableOutput("itt_tbl")),
        tabPanel("⑦ Bone · Body Composition",
          br(),
          tags$p("Effects on bone and fat are mostly mediated not by testosterone itself but by its aromatization ",
                 "product, estradiol (Finkelstein NEJM 2013). ",
                 "Turning on anastrozole in the sidebar shows the cost of that."),
        ...
        tabPanel("⑩ Clinical Trial Comparison Table",
          br(),
          tags$p("Comparison of model output against published clinical trial endpoints. The last entry is a ",
                 "result the model does not reproduce, and it has been left in deliberately."),
...
server <- function(input, output, session) {
  ...
  output$dx <- renderPrint({
    ...
    if ((tt < 300) != (ft < 65))
      cat("\n>> Total T and free T give discordant readings — SHBG is the cause.\n",
